...
assert(migrations.at(-4)==='0061_post_confirmation_integrity_guards.sql'
  &&migrations.at(-3)==='0062_runtime_authority_and_privilege_guards.sql'
  &&migrations.at(-2)==='0063_advance_principal_proof_and_overpayment.sql'
  &&migrations.at(-1)==='0064_marketplace_local_date_truth.sql','unexpected migration tail');

const production=read('apps/api/wrangler.production.template.jsonc');
for(const marker of ['"APP_RELEASE_SHA": "REQUIRED_RELEASE_COMMIT_SHA"','"SCHEDULED_OPERATIONS_ENABLED": "true"','"ACQUISITION_MAINTENANCE_ENABLED": "true"','"STAFF_ACCESS_TEAM_DOMAIN": "REQUIRED_CLOUDFLARE_ACCESS_TEAM_HTTPS_ORIGIN"','"STAFF_ACCESS_AUD": "REQUIRED_CLOUDFLARE_ACCESS_APPLICATION_AUD"','"STAFF_AUTH_ALLOWED_ORIGINS": "REQUIRED_PRODUCTION_HTTPS_ORIGIN"'])assert(production.includes(marker),`production access/readiness marker missing: ${marker}`);
...
for(const file of ['apps/api/src/operational-readiness/routes.ts','apps/api/src/production-readiness/recovery-attestation-routes.ts','apps/api/src/staff-auth/cloudflare-access.ts','apps/api/src/files/r2-object-storage.ts','docs/OPERATING_INTEGRITY_FREEZE.md','docs/SECOND_LAYER_HARDENING_FREEZE.md'])assert(existsSync(path.join(root,file)),`required production boundary missing: ${file}`);
const readiness=read('apps/api/src/operational-readiness/routes.ts');assert(readiness.includes('const TARGET_SCHEMA=64'),'readiness target schema is stale');assert(readiness.includes('APP_RELEASE_SHA'),'readiness is not bound to running release');
const attestation=read('apps/api/src/production-readiness/recovery-attestation-routes.ts');assert(attestation.includes('const TARGET_SCHEMA=64'),'recovery attestation target schema is stale');assert(attestation.includes('APP_RELEASE_SHA'),'recovery proof is not bound to running release');
...
const monitor=read('scripts/production-health-monitor.mjs');for(const marker of ["url.pathname!=='/ready'",'value.data.status','staff_access','release'])assert(monitor.includes(marker),`production monitor does not validate current readiness: ${marker}`);
...
console.log(JSON.stringify({status:'PASS',migration:'0001-0064_CONTINUOUS',staff_auth:'CLOUDFLARE_ACCESS_REQUIRED',scheduler:'PRODUCTION_REQUIRED',readiness:'/ready',recovery:'SCHEMA64_CURRENT_RELEASE_D1_R2_ATTESTATION_REQUIRED',local_external_calls:0,production_go:'OWNER_APPROVAL_AND_EXPLICIT_PROBE_REQUIRED'},null,2));
