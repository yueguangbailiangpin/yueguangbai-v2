...
const readiness=read('apps/api/src/operational-readiness/routes.ts');assert(readiness.includes(`const TARGET_SCHEMA=${schema}`),'readiness target schema is stale');assert(readiness.includes('APP_RELEASE_SHA'),'readiness is not bound to running release');
const attestation=read('apps/api/src/production-readiness/recovery-attestation-routes.ts');assert(attestation.includes(`const TARGET_SCHEMA=${schema}`),'recovery attestation target schema is stale');assert(attestation.includes('APP_RELEASE_SHA'),'recovery proof is not bound to running release');
