...
const migrations = readdirSync(migrationDirectory)
  ...
  .sort();
if (migrations.length !== 39 || migrations.at(-1) !== '0039_owner_cleanup_bd_zero_consumer_objects.sql') {
  throw new Error('expected the clean baseline 0001-0039');
}
...
try {
  ...
  if (database.prepare('SELECT schema_version FROM app_schema_state WHERE singleton_id=1')
    .get().schema_version !== 39) throw new Error('schema version');
