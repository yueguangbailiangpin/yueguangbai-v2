...
function assertIntegrity(database, label) {
  ...
  if (foreignKeys.length > 0) {
    throw new Error(
      `${label}: foreign_key_check 发现 ${foreignKeys.length} 项`,
    );
  }
}
...
function assertObjectAbsent(database, type, name) {
  ...
  `).get(type, name);
  if (row) {
    throw new Error(`${type} ${name} 不应在失败后残留`);
  }
}
...
function expectMigrationFailure(database, name, label) {
  ...
  if (!failed) {
    throw new Error(`${label}: Migration 应失败但未失败`);
  }
}

// 1. Fresh full sequence 0001-0012.
{
  const fresh = openDatabase();
  applyFiles(fresh, migrationFiles);
  if (schemaVersion(fresh) !== 12) {
    throw new Error(`fresh: schema 应为 12，实际 ${schemaVersion(fresh)}`);
  }
  assertIntegrity(fresh, 'fresh');
  fresh.close();
}

// 2. Upgrade path 0001-0009 -> 0010 -> 0011 -> 0012.
{
  const upgrade = openDatabase();
  applyFiles(upgrade, migrationFiles.slice(0, 9));
  if (schemaVersion(upgrade) !== 9) {
    throw new Error('upgrade: 0001-0009 后 schema 应为 9');
  }
  applyFiles(upgrade, ['0010_file_storage.sql']);
  if (schemaVersion(upgrade) !== 10) {
    throw new Error('upgrade: 0010 后 schema 应为 10');
  }
  applyFiles(upgrade, ['0011_pricing_rules.sql']);
  if (schemaVersion(upgrade) !== 11) {
    throw new Error('upgrade: 0011 后 schema 应为 11');
  }
  applyFiles(upgrade, ['0012_customer_auth_security.sql']);
  if (schemaVersion(upgrade) !== 12) {
    throw new Error('upgrade: 0012 后 schema 应为 12');
  }
  assertIntegrity(upgrade, 'upgrade');
  upgrade.close();
}

// 3. Negative: schema=9, direct 0011 must fail without partial DDL.
{
  ...
  if (schemaVersion(neg) !== 9) {
    throw new Error('schema9->0011: schema 应保持 9');
  }
  assertObjectAbsent(neg, 'table', 'buyer_daily_exchange_rates');
  assertObjectAbsent(neg, 'trigger', 'trg_buyer_daily_rate_events_no_update');
  assertIntegrity(neg, 'schema9->0011');
  neg.close();
}

// 4. Negative: schema=10, direct 0012 must fail without partial DDL.
{
  const neg = openDatabase();
  applyFiles(neg, migrationFiles.slice(0, 10));
  expectMigrationFailure(neg, '0012_customer_auth_security.sql', 'schema10->0012');
  if (schemaVersion(neg) !== 10) {
    throw new Error('schema10->0012: schema 应保持 10');
  }
  assertObjectAbsent(neg, 'table', 'customer_login_rate_limits');
  assertObjectAbsent(neg, 'trigger', 'trg_customer_auth_security_events_no_update');
  assertIntegrity(neg, 'schema10->0012');
  neg.close();
}

// 5. Negative: schema=11, repeat 0011 must be rejected.
{
  const neg = openDatabase();
  applyFiles(neg, migrationFiles.slice(0, 11));
  if (schemaVersion(neg) !== 11) {
    throw new Error('repeat0011@11: 前置 schema 应为 11');
  }
  expectMigrationFailure(neg, '0011_pricing_rules.sql', 'repeat0011@11');
  if (schemaVersion(neg) !== 11) {
    throw new Error('repeat0011@11: schema 应保持 11');
  }
  assertIntegrity(neg, 'repeat0011@11');
  neg.close();
}

// 6. Negative: schema=12, repeat 0012 must be rejected.
{
  const neg = openDatabase();
  applyFiles(neg, migrationFiles);
  if (schemaVersion(neg) !== 12) {
    throw new Error('repeat0012@12: 前置 schema 应为 12');
  }
  expectMigrationFailure(neg, '0012_customer_auth_security.sql', 'repeat0012@12');
  if (schemaVersion(neg) !== 12) {
    throw new Error('repeat0012@12: schema 应保持 12');
  }
...
console.log(JSON.stringify({
  status: 'PASS',
  fresh_db: 'schema 12, integrity ok, fk 0',
  upgrade_9_to_12: '9 -> 10 -> 11 -> 12',
  negative_schema9_to_0011: 'REJECTED, schema stays 9, no partial ddl',
  negative_schema10_to_0012: 'REJECTED, schema stays 10, no partial ddl',
