...
describe('backup and restore CLI expected schema', () => {
  ...
  it('creates and restores one anonymous schema-64 fixture when both values match', () => {
    const fixture = localFixture();
    const backup = run('backup-d1.mjs', backupArgs(fixture, '64'));
    ...
    expect(JSON.parse(restored.stdout)).toMatchObject({ status: 'PASS', schema_version: 64 });
  });
});
