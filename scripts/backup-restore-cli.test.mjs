...
describe('backup and restore CLI expected schema', () => {
  ...
  it('creates and restores one anonymous schema-67 fixture when both values match', () => {
    const fixture = localFixture();
    const backup = run('backup-d1.mjs', backupArgs(fixture, '67'));
    ...
    expect(JSON.parse(restored.stdout)).toMatchObject({ status: 'PASS', schema_version: 67 });
  });
});
