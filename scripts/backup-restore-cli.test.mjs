...
describe('backup and restore CLI expected schema', () => {
  ...
  it('rejects an explicit schema mismatch', () => {
    ...
    const result = run('backup-d1.mjs', backupArgs(fixture, '36'));
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('unexpected_schema_version');
  });

  it('creates and restores one anonymous schema-40 fixture when both values match', () => {
    const fixture = localFixture();
    const backup = run('backup-d1.mjs', backupArgs(fixture, '40'));
    ...
    expect(JSON.parse(restored.stdout)).toMatchObject({ status: 'PASS', schema_version: 40 });
