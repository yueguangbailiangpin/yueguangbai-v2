...
for(const marker of ['AES-256-GCM','gzip','schema_fingerprint_sha256','financial_aggregates','PRAGMA foreign_key_check','smoke_reads']) assert(backup.includes(marker),`backup evidence missing: ${marker}`);
...
for(const marker of ['backup_authentication_failed','restore_target_exists','R2_REHYDRATION_REQUIRED','daily_orders: 200','staff_count: 8']) assert(tests.includes(marker),`test evidence missing: ${marker}`);
...
console.log(JSON.stringify({status:'PASS',change:'production-readiness-backup-validation',requirements:5,scenarios:10,migration:'NO_0035_SCHEMA_34',backup:'ENCRYPTED_RESTORE_REQUIRED',reconciliation:'OFFLINE_NO_DELETE',capacity:'8_STAFF_200_ORDERS',external_gates:'8_PRODUCTION_GO_BLOCKERS',production_go:'NOT_APPROVED'},null,2));
