...
const EXPECTED_SCHEMA=72;
const migrations=readdirSync(path.join(root,'migrations')).filter((name)=>/^\d{4}_.+\.sql$/u.test(name)).sort();
assert(migrations.length===EXPECTED_SCHEMA,'repository migration count must match schema 72');
assert(migrations.every((name,index)=>Number(name.slice(0,4))===index+1),'Migration chain must be continuous');
assert(migrations.at(-1)==='0072_unified_order_day_rate_center.sql','latest migration must persist the unified order-day rate center');
...
for(const marker of ['const TARGET_SCHEMA = 72','APP_RELEASE_SHA','production_recovery_attestations','operationalAlertAttestationReady','last_backlog_count','staff_access','release'])
...
assert(recovery.includes('const TARGET_SCHEMA = 72'),'recovery attestation must target schema 72');
...
console.log(JSON.stringify({
  ...
  migrations:'0001-0072_CONTINUOUS',
