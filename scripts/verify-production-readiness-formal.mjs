...
const EXPECTED_SCHEMA=68;
const migrations=readdirSync(path.join(root,'migrations')).filter((name)=>/^\d{4}_.+\.sql$/u.test(name)).sort();
assert(migrations.length===EXPECTED_SCHEMA,'repository migration count must match schema 68');
assert(migrations.every((name,index)=>Number(name.slice(0,4))===index+1),'Migration chain must be continuous');
assert(migrations.at(-1)==='0068_customer_security_deny_password_rate_limit.sql','latest migration must enforce Customer security DENY and password-change rate limiting');
...
for(const marker of ['const TARGET_SCHEMA=68','APP_RELEASE_SHA','production_recovery_attestations','operationalAlertAttestationReady','last_backlog_count','staff_access','release'])
...
assert(recovery.includes('const TARGET_SCHEMA=68'),'recovery attestation must target schema 68');
...
console.log(JSON.stringify({
  ...
  migrations:'0001-0068_CONTINUOUS',
