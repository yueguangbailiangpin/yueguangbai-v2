...
const EXPECTED_SCHEMA=19;
const migrations=readdirSync(path.join(root,'migrations')).filter((name)=>/^\d{4}_.+\.sql$/u.test(name)).sort();
assert(migrations.length===EXPECTED_SCHEMA,'repository migration count must match schema 19');
assert(migrations.every((name,index)=>Number(name.slice(0,4))===index+1),'Migration chain must be continuous');
assert(migrations.at(-1)==='0019_read_model_views.sql','latest baseline migration must be the read model views file');
...
for(const marker of ['const TARGET_SCHEMA = 19','APP_RELEASE_SHA','production_recovery_attestations','operationalAlertAttestationReady','last_backlog_count','staff_access','release'])
...
assert(recovery.includes('const TARGET_SCHEMA = 19'),'recovery attestation must target schema 19');
