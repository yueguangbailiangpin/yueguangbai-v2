...
export const PRODUCT_SCHEDULE_TIMEZONE = MARKETPLACE_RUNTIME_DEFINITIONS.AMAZON_JP.business_timezone;
...
export interface StaffProductDetailDto extends StaffProductListItemDto { versions:readonly StaffProductVersionDto[];demands:readonly StaffProductDemandDto[];timezone:ProductScheduleTimezone;data_as_of:number }
export interface DemandReviewContextDto { demand_batch_id:string;demand_version:number;status:'SUBMITTED';seller_organization_id:string;store_id:string;product_id:string;product_version_no:number;product_name:string;task_type:DemandTaskType;target_quantity:number;reservation_deadline:number;order_deadline:number;cadence:OrderCadenceDto|null;can_publish:boolean;timezone:ProductScheduleTimezone;data_as_of:number }
