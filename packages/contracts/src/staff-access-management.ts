...
export type StaffBindingInvitationStatus =
  | 'ISSUED'
  | 'CONSUMED'
  | 'CANCELLED'
  | 'EXPIRED';

export interface StaffAccessTeamOptionDto {
  team_id: string;
  team_name: string;
...
export interface StaffAccessEmployeeDto {
  ...
  feishu_binding: {
    status: StaffFeishuBindingStatus;
...
export interface StaffBindingInvitationDto {
  ...
  team: StaffAccessTeamOptionDto | null;
  status: StaffBindingInvitationStatus;
  version: number;
  issued_at: number;
  expires_at: number;
...
export interface StaffAccessManagementOverviewDto {
  ...
  invitations: readonly StaffBindingInvitationDto[];
  available_teams: readonly StaffAccessTeamOptionDto[];
}

export interface CreateStaffBindingInvitationRequest {
  display_name: string;
  role_code: StaffRoleCode;
  team_id: string | null;
}

export interface CreateStaffBindingInvitationResponse {
  invitation: StaffBindingInvitationDto;
  invitation_path: string | null;
  replayed: boolean;
}

export interface CancelStaffBindingInvitationRequest {
  expected_version: number;
}

export interface CancelStaffBindingInvitationResponse {
  invitation: StaffBindingInvitationDto;
  replayed: boolean;
}
...
export interface StaffBindingStartRequest {
  invite_token: string;
}

export interface StaffBindingStartResponse {
  provider: 'FEISHU';
