...
// and focused tests can compile while the active composition no longer exposes them.
...
export interface StaffBindingInvitationDto {invitation_id:string;display_name:string;role:{code:StaffRoleCode;display_name:StaffRoleDisplayName};team:StaffAccessTeamOptionDto|null;status:StaffBindingInvitationStatus;version:number;issued_at:number;expires_at:number;consumed_at:number|null;cancelled_at:number|null}
