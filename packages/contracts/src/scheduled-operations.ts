...
export const SCHEDULED_OPERATION_OUTCOMES = [
  'SUCCEEDED',
  'PARTIAL',
  'FAILED',
  'SKIPPED',
...
export const SCHEDULED_OPERATION_FAILURE_CATEGORIES = [
  'adapter_unavailable',
  'delivery_failed',
  'provider_rate_limited',
  'provider_unavailable',
  'contract_rejected',
  'file_cleanup_deferred',
  'reservation_expiry_failed',
  'job_item_failed',
...
export type ScheduledOperationFailureCategory =
  (typeof SCHEDULED_OPERATION_FAILURE_CATEGORIES)[number];
export const SCHEDULED_OPERATION_CAPABILITY_SCOPES = [
  'ALL_ENABLED_MARKETPLACES',
  'LEGACY_JP_ONLY',
  'HARD_DISABLED',
] as const;
...
export const SCHEDULED_OPERATIONAL_SIGNAL_TYPES = [
  'worker_5xx',
  'job_stale',
  'lease_stuck',
  'backlog_sustained',
...
export const SCHEDULED_OPERATIONAL_SIGNAL_CATEGORIES = [
  'worker',
  'scheduler',
  'file',
  'auth',
...
export type ScheduledOperationalSignalCategory =
  (typeof SCHEDULED_OPERATIONAL_SIGNAL_CATEGORIES)[number];
export const SCHEDULED_OPERATIONAL_SIGNAL_SEVERITIES = ['WARNING', 'CRITICAL'] as const;
export type ScheduledOperationalSignalSeverity =
  (typeof SCHEDULED_OPERATIONAL_SIGNAL_SEVERITIES)[number];
export const SCHEDULED_OPERATIONAL_SIGNAL_SUMMARY_CODES = [
...
export type ScheduledOperationalSignalSummaryCode =
  (typeof SCHEDULED_OPERATIONAL_SIGNAL_SUMMARY_CODES)[number];
export const SCHEDULED_OPERATIONAL_OBSERVATION_STATES = ['BREACH', 'HEALTHY'] as const;
export type ScheduledOperationalObservationState =
  (typeof SCHEDULED_OPERATIONAL_OBSERVATION_STATES)[number];
...
export type ScheduledOperationalNotificationKind =
  (typeof SCHEDULED_OPERATIONAL_NOTIFICATION_KINDS)[number];
...
export function isScheduledOperationJobName(value: unknown): value is ScheduledOperationJobName {
  return published(value, SCHEDULED_OPERATION_JOB_NAMES);
}
export function parseScheduledOperationManualRunCommand(
  value: unknown,
...
export function parseScheduledOperationRunDto(value: unknown): ScheduledOperationRunDto {
  ...
  if (
    !isScheduledOperationJobName(record['job_name']) ||
    !published(record['outcome'], SCHEDULED_OPERATION_OUTCOMES) ||
    !count(record['processed_count']) ||
    !count(record['succeeded_count']) ||
    !count(record['failed_count']) ||
    !count(record['backlog_count']) ||
    !(
      record['failure_category'] === null ||
      published(record['failure_category'], SCHEDULED_OPERATION_FAILURE_CATEGORIES)
    )
  )
    throw new Error('invalid_scheduled_operation_run');
  return {
...
export function parseScheduledOperationHealthDto(value: unknown): ScheduledOperationHealthDto {
  ...
  if (
    ...
    !nullableTimestamp(record['last_started_at']) ||
    !nullableTimestamp(record['last_succeeded_at']) ||
    !nullableTimestamp(record['last_failed_at']) ||
    !(
      record['last_failure_category'] === null ||
      published(record['last_failure_category'], SCHEDULED_OPERATION_FAILURE_CATEGORIES)
    ) ||
    !count(record['backlog_count']) ||
    !nullableTimestamp(record['lease_expires_at']) ||
    !published(record['capability_scope'], SCHEDULED_OPERATION_CAPABILITY_SCOPES)
  )
...
export function parseScheduledOperationCommandResultDto(
  value: unknown,
...
export function parseScheduledOperationalSignalObservationDto(
  ...
  if (
    !opaqueObservationId(record['observation_id']) ||
    ...
    !(record['job_name'] === null || isScheduledOperationJobName(record['job_name'])) ||
    !published(record['observation_state'], SCHEDULED_OPERATIONAL_OBSERVATION_STATES) ||
    !timestamp(record['observed_at']) ||
    !count(record['count_value']) ||
    (record['observation_state'] === 'BREACH'
      ? record['count_value'] < 1
      : record['count_value'] !== 0)
  )
    throw new Error('invalid_scheduled_operational_signal');
  return {
...
export function parseScheduledOperationalAlertNotificationDto(
  value: unknown,
  ...
  if (
    !published(record['signal_type'], SCHEDULED_OPERATIONAL_SIGNAL_TYPES) ||
    !published(record['category'], SCHEDULED_OPERATIONAL_SIGNAL_CATEGORIES) ||
    !published(record['severity'], SCHEDULED_OPERATIONAL_SIGNAL_SEVERITIES) ||
    !published(record['summary_code'], SCHEDULED_OPERATIONAL_SIGNAL_SUMMARY_CODES) ||
    !(record['job_name'] === null || isScheduledOperationJobName(record['job_name'])) ||
    !published(record['notification_kind'], SCHEDULED_OPERATIONAL_NOTIFICATION_KINDS) ||
    (record['status'] !== 'OPEN' && record['status'] !== 'RESOLVED') ||
    (record['notification_kind'] === 'RESOLVED') !== (record['status'] === 'RESOLVED') ||
    !timestamp(record['observed_at']) ||
    !count(record['incident_version']) ||
    !count(record['count_value'])
  )
    throw new Error('invalid_scheduled_operational_notification');
  return {
    signal_type: record['signal_type'],
...
export function parseScheduledOperationalAlertDto(value: unknown): ScheduledOperationalAlertDto {
  ...
  if (
    ...
    !published(record['summary_code'], SCHEDULED_OPERATIONAL_SIGNAL_SUMMARY_CODES) ||
    !(record['job_name'] === null || isScheduledOperationJobName(record['job_name'])) ||
    !published(record['status'], SCHEDULED_OPERATIONAL_ALERT_STATUSES) ||
    !nullableTimestamp(record['first_seen_at']) ||
    !nullableTimestamp(record['last_seen_at']) ||
    !count(record['consecutive_breach_count']) ||
    !count(record['consecutive_healthy_count']) ||
    !count(record['window_count_value']) ||
    !positiveCount(record['threshold_count']) ||
    !positiveCount(record['threshold_window_ms']) ||
    !positiveCount(record['recovery_count']) ||
    !nullableTimestamp(record['opened_at']) ||
    !nullableTimestamp(record['acknowledged_at']) ||
    !nullableTimestamp(record['resolved_at']) ||
    !nullableTimestamp(record['cooldown_until']) ||
    !nullableTimestamp(record['suppressed_until']) ||
    !nullableTimestamp(record['last_notification_at']) ||
    !count(record['incident_version']) ||
    !timestamp(record['updated_at']) ||
    record['time_basis'] !== 'UTC_MS' ||
    record['display_timezone'] !== 'Asia/Shanghai' ||
    (record['status'] === 'OPEN' && record['opened_at'] === null) ||
    (record['status'] === 'ACKNOWLEDGED' &&
      (record['opened_at'] === null || record['acknowledged_at'] === null)) ||
    (record['status'] === 'RESOLVED' && record['resolved_at'] === null)
  )
    throw new Error('invalid_scheduled_operational_alert');
  return {
    signal_type: record['signal_type'],
    category: record['category'],
    severity: record['severity'],
    ...
    consecutive_breach_count: record['consecutive_breach_count'],
    consecutive_healthy_count: record['consecutive_healthy_count'],
    window_count_value: record['window_count_value'],
    threshold_count: record['threshold_count'],
    threshold_window_ms: record['threshold_window_ms'],
...
export function parseScheduledOperationalAlertAckCommandDto(
  value: unknown,
): ScheduledOperationalAlertAckCommandDto {
  const record = exactRecord(value, [
    'signal_type',
    'summary_code',
    'job_name',
  ...
  if (
    ...
    !published(record['summary_code'], SCHEDULED_OPERATIONAL_SIGNAL_SUMMARY_CODES) ||
    !(record['job_name'] === null || isScheduledOperationJobName(record['job_name'])) ||
    !positiveCount(record['incident_version'])
  )
    throw new Error('invalid_scheduled_operational_alert_ack');
  return {
    signal_type: record['signal_type'],
    summary_code: record['summary_code'],
    job_name: record['job_name'],
...
export function parseScheduledOperationalAlertAckResultDto(
  value: unknown,
  ...
  if (
    ...
    throw new Error('invalid_scheduled_operational_alert_ack_result');
  return {
    signal_type: record['signal_type'],
    summary_code: record['summary_code'],
    job_name: record['job_name'],
...
function exactRecord(value: unknown, keys: readonly string[]): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('invalid_scheduled_operation_contract');
