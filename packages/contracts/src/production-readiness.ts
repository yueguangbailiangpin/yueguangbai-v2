...
export type ReleaseGateStatus = 'LOCAL_PASS' | 'OWNER_ACTION_REQUIRED' | 'PRODUCTION_GO_BLOCKED';
...
export const PRODUCTION_ALERT_CONTROLS: readonly ProductionAlertControl[] = [
  {
    signal: 'worker_5xx',
    threshold: '5m>=3',
    diagnostic: 'request-id/status aggregate',
    kill_switch: 'deployment rollback',
    recovery: 'compatible Worker switch or forward fix',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'login_anomaly',
    threshold: '10m>=5',
    diagnostic: 'opaque security-event aggregate',
    kill_switch: 'affected auth/provider disable',
    recovery: 'session revoke and provider verification',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'job_stale_or_backlog',
    threshold: 'stale>=6h or 3 breaches/30m',
    diagnostic: 'job health/lease/backlog',
    kill_switch: 'global or per-job disable',
    recovery: 'lease takeover and bounded replay',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'file_integrity',
    threshold: '30m>=3 or any checksum mismatch',
    diagnostic: 'D1 authority plus protected manifest refs',
    kill_switch: 'file mutation/archive delete disable',
    recovery: 'verified compensation or rehydration',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'drive_dependency',
    threshold: 'any authorization/manifest failure',
    diagnostic: 'safe provider category and manifest hash',
    kill_switch: 'copy/proxy/delete disable',
    recovery: 'OAuth repair then read-back verification',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'cloudflare_access_dependency',
    threshold: '15m>=3',
    diagnostic: 'safe Access authentication category',
    kill_switch: 'Cloudflare Access policy or deployment rollback',
    recovery: 'verify Access policy/JWKS then reauthenticate',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'mcp_dependency',
    threshold: 'any auth/audit outage or sustained failures',
    diagnostic: 'safe tool/outcome aggregate',
    kill_switch: 'global or per-tool disable',
    recovery: 'Web remains authoritative; re-enable after conformance',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'd1_dependency',
    threshold: 'any integrity/FK failure or sustained errors',
    diagnostic: 'integrity, FK and request aggregate',
    kill_switch: 'stop new writes/deployment rollback',
    recovery: 'isolated verified restore or forward recovery',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'r2_dependency',
    threshold: 'any protected read/write/delete failure burst',
    diagnostic: 'safe storage operation category',
    kill_switch: 'upload/archive-delete disable',
    recovery: 'retry only after HEAD/checksum validation',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
  {
    signal: 'capacity',
    threshold: '>=80% approved D1/R2/Drive/provider envelope',
    diagnostic: 'anonymous counts and provider quotas',
    kill_switch: 'pause intake/background expansion',
    recovery: 'drain bounded backlog and owner capacity decision',
    escalation: 'PROVIDER_INDEPENDENT_REQUIRED',
  },
] as const;
...
export function validateProductionAlertControls(controls: readonly ProductionAlertControl[]): void {
  const required = [
    'worker_5xx',
    'login_anomaly',
    'job_stale_or_backlog',
    'file_integrity',
    'drive_dependency',
    'cloudflare_access_dependency',
    'mcp_dependency',
    'd1_dependency',
    'r2_dependency',
    'capacity',
  ...
  if (
    controls.length !== required.length ||
