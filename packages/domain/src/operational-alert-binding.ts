...
export function operationalAlertDescriptorFromService(
  value: unknown,
): OperationalAlertBindingDescriptor | null {
  const service = record(value);
  if (!service || !allowedKeys(service, ['binding', 'service', 'props'], ['entrypoint']))
    return null;
  ...
  if (
    !propTarget ||
    props['entrypoint'] !== entrypoint.mirror ||
    !identity ||
    !version ||
    propTarget !== target
  )
    return null;
  return {
    binding: OPERATIONAL_ALERT_BINDING_NAME,
    service_target: target,
    entrypoint: entrypoint.canonical,
    props: {
      service_target: propTarget,
      entrypoint: entrypoint.mirror,
...
export function operationalAlertDescriptorFromRuntime(value: {
  serviceTarget: unknown;
  entrypoint: unknown;
  sinkIdentity: unknown;
  sinkDeploymentVersion: unknown;
}): OperationalAlertBindingDescriptor | null {
  const target = resourceName(value.serviceTarget),
    entrypoint = runtimeEntrypoint(value.entrypoint);
  const identity = safeIdentifier(value.sinkIdentity, 8, 200),
    version = safeIdentifier(value.sinkDeploymentVersion, 7, 200);
...
export function runtimeEntrypointValue(value: string | null): string {
  return value ?? DEFAULT_OPERATIONAL_ALERT_ENTRYPOINT;
}

interface CanonicalEntrypoint {
...
function serviceEntrypoint(service: Record<string, unknown>): CanonicalEntrypoint | null {
  if (!Object.hasOwn(service, 'entrypoint'))
    return { canonical: null, mirror: DEFAULT_OPERATIONAL_ALERT_ENTRYPOINT };
...
function runtimeEntrypoint(value: unknown): CanonicalEntrypoint | null {
  if (value === DEFAULT_OPERATIONAL_ALERT_ENTRYPOINT)
    return { canonical: null, mirror: DEFAULT_OPERATIONAL_ALERT_ENTRYPOINT };
  const named = namedEntrypoint(value);
  return named ? { canonical: named, mirror: named } : null;
}
function namedEntrypoint(value: unknown): string | null {
  return typeof value === 'string' &&
    value !== DEFAULT_OPERATIONAL_ALERT_ENTRYPOINT &&
    value.length <= 128 &&
    !/REQUIRED|REPLACE|PLACEHOLDER|CHANGEME|TODO/iu.test(value) &&
...
function safeIdentifier(value: unknown, min: number, max: number): string | null {
  return typeof value === 'string' &&
    value.length >= min &&
    value.length <= max &&
    !/REQUIRED|REPLACE|PLACEHOLDER|CHANGEME|TODO/iu.test(value) &&
