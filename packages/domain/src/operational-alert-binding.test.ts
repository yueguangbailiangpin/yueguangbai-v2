...
import {
  operationalAlertDescriptorFromRuntime,
  operationalAlertDescriptorFromService,
  parseExactGitCommitSha,
  runtimeEntrypointValue,
} from './operational-alert-binding';
...
describe('operational alert binding descriptor', () => {
  it('normalizes the exact rendered service descriptor and canonical default entrypoint', () => {
    const service = {
      ...
      service: 'alerts-primary',
      props: {
        sink_deployment_version: 'deploy-001',
        entrypoint: 'default',
        sink_identity: 'service:alerts-primary',
        service_target: 'alerts-primary',
      },
    };
    const descriptor = operationalAlertDescriptorFromService(service);
    expect(descriptor).not.toBeNull();
    expect(canonicalJson(descriptor)).toBe(
      '{"binding":"OPERATIONAL_ALERT_SINK","entrypoint":null,"props":{"entrypoint":"default","service_target":"alerts-primary","sink_deployment_version":"deploy-001","sink_identity":"service:alerts-primary"},"service_target":"alerts-primary"}',
    );
    expect(
      operationalAlertDescriptorFromRuntime({
        serviceTarget: 'alerts-primary',
        entrypoint: 'default',
        sinkIdentity: 'service:alerts-primary',
  ...
  it('distinguishes omitted default, named and invalid or missing entrypoint forms', async () => {
    ...
    const dollar = operationalAlertDescriptorFromService({
      binding: 'OPERATIONAL_ALERT_SINK',
      service: 'alerts-primary',
      entrypoint: '$sink',
      props,
    });
    const underscoreDollar = operationalAlertDescriptorFromService({
      binding: 'OPERATIONAL_ALERT_SINK',
      service: 'alerts-primary',
      entrypoint: '_$sink',
      props: { ...props, entrypoint: '_$sink' },
    });
  ...
  it('rejects extra props, mismatched mirrors, placeholders and non-exact release SHAs', () => {
    const base = {
      binding: 'OPERATIONAL_ALERT_SINK',
      service: 'alerts-primary',
      entrypoint: 'AlertSinkEntrypoint',
      props: {
        service_target: 'alerts-primary',
        entrypoint: 'AlertSinkEntrypoint',
        sink_identity: 'service:alerts-primary',
    ...
    expect(
      operationalAlertDescriptorFromService({ ...base, props: { ...base.props, extra: true } }),
    ).toBeNull();
    expect(
      operationalAlertDescriptorFromService({
        ...base,
        props: { ...base.props, service_target: 'alerts-other' },
      }),
    ).toBeNull();
    expect(
      operationalAlertDescriptorFromRuntime({
        serviceTarget: 'REQUIRED_SERVICE',
        entrypoint: 'default',
        sinkIdentity: 'service:alerts-primary',
        sinkDeploymentVersion: 'deploy-001',
      }),
    ...
    for (const value of [
      ...
      expect(parseExactGitCommitSha(value)).toBeNull();
  });
});
