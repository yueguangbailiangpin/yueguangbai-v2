...
Staging SHALL report Scheduler, Acquisition Maintenance, operational alerts and production recovery as `not_required`, not `ok`. It SHALL return ready only when Schema 67, isolated object storage, valid staging Access configuration and exact release SHA are `ok`, while every production-only switch remains disabled. Production SHALL continue to require all eight published checks to equal `ok`.

#### Scenario: Disabled staging production gates are intentional

- **WHEN** the staging release has exact disabled Scheduler, Acquisition Maintenance and alert configuration plus valid required staging evidence
- **THEN** `/ready` returns 200 with four explicit `not_required` checks and four `ok` checks.

#### Scenario: Production or unknown environment is evaluated

- **WHEN** production lacks any one `ok` check, an unknown environment is supplied, or staging enables a production-only switch
