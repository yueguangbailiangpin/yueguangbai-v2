...
- [ ] 9. **T8 — staging basic activation:** under separate explicit operator authorization, create only isolated staging resources, apply migrations 0001–0070, bootstrap the first Owner and synthetic Buyer channel, inject staging-only managed Secrets, deploy the independently reviewed fixed SHA, and record only the baseline `/health`, `/ready`, binding, isolation and disabled-capability evidence. Do not record the A–H acceptance matrix or recovery evidence here.
- [ ] 10. **T9 — staging A–H acceptance:** execute and record the 67 real staging acceptance items across sections A–H as a separate evidence change after T8. Do not create resources, alter the T8 deployment contract, or include backup/restore results here.
