...
- [ ] 2.7 删除旧别名与兼容层：`marketplace_legacy_aliases` 运行引用、旧 Seller Agreement 投影残留、无用途 Feature Flag
- [ ] 2.8 删除 Rakuten/TikTok `platform_*` 运行实现与 `marketplace-adapters` 预备（保留 Registry/AMAZON_JP/禁用边界）
- [ ] 2.9 每删除类别后 typecheck + 受影响测试通过；被删能力 verifier 按 §7 留核验记录
