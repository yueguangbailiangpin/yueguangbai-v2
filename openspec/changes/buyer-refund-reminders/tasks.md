...
- [x] 1.3 Update local migration guards/inventory and add Schema 69→70 wrong-order, repeat, dirty-state, constraint, index, and trigger evidence.

## 2. Contracts and command

- [x] 2.1 Add Buyer and Staff-safe reminder DTO fields without Seller projection changes.
- [x] 2.2 Implement the session-derived owned DUE/PARTIALLY_PAID reminder command with idempotency, conditional 24-hour insert, `changes()=1`, Audit, and fail-closed rate-limit behavior.
...
- [x] 4.1 Add real local-D1 coverage for ownership concealment, idempotency replay, rate-limit rollback, immutable Audit, and migration guards.
