...
- Add a one-time, parameterized and atomic operator tool for the first Owner and synthetic Buyer registration channel on a completely empty Schema 70 staging D1.
...
- This staging bootstrap change adds no schema change and does not modify migrations 0001–0070.
...
`NO_SCHEMA_CHANGE`. Current Schema 70 already contains Staff identity, role, authorization event, Audit, idempotency and transaction assertion facts required for a governed first-owner bootstrap.
