# Seller Complete Business Loop Capability
...
The system SHALL return order amounts as decimal-string integer minor units with explicit currency and exponent, SHALL return Seller CNY facts as decimal-string fen, and SHALL return the immutable Seller agreement-rate and service-fee snapshots used by the order.

#### Scenario: USD order display

- **WHEN** a Seller reads an Amazon US formal order
- **THEN** the response identifies USD minor units, CNY quote currency, integer rate value/scale, rounding rule and snapshot version without floating-point calculation.

#### Scenario: Legacy JP order display

- **WHEN** a Seller reads a migrated JP order
- **THEN** generic fields and retained JP compatibility fields describe the same immutable values.

### Requirement: Seller agreement rates are read-only to Seller

The system SHALL allow a Seller to view the organization-and-business-currency rate snapshot used by an order and SHALL provide no Seller route that submits, confirms or edits Staff-controlled rate facts.

#### Scenario: Seller views locked rate

- **WHEN** a Seller opens an authorized formal order
- **THEN** the exact locked rate version and confirmation time are displayed read-only.

#### Scenario: Seller attempts rate mutation

- **WHEN** a Seller calls a Staff rate command or supplies rate fields in a Seller command
- **THEN** the request is rejected or ignored by a strict allowlist and no rate fact changes.
