...
The third clean-candidate run passed every non-browser sub-gate and then found that an unrelated existing local process owned Playwright's fixed port 4174. No process was terminated. Playwright now accepts a validated loopback port, and `release:check` uses isolated port 4188 by default (or an explicit `RELEASE_BROWSER_PORT`) while retaining `reuseExistingServer: false`. This changes only local test-server selection; browser assertions and application behavior are unchanged.
