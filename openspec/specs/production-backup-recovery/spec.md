...
- **THEN** `app_schema_state.schema_version=64`, full schema inventory, table row counts, `integrity_check`, `foreign_key_check`, key financial aggregates, and Staff/Buyer/Seller/order/file/scheduler/acquisition smoke reads match the backup evidence.
...
- **WHEN** `/ready` evaluates the running Schema 64 release
- **THEN** recovery passes only when an attestation exists with `schema_version=64` and `release_sha=APP_RELEASE_SHA`; older schema or another release SHA is insufficient.
...
- **WHEN** Schema 64 is current but the latest recovery attestation release SHA differs from `APP_RELEASE_SHA`
