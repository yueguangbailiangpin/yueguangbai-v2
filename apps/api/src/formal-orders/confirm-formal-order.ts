...
} from '../pricing/buyer-daily-exchange-rates';
import {
  resolveSellerAgreementRate,
} from '../pricing/seller-agreement-rates';
import {
  resolveSellerServiceFee,
} from '../pricing/seller-service-fees';
import {
...
export async function confirmFormalOrder(
  ...
  const submissionId = cleanFormalOrderIdentifier(
    input.orderEvidenceSubmissionId,
  );
  const expectedVersion = cleanFormalOrderExpectedVersion(
  ...
  const acquired = await acquireIdempotency<ConfirmFormalOrderResult>(
    database,
    {
      actorType: 'STAFF',
  ...
  try {
    ...
    const buyerRateValue = parseCnyPerJpyE8(
      buyerRate.cny_per_jpy_e8,
    );
    const sellerRateValue = parseCnyPerJpyE8(
    ...
    const statements: SqlStatement[] = [
      ...
      database.prepare(`
        INSERT INTO formal_order_financial_snapshots (
          ...
          formal_order_id,
          snapshot_version,
          buyer_rate_version_id,
          buyer_rate_version_no,
          buyer_rate_business_date,
          buyer_rate_confirmed_at,
...
async function requireFormalOrderSource(
  ...
  const row = await database.prepare(`
    ...
    JOIN buyer_customers buyer
      ON buyer.id=submission.buyer_customer_id
    JOIN buyer_channels channel
      ON channel.id=buyer.buyer_channel_id
  ...
  if (!row) {
    throw new FormalOrderError('ORDER_EVIDENCE_NOT_FOUND', 404);
  }
...
function assertFormalOrderConfirmedStatement(
  ...
  return database.prepare(`
    ...
    SELECT CASE WHEN
      ...
      AND EXISTS (
        SELECT 1
        FROM formal_order_events event
      ...
      AND EXISTS (
        ...
        FROM order_evidence_submissions submission
        WHERE submission.id=?
          AND submission.status='CONSUMED'
          AND submission.version=?
          AND submission.consumed_at=?
      )
      AND EXISTS (
        SELECT 1
        FROM buyer_customers buyer
        WHERE buyer.id=?
          AND buyer.buyer_customer_no=?
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE channel.id=?
            AND channel.next_sequence=?
            AND channel.version=?
            AND event.buyer_customer_id=?
            AND event.buyer_customer_no=?
            AND event.buyer_sequence=?
        )
      )
      AND EXISTS (
        SELECT 1
  ...
  `).bind(
    ...
    buyerNumber.allocated
      ? source.buyer_version + 1
      : source.buyer_version,
