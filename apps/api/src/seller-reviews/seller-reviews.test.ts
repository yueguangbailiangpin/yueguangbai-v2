...
describe('Phase 4C3 route and schema guardrails', () => {
  it('reuses seller session ACTIVE checks and exposes only declared routes', () => {
    ...
    expect(actorSource).toContain(
      "session.accountType !== 'SELLER_MEMBER'",
    );
    expect(actorSource).toContain("row.member_status !== 'ACTIVE'");
    expect(actorSource).toContain("row.organization_status !== 'ACTIVE'");
  });

  it('keeps schema18 and does not add refund, settlement, or profit storage', () => {
    const root = path.resolve(import.meta.dirname, '../../../..');
    ...
    expect(migrations.at(-1)).toBe('0019_product_ordering_profiles.sql');
