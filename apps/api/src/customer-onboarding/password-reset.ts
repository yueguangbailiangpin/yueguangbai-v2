...
import { deriveOneTimeToken, hashCanonicalJson, hashOneTimeToken } from '@ygb/domain';
...
export async function issueCustomerPasswordResetForSubject(
  ...
  if (verificationNote.length < 8 || verificationNote.length > 1000)
  ...
  const acquired = await acquireIdempotency<{
    reset_id: string;
    expires_at: number;
    affected_personas: readonly string[];
  }>(
    database,
    {
      actorType: 'STAFF',
  ...
  const wechatHash = await registrationPrivacyHash(
    command.tokenSecret,
    'WECHAT_ID',
    target.login_identifier_normalized,
  );
  try {
    await database.batch([
      database
        ...
        .bind(
          ...
          command.requestId,
          command.idempotencyKey,
          JSON.stringify({ customer_type: input.customerType, subject_id: input.subjectId }),
          now,
          target.account_id,
        ),
      database
        ...
        .bind(now, actor.staffId, now, target.account_id),
      database
        .prepare(
      ...
      database
        ...
        id,reset_token_id,account_id,event_type,outcome,actor_type,actor_id,reason_code,
        request_id,idempotency_key,metadata_json,created_at
      ) VALUES(?,?,?,'ISSUED','SUCCESS','STAFF',?,NULL,?,?,?,?)`,
        )
        .bind(
          crypto.randomUUID(),
          resetId,
      ...
      createAuditEventStatement(database, {
        ...
        aggregateType: 'CUSTOMER_PASSWORD_RESET',
        aggregateId: resetId,
        eventType: 'SCOPED_CUSTOMER_PASSWORD_RESET_ISSUED',
      ...
      completeIdempotencyStatement(database, acquired.claim, safe, {
        resultReferences: { reset_id: resetId, account_id: target.account_id },
        now,
      }),
      assertIdempotencyCompletionStatement(database, acquired.claim),
    ]);
  } catch (error) {
    await markIdempotencyFailed(database, acquired.claim, 'SCOPED_RESET_ISSUE_FAILED', now);
    throw error;
  }
...
async function resolveAccountTarget(
  ...
  const rows = await database
    ...
    ORDER BY member.member_number,member.id`,
    )
    .bind(subjectId)
    .all<AccountTarget>();
  if (rows.results.length !== 1)
    throw new CustomerSecurityError(
