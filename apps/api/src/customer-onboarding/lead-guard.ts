    ...
    try {
      ...
      const exists =
        type === 'BUYER'
          ? await context.env.DB.prepare(
              `SELECT buyer.id FROM wechat_identity_claims claim
      ...
      if (exists) {
        ...
        return context.json(
          apiFailure(
            'CONFLICT',
            '该微信在当前站点已经对应历史客户，请使用“历史客户开通账号”，不要重复新增',
      ...
      /* Validation remains owned by the canonical acquisition route. */
    }
    return next();
  });
}
