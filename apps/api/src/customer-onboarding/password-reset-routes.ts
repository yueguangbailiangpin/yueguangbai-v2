...
export function registerScopedCustomerPasswordResetRoutes(app: Hono<AppEnv>): void {
  app.post(
    '/api/staff/customer-onboarding/:customerType/:subjectId/password-reset',
    ...
    async (context) => {
      ...
      try {
        ...
        if (Object.keys(record).length !== 1 || typeof record['verification_note'] !== 'string')
          throw new CustomerSecurityError('VALIDATION_ERROR', 400);
        let idempotencyKey: string | null = null;
        try {
          idempotencyKey = parseIdempotencyKey(context.req.header('Idempotency-Key'));
        } catch {
          throw new CustomerSecurityError('VALIDATION_ERROR', 400);
        }
        ...
        const result = await issueCustomerPasswordResetForSubject(
          context.env.DB,
          actor,
          {
            customerType: type,
