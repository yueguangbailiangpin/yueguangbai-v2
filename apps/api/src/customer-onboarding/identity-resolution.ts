...
export async function reportIdentityResolutionCase(
  ...
  await requireMarket(database, actor, input.marketplaceCode);
  const wechat = normalizeWechatId(input.wechatId);
  const identityHash = await hashNormalizedWechat(wechat.normalized, secret);
  const existing = await database
    .prepare(
      `SELECT id,status FROM customer_identity_resolution_cases
    WHERE identity_hash=? AND customer_type=? AND marketplace_code=? AND status='OPEN' LIMIT 1`,
    )
    .bind(identityHash, input.customerType, input.marketplaceCode)
  ...
  await database.batch([
    database
      .prepare(
        `INSERT INTO customer_identity_resolution_cases(
    ...
    database
      .prepare(
        `INSERT INTO customer_identity_resolution_events(id,case_id,event_type,actor_staff_id,subject_id,reason,created_at)
      VALUES(?,?,'REPORTED',?,NULL,?,?)`,
      )
      .bind(crypto.randomUUID(), id, actor.staffId, note, now),
    createAuditEventStatement(database, {
      id: crypto.randomUUID(),
      aggregateType: 'CUSTOMER_IDENTITY_RESOLUTION',
      aggregateId: id,
      eventType: 'CUSTOMER_IDENTITY_CONFLICT_REPORTED',
...
export async function listIdentityResolutionCases(
  database: SqlDatabase,
  actor: AssignmentStaffAuthorization,
) {
  requireOwner(actor);
  const rows = await database
    .prepare(
      `SELECT id,identity_masked,customer_type,marketplace_code,reason_code,staff_note,status,
      reported_by_staff_id,resolved_subject_id,resolution_note,resolved_by_staff_id,created_at,resolved_at
...
export async function searchIdentityResolutionCandidates(
  database: SqlDatabase,
  actor: AssignmentStaffAuthorization,
  input: { customerType: CustomerType; query: string },
) {
  requireOwner(actor);
  const query = input.query.normalize('NFKC').trim();
  if (query.length < 2 || query.length > 120) validation();
  ...
  if (input.customerType === 'BUYER') {
    const rows = await database
      ...
      ORDER BY buyer.updated_at DESC,buyer.id LIMIT 30`,
      )
      .bind(query, like, like)
      .all<any>();
    return Object.freeze(
      rows.results.map((row) =>
        Object.freeze({
          ...
          reference_code: row.reference_code === null ? null : String(row.reference_code),
  ...
  const rows = await database
    ...
    ORDER BY organization.updated_at DESC,organization.id LIMIT 30`,
    )
...
export async function resolveIdentityResolutionCase(
  database: SqlDatabase,
  actor: AssignmentStaffAuthorization,
  input: { caseId: string; subjectId: string; reason: string },
) {
  requireOwner(actor);
  const reason = required(input.reason, 1000);
  const current = await readCase(database, input.caseId);
  ...
  const row = await database
    .prepare(`SELECT identity_hash FROM customer_identity_resolution_cases WHERE id=?`)
    .bind(input.caseId)
  ...
  await database.batch([
    database
      .prepare(
        `INSERT INTO customer_identity_manual_bindings(
    ...
    database
      .prepare(
        `UPDATE customer_identity_resolution_cases SET status='RESOLVED',resolved_subject_id=?,resolution_note=?,
      resolved_by_staff_id=?,resolved_at=? WHERE id=? AND status='OPEN'`,
      )
      .bind(input.subjectId, reason, actor.staffId, now, input.caseId),
    database
      .prepare(
        `INSERT INTO customer_identity_resolution_events(id,case_id,event_type,actor_staff_id,subject_id,reason,created_at)
      VALUES(?,?,'RESOLVED',?,?,?,?)`,
      )
      .bind(crypto.randomUUID(), input.caseId, actor.staffId, input.subjectId, reason, now),
    createAuditEventStatement(database, {
      id: crypto.randomUUID(),
      aggregateType: 'CUSTOMER_IDENTITY_RESOLUTION',
      aggregateId: input.caseId,
      eventType: 'CUSTOMER_IDENTITY_CONFLICT_RESOLVED',
      actor: { type: 'STAFF', id: actor.staffId, roles: [...actor.roles] },
      requestId: null,
      idempotencyKey: null,
      previousState: { status: 'OPEN' },
      nextState: { status: 'RESOLVED', subject_id: input.subjectId, binding_id: bindingId },
      reason,
      createdAt: now,
    }),
  ]);
...
export async function manualIdentityBinding(
  database: SqlDatabase,
  secret: string,
  input: { customerType: CustomerType; marketplaceCode: string; wechatId: string },
): Promise<string | null> {
  const normalized = normalizeWechatId(input.wechatId);
  const hash = await hashNormalizedWechat(normalized.normalized, secret);
  const row = await database
    .prepare(
      `SELECT subject_id FROM customer_identity_manual_bindings
    WHERE identity_hash=? AND customer_type=? AND marketplace_code=? AND status='ACTIVE' LIMIT 1`,
    )
    .bind(hash, input.customerType, input.marketplaceCode)
...
async function readCase(database: SqlDatabase, id: string): Promise<any> {
  const row = await database
    .prepare(
      `SELECT id,identity_masked,customer_type,marketplace_code,reason_code,staff_note,status,
    reported_by_staff_id,resolved_subject_id,resolution_note,resolved_by_staff_id,created_at,resolved_at
    FROM customer_identity_resolution_cases WHERE id=?`,
    )
    .bind(clean(id))
    .first<any>();
  if (!row) notFound();
...
function project(row: any) {
  return Object.freeze({
    id: String(row.id),
    identity_masked: String(row.identity_masked),
    customer_type: row.customer_type as CustomerType,
    marketplace_code: String(row.marketplace_code),
    reason_code: String(row.reason_code),
    staff_note: row.staff_note === null ? null : String(row.staff_note),
    status: row.status as 'OPEN' | 'RESOLVED' | 'CANCELLED',
    reported_by_staff_id: String(row.reported_by_staff_id),
    resolved_subject_id: row.resolved_subject_id === null ? null : String(row.resolved_subject_id),
    resolution_note: row.resolution_note === null ? null : String(row.resolution_note),
    resolved_by_staff_id:
      row.resolved_by_staff_id === null ? null : String(row.resolved_by_staff_id),
...
async function assertSubject(
  ...
  const row =
    type === 'BUYER'
      ? await database
          .prepare(
            `SELECT buyer.id,COALESCE(assignment.marketplace_code,'AMAZON_JP') AS market FROM buyer_customers buyer
      LEFT JOIN buyer_marketplace_assignments assignment ON assignment.buyer_customer_id=buyer.id WHERE buyer.id=?`,
          )
          .bind(clean(id))
          .first<any>()
      : await database
      ...
      FROM seller_organizations WHERE id=?`,
          ...
          .bind(clean(id))
          .first<any>();
  if (!row || String(row.market) !== market)
    throw new IdentityResolutionError('VALIDATION_ERROR', 400);
}
function requireDuty(actor: AssignmentStaffAuthorization, type: CustomerType) {
  const okay =
    actor.roles.has('owner') ||
    (type === 'BUYER' && actor.roles.has('pre_sales')) ||
    (type === 'SELLER' && actor.roles.has('seller_ops'));
  if (!okay) forbidden();
}
async function requireMarket(
  ...
  if (!markets.includes(market)) forbidden();
}
function requireOwner(actor: AssignmentStaffAuthorization) {
  if (!actor.roles.has('owner') || !actor.permissions.has('STAFF_MANAGE')) forbidden();
}
function optional(value: string | null, max: number) {
...
function required(value: string, max: number) {
  const v = optional(value, max);
  if (v === null || v.length < 3) validation();
  return v;
}
...
function forbidden(): never {
  throw new IdentityResolutionError('FORBIDDEN', 403);
}
function notFound(): never {
  throw new IdentityResolutionError('NOT_FOUND', 404);
}
function conflict(): never {
  throw new IdentityResolutionError('CONFLICT', 409);
}
