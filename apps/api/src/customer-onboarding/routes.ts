...
export function registerCustomerOnboardingRoutes(app:Hono<any>):void{
  app.get('/api/staff/customer-onboarding/lookup',async(context)=>{
    ...
    try{
      ...
      const direct=type==='BUYER'?await buyerMatches(context.env.DB,wechat.normalized,markets):await sellerMatches(context.env.DB,wechat.normalized,markets);
      const manual=await manualMatches(context.env.DB,type,wechat.normalized,markets,securitySecret(context));
      const merged=mergeMatches(direct,manual);
      const resolutionRequired=hasAmbiguousMarketplace(merged);
      context.header('Cache-Control','no-store');return context.json(apiSuccess({
        matches:merged,resolution_required:resolutionRequired,manual_resolution_applied:manual.length>0,
      },requestId));
    }catch(error){
...
async function buyerMatches(database:SqlDatabase,wechat:string,markets:readonly string[]|null):Promise<CustomerMatch[]>{
  ...
  return rows.results.filter((row)=>markets===null||markets.includes(row.marketplace_code??'AMAZON_JP')).map((row)=>({
    customer_type:'BUYER',subject_id:row.subject_id,display_name:row.display_name,
    marketplace_code:row.marketplace_code??'AMAZON_JP',has_portal_account:row.account_id!==null,
    historical_order_count:Number(row.formal_order_count),source_status:'HISTORICAL_UNKNOWN',
  }));
}

async function sellerMatches(database:SqlDatabase,wechat:string,markets:readonly string[]|null):Promise<CustomerMatch[]>{
  ...
  return [...dedup.values()].filter((row)=>{const canonical=row.marketplace_code==='JP'?'AMAZON_JP':row.marketplace_code;return markets===null||markets.includes(canonical);}).map((row)=>({
    customer_type:'SELLER',subject_id:row.subject_id,display_name:row.display_name,
    marketplace_code:row.marketplace_code==='JP'?'AMAZON_JP':row.marketplace_code,has_portal_account:row.account_id!==null,
    historical_order_count:Number(row.formal_order_count),source_status:'HISTORICAL_UNKNOWN',
  }));
}

async function manualMatches(database:SqlDatabase,type:'BUYER'|'SELLER',wechat:string,markets:readonly string[]|null,secret:string):Promise<CustomerMatch[]>{
  const hash=await hashNormalizedWechat(wechat,secret);const where=markets&&markets.length?`AND binding.marketplace_code IN (${markets.map(()=>'?').join(',')})`:'';
  const rows=await database.prepare(`SELECT binding.subject_id,binding.marketplace_code FROM customer_identity_manual_bindings binding
    WHERE binding.identity_hash=? AND binding.customer_type=? AND binding.status='ACTIVE' ${where}`)
    .bind(hash,type,...(markets??[])).all<{subject_id:string;marketplace_code:string}>();
  const result:CustomerMatch[]=[];
  for(const row of rows.results){
    const match=type==='BUYER'?await buyerBySubject(database,row.subject_id,row.marketplace_code):await sellerBySubject(database,row.subject_id,row.marketplace_code);
    if(match)result.push(match);
  }
  return result;
}
async function buyerBySubject(database:SqlDatabase,id:string,market:string):Promise<CustomerMatch|null>{
  const row=await database.prepare(`SELECT buyer.id AS subject_id,buyer.display_name,
    ...
    WHERE buyer.id=? AND buyer.access_status='ACTIVE'`).bind(id).first<any>();
  return row?{customer_type:'BUYER',subject_id:String(row.subject_id),display_name:String(row.display_name),marketplace_code:market,
    has_portal_account:row.account_id!==null,historical_order_count:Number(row.formal_order_count),source_status:'HISTORICAL_UNKNOWN'}:null;
}
async function sellerBySubject(database:SqlDatabase,id:string,market:string):Promise<CustomerMatch|null>{
  const row=await database.prepare(`SELECT organization.id AS subject_id,organization.organization_name AS display_name,
      ...
      (SELECT COUNT(*) FROM formal_orders formal_order WHERE formal_order.seller_organization_id=organization.id) AS formal_order_count
    FROM seller_organizations organization WHERE organization.id=? AND organization.status='ACTIVE'`).bind(id).first<any>();
  return row?{customer_type:'SELLER',subject_id:String(row.subject_id),display_name:String(row.display_name),marketplace_code:market,
    has_portal_account:row.account_id!==null,historical_order_count:Number(row.formal_order_count),source_status:'HISTORICAL_UNKNOWN'}:null;
}
function mergeMatches(...groups:readonly CustomerMatch[][]):CustomerMatch[]{const map=new Map<string,CustomerMatch>();for(const group of groups)for(const item of group)map.set(`${item.customer_type}:${item.marketplace_code}:${item.subject_id}`,item);return [...map.values()].sort((a,b)=>a.marketplace_code.localeCompare(b.marketplace_code)||a.display_name.localeCompare(b.display_name,'zh-CN'));}
function hasAmbiguousMarketplace(matches:readonly CustomerMatch[]):boolean{const counts=new Map<string,Set<string>>();for(const item of matches){const key=`${item.customer_type}:${item.marketplace_code}`;const set=counts.get(key)??new Set<string>();set.add(item.subject_id);counts.set(key,set);}return [...counts.values()].some((set)=>set.size>1);}
function requireActor(context:Context<any>):AssignmentStaffAuthorization{const actor=context.get('staffAuthorization') as AssignmentStaffAuthorization|undefined;if(!actor||actor.staffStatus!=='ACTIVE')throw new Error('FORBIDDEN');return actor;}
function securitySecret(context:Context<any>):string{const value=String(context.env.CUSTOMER_SECURITY_TOKEN_SECRET??'');if(new TextEncoder().encode(value).byteLength<32)throw new Error('DEPENDENCY');return value;}
