...
export async function resolveSellerMemberStoreAccess(database:SqlDatabase,memberId:string):Promise<SellerMemberStoreAccess|null>{
  ...
  const stores=allActiveStores
    ?await database.prepare(`SELECT id AS store_id FROM seller_stores WHERE organization_id=? AND status='ACTIVE' ORDER BY id`).bind(member.organization_id).all<StoreIdRow>()
    :await database.prepare(`SELECT store_id FROM (
        SELECT scope.store_id AS store_id
        FROM seller_member_store_scopes scope
        JOIN seller_stores store ON store.id=scope.store_id AND store.organization_id=scope.organization_id
        WHERE scope.member_id=? AND scope.organization_id=? AND scope.status='ACTIVE' AND store.status='ACTIVE'
        UNION
        SELECT grant.store_id
        FROM seller_member_portal_store_grants grant
        JOIN seller_stores store ON store.id=grant.store_id AND store.organization_id=grant.organization_id
        WHERE grant.member_id=? AND grant.organization_id=? AND grant.revoked_at IS NULL AND store.status='ACTIVE'
      ) ORDER BY store_id`).bind(member.member_id,member.organization_id,member.member_id,member.organization_id).all<StoreIdRow>();
  return Object.freeze({
    memberId:member.member_id,sellerOrganizationId:member.organization_id,role:member.member_role,allActiveStores,
