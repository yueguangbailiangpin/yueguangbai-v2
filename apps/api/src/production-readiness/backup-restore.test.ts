...
describe('encrypted D1 backup and isolated restore', () => {
  it('backs up schema 42, restores it and verifies rows, finance, relations and smoke reads', async () => {
    ...
    expect(backup.manifest.schema_version).toBe(42);
    expect(backup.manifest.release_commit_sha).toBe(RELEASE_SHA);
    expect(backup.manifest.row_counts['staff_users']).toBe(1);
    expect(backup.manifest.inventory.tables).toHaveLength(187);
    expect(backup.manifest.inventory.views).toHaveLength(10);
    expect(backup.manifest.inventory.triggers).toHaveLength(354);
    expect(backup.manifest.inventory.indexes).toHaveLength(243);
  ...
  it('fails closed before target creation for wrong, tampered or swapped provenance', async () => {
    ...
    writeFileSync(tamperedAttestationPath, JSON.stringify({
      ...first.attestation,
      anonymous_fixture: false,
    }), { mode: 0o600 });
    const malformedAttestationPath = path.join(directory, 'wrong-attestation.json');
    writeFileSync(malformedAttestationPath, JSON.stringify({
      ...first.attestation,
      unknown_field: true,
    }), { mode: 0o600 });
  ...
  it('rejects authenticated malformed, unknown and oversized manifests before restore', async () => {
    ...
    expect(() => validateBackupManifest({
      ...backup.manifest,
      row_counts: [],
    })).toThrow('invalid_backup_manifest');
    ...
    const variants = [
      ...
      },
      {
        ...
        manifest: {
          ...backup.manifest,
          malicious_unknown: 'x'.repeat(8 * 1024 * 1024),
        },
...
function writeAuthenticatedVariant(input: {
  ...
  const encryptionKey = Buffer.from(hkdfSync(
    ...
    Buffer.from('aes-256-gcm', 'utf8'), 32,
  ));
  const authenticationKey = Buffer.from(hkdfSync(
    'sha256', input.key,
    Buffer.from('yueguangbai-v2/d1-backup/v1', 'utf8'),
    Buffer.from('attestation-hmac', 'utf8'), 32,
  ));
  const original = readFileSync(input.backup.bundlePath);
  const decipher = createDecipheriv(
    'aes-256-gcm', encryptionKey, original.subarray(9, 21),
  );
  decipher.setAuthTag(original.subarray(21, 37));
  const payload = Buffer.concat([
    decipher.update(original.subarray(37)),
    decipher.final(),
  ]);
  const originalManifestLength = payload.readUInt32BE(9);
  const compressed = payload.subarray(13 + originalManifestLength);
  ...
  const rewrittenPayload = Buffer.concat([
    Buffer.from('YGBD1PAY1', 'ascii'), length, manifestBytes, compressed,
  ]);
  const iv = randomBytes(12);
  const cipher = createCipheriv('aes-256-gcm', encryptionKey, iv);
  const ciphertext = Buffer.concat([cipher.update(rewrittenPayload), cipher.final()]);
  const encrypted = Buffer.concat([
    Buffer.from('YGBD1ENC1', 'ascii'), iv, cipher.getAuthTag(), ciphertext,
  ]);
  ...
  const attestation: D1BackupAttestation = {
    ...
    attestation_hmac_sha256: createHmac('sha256', authenticationKey)
      .update(Buffer.from(stableJsonForTest(core))).digest('hex'),
  };
  const bundlePath = path.join(input.directory, 'variant.bundle');
  const attestationPath = path.join(input.directory, 'variant.attestation.json');
