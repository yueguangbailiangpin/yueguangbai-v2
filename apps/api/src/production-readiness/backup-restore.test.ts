...
describe('encrypted D1 backup and isolated restore', () => {
  it('backs up schema 41, restores it and verifies rows, finance, relations and smoke reads', async () => {
    ...
    expect(backup.manifest.schema_version).toBe(41);
    expect(backup.manifest.release_commit_sha).toBe(RELEASE_SHA);
    expect(backup.manifest.row_counts['staff_users']).toBe(1);
    expect(backup.manifest.inventory.tables).toHaveLength(180);
    expect(backup.manifest.inventory.views).toHaveLength(10);
    expect(backup.manifest.inventory.triggers).toHaveLength(330);
    expect(backup.manifest.inventory.indexes).toHaveLength(237);
