...
describe('scheduled operational alert Staff services', () => {
  it('projects only the strict UTC-ms Staff-safe alert summary', async () => {
    ...
    expect(JSON.stringify(alerts)).not.toMatch(
      /observation_id|object_key|payload|token|wechat|password|user-agent|last_error|customer/u,
    );
  });
  ...
  it('acknowledges one OPEN incident once and safely replays the same command', async () => {
    ...
    const first = await acknowledgeScheduledOperationalAlert(
      database,
      command,
      context('alert-ack-key'),
    );
    const replay = await acknowledgeScheduledOperationalAlert(
      database,
      command,
      context('alert-ack-key'),
    );
    ...
    expect(
      await database
        .prepare(
          "SELECT status,acknowledged_at,version FROM scheduled_alert_states WHERE signal_type='login_anomaly' AND job_name=''",
        )
        .first(),
    ).toEqual({ status: 'ACKNOWLEDGED', acknowledged_at: 2_000, version: 2 });
    const audits = (
      await database
        .prepare(
          "SELECT event_type,actor_id,idempotency_key,previous_state_json,next_state_json,reason,metadata_json FROM audit_events WHERE event_type='SCHEDULED_OPERATION_ALERT_ACKNOWLEDGED'",
        )
        .all()
    ).results;
    expect(audits).toHaveLength(1);
    expect(audits[0]).toMatchObject({
      actor_id: 'zz-phase3h-test-owner',
  ...
  it('rejects key conflicts, denied actors, stale incidents, and non-OPEN states', async () => {
    ...
    await expect(
      acknowledgeScheduledOperationalAlert(database, command, {
        ...context('denied-alert-key'),
        actor: actor([]),
      }),
    ...
    await expect(
      acknowledgeScheduledOperationalAlert(
        database,
        { ...command, incident_version: 2 },
        context('conflicting-alert-key'),
      ),
    ).rejects.toMatchObject({ code: 'IDEMPOTENCY_CONFLICT', status: 409 });
    await expect(
      acknowledgeScheduledOperationalAlert(database, command, context('already-acknowledged-key')),
    ...
    expect(
      (
        await database
          .prepare(
            "SELECT COUNT(*) AS count FROM audit_events WHERE event_type='SCHEDULED_OPERATION_ALERT_ACKNOWLEDGED'",
          )
          .first<{ count: number }>()
      )?.count,
    ).toBe(1);
...
async function openLoginAlert(db: SqliteDatabase) {
  await ingestScheduledOperationalSignal(db, {
    observation_id: id(1),
    signal_type: 'login_anomaly',
    summary_code: 'LOGIN_ANOMALY_DETECTED',
    job_name: null,
...
function context(idempotencyKey: string) {
  return { actor: actor(), idempotencyKey, requestId: `request-${idempotencyKey}`, now: 2_000 };
}
