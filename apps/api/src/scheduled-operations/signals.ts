...
export class MemoryOperationalAlertSink implements OperationalAlertSink {
  ...
  constructor(
    private readonly failWhen: (
      notification: ScheduledOperationalAlertNotificationDto,
    ) => boolean = () => false,
  ) {}
...
export class LocalOperationalAlertSink implements OperationalAlertSink {
  constructor(
    private readonly write: (
      notification: ScheduledOperationalAlertNotificationDto,
    ) => void | Promise<void> = (notification) => {
      console.warn(JSON.stringify({ event: 'scheduled_operational_alert', notification }));
    },
  ) {}
  async notify(value: ScheduledOperationalAlertNotificationDto): Promise<void> {
...
export function resolveOperationalAlertSink(input: {
  mode?: string;
  localSink?: OperationalAlertSink;
  boundSink?: OperationalAlertSink;
}): OperationalAlertSink | null {
  const mode = input.mode ?? 'disabled';
  if (mode === 'disabled') {
    if (input.localSink !== undefined || input.boundSink !== undefined)
      throw new Error('operational_alert_sink_disabled_with_adapter');
    return null;
  }
  if (mode === 'local') {
    if (input.boundSink !== undefined)
      throw new Error('operational_alert_local_with_bound_adapter');
    return input.localSink ?? new LocalOperationalAlertSink();
  }
  if (mode === 'bound') {
    if (input.localSink !== undefined || input.boundSink === undefined)
      throw new Error('operational_alert_bound_adapter_required');
    return input.boundSink;
  }
  throw new Error('invalid_operational_alert_mode');
}

export function safeResolveOperationalAlertSink(input: {
  mode?: string;
  localSink?: OperationalAlertSink;
  boundSink?: OperationalAlertSink;
}): OperationalAlertSink | null {
  try {
    return resolveOperationalAlertSink(input);
  } catch {
    return null;
  }
}
...
export async function ingestScheduledOperationalSignal(
  ...
  await database
    .prepare(
      `INSERT OR IGNORE INTO scheduled_operational_signals(id,signal_type,category,severity,summary_code,job_name,observation_state,observed_at,count_value,evaluated_at) VALUES(?,?,?,?,?,?,?,?,?,NULL)`,
    )
    .bind(
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const observation = await database
      .prepare(
        'SELECT signal_type,category,severity,summary_code,job_name,observation_state,observed_at,count_value,evaluated_at FROM scheduled_operational_signals WHERE id=?',
      )
      .bind(input.observation_id)
      .first<PersistedObservationRow>();
    if (!observation || !sameObservation(input, policy, observation))
      throw new Error('scheduled_operational_signal_id_conflict');
    ...
    if (current && input.observed_at < current.last_evaluated_at) {
      const stale = await database
        .prepare(
          'UPDATE scheduled_operational_signals SET evaluated_at=? WHERE id=? AND evaluated_at IS NULL',
        )
        .bind(input.observed_at, input.observation_id)
        .run();
      if (statementChangedOnce(stale))
        return {
          disposition: 'STALE',
          notification: 'NONE',
          status: current.status,
          incident_version: current.incident_version,
        };
      continue;
    }
    ...
    if (!computed.notification) {
      return {
        ...
        notification:
          computed.suppressed_until !== null && computed.suppressed_until > input.observed_at
            ? 'SUPPRESSED'
            : 'NONE',
        status: computed.status,
        incident_version: computed.incident_version,
      };
    }
    try {
      ...
      await database
        .prepare(
          'UPDATE scheduled_alert_states SET last_notification_at=?,updated_at=? WHERE signal_type=? AND job_name=? AND summary_code=? AND (last_notification_at IS NULL OR last_notification_at<?)',
        )
        .bind(
          input.observed_at,
          input.observed_at,
      ...
      await database
        .prepare(
          'UPDATE scheduled_alert_states SET suppressed_until=COALESCE(cooldown_until,?),updated_at=? WHERE signal_type=? AND job_name=? AND summary_code=?',
        )
        .bind(input.observed_at, input.observed_at, input.signal_type, jobKey, policy.summaryCode)
        .run();
...
export async function evaluatePersistedScheduledJobSignals(
  ...
  input: {
    evaluationId: string;
    now: number;
    sink?: OperationalAlertSink | null;
    disabledJobs?: readonly ScheduledOperationJobName[];
  },
): Promise<OperationalSignalEvaluationResult[]> {
  if (
    !/^[0-9a-f]{64}$/u.test(input.evaluationId) ||
    !Number.isSafeInteger(input.now) ||
    input.now < 0
  )
    throw new Error('invalid_scheduled_operational_evaluation');
  const jobs = await database
    .prepare(
      `SELECT job_name,lease_token,lease_expires_at,last_started_at,last_succeeded_at,last_failure_category,last_backlog_count,updated_at FROM scheduled_job_states WHERE enabled=1 ORDER BY job_name`,
    )
    .all<{
  ...
  for (const job of jobs.results) {
    ...
    results.push(
      await ingestDerived(
        ...
        input,
        {
          ...
          summaryCode: 'JOB_SUCCESS_STALE',
          jobName: job.job_name,
          breach: input.now - baseline >= JOB_SUCCESS_STALE_AFTER_MS,
          countValue: 1,
        },
        input.sink,
      ),
    ...
    results.push(
      await ingestDerived(
        ...
        input,
        {
          signalType: 'lease_stuck',
          summaryCode: 'JOB_LEASE_STUCK',
          jobName: job.job_name,
          breach:
            job.lease_token !== null &&
            job.lease_expires_at !== null &&
            input.now - job.lease_expires_at >= LEASE_STUCK_GRACE_MS,
          countValue: 1,
        },
        input.sink,
      ),
    ...
    results.push(
      await ingestDerived(
        ...
        input,
        {
          signalType: 'backlog_sustained',
          summaryCode: 'JOB_BACKLOG_SUSTAINED',
          jobName: job.job_name,
          breach: job.last_backlog_count > 0,
          countValue: job.last_backlog_count,
        },
        input.sink,
      ),
    );
    if (job.job_name === 'file_orphan_cleanup' || job.job_name === 'drive_archive')
      results.push(
        await ingestDerived(
          database,
          input,
          {
            signalType: 'file_failure',
            summaryCode: 'FILE_PROCESSING_FAILURE',
            jobName: job.job_name,
            breach:
              job.job_name === 'file_orphan_cleanup'
                ? job.last_failure_category === 'file_cleanup_deferred'
                : job.last_failure_category !== null,
            countValue: 1,
          },
          input.sink,
        ),
  ...
  for (const global of GLOBAL_RECOVERY_SIGNALS) {
    ...
    const recent = await database
      .prepare(
        "SELECT 1 AS found FROM scheduled_operational_signals WHERE signal_type=? AND summary_code=? AND job_name IS NULL AND observation_state='BREACH' AND observed_at>? LIMIT 1",
      )
      .bind(global.signalType, global.summaryCode, input.now - global.quietWindowMs)
      .first<{ found: number }>();
...
export async function recordWorker5xxSignal(
  ...
  if (
    ...
    throw new Error('invalid_worker_5xx_signal');
  const observationId = await hashCanonicalJson({
    kind: 'WORKER_5XX',
    request_id: input.requestId,
  });
  return ingestScheduledOperationalSignal(
    database,
    {
...
function policyFor(input: ScheduledOperationalSignalObservationDto): SignalPolicy {
  switch (input.signal_type) {
    ...
    case 'file_failure':
      ...
      if (input.job_name !== 'file_orphan_cleanup' && input.job_name !== 'drive_archive')
        throw new Error('invalid_scheduled_operational_signal_scope');
      return policy(
        'file',
        'WARNING',
    ...
    case 'external_adapter_failure':
      ...
      if (input.summary_code === 'PRIMARY_ALERT_SINK_FAILURE')
        return policy(
          'external',
          'CRITICAL',
          'PRIMARY_ALERT_SINK_FAILURE',
          1,
          5 * MINUTE_MS,
          30 * MINUTE_MS,
          'COUNT',
...
function assertShape(
  input: ScheduledOperationalSignalObservationDto,
  summaryCode: ScheduledOperationalSignalSummaryCode,
  requiresJob: boolean,
) {
  if (
    input.summary_code !== summaryCode ||
    (requiresJob ? input.job_name === null : input.job_name !== null)
  )
    throw new Error('invalid_scheduled_operational_signal_scope');
}

function computeAlertState(
  current: AlertStateRow | null,
  input: ScheduledOperationalSignalObservationDto,
  policy: SignalPolicy,
  ...
  const state: ComputedAlertState = {
    ...
    summary_code: policy.summaryCode,
    status,
    first_seen_at: firstSeen,
    last_seen_at: now,
    consecutive_breach_count: breachCount,
    consecutive_healthy_count: healthyCount,
    window_started_at: windowStarted,
    window_count_value: windowCount,
    threshold_count: policy.thresholdCount,
    threshold_window_ms: policy.thresholdWindowMs,
    ...
    last_notification_at: base.last_notification_at,
  ...
  if (notificationKind !== null) {
    state.notification = parseScheduledOperationalAlertNotificationDto({
      ...
      status: notificationKind === 'RESOLVED' ? 'RESOLVED' : 'OPEN',
      observed_at: now,
      incident_version: incidentVersion,
      count_value: windowCount,
    });
  }
  return state;
}
...
function initialState(
  ...
  return {
    ...
    window_started_at: input.observed_at,
...
function upsertAlertStateStatement(
  ...
  return database
    ...
    .bind(
      ...
      state.window_count_value,
      state.threshold_count,
      state.threshold_window_ms,
...
async function readAlertState(
  database: SqlDatabase,
  signalType: ScheduledOperationalSignalType,
  jobKey: string,
  summaryCode: ScheduledOperationalSignalSummaryCode,
) {
  return database
    .prepare(
      'SELECT signal_type,job_name,category,severity,summary_code,status,first_seen_at,last_seen_at,consecutive_breach_count,consecutive_healthy_count,window_started_at,window_count_value,threshold_count,threshold_window_ms,recovery_count,opened_at,acknowledged_at,resolved_at,cooldown_until,suppressed_until,last_notification_at,last_evaluated_at,incident_version,version,updated_at FROM scheduled_alert_states WHERE signal_type=? AND job_name=? AND summary_code=?',
    )
    .bind(signalType, jobKey, summaryCode)
    .first<AlertStateRow>();
}
...
function sameObservation(
  ...
  return (
    row.signal_type === input.signal_type &&
    row.category === policy.category &&
    row.severity === policy.severity &&
    row.summary_code === policy.summaryCode &&
    row.job_name === input.job_name &&
    row.observation_state === input.observation_state &&
    row.observed_at === input.observed_at &&
    row.count_value === input.count_value
  );
}
...
function resultFromCurrent(
  ...
  current: AlertStateRow | null,
): OperationalSignalEvaluationResult {
  return {
    disposition,
    notification: 'NONE',
    status: current?.status ?? 'RESOLVED',
    incident_version: current?.incident_version ?? 0,
  };
}
...
function validateSafeSourceFact(value: string, observedAt: number) {
  if (
    ...
    throw new Error('invalid_operational_signal_source');
}
...
async function recordAlertSinkFailure(
  ...
  await ingestScheduledOperationalSignal(database, {
    observation_id: observationId,
    signal_type: 'external_adapter_failure',
...
async function ingestDerived(
  database: SqlDatabase,
  evaluation: { evaluationId: string; now: number },
  derived: {
    signalType: ScheduledOperationalSignalType;
    summaryCode: ScheduledOperationalSignalSummaryCode;
  ...
  sink: OperationalAlertSink | null | undefined,
) {
  const observationState = derived.breach ? 'BREACH' : 'HEALTHY';
  const observationId = await hashCanonicalJson({
    evaluation_id: evaluation.evaluationId,
    signal_type: derived.signalType,
    job_name: derived.jobName,
  });
  return ingestScheduledOperationalSignal(
    database,
    {
      observation_id: observationId,
      signal_type: derived.signalType,
      summary_code: derived.summaryCode,
      job_name: derived.jobName,
      observation_state: observationState,
      observed_at: evaluation.now,
      count_value: derived.breach ? derived.countValue : 0,
    },
    sink === undefined ? {} : { sink },
  );
}
...
const GLOBAL_RECOVERY_SIGNALS = [
  ...
  {
    signalType: 'login_anomaly',
    summaryCode: 'LOGIN_ANOMALY_DETECTED',
    quietWindowMs: 10 * MINUTE_MS,
  },
  {
    signalType: 'external_adapter_failure',
    summaryCode: 'PRIMARY_ALERT_SINK_FAILURE',
    quietWindowMs: 5 * MINUTE_MS,
  },
...
async function ingestGlobalHealthy(
  database: SqlDatabase,
  evaluation: { evaluationId: string; now: number },
  global: (typeof GLOBAL_RECOVERY_SIGNALS)[number],
  sink: OperationalAlertSink | null | undefined,
) {
  const observationId = await hashCanonicalJson({
    evaluation_id: evaluation.evaluationId,
    signal_type: global.signalType,
    summary_code: global.summaryCode,
  });
  return ingestScheduledOperationalSignal(
    database,
    {
