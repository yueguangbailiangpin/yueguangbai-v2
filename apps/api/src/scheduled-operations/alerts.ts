...
export async function acknowledgeScheduledOperationalAlert(
  ...
  const requestHash = await hashCanonicalJson({
    ...
    summary_code: command.summary_code,
    job_name: command.job_name,
    incident_version: command.incident_version,
  });
  ...
  try {
    const state = await database
      .prepare(
        'SELECT status,incident_version,version FROM scheduled_alert_states WHERE signal_type=? AND job_name=? AND summary_code=?',
      )
      .bind(command.signal_type, jobKey, command.summary_code)
      .first<AlertStateVersionRow>();
    if (!state) throw new ScheduledOperationCommandError('NOT_FOUND', 404);
    if (state.status !== 'OPEN' || state.incident_version !== command.incident_version)
      throw new ScheduledOperationCommandError('STATE_CONFLICT', 409);
    ...
    await database.batch([
      database
        .prepare(
          "UPDATE scheduled_alert_states SET status='ACKNOWLEDGED',acknowledged_at=?,version=version+1,updated_at=MAX(updated_at,?) WHERE signal_type=? AND job_name=? AND summary_code=? AND status='OPEN' AND incident_version=? AND version=?",
        )
        .bind(
          now,
          now,
          command.signal_type,
          jobKey,
          command.summary_code,
          command.incident_version,
          state.version,
        ),
      changedOnce(database),
      createAuditEventStatement(database, {
        id: crypto.randomUUID(),
        aggregateType: 'SCHEDULED_OPERATION_ALERT',
        aggregateId: targetId,
        eventType: 'SCHEDULED_OPERATION_ALERT_ACKNOWLEDGED',
        actor: { type: 'STAFF', id: context.actor.staffId, roles: [...context.actor.roles] },
...
async function acquire(
  ...
  try {
    return await acquireIdempotency<ScheduledOperationalAlertAckResultDto>(
      database,
      {
        actorType: 'STAFF',
        actorId: input.actor.staffId,
        action: 'SCHEDULED_OPERATION_ALERT_ACK',
        targetType: 'SCHEDULED_OPERATION_ALERT',
...
function parseResult(value: unknown) {
  try {
    return parseScheduledOperationalAlertAckResultDto(value);
  } catch {
...
function safeErrorCode(error: unknown) {
  ...
  return typeof code === 'string' && /^[A-Z_]{1,100}$/u.test(code) ? code : 'STATE_CONFLICT';
}
