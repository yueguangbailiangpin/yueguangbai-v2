...
describe('scheduled operations', () => {
  ...
  it('allows only one concurrent outbox scheduler to hold the job lease', async () => {
    ...
    const first = runScheduledOperations(database,{now:2000,only:'outbox_delivery',outboxAdapter:{deliver:async()=>blocked}});
  ...
  it('does not write leases, runs, or business facts when disabled', async () => {
    ...
    expect((await database.prepare('SELECT COUNT(*) AS count FROM scheduled_job_states').first<{count:number}>())?.count).toBe(0);
  });
  ...
  it('uses bounded exponential outbox retry without exposing payload in run facts', async () => {
    ...
    expect(run[0]).toMatchObject({ outcome: 'FAILED', failed_count: 1, backlog_count: 1, failure_category: 'adapter_unavailable' });
  ...
  it('quarantines a poison outbox event and dry-run never claims it', async () => {
    ...
    const dry = await runScheduledOperations(database,{now:2000,only:'outbox_delivery',dryRun:true});
    ...
    const dead = await database.prepare("SELECT source_id FROM scheduled_dead_letters WHERE source_id='outbox-poison-1'").first();
  ...
  it('does not send a committed outbox event twice across duplicate cron runs', async () => {
    ...
    expect((await runScheduledOperations(database,{now:2000,only:'outbox_delivery',outboxAdapter:adapter}))[0]?.outcome).toBe('SUCCEEDED');
    expect((await runScheduledOperations(database,{now:2000,only:'outbox_delivery',outboxAdapter:adapter}))[0]?.processed_count).toBe(0);
    expect(sends).toBe(1);
  });

  it('resumes reservation expiry without duplicate release, audit, or message facts', async () => {
    database=createMigratedTestDatabase();
    seedScheduledReservationFixture(database,{statuses:['PENDING_REVIEW','PENDING_REVIEW'],holdExpiresAt:[1000,1000]});

    const first=await runScheduledOperations(database,{now:2000,only:'reservation_expiry',batchSize:1});
    expect(first[0]).toMatchObject({outcome:'SUCCEEDED',processed_count:1,succeeded_count:1,failed_count:0,backlog_count:1});
    const partialState=await database.prepare("SELECT cursor_json,version FROM scheduled_job_states WHERE job_name='reservation_expiry'").first<{cursor_json:string|null;version:number}>();
    expect(JSON.parse(partialState?.cursor_json ?? 'null')).toEqual({due:1000,id:'reservation-scheduled-1'});
    expect(await reservationExpiryFactCounts(database)).toEqual({expired:1,events:1,audits:1,outbox:1});

    const second=await runScheduledOperations(database,{now:2000,only:'reservation_expiry',batchSize:1});
    expect(second[0]).toMatchObject({outcome:'SUCCEEDED',processed_count:1,succeeded_count:1,failed_count:0});
    const completedState=await database.prepare("SELECT cursor_json,version,last_succeeded_at FROM scheduled_job_states WHERE job_name='reservation_expiry'").first<{cursor_json:string|null;version:number;last_succeeded_at:number}>();
    expect(completedState?.cursor_json).toBeNull();
    expect(completedState?.version).toBeGreaterThan(partialState?.version ?? 0);
    expect(await reservationExpiryFactCounts(database)).toEqual({expired:2,events:2,audits:2,outbox:2});

    const duplicate=await runScheduledOperations(database,{now:2000,only:'reservation_expiry',batchSize:1});
    expect(duplicate[0]?.processed_count).toBe(0);
    expect(await reservationExpiryFactCounts(database)).toEqual({expired:2,events:2,audits:2,outbox:2});
    const stale=await database.prepare("UPDATE scheduled_job_states SET cursor_json='stale',last_succeeded_at=999999 WHERE job_name='reservation_expiry' AND lease_token='late-reservation-worker' AND version=?").bind(completedState?.version ?? 0).run();
    expect(stale.meta.changes).toBe(0);
    const afterStale=await database.prepare("SELECT cursor_json,last_succeeded_at FROM scheduled_job_states WHERE job_name='reservation_expiry'").first<{cursor_json:string|null;last_succeeded_at:number}>();
  ...
  it('previews only due legacy JP instructions and leaves business facts unchanged', async () => {
    database=createMigratedTestDatabase();
    seedScheduledReservationFixture(database,{statuses:['APPROVED','APPROVED','APPROVED'],canonicalUsIndexes:[3]});
    const due=await seedScheduledInstruction(database,{suffix:'scheduled-due',reservationIndex:1,publishedAt:1000});
    const future=await seedScheduledInstruction(database,{suffix:'scheduled-future',reservationIndex:2,publishedAt:30_000_000});
    const us=await seedScheduledInstruction(database,{suffix:'scheduled-us',reservationIndex:3,publishedAt:1000});
    const before=await instructionBusinessFactCounts(database);

    const preview=await runScheduledOperations(database,{now:25_000_000,only:'instruction_expiry',dryRun:true,batchSize:1});
    expect(preview[0]).toMatchObject({outcome:'SUCCEEDED',processed_count:0,succeeded_count:0,failed_count:0,backlog_count:1});
    expect(await instructionBusinessFactCounts(database)).toEqual(before);
    expect(await instructionStatuses(database,[due.instructionId,future.instructionId,us.instructionId])).toEqual(['ACTIVE','ACTIVE','ACTIVE']);

    const executed=await runScheduledOperations(database,{now:25_000_000,only:'instruction_expiry',batchSize:1});
    expect(executed[0]).toMatchObject({outcome:'SUCCEEDED',processed_count:1,succeeded_count:1,failed_count:0,backlog_count:0});
    expect(await instructionStatuses(database,[due.instructionId,future.instructionId,us.instructionId])).toEqual(['EXPIRED','ACTIVE','ACTIVE']);
  });
  ...
  it('persists an instruction continuation marker and clears it after a completed JP round', async () => {
    ...
    expect(completedState?.cursor_json).toBeNull();
    expect(await instructionStatuses(database,[firstInstruction.instructionId,secondInstruction.instructionId])).toEqual(['EXPIRED','EXPIRED']);
  });
  ...
  it('records a failed instruction round without refreshing last success', async () => {
    ...
    const fixture=await seedScheduledInstruction(database,{suffix:'scheduled-failure',reservationIndex:1,publishedAt:1000});
    database.exec("INSERT INTO scheduled_job_states(job_name,last_succeeded_at,updated_at) VALUES('instruction_expiry',123,123); CREATE TRIGGER fixture_instruction_expiry_failure BEFORE UPDATE ON order_instructions WHEN OLD.id='phase3g-instruction-scheduled-failure' BEGIN SELECT RAISE(ABORT,'fixture_instruction_failure'); END;");

    const failed=await runScheduledOperations(database,{now:25_000_000,only:'instruction_expiry',batchSize:1});
    expect(failed[0]).toMatchObject({outcome:'FAILED',processed_count:1,succeeded_count:0,failed_count:1,backlog_count:1,failure_category:'job_item_failed'});
    const state=await database.prepare("SELECT last_succeeded_at,last_failed_at,last_failure_category FROM scheduled_job_states WHERE job_name='instruction_expiry'").first<{last_succeeded_at:number;last_failed_at:number;last_failure_category:string}>();
    expect(state).toEqual({last_succeeded_at:123,last_failed_at:25_000_000,last_failure_category:'job_item_failed'});
...
function seedScheduledReservationFixture(database: SqliteDatabase, input: {statuses: readonly ReservationFixtureStatus[];holdExpiresAt?: readonly number[];canonicalUsIndexes?: readonly number[]}): void {
  ...
  database.exec(`
    ...
    ${buyers.map((index)=>`INSERT INTO customer_identity_subjects(id,subject_type,created_at) VALUES ('buyer-scheduled-subject-${index}','BUYER_CUSTOMER',1);
    ...
    VALUES ('buyer-scheduled-${index}','buyer-scheduled-subject-${index}','JP','buyer-channel-scheduled',NULL,NULL,NULL,'测试买家${index}','ACTIVE','CLEAR',1,1,1,1,NULL);`).join('\n')}
  ...
  `);
  for (const index of input.canonicalUsIndexes ?? []) database.exec(`UPDATE buyer_marketplace_assignments SET marketplace_code='AMAZON_US',version=version+1,updated_at=2 WHERE buyer_customer_id='buyer-scheduled-${index}'`);
  database.exec(input.statuses.map((status,offset)=>{
    const index=offset+1; const approvedStatus=status==='APPROVED'; const hold=input.holdExpiresAt?.[offset] ?? 1000;
...
function seedScheduledInstruction(database: SqliteDatabase, input: {suffix:string;reservationIndex:number;publishedAt:number}) {
...
async function reservationExpiryFactCounts(database: SqliteDatabase): Promise<{expired:number;events:number;audits:number;outbox:number}> {
  const row=await database.prepare(`SELECT
    ...
    (SELECT COUNT(*) FROM integration_outbox WHERE event_type='RESERVATION_EXPIRED') AS outbox`).first<{expired:number;events:number;audits:number;outbox:number}>();
  return {expired:Number(row?.expired??0),events:Number(row?.events??0),audits:Number(row?.audits??0),outbox:Number(row?.outbox??0)};
}

async function instructionBusinessFactCounts(database: SqliteDatabase): Promise<{expired:number;events:number;audits:number;outbox:number;idempotency:number}> {
  const row=await database.prepare(`SELECT
    ...
    (SELECT COUNT(*) FROM command_idempotency_records) AS idempotency`).first<{expired:number;events:number;audits:number;outbox:number;idempotency:number}>();
  return {expired:Number(row?.expired??0),events:Number(row?.events??0),audits:Number(row?.audits??0),outbox:Number(row?.outbox??0),idempotency:Number(row?.idempotency??0)};
}

async function instructionStatuses(database: SqliteDatabase, ids: readonly string[]): Promise<string[]> {
  const statuses: string[]=[];
  for (const id of ids) statuses.push((await database.prepare('SELECT status FROM order_instructions WHERE id=?').bind(id).first<{status:string}>())?.status ?? 'MISSING');
  return statuses;
}
