...
const workerObservation = (value: number, observedAt: number, countValue = 1) => ({
...
describe('scheduled operational signal evaluation', () => {
  it('opens only at the threshold and makes duplicate observations idempotent', async () => {
    ...
    expect(
      (await ingestScheduledOperationalSignal(database, workerObservation(1, 1_000), { sink }))
        .status,
    ).toBe('RESOLVED');
    expect(
      (await ingestScheduledOperationalSignal(database, workerObservation(2, 2_000), { sink }))
        .status,
    ).toBe('RESOLVED');
    expect(
      (await ingestScheduledOperationalSignal(database, workerObservation(2, 2_000), { sink }))
    ...
    expect(
      await ingestScheduledOperationalSignal(database, workerObservation(3, 3_000), { sink }),
    ).toMatchObject({ status: 'OPEN', notification: 'SENT', incident_version: 1 });
    expect(sink.notifications).toHaveLength(1);
    expect(sink.notifications[0]).toMatchObject({
      category: 'worker',
      severity: 'CRITICAL',
      summary_code: 'WORKER_5XX_THRESHOLD',
    ...
    expect(state).toMatchObject({
      status: 'OPEN',
      window_count_value: 3,
      threshold_count: 3,
      threshold_window_ms: 300_000,
      consecutive_breach_count: 3,
      incident_version: 1,
      version: 3,
      last_notification_at: 3_000,
    });
  ...
  it('suppresses cooldown reminders, resolves after two healthy scans, and reopens a new incident', async () => {
    ...
    const suppressed = await ingestScheduledOperationalSignal(
      database,
      workerObservation(11, 2_000),
      { sink },
    );
    ...
    expect(sink.notifications.map((entry) => entry.notification_kind)).toEqual([
      'OPENED',
      'REMINDER',
    ]);

    await ingestScheduledOperationalSignal(
      database,
      { ...workerObservation(13, 1_803_000, 0), observation_state: 'HEALTHY' },
      { sink },
    );
    const resolved = await ingestScheduledOperationalSignal(
      database,
      { ...workerObservation(14, 1_804_000, 0), observation_state: 'HEALTHY' },
      { sink },
    );
    ...
    const recurrence = await ingestScheduledOperationalSignal(
      database,
      workerObservation(15, 1_805_000, 3),
      { sink },
    );
    ...
    expect(await alertState(database, 'worker_5xx', '')).toMatchObject({
      status: 'OPEN',
      incident_version: 2,
      resolved_at: null,
    });
  ...
  it('requires three distinct healthy-window scans before sustained backlog opens', async () => {
    ...
    const third = await evaluatePersistedScheduledJobSignals(database, {
      evaluationId: id(103),
      now: 4_000,
      sink,
    });
    expect(third.some((entry) => entry.status === 'OPEN' && entry.notification === 'SENT')).toBe(
      true,
    );
    ...
    const duplicate = await evaluatePersistedScheduledJobSignals(database, {
      evaluationId: id(103),
      now: 4_000,
      sink,
    });
  ...
  it('derives stale success, stuck lease, and file failures from scheduler facts', async () => {
    database = createMigratedTestDatabase();
    database.exec(
      "INSERT INTO scheduled_job_states(job_name,lease_token,lease_expires_at,last_succeeded_at,last_failure_category,last_backlog_count,updated_at) VALUES('file_orphan_cleanup','stuck',1000,0,'file_cleanup_deferred',0,0)",
    );
    const sink = new MemoryOperationalAlertSink();
    ...
    expect(sink.notifications.map((entry) => entry.signal_type).sort()).toEqual([
      'file_failure',
      'job_stale',
      'lease_stuck',
    ]);
    expect(await alertState(database, 'job_stale', 'file_orphan_cleanup')).toMatchObject({
      status: 'OPEN',
      category: 'scheduler',
      severity: 'WARNING',
    });
    expect(await alertState(database, 'lease_stuck', 'file_orphan_cleanup')).toMatchObject({
      status: 'OPEN',
      category: 'scheduler',
      severity: 'CRITICAL',
    });
    expect(await alertState(database, 'file_failure', 'file_orphan_cleanup')).toMatchObject({
      status: 'OPEN',
      category: 'file',
      threshold_count: 3,
    });
  ...
  it('uses a fixed authentication anomaly policy', async () => {
    ...
    for (let index = 0; index < 5; index += 1)
      await recordLoginAnomalySignal(database, {
        securityEventId: `safe-login-event-${index}`,
    ...
    expect(
      (
        await recordLoginAnomalySignal(database, {
          securityEventId: 'safe-login-event-0',
          observedAt: 1_000,
          sink,
        })
      ).disposition,
  ...
  it('contains sink failures and records a fixed primary-adapter failure signal', async () => {
    ...
    const result = await ingestScheduledOperationalSignal(
      database,
      workerObservation(401, 1_000, 3),
      { sink },
    );
    expect(result).toMatchObject({ status: 'OPEN', notification: 'FAILED' });
    expect(sink.notifications).toHaveLength(0);
    expect(await alertState(database, 'external_adapter_failure', '')).toMatchObject({
      status: 'OPEN',
      summary_code: 'PRIMARY_ALERT_SINK_FAILURE',
      category: 'external',
      severity: 'CRITICAL',
      incident_version: 1,
    });
  ...
  it('automatically resolves event-based alerts after two quiet scheduled evaluations', async () => {
    ...
    expect(await alertState(database, 'worker_5xx', '', 'WORKER_5XX_THRESHOLD')).toMatchObject({
      status: 'RESOLVED',
  ...
  it('defaults the primary adapter to disabled and validates local-only configuration', async () => {
    ...
    expect(() => resolveOperationalAlertSink({ mode: 'external' })).toThrow(
      'invalid_operational_alert_mode',
    );
    expect(() =>
      resolveOperationalAlertSink({
        mode: 'disabled',
        localSink: new MemoryOperationalAlertSink(),
      }),
    ).toThrow('operational_alert_sink_disabled_with_adapter');
    const facts: unknown[] = [];
    const local = new LocalOperationalAlertSink((notification) => {
  ...
  it('collects handled Worker 5xx responses without persisting route or exception detail', async () => {
    ...
    app.get('/unhandled', () => {
      throw new Error('token=private-customer-secret');
    });
    vi.spyOn(console, 'error').mockImplementation(() => undefined);
    for (let index = 0; index < 3; index += 1) {
      const response = await app.request(
        `/private/customer/customer-${index}`,
        {},
        {
          DB: database,
    ...
    const serialized = JSON.stringify(
      (await database.prepare('SELECT * FROM scheduled_operational_signals').all()).results,
    );
  ...
  it('rejects arbitrary summaries, scopes, labels, and observation-id collisions', async () => {
    ...
    ).rejects.toThrow('invalid_scheduled_operation_contract');
    await expect(
      ingestScheduledOperationalSignal(database, { ...valid, count_value: 2 }),
    ).rejects.toThrow('scheduled_operational_signal_id_conflict');
    await expect(
      ingestScheduledOperationalSignal(database, {
        ...workerObservation(502, 2_000),
        summary_code: 'LOGIN_ANOMALY_DETECTED',
      }),
    ...
    const columns = await database
      .prepare('PRAGMA table_info(scheduled_operational_signals)')
      .all<{ name: string }>();
    expect(columns.results.map((entry) => entry.name)).toEqual([
      'id',
      'signal_type',
      'category',
      'severity',
    ...
    ).not.toMatch(/orders|private|token|secret|wechat|object_key/u);
  });
});
...
async function alertState(
  ...
  return summaryCode
    ? db
        .prepare(
          'SELECT * FROM scheduled_alert_states WHERE signal_type=? AND job_name=? AND summary_code=?',
        )
        .bind(signalType, jobName, summaryCode)
        .first<Record<string, unknown>>()
    : db
        .prepare('SELECT * FROM scheduled_alert_states WHERE signal_type=? AND job_name=?')
        .bind(signalType, jobName)
        .first<Record<string, unknown>>();
}
