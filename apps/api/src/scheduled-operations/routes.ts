...
function runtime(context:Context<AppEnv>) { return {enabled:context.env.SCHEDULED_OPERATIONS_ENABLED==='true',disabledJobs:disabledJobs(context.env.SCHEDULED_OPERATIONS_DISABLED_JOBS),storage:context.env.FILE_OBJECT_STORAGE??null,outboxAdapter:context.env.OUTBOX_DELIVERY_ADAPTER??null}; }
