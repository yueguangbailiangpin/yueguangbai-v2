...
async function health(context: Context<AppEnv>): Promise<Response> {
  ...
  const driveEnabled=context.env.DRIVE_ARCHIVE_ENABLED==='true'&&context.env.DRIVE_ARCHIVE_COPY_ENABLED==='true'
    && Boolean(context.env.DRIVE_ARCHIVE_ADAPTER)&&Boolean(context.env.FILE_OBJECT_STORAGE);
  const jobs=SCHEDULED_JOB_NAMES.map((jobName)=>{ const row=states.get(jobName); const hardDisabled=jobName==='feishu_sync'||(jobName==='drive_archive'&&!driveEnabled); return parseScheduledOperationHealthDto({job_name:jobName,enabled:hardDisabled?false:row?.enabled!==0,last_started_at:row?.last_started_at??null,last_succeeded_at:row?.last_succeeded_at??null,last_failed_at:row?.last_failed_at??null,last_failure_category:row?.last_failure_category??null,backlog_count:Number(row?.backlog_count??0),lease_expires_at:row?.lease_expires_at??null,capability_scope:jobName==='instruction_expiry'?'LEGACY_JP_ONLY':hardDisabled?'HARD_DISABLED':'ALL_ENABLED_MARKETPLACES'}); });
...
function runtime(context:Context<AppEnv>) { return {enabled:context.env.SCHEDULED_OPERATIONS_ENABLED==='true',disabledJobs:disabledJobs(context.env.SCHEDULED_OPERATIONS_DISABLED_JOBS),storage:context.env.FILE_OBJECT_STORAGE??null,outboxAdapter:context.env.OUTBOX_DELIVERY_ADAPTER??null,driveAdapter:context.env.DRIVE_ARCHIVE_ADAPTER??null,driveArchiveEnabled:context.env.DRIVE_ARCHIVE_ENABLED==='true',driveArchiveCopyEnabled:context.env.DRIVE_ARCHIVE_COPY_ENABLED==='true',driveArchiveProxyReadEnabled:context.env.DRIVE_ARCHIVE_PROXY_READ_ENABLED==='true',driveArchiveR2DeleteEnabled:context.env.DRIVE_ARCHIVE_R2_DELETE_ENABLED==='true'}; }
