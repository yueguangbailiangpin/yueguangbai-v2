...
async function health(context: Context<AppEnv>): Promise<Response> {
  ...
  const rows = await context.env.DB.prepare(
    'SELECT job_name,enabled,last_started_at,last_succeeded_at,last_failed_at,last_failure_category,last_backlog_count AS backlog_count,lease_expires_at FROM scheduled_job_states ORDER BY job_name',
  ).all<{
    job_name: string;
    enabled: number;
  ...
  const jobs = SCHEDULED_JOB_NAMES.map((jobName) => {
    ...
    return parseScheduledOperationHealthDto({
      ...
      last_failure_category: row?.last_failure_category ?? null,
      backlog_count: Number(row?.backlog_count ?? 0),
      lease_expires_at: row?.lease_expires_at ?? null,
      capability_scope:
        jobName === 'instruction_expiry'
          ? 'LEGACY_JP_ONLY'
          : hardDisabled
            ? 'HARD_DISABLED'
  ...
  return context.json(
    apiSuccess(
      { jobs, alerts, time_basis: 'UTC_MS' as const, display_timezone: 'Asia/Shanghai' as const },
...
async function acknowledgeAlert(context: Context<AppEnv>): Promise<Response> {
  ...
  const result = await acknowledgeScheduledOperationalAlert(context.env.DB, command, {
    actor,
  ...
  return context.json(
    apiSuccess(
      { acknowledgement: parseScheduledOperationalAlertAckResultDto(result) },
      context.get('requestId'),
    ),
  );
}
...
async function runManually(context: Context<AppEnv>): Promise<Response> {
  ...
  if (!isScheduledOperationJobName(job))
  ...
  const result = await runScheduledOperationManually(
    context.env.DB,
    runtime(context),
    { jobName: job, command },
...
function withErrors(handler: (context: Context<AppEnv>) => Promise<Response>) {
  return async (context: Context<AppEnv>) => {
    try {
      ...
      return context.json(
        apiFailure(code, message(code), context.get('requestId')),
        safeStatus(statusValue),
      );
    }
  };
}
function safeStatus(value: unknown): 400 | 403 | 404 | 409 | 503 {
  switch (value) {
    case 400:
      return 400;
    case 403:
