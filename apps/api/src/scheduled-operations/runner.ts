import type { ObjectStorageAdapter, SqlDatabase, StaffPermissionCode, StaffRoleCode } from '@ygb/contracts';
...
async function runOne(database: SqlDatabase, job: ScheduledJobName, input: Required<Pick<Parameters<typeof runScheduledOperations>[1], 'now'>> & Parameters<typeof runScheduledOperations>[1]): Promise<SafeJobRun> {
  ...
  try {
    ...
    await database.prepare(`UPDATE scheduled_job_states SET lease_token=NULL, lease_expires_at=NULL, cursor_json=?, version=version+1, last_succeeded_at=CASE WHEN ?='SUCCEEDED' THEN ? ELSE last_succeeded_at END, last_failed_at=CASE WHEN ?='SUCCEEDED' THEN last_failed_at ELSE ? END, last_backlog_count=?, last_failure_category=?, updated_at=? WHERE job_name=? AND lease_token=?`).bind(result.cursorJson ?? null,outcome,input.now,outcome,input.now,result.backlog,category,input.now,job,token).run();
...
async function execute(database: SqlDatabase, job: ScheduledJobName, input: Parameters<typeof runScheduledOperations>[1] & {now:number}): Promise<{processed:number;succeeded:number;failed:number;backlog:number;failureCategory?:string | undefined;cursorJson?:string | undefined}> {
  ...
  if (job === 'instruction_expiry') {
    ...
    const r = await runOrderInstructionExpiryScan(database, { marketplaceCode: 'JP', limit: batchSize }, { actor, idempotencyKey: `scheduled:instruction-expiry:${state?.version ?? 0}`, now: input.now });
