...
} from '../order-instructions/expiry-scan';
...
export interface SafeJobRun {
  job_name: ScheduledJobName;
  outcome: 'SUCCEEDED' | 'PARTIAL' | 'FAILED' | 'SKIPPED' | 'DISABLED';
...
const SYSTEM_SCHEDULER_ACTOR = Object.freeze({
  ...
  memberTeamIds: Object.freeze([]),
  leaderTeamIds: Object.freeze([]),
...
async function runOne(
  ...
  input: Required<Pick<Parameters<typeof runScheduledOperations>[1], 'now'>> &
    Parameters<typeof runScheduledOperations>[1],
): Promise<SafeJobRun> {
  ...
  `,
    ...
    .bind(job, token, input.now + LEASE_MS, input.now, input.now)
    .first<{ job_name: string }>();
  if (!acquired)
    return record(database, job, input.trigger ?? 'CRON', 'SKIPPED', 0, 0, 0, 0, null, input.now);
  await database
    .prepare(
      `UPDATE scheduled_job_states SET last_started_at=?, updated_at=? WHERE job_name=? AND lease_token=?`,
    )
    .bind(input.now, input.now, job, token)
    .run();
  try {
    ...
    const completed = await database
      .prepare(
        `UPDATE scheduled_job_states SET lease_token=NULL, lease_expires_at=NULL, cursor_json=?, version=version+1, last_succeeded_at=CASE WHEN ?='SUCCEEDED' THEN ? ELSE last_succeeded_at END, last_failed_at=CASE WHEN ?='SUCCEEDED' THEN last_failed_at ELSE ? END, last_backlog_count=?, last_failure_category=?, updated_at=? WHERE job_name=? AND lease_token=?`,
      )
      .bind(
        result.cursorJson ?? null,
        outcome,
        input.now,
        outcome,
        input.now,
      ...
      .run();
    if (!statementChangedOnce(completed))
      return record(
        database,
        job,
        input.trigger ?? 'CRON',
    ...
    const completed = await database
      .prepare(
        `UPDATE scheduled_job_states SET lease_token=NULL, lease_expires_at=NULL, last_failed_at=?, last_failure_category=?, updated_at=? WHERE job_name=? AND lease_token=?`,
      )
      .bind(input.now, category, input.now, job, token)
      .run();
    if (!statementChangedOnce(completed))
      return record(
        database,
...
async function execute(
  database: SqlDatabase,
  job: ScheduledJobName,
  input: Parameters<typeof runScheduledOperations>[1] & { now: number },
): Promise<{
  processed: number;
  succeeded: number;
  failed: number;
  ...
  const batchSize =
    ...
    Number(input.batchSize) > 0 &&
    Number(input.batchSize) <= BATCH
  ...
  if (job === 'reservation_expiry') {
    ...
    const rows = await database
      .prepare(
        `SELECT id,version,due_at FROM (SELECT id,version,CASE WHEN status='PENDING_REVIEW' THEN hold_expires_at ELSE order_deadline_snapshot END AS due_at FROM product_reservations WHERE status IN ('PENDING_REVIEW','APPROVED')) WHERE due_at<=? AND (? IS NULL OR due_at>? OR (due_at=? AND id>?)) ORDER BY due_at,id LIMIT ?`,
      )
      .bind(
        input.now,
        cursor?.due ?? null,
      ...
      .all<{ id: string; version: number; due_at: number }>();
    let succeeded = 0;
    let failed = 0;
    if (input.dryRun)
      return {
        ...
        succeeded: 0,
        failed: 0,
        backlog: await countReservationExpiryCandidates(database, input.now),
        cursorJson: state?.cursor_json ?? undefined,
      };
    let last: { id: string; due_at: number } | undefined;
    for (const row of rows.results.slice(0, batchSize)) {
      if (input.deadlineReached?.()) break;
      try {
        await expireReservation(
          database,
          { reservationId: row.id, expectedVersion: Number(row.version) },
          {
            idempotencyKey: `scheduled:reservation-expiry:${row.id}:${row.due_at}`,
            now: input.now,
          },
        );
        succeeded += 1;
    ...
    const cursorJson = more
      ? last
        ? JSON.stringify({ due: last.due_at, id: last.id })
        : (state?.cursor_json ?? undefined)
      : undefined;
    return {
      processed,
      succeeded,
      failed,
      backlog: await countReservationExpiryCandidates(database, input.now),
      failureCategory: failed ? 'reservation_expiry_failed' : undefined,
      cursorJson,
    };
  }
  if (job === 'instruction_expiry') {
    ...
    const r = await runOrderInstructionExpiryScan(
      ...
      {
        actor,
        idempotencyKey: `scheduled:instruction-expiry:${state?.version ?? 0}`,
        now: input.now,
      },
    );
    if (r.completed)
    ...
    return {
      ...
      failureCategory: r.failed ? 'job_item_failed' : undefined,
    };
  }
  if (job === 'file_orphan_cleanup') {
    if (!input.storage)
      return {
        processed: 0,
        succeeded: 0,
...
async function countReservationExpiryCandidates(
  ...
  const row = await database
    .prepare(
      "SELECT COUNT(*) AS count FROM product_reservations WHERE (status='PENDING_REVIEW' AND hold_expires_at<=?) OR (status='APPROVED' AND order_deadline_snapshot<=?)",
    )
    .bind(now, now)
    .first<{ count: number }>();
  return Number(row?.count ?? 0);
}
async function countFileOrphanCleanupCandidates(
  database: SqlDatabase,
  now: number,
): Promise<number> {
  const row = await database
...
function classify(error: unknown): string {
  return error instanceof Error && error.name === 'StaffAuthError'
    ? 'dependency_unavailable'
    : 'job_execution_failed';
}
function parseCursor(value: string | null | undefined): { due: number; id: string } | null {
  try {
...
async function record(
  ...
  await database
    .prepare(
      `INSERT INTO scheduled_job_runs (id,job_name,trigger_type,outcome,processed_count,succeeded_count,failed_count,backlog_count,failure_category,request_id,started_at,finished_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    )
    .bind(
      crypto.randomUUID(),
      job,
