...
export class ColdArchiveCommandError extends Error {
  constructor(
    public readonly code: ClosureErrorCode,
    public readonly status: 400 | 403 | 404 | 409 | 503,
  ) {
    super(code);
    this.name = 'ColdArchiveCommandError';
  }
}
...
export async function recordOrderBusinessClosure(
  ...
  input: {
    ...
    notApplicable: readonly ArchiveComponent[];
    reason: string;
  },
  command: {
  ...
  const requestHash = await hashCanonicalJson({
    action: 'CLOSE_ORDER_ARCHIVE',
    formal_order_id: formalOrderId,
    expected_version: expectedVersion,
    not_applicable: notApplicable,
    reason,
  });
  const acquired = await acquireIdempotency<OrderArchiveClosureResultDto>(
    database,
    {
      actorType: 'STAFF',
    ...
    { now },
  ).catch(translateIdempotency);
  if (acquired.kind === 'REPLAY') return { ...acquired.response, replayed: true };
  try {
    await command.afterClaimed?.();
    const existing = await database
      .prepare(
        `SELECT formal_order_id,status,version,business_closed_at,archive_due_at,
    ...
    if ((existing?.version ?? 0) !== expectedVersion)
      throw new ColdArchiveCommandError('VERSION_CONFLICT', 409);
    if (existing?.status === 'CLOSED') throw new ColdArchiveCommandError('STATE_CONFLICT', 409);
    const order = await database
      .prepare(`SELECT confirmed_at FROM formal_orders WHERE id=? AND status='CONFIRMED'`)
      .bind(formalOrderId)
    ...
    const refund = await refundCompletion(
      database,
      formalOrderId,
      explicit.has('buyer_refund'),
      order.confirmed_at,
    );
    const principal = await payableCompletion(
      database,
      formalOrderId,
      'SELLER_PRINCIPAL',
      explicit.has('seller_principal'),
      order.confirmed_at,
    );
    const fee = await payableCompletion(
      database,
      formalOrderId,
      'SELLER_SERVICE_FEE',
      explicit.has('seller_service_fee'),
      order.confirmed_at,
    );
    const businessClosedAt = Math.max(
      order.confirmed_at,
      review.completedAt,
      refund.completedAt,
      principal.completedAt,
      fee.completedAt,
    );
    const response: OrderArchiveClosureResultDto = {
      ...
      archive_due_at: archiveDueAt(businessClosedAt),
      review_state: review.state,
      buyer_refund_state: refund.state,
      seller_principal_state: principal.state,
      seller_service_fee_state: fee.state,
      replayed: false,
    };
    const mutation = existing
      ? database
          .prepare(
            `UPDATE order_archive_closures SET review_state=?,buyer_refund_state=?,seller_principal_state=?,
        ...
        WHERE formal_order_id=? AND status='REOPENED' AND version=?`,
          )
          .bind(
            review.state,
            refund.state,
      ...
      : database
          .prepare(
            `INSERT INTO order_archive_closures(formal_order_id,review_state,buyer_refund_state,
...
export async function reopenOrderBusinessClosure(
  ...
  requireOwner(command.actor);
  const formalOrderId = safeId(input.formalOrderId);
  const expectedVersion = version(input.expectedVersion, false);
  const reason = safeReason(input.reason);
  const now = safeNow(command.now ?? Date.now());
  const requestHash = await hashCanonicalJson({
    action: 'REOPEN_ORDER_ARCHIVE',
    formal_order_id: formalOrderId,
    expected_version: expectedVersion,
    reason,
  });
  ...
  try {
    ...
    const response: OrderArchiveClosureResultDto = {
      ...
      status: 'REOPENED',
      version: expectedVersion + 1,
      business_closed_at: source.business_closed_at,
      archive_due_at: source.archive_due_at,
      review_state: source.review_state,
      buyer_refund_state: source.buyer_refund_state,
      seller_principal_state: source.seller_principal_state,
      seller_service_fee_state: source.seller_service_fee_state,
      replayed: false,
    };
...
function completion(
  ...
  input: {
    ...
    response: OrderArchiveClosureResultDto;
    existing: ClosureRow | null;
    reason: string;
    command: {
      actor: AssignmentStaffAuthorization;
      idempotencyKey: string;
    ...
    claim: Parameters<typeof completeIdempotencyStatement>[1];
    now: number;
  },
): SqlStatement[] {
  return [
    ...
    createAuditEventStatement(database, {
      ...
      aggregateType: 'ORDER_ARCHIVE_CLOSURE',
      aggregateId: input.response.formal_order_id,
      eventType:
        input.response.status === 'CLOSED' ? 'ORDER_ARCHIVE_CLOSED' : 'ORDER_ARCHIVE_REOPENED',
      actor: {
        type: 'STAFF',
        id: input.command.actor.staffId,
        roles: [...input.command.actor.roles],
      },
      requestId: input.command.requestId ?? null,
      idempotencyKey: input.claim.idempotencyKey,
      previousState: input.existing,
      nextState: input.response,
      reason: input.reason,
      createdAt: input.now,
    }),
    completeIdempotencyStatement(database, input.claim, input.response, {
      resultReferences: {
        formal_order_id: input.response.formal_order_id,
        closure_version: input.response.version,
      },
      now: input.now,
    }),
    assertClosure(database, input.response),
...
async function reviewCompletion(
  ...
  baseline: number,
): Promise<ComponentCompletion> {
  const row = await database
    .prepare(`SELECT status,decided_at FROM review_cases WHERE formal_order_id=?`)
    .bind(orderId)
    .first<{ status: string; decided_at: number | null }>();
  if (!row) return explicitNotApplicable(notApplicable, baseline);
  if (notApplicable || row.status !== 'APPROVED' || row.decided_at === null) incomplete();
  return { state: 'COMPLETED', completedAt: row.decided_at };
}
async function refundCompletion(
  database: SqlDatabase,
  orderId: string,
  notApplicable: boolean,
  baseline: number,
  ...
  const row = await database
    .prepare(
      `SELECT balance.status,COALESCE((SELECT MAX(entry.created_at) FROM buyer_refund_payment_entries entry
  ...
  if (!row) return explicitNotApplicable(notApplicable, baseline);
  if (notApplicable || row.status !== 'PAID') incomplete();
  return { state: 'COMPLETED', completedAt: row.completed_at };
}
async function payableCompletion(
  database: SqlDatabase,
  orderId: string,
  type: 'SELLER_PRINCIPAL' | 'SELLER_SERVICE_FEE',
  notApplicable: boolean,
  baseline: number,
): Promise<ComponentCompletion> {
  const row = await database
    .prepare(
      `SELECT balance.derived_status,COALESCE((SELECT MAX(allocation.created_at)
    ...
    FROM seller_payable_balances balance WHERE balance.formal_order_id=? AND balance.payable_type=?`,
    )
    .bind(orderId, type)
    .first<{ derived_status: string; completed_at: number }>();
  if (!row) return explicitNotApplicable(notApplicable, baseline);
  if (notApplicable || row.derived_status !== 'PAID') incomplete();
  return { state: 'COMPLETED', completedAt: row.completed_at };
}
function explicitNotApplicable(explicit: boolean, baseline: number): ComponentCompletion {
  if (!explicit) incomplete();
  return { state: 'NOT_APPLICABLE', completedAt: baseline };
}
function incomplete(): never {
  throw new ColdArchiveCommandError('STATE_CONFLICT', 409);
}
...
function components(values: readonly ArchiveComponent[]): ArchiveComponent[] {
  if (
    !Array.isArray(values) ||
    new Set(values).size !== values.length ||
    values.some((value) => !ARCHIVE_COMPONENTS.includes(value))
  )
    throw new ColdArchiveCommandError('VALIDATION_ERROR', 400);
  return [...values].sort() as ArchiveComponent[];
}
...
function version(value: number, allowZero: boolean): number {
...
function normalize(error: unknown): ColdArchiveCommandError {
  if (error instanceof ColdArchiveCommandError) return error;
  if (error instanceof IdempotencyError) {
    if (error.code === 'IDEMPOTENCY_CONFLICT')
      return new ColdArchiveCommandError('IDEMPOTENCY_CONFLICT', 409);
    if (error.code === 'REQUEST_IN_PROGRESS')
      return new ColdArchiveCommandError('REQUEST_IN_PROGRESS', 409);
    if (error.code === 'VALIDATION_ERROR')
      return new ColdArchiveCommandError('VALIDATION_ERROR', 400);
  }
  return new ColdArchiveCommandError('DEPENDENCY_UNAVAILABLE', 503);
}
