...
interface Attempt {
  ...
  request_hash: string;
  expected_archive_version: number;
...
export async function rehydrateArchivedFile(
  ...
  const fileObjectId = safe(input.fileObjectId, 120);
  const expectedArchiveVersion = positive(input.expectedArchiveVersion);
  const now = timestamp(command.now ?? Date.now());
  const requestHash = await hashCanonicalJson({
    action: 'REHYDRATE_DRIVE_ARCHIVE',
    file_object_id: fileObjectId,
    expected_archive_version: expectedArchiveVersion,
  });
  const acquired = await acquireIdempotency<FileDriveRehydrationResultDto>(
    database,
    {
      actorType: 'STAFF',
  ...
  try {
    ...
    const source = await database
      .prepare(
        `SELECT archive.drive_file_id,archive.version,manifest.byte_size,
    ...
    if (source.version !== expectedArchiveVersion)
      throw new ColdArchiveCommandError('VERSION_CONFLICT', 409);
    const id = await hashCanonicalJson({
      kind: 'FILE_DRIVE_REHYDRATION',
      staff_id: command.actor.staffId,
      idempotency_key: acquired.claim.idempotencyKey,
    });
    attempt = await database
      .prepare(
        `SELECT id,status,version,attempt_count,request_hash,expected_archive_version,
      file_object_id FROM file_drive_rehydrations WHERE requested_by_staff_id=? AND idempotency_key=?`,
      )
      .bind(command.actor.staffId, acquired.claim.idempotencyKey)
      .first<Attempt>();
    if (
      attempt &&
      (attempt.request_hash !== requestHash ||
        attempt.file_object_id !== fileObjectId ||
        attempt.expected_archive_version !== expectedArchiveVersion)
    )
      throw new ColdArchiveCommandError('IDEMPOTENCY_CONFLICT', 409);
    if (!attempt) {
      attempt = {
        id,
        status: 'STARTED',
      ...
      if (attempt.status === 'COMPLETED')
        throw new ColdArchiveCommandError('DEPENDENCY_UNAVAILABLE', 503);
    ...
    await verifyBytes(read.bytes, source.mime_type, source.byte_size, source.sha256);
    const existing = await r2.headObject(source.object_key);
    if (existing) {
      if (
        existing.byteSize !== source.byte_size ||
        existing.contentType !== source.mime_type ||
        existing.checksumSha256 !== source.sha256
      )
    ...
    if (
      !head ||
      head.byteSize !== source.byte_size ||
      head.contentType !== source.mime_type ||
      head.checksumSha256 !== source.sha256
    )
      throw new Error('archive_rehydration_r2_mismatch');
    const response: FileDriveRehydrationResultDto = {
      file_object_id: fileObjectId,
      status: 'COMPLETED',
      archive_version: expectedArchiveVersion,
      replayed: false,
    };
    await database.batch([
      database
        .prepare(
          `UPDATE file_drive_rehydrations SET status='COMPLETED',version=version+1,updated_at=MAX(?,updated_at+1),
        completed_at=? WHERE id=? AND status='STARTED' AND version=?`,
        )
        .bind(now, now, attempt.id, attempt.version),
      ...
      audit(database, {
        eventType: 'DRIVE_ARCHIVE_REHYDRATION_COMPLETED',
        fileObjectId,
        actor: command.actor,
        requestId: command.requestId ?? null,
        idempotencyKey: acquired.claim.idempotencyKey,
        archiveVersion: expectedArchiveVersion,
        now,
      }),
      completeIdempotencyStatement(database, acquired.claim, response, {
        resultReferences: { file_object_id: fileObjectId, archive_version: expectedArchiveVersion },
        now,
      }),
      database
        .prepare(
        ...
        .bind(attempt.id, attempt.version + 1),
    ...
    if (attempt?.status === 'STARTED')
      ...
      await markIdempotencyFailed(database, acquired.claim, normalized.code, now).catch(
        () => false,
      );
    throw normalized;
  }
...
function startStatements(
  ...
  const mutation = input.insert
    ? database
        .prepare(
          `INSERT INTO file_drive_rehydrations(id,file_object_id,target_object_key,status,
    ...
    attempt_count,version,created_at,updated_at,completed_at) VALUES(?,?,?,'STARTED',?,?,?,NULL,?,?,?,?,1,?,?,NULL)`,
        ...
        .bind(
          input.attempt.id,
          input.attempt.file_object_id,
          input.source.object_key,
          input.source.sha256,
          input.source.version,
          input.attempt.request_hash,
          input.actor.staffId,
          input.requestId,
          input.claim.idempotencyKey,
          input.attempt.attempt_count,
          input.now,
          input.now,
        )
    : database
        .prepare(
          `UPDATE file_drive_rehydrations SET status='STARTED',failure_category=NULL,completed_at=NULL,
      attempt_count=attempt_count+1,version=version+1,updated_at=MAX(?,updated_at+1)
      WHERE id=? AND status IN ('STARTED','FAILED') AND version=?`,
        )
        .bind(input.now, input.attempt.id, input.previousVersion);
  return [
    mutation,
    changedOnce(database),
    archiveEvent(
    ...
    audit(database, {
      eventType: 'DRIVE_ARCHIVE_REHYDRATION_STARTED',
      fileObjectId: input.attempt.file_object_id,
      actor: input.actor,
      requestId: input.requestId,
      idempotencyKey: input.claim.idempotencyKey,
      archiveVersion: input.source.version,
      now: input.now,
    }),
    database
      .prepare(
      ...
      .bind(input.attempt.id, input.attempt.version, input.attempt.attempt_count),
  ];
}
async function failAttempt(
  ...
  await database.batch([
    database
      .prepare(
        `UPDATE file_drive_rehydrations SET status='FAILED',failure_category=?,
    completed_at=?,version=version+1,updated_at=MAX(?,updated_at+1) WHERE id=? AND status='STARTED' AND version=?`,
      )
      .bind(input.category, input.now, input.now, input.attempt.id, input.attempt.version),
    changedOnce(database),
    archiveEvent(
      database,
      input.fileObjectId,
    ...
    audit(database, {
      eventType: 'DRIVE_ARCHIVE_REHYDRATION_FAILED',
    ...
    database
      .prepare(
        `UPDATE command_idempotency_records SET status='FAILED',
    ...
    database
      ...
      .bind(input.attempt.id, input.attempt.version + 1),
  ]);
}
...
function audit(
  ...
  return createAuditEventStatement(database, {
    ...
    aggregateType: 'FILE_DRIVE_REHYDRATION',
    aggregateId: input.fileObjectId,
    ...
    nextState: {
      ...
      archive_version: input.archiveVersion,
