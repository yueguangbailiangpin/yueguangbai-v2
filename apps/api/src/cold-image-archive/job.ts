...
const MAX_BATCH=50;
interface Candidate { file_object_id:string; purpose:string; archive_due_at:number; }
...
interface SourceFile {
  object_key:string; client_file_name:string; detected_mime:SupportedFileMime;
  uploaded_byte_size:number; uploaded_sha256:string;
}

export interface DriveArchiveBatchResult {
  processed:number; succeeded:number; failed:number; backlog:number;
  failureCategory?:string;
}

export async function reconcileDriveArchiveBatch(
  database:SqlDatabase,drive:DriveArchiveAdapter,input:{now?:number;limit?:number;deadlineReached?:()=>boolean},
):Promise<{processed:number;succeeded:number;failed:number}> {
  const now=input.now??Date.now();
  const limit=Number.isSafeInteger(input.limit)&&Number(input.limit)>0&&Number(input.limit)<=10?Number(input.limit):5;
  ...
  `).bind(limit).all<{file_object_id:string;drive_file_id:string;byte_size:number;mime_type:SupportedFileMime;sha256:string}>();
  ...
  for(const row of rows.results){
    ...
    try{
      ...
      await database.prepare(`INSERT INTO file_drive_archive_reconciliations(id,file_object_id,result,
        failure_category,checked_byte_size,checked_mime,checked_sha256,checked_at)
        VALUES(?,?,'HEALTHY',NULL,?,?,?,?)`).bind(crypto.randomUUID(),row.file_object_id,
          value.byteSize,value.mimeType,await sha256Hex(value.bytes),now).run();
      succeeded+=1;
    }catch(error){
      const category=classifyReconciliation(error);
      await database.prepare(`INSERT INTO file_drive_archive_reconciliations(id,file_object_id,result,
        failure_category,checked_byte_size,checked_mime,checked_sha256,checked_at)
        VALUES(?,?,'FAILED',?,NULL,NULL,NULL,?)`).bind(crypto.randomUUID(),row.file_object_id,category,now).run();
...
export async function runDriveArchiveBatch(
  ...
  input:{now?:number;limit?:number;copyEnabled:boolean;proxyReadEnabled:boolean;r2DeleteEnabled:boolean;dryRun?:boolean;deadlineReached?:()=>boolean},
):Promise<DriveArchiveBatchResult> {
  const now=input.now??Date.now();
  const limit=Number.isSafeInteger(input.limit)&&Number(input.limit)>0&&Number(input.limit)<=MAX_BATCH?Number(input.limit):MAX_BATCH;
  const controls=await readControls(database);
  if (!input.copyEnabled || controls.copy_enabled!==1) return {processed:0,succeeded:0,failed:0,backlog:await countBacklog(database,now)};
  ...
  for (const candidate of candidates.slice(0,limit)) {
    ...
    try {
      const outcome=await processCandidate(database,r2,drive,candidate,{
        now,
        allowDelete:input.proxyReadEnabled&&input.r2DeleteEnabled
          && controls.proxy_read_enabled===1&&controls.r2_delete_enabled===1,
      });
  ...
  return {processed,succeeded,failed,backlog:await countBacklog(database,now),
    ...(failed?{failureCategory:'job_item_failed'}:{})};
}

async function processCandidate(
  database:SqlDatabase,r2:ObjectStorageAdapter,drive:DriveArchiveAdapter,candidate:Candidate,
  ...
  await database.batch([
    database.prepare(`INSERT INTO file_drive_archives(
      ...
      .bind(candidate.file_object_id,candidate.purpose,candidate.archive_due_at,input.now,input.now),
  ...
  const leased=await database.prepare(`
    ...
    RETURNING *
  `).bind(token,input.now+LEASE_MS,input.now,candidate.file_object_id,input.now,input.now,input.now)
    .first<ArchiveRow>();
  ...
  try {
    ...
    if (leased.status==='DRIVE_VERIFIED') {
      if (!input.allowDelete) { await release(database,leased.file_object_id,token,input.now); return 'SUCCEEDED'; }
    ...
    await verifyBytes(bytes,source.detected_mime,source.uploaded_byte_size,source.uploaded_sha256);
    let copying=leased;
    if (copying.status==='R2_HOT') copying=await startCopy(database,copying,token,input.now);
    if (!copying.drive_file_id) {
      const upload=await drive.upload({fileObjectId:copying.file_object_id,fileName:source.client_file_name,
        mimeType:source.detected_mime,byteSize:source.uploaded_byte_size,sha256:source.uploaded_sha256,
        bytes,resumeSessionKey:copying.resumable_session_key});
      if (!upload.completed || !upload.fileId) {
        await saveResume(database,copying,token,upload.resumeSessionKey,input.now);
        return 'DEFERRED';
      }
      copying=await saveUploadedIdentity(database,copying,token,{...upload,fileId:upload.fileId},input.now);
    }
    if (!copying.drive_file_id) throw tagged('d1_conflict');
    const readBack=await drive.readFile(copying.drive_file_id);
    await verifyBytes(readBack.bytes,source.detected_mime,source.uploaded_byte_size,source.uploaded_sha256);
    if (readBack.mimeType!==source.detected_mime || readBack.byteSize!==source.uploaded_byte_size) {
      throw tagged('manifest_mismatch');
    }
    const verified=await finalizeManifest(database,copying,token,source,input.now);
    if (!input.allowDelete) { await release(database,verified.file_object_id,token,input.now); return 'SUCCEEDED'; }
    const pending=await transitionToDeletePending(database,verified,token,input.now);
    return await deleteR2(database,r2,drive,pending,token,input);
  } catch (error) {
    await failArchive(database,leased.file_object_id,token,classify(error),input.now).catch(()=>undefined);
    throw error;
  }
}

async function eligibleCandidates(database:SqlDatabase,now:number,limit:number):Promise<Candidate[]> {
  ...
  `).bind(now,now,limit).all<Candidate>();
...
async function requireSource(database:SqlDatabase,fileObjectId:string):Promise<SourceFile> {
  ...
  `).bind(fileObjectId).first<SourceFile>();
  if (!row) throw tagged('d1_conflict');
  return row;
}

async function startCopy(database:SqlDatabase,row:ArchiveRow,token:string,now:number):Promise<ArchiveRow> {
  return atomicTransition(database,row.file_object_id,token,`UPDATE file_drive_archives SET status='DRIVE_COPYING',attempt_count=attempt_count+1,
    next_retry_at=NULL,last_failure_category=NULL,version=version+1,updated_at=MAX(?,updated_at+1)
    WHERE file_object_id=? AND status='R2_HOT' AND lease_token=?`,[now,row.file_object_id,token],
    'COPY_STARTED','R2_HOT','DRIVE_COPYING',row.version+1,null,now);
}
async function saveResume(database:SqlDatabase,row:ArchiveRow,token:string,session:string|null,now:number):Promise<void> {
  await atomicMutation(database,database.prepare(`UPDATE file_drive_archives SET resumable_session_key=?,lease_token=NULL,
    lease_expires_at=NULL,next_retry_at=?,version=version+1,updated_at=MAX(?,updated_at+1)
    WHERE file_object_id=? AND status='DRIVE_COPYING' AND lease_token=?`).bind(session,now,now,row.file_object_id,token),
    archiveEventStatement(database,row.file_object_id,'COPY_RESUMED','DRIVE_COPYING','DRIVE_COPYING',row.version+1,null,now));
}
async function saveUploadedIdentity(database:SqlDatabase,row:ArchiveRow,token:string,upload:{fileId:string;folderId:string;ownerAccountKey:string},now:number):Promise<ArchiveRow> {
  return atomicTransition(database,row.file_object_id,token,`UPDATE file_drive_archives SET drive_file_id=?,drive_folder_id=?,owner_account_key=?,
  ...
  'DRIVE_UPLOAD_RECORDED','DRIVE_COPYING','DRIVE_COPYING',row.version+1,null,now);
}
async function finalizeManifest(database:SqlDatabase,row:ArchiveRow,token:string,source:SourceFile,now:number):Promise<ArchiveRow> {
  if(!row.drive_file_id||!row.drive_folder_id||!row.owner_account_key) throw tagged('d1_conflict');
  const id=crypto.randomUUID();
  await database.batch([
    database.prepare(`INSERT INTO file_drive_archive_manifests(id,file_object_id,drive_file_id,drive_folder_id,
      owner_account_key,byte_size,mime_type,sha256,verified_at,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`)
      .bind(id,row.file_object_id,row.drive_file_id,row.drive_folder_id,row.owner_account_key,
        source.uploaded_byte_size,source.detected_mime,source.uploaded_sha256,now,now),
    database.prepare(`UPDATE file_drive_archives SET status='DRIVE_VERIFIED',uploaded_byte_size=?,uploaded_mime=?,
      ...
      .bind(source.uploaded_byte_size,source.detected_mime,source.uploaded_sha256,now,now,row.file_object_id,token),
    changedOnceStatement(database),
    archiveEventStatement(database,row.file_object_id,'DRIVE_VERIFIED','DRIVE_COPYING','DRIVE_VERIFIED',row.version+1,null,now),
    database.prepare(`INSERT INTO transaction_assertions(assertion_value) SELECT CASE WHEN EXISTS(
      ...
      AND archive.status='DRIVE_VERIFIED' AND archive.lease_token=?) THEN 1 ELSE 0 END`).bind(row.file_object_id,token),
  ]);
  const current=await database.prepare('SELECT * FROM file_drive_archives WHERE file_object_id=?').bind(row.file_object_id).first<ArchiveRow>();
  if(!current) throw tagged('d1_conflict'); return current;
}
async function transitionToDeletePending(database:SqlDatabase,row:ArchiveRow,token:string,now:number):Promise<ArchiveRow> {
  return atomicTransition(database,row.file_object_id,token,`UPDATE file_drive_archives SET status='R2_DELETE_PENDING',
    version=version+1,updated_at=MAX(?,updated_at+1) WHERE file_object_id=? AND status='DRIVE_VERIFIED'
    AND lease_token=?`,[now,row.file_object_id,token],
    'R2_DELETE_REQUESTED','DRIVE_VERIFIED','R2_DELETE_PENDING',row.version+1,null,now);
}
async function deleteR2(database:SqlDatabase,r2:ObjectStorageAdapter,drive:DriveArchiveAdapter,row:ArchiveRow,token:string,input:{now:number;allowDelete:boolean}):Promise<'SUCCEEDED'|'DEFERRED'> {
  const controls=await readControls(database);
  if(!input.allowDelete||controls.copy_enabled!==1||controls.proxy_read_enabled!==1||controls.r2_delete_enabled!==1) {
    await release(database,row.file_object_id,token,input.now); return 'DEFERRED';
  }
  ...
  try { await r2.deleteObject(source.object_key); }
  catch(error) { await failArchive(database,row.file_object_id,token,'r2_delete_failed',input.now); throw error; }
  await atomicMutation(database,database.prepare(`UPDATE file_drive_archives SET status='DRIVE_ARCHIVED',r2_deleted_at=?,
    ...
    AND lease_token=?`).bind(input.now,input.now,input.now,row.file_object_id,token),
    archiveEventStatement(database,row.file_object_id,'DRIVE_ARCHIVED','R2_DELETE_PENDING','DRIVE_ARCHIVED',row.version+1,null,input.now));
  return 'SUCCEEDED';
}
async function verifyDriveCopy(drive:DriveArchiveAdapter,row:ArchiveRow,source:SourceFile):Promise<void> {
  if(!row.drive_file_id||row.uploaded_byte_size===null||row.uploaded_mime===null||!row.uploaded_sha256) {
    throw tagged('d1_conflict');
  }
  const readBack=await drive.readFile(row.drive_file_id);
  await verifyBytes(readBack.bytes,source.detected_mime,source.uploaded_byte_size,source.uploaded_sha256);
  if(readBack.byteSize!==row.uploaded_byte_size||readBack.mimeType!==row.uploaded_mime
    ||source.uploaded_byte_size!==row.uploaded_byte_size||source.detected_mime!==row.uploaded_mime
    ||source.uploaded_sha256!==row.uploaded_sha256) throw tagged('manifest_mismatch');
}
async function stillEligibleForDeletion(database:SqlDatabase,fileObjectId:string,now:number):Promise<boolean>{
  const row=await database.prepare(`WITH associated(formal_order_id) AS (
  ...
  ) THEN 1 ELSE 0 END) AS eligible FROM associated`).bind(fileObjectId,fileObjectId,fileObjectId,fileObjectId,now)
    .first<{total:number;eligible:number|null}>();
  return Number(row?.total??0)>0&&Number(row?.eligible??0)===Number(row?.total??0);
}
async function release(database:SqlDatabase,fileId:string,token:string,now:number):Promise<void> {
  const result=await database.prepare(`UPDATE file_drive_archives SET lease_token=NULL,lease_expires_at=NULL,
    version=version+1,updated_at=MAX(?,updated_at+1) WHERE file_object_id=? AND lease_token=?`).bind(now,fileId,token).run();
  if(!statementChangedOnce(result)) throw tagged('d1_conflict');
}
async function failArchive(database:SqlDatabase,fileId:string,token:string,category:string,now:number):Promise<void> {
  const current=await database.prepare(`SELECT status,version FROM file_drive_archives WHERE file_object_id=? AND lease_token=?`)
    .bind(fileId,token).first<{status:FileDriveArchiveState;version:number}>();
  if(!current)throw tagged('d1_conflict');
  await atomicMutation(database,database.prepare(`UPDATE file_drive_archives SET lease_token=NULL,lease_expires_at=NULL,
...
function archiveEventStatement(database:SqlDatabase,fileId:string,type:string,previous:string,next:string,version:number,failure:string|null,now:number) {
...
function changedOnceStatement(database:SqlDatabase){return database.prepare(`INSERT INTO transaction_assertions(assertion_value)
  SELECT CASE WHEN changes()=1 THEN 1 ELSE 0 END`);}
async function atomicMutation(database:SqlDatabase,mutation:ReturnType<SqlDatabase['prepare']>,audit:ReturnType<SqlDatabase['prepare']>):Promise<void>{
  await database.batch([mutation,changedOnceStatement(database),audit]);
}
async function atomicTransition(database:SqlDatabase,fileId:string,token:string,sql:string,binds:readonly unknown[],
  type:string,previous:string,next:string,version:number,failure:string|null,now:number):Promise<ArchiveRow>{
  let mutation=database.prepare(sql);mutation=mutation.bind(...binds);
  await atomicMutation(database,mutation,archiveEventStatement(database,fileId,type,previous,next,version,failure,now));
  const updated=await database.prepare(`SELECT * FROM file_drive_archives WHERE file_object_id=? AND lease_token=? AND version=?`)
    .bind(fileId,token,version).first<ArchiveRow>();
  if(!updated)throw tagged('d1_conflict');
  return updated;
}
async function readControls(database:SqlDatabase):Promise<{copy_enabled:number;proxy_read_enabled:number;r2_delete_enabled:number}> {
  const row=await database.prepare('SELECT copy_enabled,proxy_read_enabled,r2_delete_enabled FROM drive_archive_controls WHERE singleton_id=1').first<{copy_enabled:number;proxy_read_enabled:number;r2_delete_enabled:number}>();
  if(!row) throw new Error('drive_archive_controls_missing'); return row;
}
async function countBacklog(database:SqlDatabase,now:number):Promise<number> { return (await eligibleCandidates(database,now,MAX_BATCH+1)).length; }
async function recordFailureSignal(database:SqlDatabase,fileId:string,now:number):Promise<void> {
  const id=await hashCanonicalJson({kind:'DRIVE_ARCHIVE_FAILURE',file_object_id:fileId,observed_at:now});
  await database.prepare(`INSERT OR IGNORE INTO scheduled_job_states(job_name,updated_at)
    VALUES('drive_archive',?)`).bind(now).run();
  await ingestScheduledOperationalSignal(database,{observation_id:id,signal_type:'file_failure',
    summary_code:'FILE_PROCESSING_FAILURE',job_name:'drive_archive',observation_state:'BREACH',
    observed_at:now,count_value:1});
}
function classify(error:unknown):string { if(error instanceof Error){ if(error.name==='DriveAuthorizationError')return'authorization_failed'; if(error.message==='manifest_mismatch')return'manifest_mismatch'; if(error.message.includes('missing'))return'drive_missing'; if(error.message.includes('read'))return'read_back_failed'; if(error.message.includes('drive'))return'upload_failed'; } return'adapter_unavailable'; }
function classifyReconciliation(error:unknown):'authorization_failed'|'read_back_failed'|'manifest_mismatch'|'drive_missing' { if(error instanceof Error){if(error.name==='DriveAuthorizationError')return'authorization_failed';if(error.message==='manifest_mismatch')return'manifest_mismatch';if(error.message.includes('missing'))return'drive_missing';}return'read_back_failed'; }
function retryDelay(category:string):number { return category==='authorization_failed'||category==='manifest_mismatch'?3_600_000:60_000; }
function tagged(message:string):Error { return new Error(message); }
