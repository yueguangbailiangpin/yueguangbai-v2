...
export async function submitBuyerDailyExchangeRate(
  ...
  const acquired =
    await acquireIdempotency<SubmitBuyerDailyExchangeRateResult>(
      database,
      {
        actorType: 'STAFF',
...
async function decideBuyerDailyExchangeRate(
  ...
  const reason = input.decision === 'REJECT'
    ...
    : null;
  const now = cleanEpochMilliseconds(command.now ?? Date.now());
  const action = input.decision === 'CONFIRM'
  ...
  const acquired =
    await acquireIdempotency<DecideBuyerDailyExchangeRateResult>(
      database,
      {
        actorType: 'STAFF',
        actorId: command.actor.staffId,
  ...
  try {
    ...
    await database.batch([
      ...
      completeIdempotencyStatement(
        ...
        acquired.claim,
        response,
        { resultReferences: { rate_id: rateId }, now },
      ),
      assertBuyerRateDecided(
        database,
        acquired.claim,
        response,
      ),
...
async function readLatestBuyerRate(
  ...
  const row = await database.prepare(`
    ...
    FROM buyer_daily_exchange_rates
    WHERE business_date=?
  `).bind(businessDate).first<LatestBuyerRateRow>();
  return {
...
async function requireBuyerRate(
  ...
  const row = await database.prepare(`
    ...
    FROM buyer_daily_exchange_rates
    WHERE id=?
  `).bind(rateId).first<BuyerRateRow>();
