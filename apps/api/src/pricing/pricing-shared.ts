...
export function requireSellerOpsSubmitter(
  actor: PricingStaffActor,
): void {
  validateActor(actor);
  if (!actor.roles.includes('seller_ops')) {
    throw new PricingError('FORBIDDEN', 403);
  }
}

export function requireOwnerConfirmer(
  actor: PricingStaffActor,
): void {
  validateActor(actor);
...
export function cleanPricingIdentifier(
  raw: string,
  maximum = 120,
): string {
...
export function cleanPricingReason(
  raw: string,
): string {
