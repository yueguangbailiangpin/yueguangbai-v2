...
import {
  ...
  type OperationalAlertServiceBinding,
  type OperationalAlertVerificationChallenge,
  type OperationalAlertVerificationReceipt,
} from './alert-sink-contract';
import {
  OPERATIONAL_ALERT_ATTESTATION_AGGREGATE,
  OPERATIONAL_ALERT_ATTESTATION_EVENT,
  registerOperationalAlertAttestationRoutes,
} from './alert-attestation';
import { registerOperationalReadinessRoutes } from './routes';

const RELEASE = 'a'.repeat(40),
  IDENTITY = 'service:operations-primary',
  VERSION = 'deploy-001',
  SERVICE = 'ygb-operational-alerts',
...
describe('operational alert production readiness', () => {
  it('rejects local self-certification, invalid release identifiers and descriptor drift', async () => {
    ...
    const base = productionBindings(new ReceiptSink());
  ...
  it(
    'fails closed for missing, expired, cross-release or incomplete immutable attestations',
    async () => {
      database = createMigratedTestDatabase();
      const base = productionBindings(new ReceiptSink());
      ...
      database.close();
      database = createMigratedTestDatabase();
      await seedAttestation({ release_sha: 'c'.repeat(40) });
      expect(await alertCheck(base, NOW)).toBe('failed');
      database.close();
      database = createMigratedTestDatabase();
      await seedAttestation({ verified_receipts: [] });
      expect(await alertCheck(base, NOW)).toBe('failed');
    },
  ...
  it('allows disabled only outside production and local console only in local development', async () => {
    database = createMigratedTestDatabase();
    expect(
      await alertCheck({ APP_ENVIRONMENT: 'local', OPERATIONAL_ALERT_MODE: 'disabled' }, NOW),
    ).toBe('ok');
    expect(
      await alertCheck({ APP_ENVIRONMENT: 'local', OPERATIONAL_ALERT_MODE: 'local' }, NOW),
    ).toBe('ok');
    expect(
      await alertCheck({ APP_ENVIRONMENT: 'staging', OPERATIONAL_ALERT_MODE: 'disabled' }, NOW),
    ).toBe('not_required');
    expect(
      await alertCheck({ APP_ENVIRONMENT: 'staging', OPERATIONAL_ALERT_MODE: 'local' }, NOW),
    ).toBe('failed');
  });
  ...
  it('reports disabled staging-only production gates as not required without weakening production', async () => {
    ...
    await expect(selfOriginTeam.json()).resolves.toMatchObject({
      data: { checks: { staff_access: 'failed' } },
    });
  ...
  it('fails staging readiness when the empty-bucket object storage probe rejects', async () => {
    ...
    expect(storageHeadCalls).toBe(1);
    await expect(response.json()).resolves.toMatchObject({
      data: { status: 'not_ready', checks: { object_storage: 'failed' } },
    });
  });
  ...
  it('calls all three safe RPC challenges and atomically records only verified receipt summaries', async () => {
    ...
    const now = Date.now();
    const body = { expires_at: now + 60_000, evidence_reference: 'operator-runbook-evidence-001' };
    expect(
      (
        await attestationRequest(
          token,
          'alert-attestation-missing-origin',
          body,
          new ReceiptSink(),
          { Origin: null },
        )
    ...
    expect(
      (
        await attestationRequest(
          token,
          'alert-attestation-foreign-origin',
          body,
          new ReceiptSink(),
    ...
    expect(
      (
        await attestationRequest(
          token,
          'alert-attestation-extra-body',
          { ...body, delivery_result: 'PASS' },
          new ReceiptSink(),
        )
      ).status,
    ).toBe(400);
    const invoke = (value: Record<string, unknown>) =>
      attestationRequest(token, 'alert-attestation-command-001', value, sink);
    const first = await invoke(body);
    expect(first.status).toBe(201);
    const firstBody = (await first.json()) as {
      data: { attestation_id: string; verified_receipts: unknown[] };
    };
    expect(firstBody.data.verified_receipts).toHaveLength(3);
    expect(sink.calls.map((value) => value.challenge_type)).toEqual([
      'DELIVERY',
      'FAILURE',
      'RECOVERY',
    ]);
    expect(
      sink.calls.every((value) => value.simulation_mode === 'SAFE_NO_PRODUCTION_DISRUPTION'),
    ).toBe(true);
    const replay = await invoke(body);
    expect(replay.status).toBe(200);
    await expect(replay.json()).resolves.toMatchObject({
      data: { attestation_id: firstBody.data.attestation_id },
    });
    expect(sink.calls).toHaveLength(3);
    const conflict = await invoke({ ...body, evidence_reference: 'operator-runbook-evidence-002' });
    ...
    expect(() =>
      database!.raw
        .prepare(`UPDATE audit_events SET reason='tampered' WHERE id=?`)
        .run(firstBody.data.attestation_id),
    ).toThrow('audit_events_are_immutable');
  });

  it('rejects malformed, stale, duplicated or failed RPC receipts with 503 and zero success ghosts', async () => {
    const { token } = await ownerSession();
    const now = Date.now(),
      body = { expires_at: now + 60_000, evidence_reference: 'operator-negative-evidence-001' };
    const corruptions: Array<
      (
        receipt: OperationalAlertVerificationReceipt,
        challenge: OperationalAlertVerificationChallenge,
        index: number,
      ) => unknown
    > = [
      (r) => ({ ...r, nonce: 'wrong-nonce' }),
      (r) => ({ ...r, release_sha: 'c'.repeat(40) }),
      (r) => ({ ...r, binding_fingerprint: 'd'.repeat(64) }),
      (r) => ({ ...r, challenge_type: 'RECOVERY' }),
      (r) => ({ ...r, sink_identity: 'service:operations-other' }),
      (r) => ({ ...r, sink_deployment_version: 'deploy-999' }),
      (r) => ({ ...r, observed_outcome: 'WRONG' }),
      (r) => ({ ...r, issued_at: Date.now() + 60_000 }),
      (r) => {
        const value = { ...r };
        delete (value as Partial<OperationalAlertVerificationReceipt>).nonce;
        return value;
      },
      (r) => ({ ...r, expires_at: Date.now() - 1 }),
      () => ({ unknown: true }),
    ];
    let index = 0;
    for (const corrupt of corruptions) {
      const response = await attestationRequest(
        token,
        `alert-negative-${index}`,
        body,
        new ReceiptSink(corrupt),
      );
      expect(response.status, `corruption ${index}`).toBe(503);
      index += 1;
    }
    const duplicate = new ReceiptSink((receipt, _challenge, receiptIndex) => ({
      ...receipt,
      receipt_id: receiptIndex === 0 ? 'receipt-duplicate' : 'receipt-duplicate',
    }));
    expect(
      (await attestationRequest(token, 'alert-negative-duplicate', body, duplicate)).status,
    ).toBe(503);
    const failed = new ReceiptSink(undefined, true);
    expect(
      (await attestationRequest(token, 'alert-negative-rpc-failure', body, failed)).status,
    ).toBe(503);
    expect(
      database!.raw
        .prepare(
...
class ReceiptSink implements OperationalAlertServiceBinding {
  ...
  async verifyOperationalAlertChallenge(
    ...
    if (this.fail) throw new Error('sink_rpc_failed');
    const index = this.calls.length;
    this.calls.push(challenge);
    const receipt: OperationalAlertVerificationReceipt = {
      protocol_version: 'moonwhite-operational-alert-verification-v1',
      receipt_id: `receipt-${challenge.challenge_type.toLowerCase()}-${challenge.challenge_id}`,
      challenge_id: challenge.challenge_id,
      challenge_type: challenge.challenge_type,
      nonce: challenge.nonce,
      release_sha: challenge.release_sha,
      binding_fingerprint: challenge.binding_fingerprint,
      sink_identity: challenge.sink_identity,
      sink_deployment_version: challenge.sink_deployment_version,
...
function productionBindings(sink: OperationalAlertServiceBinding): Partial<AppBindings> {
  return {
    APP_ENVIRONMENT: 'production',
    APP_RELEASE_SHA: RELEASE,
    OPERATIONAL_ALERT_MODE: 'bound',
    OPERATIONAL_ALERT_SINK: sink,
    OPERATIONAL_ALERT_SINK_SERVICE: SERVICE,
    OPERATIONAL_ALERT_SINK_ENTRYPOINT: ENTRYPOINT,
    OPERATIONAL_ALERT_SINK_IDENTITY: IDENTITY,
    OPERATIONAL_ALERT_SINK_DEPLOYMENT_VERSION: VERSION,
    OPERATIONAL_ALERT_SINK_CONFIG_FINGERPRINT: FINGERPRINT,
  };
}
async function alertCheck(bindings: Partial<AppBindings>, now: number): Promise<string> {
  const response = await ready(bindings, now);
  const body = (await response.json()) as { data: { checks: { operational_alerts: string } } };
  return body.data.checks.operational_alerts;
}
async function ready(bindings: Partial<AppBindings>, now: number): Promise<Response> {
...
async function seedAttestation(overrides: Record<string, unknown> = {}): Promise<void> {
  const receipts = ['DELIVERY', 'FAILURE', 'RECOVERY'].map((type, index) => ({
    receipt_id: `stored-receipt-${index}`,
    challenge_id: `stored-challenge-${index}`,
    challenge_type: type,
    observed_outcome:
      type === 'DELIVERY'
        ? 'DELIVERED'
    ...
    sink_deployment_version: VERSION,
  }));
  const value = {
    ...
    sink_deployment_version: VERSION,
    sink_config_fingerprint: FINGERPRINT,
    verified_at: NOW - 60_000,
    expires_at: NOW + 60_000,
    evidence_reference: 'operator-evidence-ticket-001',
    verified_by_staff_id: 'staff-owner',
    verified_receipts: receipts,
    ...overrides,
  };
  await createAuditEventStatement(database!, {
    id: 'audit-attestation-001',
    aggregateType: OPERATIONAL_ALERT_ATTESTATION_AGGREGATE,
    aggregateId: `${RELEASE}:${FINGERPRINT}`,
    eventType: OPERATIONAL_ALERT_ATTESTATION_EVENT,
    actor: { type: 'STAFF', id: 'staff-owner', roles: ['owner'] },
    nextState: value,
    createdAt: NOW - 60_000,
  }).run();
}
async function ownerSession(): Promise<{ token: string }> {
  database = createMigratedTestDatabase();
  database.exec(
  ...
  await createInternalStaffSession(database, {
    token,
    identity: {
      identity_id: 'alert-owner-identity',
      staff_id: 'staff-alert-owner',
      identity_status: 'ACTIVE',
      identity_user_id: null,
      display_name: '告警证明管理员',
      staff_status: 'ACTIVE',
      authorization_version: 1,
      session_version: 1,
    },
    requestId: 'alert-attestation-session',
    now,
...
async function attestationRequest(
  ...
  const headers = new Headers({
    ...
    Cookie: `${STAFF_SESSION_COOKIE_NAME}=${token}`,
  });
  ...
  return app.request(
    'https://app.example.test/api/staff/production-readiness/operational-alert-attestations',
    { method: 'POST', headers, body: JSON.stringify(body) },
    {
      DB: database!,
      ...productionBindings(sink),
      CUSTOMER_SECURITY_TOKEN_SECRET: 'alert-test-secret-with-more-than-thirty-two-bytes',
    },
  );
}
