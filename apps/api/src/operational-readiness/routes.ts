...
export function registerOperationalReadinessRoutes(app:Hono<any>):void{
  app.get('/ready',async(context)=>{
    const now=Date.now();
    const result=await evaluateReadiness(context.env.DB,context.env.FILE_OBJECT_STORAGE??null,now).catch(()=>({ready:false,schema:false,scheduler:false,acquisition_maintenance:false,object_storage:false,recovery:false}));
    context.header('Cache-Control','no-store');
    return context.json(apiSuccess({status:result.ready?'ready' as const:'not_ready' as const,checks:{schema:result.schema?'ok':'failed',scheduler:result.scheduler?'ok':'failed',acquisition_maintenance:result.acquisition_maintenance?'ok':'failed',object_storage:result.object_storage?'ok':'failed',recovery:result.recovery?'ok':'failed'},timestamp:now},String(context.get('requestId')??crypto.randomUUID())),result.ready?200:503);
  });
}
...
async function evaluateReadiness(database:SqlDatabase,storage:ObjectStorageAdapter|null,now:number){
  ...
  const scheduler=REQUIRED_JOBS.every((name)=>{const row=byName.get(name);if(!row||row.last_succeeded_at===null)return false;const succeeded=Number(row.last_succeeded_at),failed=row.last_failed_at===null?null:Number(row.last_failed_at);return now-succeeded<=MAX_JOB_STALENESS_MS&&(failed===null||succeeded>=failed);});
