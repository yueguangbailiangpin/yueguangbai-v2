...
export async function readFinanceSummary(
  ...
  return Object.freeze({
    ...
    seller_unallocated_credit_cny_fen:
      await readUnallocatedCredit(database, filters),
    data_as_of: dataAsOf,
    filters,
  });
}
...
export async function readFinanceOrderPage(
  ...
  const visibleRows = result.results.slice(0, input.limit);
  const visible = visibleRows.map(mapPosition);
...
export async function readFinanceGroups(
  ...
  for (const [key, rows] of [...grouped].sort(
...
export async function readFinanceExceptions(
  ...
  return Object.freeze(rows.filter(
    ...
    suggested_actions: Object.freeze([
      suggestedAction(row.finance_status),
    ]),
  })));
}
...
function positionWhere(
  ...
  if (cursor !== null) {
    clauses.push(
      '(position.confirmed_at<? OR '
      + '(position.confirmed_at=? AND position.formal_order_id<?))',
    );
    values.push(
      cursor.confirmed_at,
      cursor.confirmed_at,
...
function mapPosition(row: PositionRow): InternalOrderFinancePositionDto {
  return Object.freeze({
    ...
    seller_principal_due_cny_fen: integer(
      row.seller_principal_due_cny_fen,
    ),
    ...
    buyer_refund_overpaid_cny_fen: integer(
      row.buyer_refund_overpaid_cny_fen,
    ),
    attributed_cash_net_cny_fen: integer(
      row.attributed_cash_net_cny_fen,
    ),
...
async function readUnallocatedCredit(
  ...
  const result = await database.prepare(`
    SELECT unallocated_credit_cny_fen AS value
...
function groupKey(
  ...
  groupBy: FinanceGroupBy,
  basis: InternalFinanceFilters['date_basis'],
): string | null {
