...
export function registerBuyerReviewRoutes(app: Hono<any>): void {
  ...
  app.get(
    ...
    session,
    withBuyerReviewErrors(listEligibleOrders),
  );
  app.post(
    '/api/buyer-portal/reviews',
    customerAuthOriginGuard(),
    session,
    withBuyerReviewErrors(createReview),
  );
  app.get(
    '/api/buyer-portal/reviews',
    session,
    withBuyerReviewErrors(listOwnReviews),
  );
  app.get(
    '/api/buyer-portal/reviews/:id',
    session,
    withBuyerReviewErrors(getOwnReview),
  );
  app.post(
    '/api/buyer-portal/reviews/:id/resubmit',
    customerAuthOriginGuard(),
    session,
    withBuyerReviewErrors(resubmitReview),
  );
  app.post(
    '/api/buyer-portal/reviews/:id/withdraw',
    customerAuthOriginGuard(),
    session,
    withBuyerReviewErrors(withdrawOwnReview),
  );
  app.post(
...
async function listEligibleOrders(context: Context<any>): Promise<Response> {
  ...
  const page = await listBuyerReviewEligibleOrders(
    context.env.DB,
    buyer,
    {
      limit: parseBuyerReviewPageLimit(
        singleQuery(url, 'limit') ?? undefined,
      ),
      cursor: decodeEligibleReviewOrderCursor(
...
async function listOwnReviews(context: Context<any>): Promise<Response> {
  ...
  const url = new URL(context.req.url);
  rejectUnknownOrRepeatedQuery(url);
  const page = await listBuyerReviews(
    context.env.DB,
    buyer,
    {
      limit: parseBuyerReviewPageLimit(
      ...
      cursor: decodeBuyerReviewCursor(
        singleQuery(url, 'cursor') ?? undefined,
      ),
    },
  );
...
async function getOwnReview(context: Context<any>): Promise<Response> {
  const buyer = await requireBuyerPortalContext(context);
  const review = await getBuyerReview(
    context.env.DB,
    buyer,
    requireRouteId(context),
  );
  return success(context, { review });
}
...
async function withdrawOwnReview(context: Context<any>): Promise<Response> {
  ...
  const result = await withdrawReview(
    context.env.DB,
    {
      reviewCaseId,
      expectedVersion: body.expected_version,
...
async function createReviewFileReadIntent(
  context: Context<any>,
): Promise<Response> {
  const buyer = await requireBuyerPortalContext(context);
  assertBuyerReviewBusinessAccess(buyer);
  ...
  const reviewCaseId = requireRouteId(context);
  const fileEntityLinkId = requireFileLinkRouteId(context);
...
function parseReadIntentBody(
  value: unknown,
): CreateBuyerReviewFileReadIntentRequest {
  const body = requireRecord(value);
  requireAllowedKeys(
    body,
  ...
  return {
    expected_file_version: positiveVersion(body['expected_file_version']),
  };
}

function evidenceFiles(
  value: unknown,
): SubmitBuyerReviewRequest['evidence_files'] {
  ...
  if (new Set(files.map((file) => file.file_object_id)).size
    !== files.length) {
    validationError();
  }
...
function withBuyerReviewErrors(
