...
export async function readFinancialReportingProjection(
  ...
  const [sellerCash, buyerCash, advanceCash, sellerPayables, buyerRefunds, profit, adjustments] =
    await Promise.all([
      database
        .prepare(
          `SELECT CAST(COALESCE(SUM(flow.amount_cny_fen),0) AS TEXT) AS amount FROM (
      ...
      database
        .prepare(
          `SELECT CAST(COALESCE(SUM(CASE entry.entry_type WHEN 'PAYMENT' THEN entry.amount_cny_fen ELSE -entry.amount_cny_fen END),0) AS TEXT) AS amount
      ...
      WHERE NOT EXISTS(SELECT 1 FROM buyer_advance_principal_settlements settlement WHERE settlement.buyer_refund_payment_entry_id=entry.id)
        AND ((entry.entry_type='PAYMENT' AND entry.paid_at>=? AND entry.paid_at<? AND entry.paid_at<=?)
          OR (entry.entry_type='REVERSAL' AND entry.reversed_at>=? AND entry.reversed_at<? AND entry.reversed_at<=?))`,
        )
        .bind(
          range.fromEpoch,
          range.toExclusiveEpoch,
      ...
      WHERE balance.due_at>=? AND balance.due_at<? AND balance.due_at<=?`,
        )
        .bind(range.fromEpoch, range.toExclusiveEpoch, now)
        .first<ThreeAmountRow>(),
      database
        .prepare(
          `SELECT
      ...
      WHERE ledger.created_at>=? AND ledger.created_at<? AND ledger.created_at<=?`,
        )
        .bind(range.fromEpoch, range.toExclusiveEpoch, now)
        .first<ThreeAmountRow>(),
      database
      ...
      FROM internal_order_finance_positions WHERE confirmed_at<=?`,
        )
        .bind(range.fromDate, range.toDate, range.fromDate, range.toDate, now, now)
        .first<ProfitRow>(),
      database
        .prepare(
          `SELECT
      ...
      FROM formal_order_financial_adjustments WHERE created_at>=? AND created_at<? AND created_at<=?`,
        )
        .bind(range.fromEpoch, range.toExclusiveEpoch, now)
        .first<AdjustmentRow>(),
    ]);
  ...
  const projectedAdjustment = money(adjustments?.projected),
    completedAdjustment = money(adjustments?.completed);
...
function money(value: string | number | null | undefined): bigint {
  ...
  if (!/^-?(?:0|[1-9][0-9]*)$/u.test(normalized))
    throw new Error('invalid_financial_projection_amount');
