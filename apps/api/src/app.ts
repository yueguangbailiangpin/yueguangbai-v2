...
export function createApp(): Hono<AppEnv> {
  ...
  app.use('*', async (context, next) => {
    ...
    if (context.res.status >= 500 && !context.get('errorLogged')) {
      ...
      if (context.env?.DB) {
        ...
        await recordWorker5xxSignal(context.env.DB, {
          requestId,
          observedAt: Date.now(),
          ...(sink ? { sink } : {}),
        }).catch(() => undefined);
  ...
  app.onError(async (_error, context) => {
    ...
    return context.json(
      apiFailure('DEPENDENCY_UNAVAILABLE', '服务暂时不可用，请稍后重试', requestId),
      500,
    );
  });
...
export function configuredAlertSink(
  bindings: Pick<
    AppBindings,
    'APP_ENVIRONMENT' | 'OPERATIONAL_ALERT_MODE' | 'OPERATIONAL_ALERT_SINK'
  >,
  ...
  try {
    ...
    return resolveOperationalAlertSink({
      ...(mode !== undefined ? { mode } : {}),
      ...(bindings.OPERATIONAL_ALERT_SINK && mode === 'bound'
        ? { boundSink: bindings.OPERATIONAL_ALERT_SINK }
        : {}),
      ...(bindings.OPERATIONAL_ALERT_SINK && mode !== 'bound'
