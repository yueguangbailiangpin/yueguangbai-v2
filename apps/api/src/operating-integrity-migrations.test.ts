...
describe('frozen operating integrity migrations 0051-0064',()=>{
  it('reaches schema 69 with advance cash and local-date guards installed',async()=>{
    database=createMigratedTestDatabase();
    const state=await database.prepare(`SELECT schema_version FROM app_schema_state WHERE singleton_id=1`).first<{schema_version:number}>();expect(Number(state?.schema_version)).toBe(69);
    const required=[
