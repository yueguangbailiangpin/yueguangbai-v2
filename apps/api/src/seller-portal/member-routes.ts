...
class MemberError extends Error{constructor(public code:'VALIDATION_ERROR'|'FORBIDDEN'|'NOT_FOUND'|'CONFLICT'|'RATE_LIMITED'|'DEPENDENCY_UNAVAILABLE',public status:400|403|404|409|429|503){super(code)}}
...
export function registerSellerMemberRoutes(app:Hono<AppEnv>):void{
  ...
  app.post('/api/seller-portal/member-invitations/:id/revoke',origin,session,wrap(revokeInvitation));
...
async function issueInvitation(context:Context<AppEnv>){
  ...
  const wechat=normalizeWechatId(body['wechat_id']),display=text(body['display_name'],100),stores=[...new Set((body['store_ids'] as string[]).map(clean))].sort();if(stores.length<1||stores.length>100)validation();
  const count=await context.env.DB.prepare(`SELECT COUNT(*) AS count FROM seller_stores WHERE organization_id=? AND status='ACTIVE' AND id IN (${stores.map(()=>'?').join(',')})`).bind(actor.sellerOrganizationId,...stores).first<{count:number}>();if(Number(count?.count??0)!==stores.length)validation();
  const membership=await context.env.DB.prepare(`SELECT member.id FROM wechat_identity_claims claim JOIN seller_organization_members member ON member.identity_subject_id=claim.identity_subject_id WHERE claim.normalized_wechat=? AND claim.status IN('ACTIVE','RESERVED') AND member.status='ACTIVE' LIMIT 1`).bind(wechat.normalized).first();if(membership)throw new MemberError('CONFLICT',409);
  await expireInvitations(context.env.DB,actor.sellerOrganizationId,Date.now());
  const active=await context.env.DB.prepare(`SELECT 1 AS present FROM seller_member_invitations WHERE organization_id=? AND invited_wechat_normalized=? AND status='ACTIVE'`).bind(actor.sellerOrganizationId,wechat.normalized).first();if(active)throw new MemberError('CONFLICT',409);
  const now=Date.now(),token=randomToken(),hash=await hashOneTimeToken(token),id=crypto.randomUUID(),expires=now+TTL;
  await context.env.DB.batch([
    context.env.DB.prepare(`INSERT INTO seller_member_invitations(id,token_hash,organization_id,invited_wechat_normalized,invited_wechat_display,invited_display_name,invited_role,store_scope_json,issued_by_member_id,status,version,issued_at,expires_at,consumed_at,consumed_member_id,consumed_account_id,revoked_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,'ACTIVE',1,?,?,NULL,NULL,NULL,NULL,?,?)`).bind(id,hash,actor.sellerOrganizationId,wechat.normalized,wechat.display,display,role,JSON.stringify(stores),actor.memberId,now,expires,now,now),
...
async function completeInvitation(context:Context<AppEnv>){
  ...
  const existing=await existingIdentity(context.env.DB,wechat.normalized);if(existing?.account_id){if(existing.account_status!=='ACTIVE'||!existing.algorithm||!existing.iterations||!existing.salt_base64url||!existing.hash_base64url)throw new MemberError('CONFLICT',409);const valid=await verifyCustomerPassword(body['password'],{algorithm:existing.algorithm,iterations:Number(existing.iterations),saltBase64Url:existing.salt_base64url,hashBase64Url:existing.hash_base64url});if(!valid)throw new MemberError('CONFLICT',409);}
  if(existing){const seller=await context.env.DB.prepare(`SELECT 1 AS present FROM seller_organization_members WHERE identity_subject_id=? AND status='ACTIVE' LIMIT 1`).bind(existing.identity_subject_id).first();if(seller)throw new MemberError('CONFLICT',409);}
  const org=await context.env.DB.prepare(`SELECT seller_code,organization_name,next_member_number,status FROM seller_organizations WHERE id=?`).bind(invitation.row.organization_id).first<{seller_code:string;organization_name:string;next_member_number:number;status:string}>();if(!org||org.status!=='ACTIVE')throw new MemberError('CONFLICT',409);
  const number=Number(org.next_member_number);if(!Number.isSafeInteger(number)||number<1)throw new MemberError('DEPENDENCY_UNAVAILABLE',503);
  const subjectId=existing?.identity_subject_id??crypto.randomUUID(),accountId=existing?.account_id??crypto.randomUUID(),memberId=crypto.randomUUID();const credential=existing?.account_id?null:await hashCustomerPassword(body['password']);const statements:SqlStatement[]=[];
  if(!existing){statements.push(context.env.DB.prepare(`INSERT INTO customer_identity_subjects(id,subject_type,created_at) VALUES(?,'SELLER_ORG_MEMBER',?)`).bind(subjectId,now),context.env.DB.prepare(`INSERT INTO wechat_identity_claims(id,identity_subject_id,display_wechat,normalized_wechat,status,version,acquired_at,reserved_at,released_at,created_at,updated_at,identity_subject_type) VALUES(?,?,?,?,'ACTIVE',1,?,NULL,NULL,?,?,'SELLER_ORG_MEMBER')`).bind(crypto.randomUUID(),subjectId,wechat.display,wechat.normalized,now,now,now));}
  if(!existing?.account_id&&credential){statements.push(context.env.DB.prepare(`INSERT INTO customer_login_accounts(id,identity_subject_id,account_type,login_identifier_display,login_identifier_normalized,status,session_version,password_change_required,version,created_at,updated_at,activated_at,disabled_at,registration_source) VALUES(?,?,'SELLER_MEMBER',?,?,'ACTIVE',1,0,1,?,?,?,NULL,'SELF_REGISTRATION_CLAIM')`).bind(accountId,subjectId,wechat.display,wechat.normalized,now,now,now),context.env.DB.prepare(`INSERT INTO customer_password_credentials(account_id,algorithm,iterations,salt_base64url,hash_base64url,password_version,created_at,updated_at) VALUES(?,?,?,?,?,1,?,?)`).bind(accountId,credential.algorithm,credential.iterations,credential.saltBase64Url,credential.hashBase64Url,now,now));}
  statements.push(
    context.env.DB.prepare(`INSERT INTO seller_organization_members(id,identity_subject_id,organization_id,member_number,username_fallback,display_name,role,primary_owner,status,version,created_at,updated_at,activated_at,disabled_at) VALUES(?,?,?,?,?, ?,?,0,'ACTIVE',1,?,?,?,NULL)`).bind(memberId,subjectId,invitation.row.organization_id,number,`${org.seller_code}-member-${number}`,invitation.row.invited_display_name,invitation.row.invited_role,now,now,now),
    context.env.DB.prepare(`UPDATE seller_organizations SET next_member_number=next_member_number+1,version=version+1,updated_at=? WHERE id=? AND next_member_number=?`).bind(now,invitation.row.organization_id,number),
    context.env.DB.prepare(`INSERT OR IGNORE INTO customer_account_personas(account_id,identity_subject_id,persona_type,buyer_customer_id,seller_member_id,created_at) VALUES(?,?,'SELLER_MEMBER',NULL,?,?)`).bind(accountId,subjectId,memberId,now),
    ...parseStores(invitation.row.store_scope_json).map((store)=>context.env.DB.prepare(`INSERT INTO seller_member_portal_store_grants(member_id,organization_id,store_id,granted_by_member_id,created_at,revoked_at) VALUES(?,?,?,?,?,NULL)`).bind(memberId,invitation.row.organization_id,store,invitation.row.issued_by_member_id,now)),
    context.env.DB.prepare(`UPDATE seller_member_invitations SET status='CONSUMED',version=version+1,consumed_at=?,consumed_member_id=?,consumed_account_id=?,updated_at=? WHERE id=? AND status='ACTIVE' AND expires_at>?`).bind(now,memberId,accountId,now,invitation.row.id,now),
    event(context.env.DB,invitation.row.id,'CONSUMED','CUSTOMER',accountId,now),
  );
  await context.env.DB.batch(statements);
  const sessionVersion=Number(existing?.session_version??1),secret=requireCustomerSessionSecret(context.env.CUSTOMER_SESSION_SECRET);const token=await issueCustomerSession({accountId,identitySubjectId:subjectId,accountType:'SELLER_MEMBER',availablePersonas:['SELLER_MEMBER'],sessionVersion,passwordChangeRequired:false},secret,{now,ttlMs:CUSTOMER_SESSION_TTL_MS});writeCustomerSessionCookie(context,token);
