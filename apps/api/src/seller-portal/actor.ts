...
export async function resolveSellerPortalActor(
  context: Context<any>,
): Promise<SellerPortalActor> {
  ...
  const row = await requireSellerMemberByIdentity(
    context.env.DB,
  ...
  const me: SellerPortalMeDto = Object.freeze({
    ...
    access: Object.freeze({
      read_scope: access.allActiveStores
        ? 'ORGANIZATION' as const
        : 'ASSIGNED_STORES' as const,
...
export function requireSellerPortalWriteRole(
  actor: SellerPortalActor,
): void {
  if (!actor.canManageProducts
    || (actor.role !== 'OWNER' && actor.role !== 'OPERATIONS')) {
    throw new SellerPortalError('FORBIDDEN', 403);
  }
}
...
async function requireSellerMemberByIdentity(
  ...
  `).bind(identitySubjectId).first<SellerMemberRow>();
