...
interface ExistingIdentityRow{
  claim_id:string;identity_subject_id:string;claim_status:'ACTIVE'|'RESERVED';
  account_id:string|null;account_status:string|null;session_version:number|null;
  algorithm:'PBKDF2_SHA256'|null;iterations:number|null;salt_base64url:string|null;hash_base64url:string|null;
}
interface InvitationTarget{
  organizationId:string;organizationName:string;memberId:string|null;leadId:string|null;kind:Kind;
}
...
export async function issueSellerRegistrationInvitation(
  ...
  const requestHash=await hashCanonicalJson({
    action:'ISSUE_SELLER_REGISTRATION_INVITATION',kind:target.kind,seller_organization_id:target.organizationId,
    seller_member_id:target.memberId,normalized_wechat:wechat.normalized,
  });
...
export async function readSellerInvitationContext(database:SqlDatabase,token:string,now=Date.now()){
  ...
  return{invitation_valid:true as const,seller_name:String(row.organization_name),marketplace_code:String(row.marketplace_code),
    wechat_hint:maskWechat(String(row.wechat_display)),onboarding_kind:row.onboarding_kind as Kind,
    existing_moonwhite_account:identity?.account_id!==null&&identity?.account_status==='ACTIVE',expires_at:Number(row.expires_at)};
}
...
export async function completeSellerRegistration(
  ...
  const organization=await database.prepare(`SELECT id,seller_code,organization_name,marketplace_code,status,next_member_number
    FROM seller_organizations WHERE id=?`).bind(invitation.seller_organization_id).first<OrgRow>();
  if(!organization||organization.status!=='ACTIVE'||organization.marketplace_code!=='JP')throw new SellerRegistrationError('CONFLICT',409);
  ...
  if(identity?.account_id!==null&&identity?.account_id!==undefined){
    if(identity.account_status!=='ACTIVE'||!identity.algorithm||!identity.iterations||!identity.salt_base64url||!identity.hash_base64url)
      throw new SellerRegistrationError('CONFLICT',409);
    const valid=await verifyCustomerPassword(input.password,{
      algorithm:identity.algorithm,iterations:Number(identity.iterations),saltBase64Url:identity.salt_base64url,hashBase64Url:identity.hash_base64url,
    });
    if(!valid)throw new SellerRegistrationError('CONFLICT',409);
  }
  const memberPlan=await planSellerMember(database,organization,invitation.seller_member_id,wechat,identity,now);
  ...
  const requestHash=await hashCanonicalJson({action:'COMPLETE_SELLER_REGISTRATION',invitation_id:invitation.id,
    normalized_wechat:wechat.normalized,account_id:accountId,member_id:memberPlan.memberId,
  ...
  const acquired=await acquireIdempotency<any>(database,{actorType:'CUSTOMER_INVITATION',actorId:invitation.id,
    ...
    idempotencyKey:command.idempotencyKey,requestHash},{now});
  if(acquired.kind==='REPLAY')return{...acquired.response,replayed:true};
  const safe={account_id:accountId,identity_subject_id:memberPlan.identitySubjectId,seller_organization_id:invitation.seller_organization_id,
    seller_member_id:memberPlan.memberId,session_version:sessionVersion,onboarding_kind:invitation.onboarding_kind};
  ...
  if(needsAccount&&credential){
    statements.push(
      ...
      ) VALUES(?,?,'SELLER_MEMBER',?,?,'ACTIVE',1,0,1,?,?,?,NULL,'SELF_REGISTRATION_CLAIM')`).bind(
        accountId,memberPlan.identitySubjectId,wechat.display,wechat.normalized,now,now,now),
  ...
  statements.push(
    database.prepare(`INSERT OR IGNORE INTO customer_account_personas(
      account_id,identity_subject_id,persona_type,buyer_customer_id,seller_member_id,created_at
    ) VALUES(?,?,'SELLER_MEMBER',NULL,?,?)`).bind(accountId,memberPlan.identitySubjectId,memberPlan.memberId,now),
    database.prepare(`UPDATE customer_seller_invitations SET status='CONSUMED',version=version+1,
      ...
      WHERE id=? AND status='ACTIVE' AND expires_at>? AND version=?`).bind(
      memberPlan.memberId,now,accountId,now,invitation.id,now,invitation.version),
    ...
    createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'SELLER_ORGANIZATION',aggregateId:invitation.seller_organization_id,
      ...
      requestId:command.requestId,idempotencyKey:command.idempotencyKey,nextState:{account_id:accountId,
        seller_member_id:memberPlan.memberId,onboarding_kind:invitation.onboarding_kind},createdAt:now}),
    completeIdempotencyStatement(database,acquired.claim,safe,{resultReferences:{account_id:accountId,seller_organization_id:invitation.seller_organization_id,seller_member_id:memberPlan.memberId},now}),
    assertIdempotencyCompletionStatement(database,acquired.claim),
    database.prepare(`INSERT INTO transaction_assertions(assertion_value)
      ...
      THEN 1 ELSE 0 END`).bind(accountId,accountId,memberPlan.memberId,memberPlan.memberId,invitation.seller_organization_id),
  );
  try{await database.batch(statements);}catch(error){await markIdempotencyFailed(database,acquired.claim,'SELLER_REGISTRATION_FAILED',now);throw error;}
  return{...safe,replayed:false};
}
...
async function ensureHistoricalSellerOrganization(
  database:SqlDatabase,organizationIdRaw:string,normalizedWechat:string,override?:{leadId:string;kind:Kind},
):Promise<InvitationTarget>{
  ...
  if(memberId===null){
    const primary=await database.prepare(`SELECT id,identity_subject_id,display_name,member_number,primary_owner
      FROM seller_organization_members WHERE organization_id=? AND primary_owner=1 AND status='ACTIVE'
      ORDER BY member_number,id LIMIT 2`).bind(organizationId).all<MemberRow>();
    ...
    if(primary.results.length===1){
      ...
      const claims=await database.prepare(`SELECT normalized_wechat FROM wechat_identity_claims
        WHERE identity_subject_id=? AND status IN('ACTIVE','RESERVED')`).bind(owner.identity_subject_id).all<{normalized_wechat:string}>();
      if(claims.results.length>1||claims.results.some((row)=>row.normalized_wechat!==normalizedWechat))
        throw new SellerRegistrationError('CONFLICT',409);
      memberId=owner.id;
    }
  }
  return{organizationId,organizationName:org.organization_name,memberId,leadId:override?.leadId??null,
...
async function planSellerMember(
  ...
  if(invitedMemberId!==null){
    const member=await database.prepare(`SELECT id,identity_subject_id,display_name,member_number,primary_owner
      FROM seller_organization_members WHERE id=? AND organization_id=? AND status='ACTIVE'`).bind(
        invitedMemberId,organization.id).first<MemberRow>();
    if(!member)throw new SellerRegistrationError('CONFLICT',409);
    if(identity&&identity.identity_subject_id!==member.identity_subject_id)throw new SellerRegistrationError('CONFLICT',409);
    const claims=await database.prepare(`SELECT normalized_wechat FROM wechat_identity_claims
      WHERE identity_subject_id=? AND status IN('ACTIVE','RESERVED')`).bind(member.identity_subject_id).all<{normalized_wechat:string}>();
    if(claims.results.length>1||claims.results.some((row)=>row.normalized_wechat!==wechat.normalized))throw new SellerRegistrationError('CONFLICT',409);
    if(claims.results.length===0){
      statements.push(database.prepare(`INSERT INTO wechat_identity_claims(
        ...
        crypto.randomUUID(),member.identity_subject_id,wechat.display,wechat.normalized,now,now,now));
    }
  ...
  if(identity===null){
    statements.push(
      database.prepare(`INSERT INTO customer_identity_subjects(id,subject_type,created_at) VALUES(?,'SELLER_ORG_MEMBER',?)`).bind(identitySubjectId,now),
      database.prepare(`INSERT INTO wechat_identity_claims(
        id,identity_subject_id,display_wechat,normalized_wechat,status,version,acquired_at,reserved_at,released_at,
        created_at,updated_at,identity_subject_type
      ) VALUES(?,?,?,?,'ACTIVE',1,?,NULL,NULL,?,?,'SELLER_ORG_MEMBER')`).bind(
        crypto.randomUUID(),identitySubjectId,wechat.display,wechat.normalized,now,now,now),
    );
  }else{
    const sellerMembership=await database.prepare(`SELECT id FROM seller_organization_members
      WHERE identity_subject_id=? AND status='ACTIVE' LIMIT 1`).bind(identity.identity_subject_id).first();
    if(sellerMembership)throw new SellerRegistrationError('CONFLICT',409);
  }
  const number=Number(organization.next_member_number);if(!Number.isSafeInteger(number)||number<1)throw new SellerRegistrationError('DEPENDENCY_UNAVAILABLE',503);
  ...
  statements.push(
    database.prepare(`INSERT INTO seller_organization_members(
      ...
      memberId,identitySubjectId,organization.id,number,`${organization.seller_code}-owner-${number}`,
      organization.organization_name.slice(0,100),now,now,now),
    database.prepare(`UPDATE seller_organizations SET next_member_number=next_member_number+1,version=version+1,updated_at=?
      WHERE id=? AND next_member_number=?`).bind(now,organization.id,number),
  );
  return{memberId,identitySubjectId,statements};
}

async function loadWechatIdentity(database:SqlDatabase,normalizedWechat:string):Promise<ExistingIdentityRow|null>{
  const rows=await database.prepare(`SELECT claim.id AS claim_id,claim.identity_subject_id,claim.status AS claim_status,
    ...
    WHERE claim.normalized_wechat=? AND claim.status IN('ACTIVE','RESERVED')`).bind(normalizedWechat).all<ExistingIdentityRow>();
