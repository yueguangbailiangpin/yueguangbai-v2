...
export async function createStaffAccount(
  ...
  const markets=await normalizedMarkets(database,input.roleCode,input.marketplaceCodes,null);
  ...
  await assertPrimaryMarketplaceRoleAvailable(database,input.roleCode,markets,null);
  const statements:SqlStatement[]=[
    ...
    ...markets.map((market)=>database.prepare(`INSERT INTO staff_marketplace_scopes(id,staff_id,role_code,marketplace_code,status,assigned_by_staff_id,assigned_at,revoked_at,reason,created_at,updated_at)
      VALUES(?,?,?,?,'ACTIVE',?,?,NULL,'STAFF_ACCOUNT_CREATED',?,?)`).bind(crypto.randomUUID(),staffId,input.roleCode,market,actor.staffId,now,now,now)),
    database.prepare(`INSERT INTO staff_authorization_events(id,staff_id,authorization_version,event_type,actor_staff_id,request_id,idempotency_key,change_summary_json,created_at)
      VALUES(?, ?, 1, 'STAFF_PROVISIONED', ?, NULL, NULL, ?, ?)`).bind(crypto.randomUUID(),staffId,actor.staffId,canonicalJson({email,role_code:input.roleCode,marketplace_codes:markets}),now),
    createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'STAFF',aggregateId:staffId,eventType:'STAFF_EMAIL_ACCOUNT_CREATED',
      ...
      nextState:{display_name:displayName,email,role_code:input.roleCode,marketplace_codes:markets,status:'ACTIVE'},createdAt:now}),
  ];
...
export async function updateStaffAccount(
  ...
  const displayName=text(input.displayName,100); const email=normalizeStaffEmail(input.email);
  if(!email||!isStaffRoleCode(input.roleCode))validation();
  const markets=await normalizedMarkets(database,input.roleCode,input.marketplaceCodes,staffId);
  await assertEmailAvailable(database,email,staffId);
  await assertPrimaryMarketplaceRoleAvailable(database,input.roleCode,markets,staffId);
  ...
  statements.push(database.prepare(`UPDATE staff_marketplace_scopes SET status='REVOKED',revoked_at=?,reason='STAFF_ACCOUNT_SCOPE_CHANGED',updated_at=?
    WHERE staff_id=? AND status='ACTIVE'`).bind(now,now,staffId));
  for(const market of markets)statements.push(database.prepare(`INSERT INTO staff_marketplace_scopes(id,staff_id,role_code,marketplace_code,status,assigned_by_staff_id,assigned_at,revoked_at,reason,created_at,updated_at)
    VALUES(?,?,?,?,'ACTIVE',?,?,NULL,'STAFF_ACCOUNT_SCOPE_CHANGED',?,?)`).bind(crypto.randomUUID(),staffId,input.roleCode,market,actor.staffId,now,now,now));
  statements.push(database.prepare(`INSERT INTO staff_authorization_events(id,staff_id,authorization_version,event_type,actor_staff_id,request_id,idempotency_key,change_summary_json,created_at)
    ...
    .bind(crypto.randomUUID(),actor.staffId,canonicalJson({display_name:displayName,email,role_code:input.roleCode,marketplace_codes:markets}),now,staffId));
  statements.push(createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'STAFF',aggregateId:staffId,eventType:'STAFF_ACCESS_PROFILE_CHANGED',
    ...
    nextState:{display_name:displayName,email,role_code:input.roleCode,marketplace_codes:markets,version:nextVersion},createdAt:now}));
  await database.batch(statements);
  return readStaffAccessEmployee(database,staffId);
}
...
async function normalizedMarkets(database:SqlDatabase,role:StaffRoleCode,values:readonly string[],excludeStaffId:string|null):Promise<string[]>{
...
async function assertPrimaryMarketplaceRoleAvailable(database:SqlDatabase,role:StaffRoleCode,markets:readonly string[],excludeStaffId:string|null):Promise<void>{
  if(role==='owner'||markets.length===0)return;
  const placeholders=markets.map(()=>'?').join(',');
  const row=await database.prepare(`SELECT scope.staff_id FROM staff_marketplace_scopes scope
    ...
    WHERE scope.role_code=? AND scope.marketplace_code IN (${placeholders}) AND scope.status='ACTIVE' AND staff.status='ACTIVE'
      AND (? IS NULL OR scope.staff_id<>?) LIMIT 1`).bind(role,...markets,excludeStaffId,excludeStaffId).first<{staff_id:string}>();
