...
export class RouteBoundFileAuthorizationService implements FileAuthorizationService {
  ...
  async assertCanRead(actor: FileActor, resource: FileAuthorizationResource): Promise<void> {
    ...
    if (
      ...
      deny();
    if (!this.staffAuthorization.permissions.has(readPermissionForPurpose(resource.purpose)))
      deny();
  ...
  private assertOwnedUpload(actor: FileActor, resource: FileAuthorizationResource): void {
    this.assertActor(actor);
    if (
      resource.ownerActorType !== actor.type ||
  ...
  private assertStaffUploadPermission(purpose: FilePurpose): void {
    ...
    if (!this.staffAuthorization?.permissions.has(writePermissionForPurpose(purpose))) deny();
  }
  private async assertStaffEntityScope(resource: FileAuthorizationResource): Promise<void> {
    if (!resource.entityType || !resource.entityId || !this.staffDataScope) concealNotFound();
    ...
    if (
      authority.buyerCustomerId &&
      scopeAllowsBuyer(this.staffDataScope, authority.buyerCustomerId)
    )
      return;
    if (
      authority.sellerOrganizationId &&
      scopeAllowsSellerOrganization(this.staffDataScope, authority.sellerOrganizationId)
    )
      return;
    concealNotFound();
  }
...
async function resolveEntityAuthority(
  ...
  entityType: string,
  entityId: string,
): Promise<{ buyerCustomerId: string | null; sellerOrganizationId: string | null }> {
  switch (entityType) {
    case 'ORDER':
      return authority(
        await database
          .prepare(
            `SELECT buyer_customer_id,seller_organization_id FROM formal_orders WHERE id=? UNION ALL SELECT submission.buyer_customer_id,reservation.organization_id AS seller_organization_id FROM order_evidence_versions version JOIN order_evidence_submissions submission ON submission.id=version.submission_id JOIN product_reservations reservation ON reservation.id=submission.reservation_id WHERE version.id=? LIMIT 1`,
          )
    ...
    case 'ORDER_EVIDENCE_SUBMISSION':
      return authority(
        await database
          .prepare(
            `SELECT submission.buyer_customer_id,reservation.organization_id AS seller_organization_id FROM order_evidence_submissions submission JOIN product_reservations reservation ON reservation.id=submission.reservation_id WHERE submission.id=?`,
          )
          .bind(entityId)
          .first<AuthorityRow>(),
      );
    case 'ORDER_INSTRUCTION_VERSION':
      return authority(
        await database
          .prepare(
            `SELECT instruction.buyer_customer_id,reservation.organization_id AS seller_organization_id FROM order_instruction_versions version JOIN order_instructions instruction ON instruction.id=version.instruction_id JOIN product_reservations reservation ON reservation.id=instruction.reservation_id WHERE version.id=?`,
          )
          .bind(entityId)
          .first<AuthorityRow>(),
      );
    case 'REVIEW':
      return authority(
        await database
          .prepare(
            `SELECT review.buyer_customer_id,formal_order.seller_organization_id FROM review_cases review JOIN formal_orders formal_order ON formal_order.id=review.formal_order_id WHERE review.id=?`,
          )
          .bind(entityId)
          .first<AuthorityRow>(),
      );
    ...
    case 'SELLER_SETTLEMENT':
      return authority(
        await database
          .prepare(
            `SELECT NULL AS buyer_customer_id,seller_organization_id FROM seller_payments WHERE id=? UNION ALL SELECT NULL AS buyer_customer_id,seller_organization_id FROM seller_payables WHERE id=? LIMIT 1`,
          )
          .bind(entityId, entityId)
          .first<AuthorityRow>(),
      );
    case 'PRODUCT_APPLICATION':
      return authority(
        await database
          .prepare(
            `SELECT NULL AS buyer_customer_id,organization_id AS seller_organization_id FROM product_applications WHERE id=?`,
          )
          .bind(entityId)
          .first<AuthorityRow>(),
      );
    case 'PRODUCT_VERSION':
      return authority(
        await database
          .prepare(
            `SELECT NULL AS buyer_customer_id,product.organization_id AS seller_organization_id FROM product_versions version JOIN products product ON product.id=version.product_id WHERE version.id=?`,
          )
          .bind(entityId)
          .first<AuthorityRow>(),
      );
