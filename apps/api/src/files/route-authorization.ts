...
export class RouteBoundFileAuthorizationService implements FileAuthorizationService {
  ...
  private assertOwnedUpload(actor:FileActor,resource:FileAuthorizationResource):void{this.assertActor(actor);if(resource.ownerActorType!==actor.type||resource.ownerActorId!==actor.id||this.allowedUploads.get(resource.purpose)!==resource.visibility)deny();this.assertStaffUploadPermission(resource.purpose);}
...
function readPermissionForPurpose(purpose:FilePurpose):StaffPermissionCode{switch(purpose){case 'ORDER_EVIDENCE':case 'ORDER_EVIDENCE_INTERNAL_COMMUNICATION':case 'ORDER_INSTRUCTION_KEYWORD_IMAGE':return 'ORDER_VIEW';case 'REVIEW_EVIDENCE':return 'REVIEW_VIEW';case 'PRODUCT_APPLICATION_IMAGE':case 'PRODUCT_IMAGE':return 'PRODUCT_VIEW';case 'BUYER_REFUND_PROOF':return 'BUYER_REFUND_VIEW';case 'SELLER_SETTLEMENT_PROOF':return 'SELLER_SETTLEMENT_VIEW';default:return 'AUDIT_VIEW';}}
function writePermissionForPurpose(purpose:FilePurpose):StaffPermissionCode{switch(purpose){case 'ORDER_EVIDENCE_INTERNAL_COMMUNICATION':return 'ORDER_CONFIRM';case 'BUYER_REFUND_PROOF':return 'BUYER_REFUND_RECORD';case 'SELLER_SETTLEMENT_PROOF':return 'SELLER_SETTLEMENT_RECORD';case 'PRODUCT_IMAGE':return 'PRODUCT_REVIEW';default:return readPermissionForPurpose(purpose);}}
async function resolveEntityAuthority(database:SqlDatabase,entityType:string,entityId:string):Promise<{buyerCustomerId:string|null;sellerOrganizationId:string|null}>{
  switch(entityType){
    case 'ORDER':return authority(await database.prepare(`SELECT buyer_customer_id,seller_organization_id FROM formal_orders WHERE id=? UNION ALL SELECT submission.buyer_customer_id,reservation.organization_id AS seller_organization_id FROM order_evidence_versions version JOIN order_evidence_submissions submission ON submission.id=version.submission_id JOIN product_reservations reservation ON reservation.id=submission.reservation_id WHERE version.id=? LIMIT 1`).bind(entityId,entityId).first<AuthorityRow>());
