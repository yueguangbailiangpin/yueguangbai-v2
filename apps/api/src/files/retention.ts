...
export async function reconcileUnlinkedFileRetention(
  ...
  const planRows=await database.prepare(`SELECT object.id,object.object_key,object.status,object.delete_attempt_count
    ...
    ORDER BY object.updated_at,object.id LIMIT ?`).bind(
      now,now-VERIFIED_UNLINKED_TTL_MS,now-UPLOADED_OR_REJECTED_TTL_MS,limit,
    ).all<Candidate>();
  ...
  for(const row of planRows.results){
    ...
    const result=await database.prepare(`UPDATE file_objects
      ...
      WHERE id=? AND status=?
        ...
        AND NOT EXISTS(SELECT 1 FROM order_instruction_asset_items item WHERE item.file_object_id=file_objects.id AND item.status='ORPHANED')`).bind(now,now,row.id,row.status,now).run();
  ...
  const deleteRows=await database.prepare(`SELECT object.id,object.object_key,object.status,object.delete_attempt_count
    ...
    WHERE object.status='DELETION_PENDING' AND object.next_delete_at<=?
      ...
      AND NOT EXISTS(SELECT 1 FROM file_read_intents read_intent WHERE read_intent.file_object_id=object.id AND read_intent.status='ISSUED' AND read_intent.expires_at>?)
      AND NOT EXISTS(SELECT 1 FROM order_instruction_asset_items item WHERE item.file_object_id=object.id AND item.status='ORPHANED')
    ORDER BY object.next_delete_at,object.updated_at,object.id LIMIT ?`).bind(now,now,limit).all<Candidate>();
    ...
    try{
      ...
      const result=await database.prepare(`UPDATE file_objects
        ...
        WHERE id=? AND status='DELETION_PENDING'
          AND NOT EXISTS(SELECT 1 FROM file_entity_links link WHERE link.file_object_id=file_objects.id AND link.revoked_at IS NULL)
          AND NOT EXISTS(SELECT 1 FROM file_read_intents read_intent WHERE read_intent.file_object_id=file_objects.id AND read_intent.status='ISSUED' AND read_intent.expires_at>?)`).bind(now,now,row.id,now).run();
...
async function countBacklog(database:SqlDatabase,now:number):Promise<number>{
  const row=await database.prepare(`SELECT COUNT(*) AS count FROM file_objects object
    ...
    AND NOT EXISTS(SELECT 1 FROM order_instruction_asset_items item WHERE item.file_object_id=object.id AND item.status='ORPHANED')`).bind(
      now,now-VERIFIED_UNLINKED_TTL_MS,now-UPLOADED_OR_REJECTED_TTL_MS,now,
    ).first<{count:number}>();
