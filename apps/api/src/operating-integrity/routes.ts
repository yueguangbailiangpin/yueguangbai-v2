import {
  apiFailure,apiSuccess,FORMAL_ORDER_OPERATIONAL_EVENT_TYPES,REVIEW_VISIBILITY_STATUSES,
  type FileActor,type ReviewVisibilityStatus,type FormalOrderOperationalEventType,
  type SqlDatabase,type SqlStatement,
} from '@ygb/contracts';
...
import {
  assertBuyerRefundProofFilesUnused,listBuyerRefundProofFiles,
...
async function recordOrderEvent(context:Context<AppEnv>){
  ...
  const acquired=await command(context,actor,'RECORD_FORMAL_ORDER_OPERATIONAL_EVENT','FORMAL_ORDER',order.id,{event_type:type,reason},now);
  if(acquired.kind==='REPLAY')return ok(context,acquired.response,201);
  const eventId=crypto.randomUUID();const response={event:{event_id:eventId,formal_order_id:order.id,event_type:type,reason,actor_staff_id:actor.staffId,created_at:now}};
  await commit(context,acquired.claim,'ORDER_EVENT_FAILED',[
    ...
    createAuditEventStatement(context.env.DB,{id:crypto.randomUUID(),aggregateType:'FORMAL_ORDER',aggregateId:order.id,eventType:`FORMAL_ORDER_${type}`,actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:requestIdFromContext(context),idempotencyKey:acquired.claim.idempotencyKey,nextState:{operational_event_id:eventId,event_type:type,reason},createdAt:now}),
    completeIdempotencyStatement(context.env.DB,acquired.claim,response,{resultReferences:{event_id:eventId},now}),
...
async function recordFinancialAdjustment(context:Context<AppEnv>){
  ...
  if(typeof source==='string'){const found=await context.env.DB.prepare(`SELECT 1 AS present FROM formal_order_operational_events WHERE id=? AND formal_order_id=?`).bind(source,order.id).first();if(!found)validation();}
  const now=Date.now();const acquired=await command(context,actor,'RECORD_FORMAL_ORDER_PROFIT_ADJUSTMENT','FORMAL_ORDER',order.id,{adjustment_scope:scope,amount_cny_fen:amount,reason,source_operational_event_id:source},now);
  if(acquired.kind==='REPLAY')return ok(context,acquired.response,201);
  const adjustmentId=crypto.randomUUID();const response={adjustment:{adjustment_id:adjustmentId,formal_order_id:order.id,source_operational_event_id:source,adjustment_scope:scope,amount_cny_fen:String(amount),reason,actor_staff_id:actor.staffId,created_at:now}};
  await commit(context,acquired.claim,'PROFIT_ADJUSTMENT_FAILED',[
    ...
    createAuditEventStatement(context.env.DB,{id:crypto.randomUUID(),aggregateType:'FORMAL_ORDER_FINANCIAL_ADJUSTMENT',aggregateId:adjustmentId,eventType:'FORMAL_ORDER_FINANCIAL_ADJUSTMENT_RECORDED',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:requestIdFromContext(context),idempotencyKey:acquired.claim.idempotencyKey,nextState:{formal_order_id:order.id,adjustment_scope:scope,amount_cny_fen:String(amount),reason,source_operational_event_id:source},createdAt:now}),
    completeIdempotencyStatement(context.env.DB,acquired.claim,response,{resultReferences:{adjustment_id:adjustmentId},now}),
    assertIdempotencyCompletionStatement(context.env.DB,acquired.claim),
  ],now);
  return ok(context,response,201);
}
...
async function recordReviewVisibility(context:Context<AppEnv>){
  ...
  const observationId=crypto.randomUUID();const response={observation:{observation_id:observationId,review_case_id:review.id,formal_order_id:review.formalOrderId,visibility_status:status,note,observed_at:observed,actor_staff_id:actor.staffId,created_at:now}};
  await commit(context,acquired.claim,'REVIEW_VISIBILITY_FAILED',[
    ...
    createAuditEventStatement(context.env.DB,{id:crypto.randomUUID(),aggregateType:'REVIEW_CASE',aggregateId:review.id,eventType:'REVIEW_VISIBILITY_OBSERVED',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:requestIdFromContext(context),idempotencyKey:acquired.claim.idempotencyKey,nextState:{visibility_status:status,note,observed_at:observed},createdAt:now}),
    completeIdempotencyStatement(context.env.DB,acquired.claim,response,{resultReferences:{observation_id:observationId},now}),assertIdempotencyCompletionStatement(context.env.DB,acquired.claim),
  ],now);
  return ok(context,response,201);
}
...
async function recordAdvancePayment(context:Context<AppEnv>){
  ...
  const body=await json(context,['paid_at','payment_channel','note','proof_files']);const now=Date.now();const paid=cleanOperatingPaymentTimestamp(body['paid_at'],now);const channel=paymentChannel(body['payment_channel']);const note=optionalText(body['note'],2000);const proofs=parseProofFiles(body['proof_files']);
  const amount=await authoritativeAdvanceAmount(context.env.DB,order.id);
  const outstanding=await context.env.DB.prepare(`SELECT 1 AS present FROM buyer_advance_principal_entries payment WHERE payment.formal_order_id=? AND payment.entry_type='PAYMENT' AND payment.amount_cny_fen>COALESCE((SELECT SUM(reversal.amount_cny_fen) FROM buyer_advance_principal_entries reversal WHERE reversal.entry_type='REVERSAL' AND reversal.original_payment_entry_id=payment.id),0) LIMIT 1`).bind(order.id).first();if(outstanding)throw new IntegrityError('CONFLICT',409);
  ...
  try{
    ...
    await assertBuyerRefundProofFilesUnused(context.env.DB,proofs.map((proof)=>proof.fileObjectId));
    const already=await context.env.DB.prepare(`SELECT 1 AS present FROM buyer_advance_principal_entry_files WHERE file_object_id IN (${proofs.map(()=>'?').join(',')}) LIMIT 1`).bind(...proofs.map((proof)=>proof.fileObjectId)).first();if(already)throw new IntegrityError('CONFLICT',409);
    const entryId=crypto.randomUUID();const fileActor:FileActor={type:'STAFF',id:actor.staffId,roles:[...actor.roles]};const prepared:{fileObjectId:string;linkId:string;rowId:string;statements:readonly SqlStatement[]}[]=[];
    for(const proof of proofs){const link=await createExplicitAudienceFileLinkStatements(context.env.DB,new AdvanceProofAuthorization(actor),{fileObjectId:proof.fileObjectId,expectedFileVersion:proof.expectedFileVersion,entityType:'BUYER_REFUND',entityId:entryId,grants:[{subjectType:'STAFF_INTERNAL',permissionCode:'BUYER_REFUND_VIEW',scope:{type:'GLOBAL'}}]},{actor:fileActor,idempotencyKey:acquired.claim.idempotencyKey,requestId:requestIdFromContext(context),now});prepared.push({fileObjectId:proof.fileObjectId,linkId:link.result.linkId,rowId:crypto.randomUUID(),statements:link.statements});}
    const response={entry:{entry_id:entryId,formal_order_id:order.id,buyer_customer_id:order.buyerCustomerId,entry_type:'PAYMENT' as const,original_payment_entry_id:null,amount_cny_fen:String(amount),paid_at:paid,reversed_at:null,china_business_date:chinaBusinessDate(paid),payment_channel:channel,note,actor_staff_id:actor.staffId,created_at:now}};
    const statements:SqlStatement[]=[context.env.DB.prepare(`INSERT INTO buyer_advance_principal_entries(id,formal_order_id,buyer_customer_id,entry_type,original_payment_entry_id,amount_cny_fen,paid_at,reversed_at,china_business_date,payment_channel,note,actor_staff_id,created_at) VALUES(?,?,?,'PAYMENT',NULL,?,?,NULL,?,?,?,?,?)`).bind(entryId,order.id,order.buyerCustomerId,amount,paid,chinaBusinessDate(paid),channel,note,actor.staffId,now)];
    for(const proof of prepared)statements.push(...proof.statements,context.env.DB.prepare(`INSERT INTO buyer_advance_principal_entry_files(id,advance_payment_entry_id,file_object_id,file_entity_link_id,created_at) VALUES(?,?,?,?,?)`).bind(proof.rowId,entryId,proof.fileObjectId,proof.linkId,now));
    statements.push(createAuditEventStatement(context.env.DB,{id:crypto.randomUUID(),aggregateType:'BUYER_ADVANCE_PRINCIPAL',aggregateId:entryId,eventType:'BUYER_ADVANCE_PRINCIPAL_PAID',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:requestIdFromContext(context),idempotencyKey:acquired.claim.idempotencyKey,nextState:{formal_order_id:order.id,amount_cny_fen:String(amount),paid_at:paid,payment_channel:channel,proof_file_count:prepared.length},createdAt:now}),completeIdempotencyStatement(context.env.DB,acquired.claim,response,{resultReferences:{entry_id:entryId,proof_file_ids:prepared.map((proof)=>proof.fileObjectId)},now}),assertIdempotencyCompletionStatement(context.env.DB,acquired.claim));
    await context.env.DB.batch(statements);return ok(context,response,201);
...
async function reverseAdvancePayment(context:Context<AppEnv>){
  ...
  const body=await json(context,['reason']);const amount=Number(original.amount_cny_fen);if(!Number.isSafeInteger(amount)||amount<=0)throw new IntegrityError('CONFLICT',409);const reason=text(body['reason'],3,2000);const prior=await context.env.DB.prepare(`SELECT 1 AS present FROM buyer_advance_principal_entries WHERE entry_type='REVERSAL' AND original_payment_entry_id=? LIMIT 1`).bind(paymentId).first();if(prior)throw new IntegrityError('CONFLICT',409);
  ...
  const entryId=crypto.randomUUID();const response={reversal:{entry_id:entryId,original_payment_entry_id:paymentId,amount_cny_fen:String(amount),reversed_at:now,reason}};
  await commit(context,acquired.claim,'ADVANCE_PRINCIPAL_REVERSAL_FAILED',[
    ...
    createAuditEventStatement(context.env.DB,{id:crypto.randomUUID(),aggregateType:'BUYER_ADVANCE_PRINCIPAL',aggregateId:entryId,eventType:'BUYER_ADVANCE_PRINCIPAL_REVERSED',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:requestIdFromContext(context),idempotencyKey:acquired.claim.idempotencyKey,nextState:{original_payment_entry_id:paymentId,amount_cny_fen:String(amount),reason},createdAt:now}),
    completeIdempotencyStatement(context.env.DB,acquired.claim,response,{resultReferences:{reversal_entry_id:entryId},now}),assertIdempotencyCompletionStatement(context.env.DB,acquired.claim),
  ],now);
  return ok(context,response,201);
}
...
class AdvanceProofAuthorization implements FileAuthorizationService{
  ...
  assertCanLink(fileActor:FileActor,resource:FileAuthorizationResource):void{if(fileActor.type!=='STAFF'||fileActor.id!==this.actor.staffId||!this.actor.permissions.has('BUYER_REFUND_RECORD')||resource.ownerActorType!=='STAFF'||resource.ownerActorId!==this.actor.staffId||resource.purpose!=='BUYER_REFUND_PROOF'||resource.visibility!=='INTERNAL_ONLY'||resource.entityType!=='BUYER_REFUND')throw new IntegrityError('CONFLICT',409);}
...
export async function authoritativeAdvanceAmount(database:SqlDatabase,formalOrderId:string):Promise<number>{const row=await database.prepare(`SELECT buyer_expected_principal_cny_fen AS amount FROM formal_order_financial_snapshots WHERE formal_order_id=?`).bind(formalOrderId).first<{amount:number}>();const amount=Number(row?.amount);if(!Number.isSafeInteger(amount)||amount<=0)throw new IntegrityError('CONFLICT',409);return amount;}
