...
describe('frozen operating integrity migrations 0051-0064', () => {
  it('reaches current schema 73 with advance cash and local-date guards installed', async () => {
    ...
    const required = [
      ...
      'uq_staff_marketplace_role_primary',
      'uq_acquisition_lead_active_identity_market',
    ...
    const rows = await database
      .prepare(
        `SELECT type,name,sql FROM sqlite_schema WHERE name IN (${required.map(() => '?').join(',')})`,
      )
      .bind(...required)
    ...
    expect(
      await database
        .prepare(
          `SELECT name FROM sqlite_schema WHERE type='index' AND name='uq_acquisition_active_identity_per_type'`,
        )
        .first(),
    ).toBeNull();
    const marketIndex = await database
      .prepare(
        `SELECT sql FROM sqlite_schema WHERE type='index' AND name='uq_acquisition_lead_active_identity_market'`,
      )
      .first<{ sql: string }>();
    expect(marketIndex?.sql).toContain('lead_type,marketplace_code,identity_hash');
    const scope = await database
      .prepare(
        `SELECT name,type,"notnull",dflt_value FROM pragma_table_info('staff_marketplace_scopes') WHERE name='scope_kind'`,
      )
      .first<any>();
    ...
    expect(
      await database
        .prepare(
          `SELECT name FROM pragma_table_info('formal_orders') WHERE name='canonical_marketplace_code'`,
        )
        .first(),
    ).not.toBeNull();
    expect(
      await database
        .prepare(
          `SELECT name FROM pragma_table_info('formal_orders') WHERE name='marketplace_business_date'`,
        )
        .first(),
    ).not.toBeNull();
  });
  ...
  it('keeps the migration-controlled Marketplace runtime mirror equal to the typed registry', async () => {
    ...
    const rows = await database
      .prepare(
        `SELECT marketplace_code,legacy_order_code,business_timezone,reporting_timezone,currency_code,currency_exponent FROM marketplace_runtime_config ORDER BY marketplace_code`,
      )
      .all<any>();
    const actual = rows.results.map((row) => ({
      marketplace_code: String(row.marketplace_code),
      legacy_order_code: String(row.legacy_order_code),
      business_timezone: String(row.business_timezone),
      reporting_timezone: String(row.reporting_timezone),
      currency_code: String(row.currency_code),
      currency_exponent: Number(row.currency_exponent),
    }));
    const expected = Object.values(MARKETPLACE_RUNTIME_DEFINITIONS)
      .map((row) => ({
        marketplace_code: row.marketplace_code,
        legacy_order_code: row.legacy_order_code,
      ...
      .sort((left, right) => left.marketplace_code.localeCompare(right.marketplace_code));
  ...
  it('allows support coverage while keeping exactly one active primary per role and marketplace', async () => {
    ...
    const supportCount = await database
      .prepare(
        `SELECT COUNT(*) AS count FROM staff_marketplace_scopes WHERE role_code='pre_sales' AND marketplace_code='AMAZON_JP' AND status='ACTIVE' AND scope_kind='SUPPORT'`,
      )
      .first<{ count: number }>();
    ...
    await expect(
      database
        .prepare(
          `INSERT INTO staff_marketplace_scopes(id,staff_id,role_code,marketplace_code,status,assigned_by_staff_id,assigned_at,revoked_at,reason,created_at,updated_at,scope_kind) VALUES('integrity-second-primary','integrity-staff-primary2','pre_sales','AMAZON_JP','ACTIVE','integrity-staff-primary',2,NULL,'TEST',2,2,'PRIMARY')`,
        )
        .run(),
    ).rejects.toThrow();
  });
  ...
  it('restores a re-enabled employee as primary only when no other active primary exists', async () => {
    ...
    const scope = await database
      .prepare(
        `SELECT scope_kind FROM staff_marketplace_scopes WHERE id='integrity-reactivate-scope'`,
      )
      .first<{ scope_kind: string }>();
  ...
  it('keeps operational channel audience and staff-label safeguards in the database', async () => {
    ...
    const activeGrant = await database
      .prepare(
        `SELECT COUNT(*) AS count FROM staff_permission_overrides WHERE status='ACTIVE' AND effect='GRANT'`,
      )
      .first<{ count: number }>();
    expect(Number(activeGrant?.count)).toBe(0);
    const channelTrigger = await database
        .prepare(
          `SELECT sql FROM sqlite_schema WHERE type='trigger' AND name='trg_acquisition_channel_no_new_both'`,
        )
        .first<{ sql: string }>(),
      labelTrigger = await database
        .prepare(
          `SELECT sql FROM sqlite_schema WHERE type='trigger' AND name='trg_acquisition_channel_staff_label_immutable'`,
        )
        .first<{ sql: string }>();
