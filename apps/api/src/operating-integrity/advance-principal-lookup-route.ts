...
async function lookup(context:Context<AppEnv>){
  ...
  try{
    ...
    const value=rows.results[0];if(!actor.roles.has('owner')){const markets=await resolveStaffMarketplaceCodes(context.env.DB,actor);if(!markets.includes(String(value.canonical_marketplace_code)))return context.json(apiFailure('NOT_FOUND','没有找到唯一的正式订单',requestId),404);}
    const policy=await readFormalOrderBusinessCapabilities(context.env.DB,String(value.formal_order_id));
    const role=(...allowed:string[])=>[...actor.roles].some((candidate)=>allowed.includes(candidate));
    const actions={
      record_order_event:capability(role('owner','seller_ops'),role('owner','seller_ops')?null:'ROLE_NOT_ALLOWED'),
      record_review_visibility:capability(
        role('owner','pre_sales')&&value.review_case_id!==null&&String(value.review_status)==='APPROVED',
