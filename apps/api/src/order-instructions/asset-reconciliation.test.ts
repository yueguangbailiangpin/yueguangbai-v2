...
const actor = {
  ...
  permissions: new Set(['ORDER_INSTRUCTION_MANAGE'] as const),
  memberTeamIds: [],
  leaderTeamIds: [],
};
function seed(id: string, active = false): void {
  ...
  db!.exec(
    'PRAGMA foreign_keys=OFF; DROP TRIGGER IF EXISTS trg_file_objects_intent_guard; DROP TRIGGER IF EXISTS trg_file_entity_links_verified_guard',
  );
  db!.exec(
  ...
  if (active)
    db!.exec(
      `INSERT INTO file_entity_links(id,file_object_id,entity_type,entity_id,purpose,authorization_mode,visibility,linked_by_actor_type,linked_by_actor_id,created_at,revoked_at) VALUES('link-${id}','${id}','ORDER','e','ORDER_EVIDENCE','EXPLICIT_AUDIENCES','INTERNAL_ONLY','SYSTEM','test',1,NULL)`,
    );
  db!.exec('PRAGMA foreign_keys=ON');
}
describe('asset orphan reconciliation', () => {
  it('dry-run previews eligible candidates without storage or database effects', async () => {
    ...
    const r = await reconcileInstructionAssetOrphans(
      ...
      storage,
      { limit: 1, dryRun: true },
      { actor, idempotencyKey: 'dry-run-key', now: 2 },
    );
    expect(r).toMatchObject({ scanned: 1, deleted: 0, dry_run: true, backlog_count: 1 });
    expect(deletes).toBe(0);
    expect(
      (await db.prepare("SELECT status FROM file_objects WHERE id='obj-a'").first())?.['status'],
    ).toBe('DELETION_PENDING');
    expect(
      (await db.prepare('SELECT COUNT(*) AS count FROM command_idempotency_records').first())?.[
        'count'
      ],
    ).toBe(0);
  });
  it('excludes active links', async () => {
    db = createMigratedTestDatabase();
    seed('obj-b', true);
    const r = await reconcileInstructionAssetOrphans(
      db,
      ...
      { dryRun: true },
      { actor, idempotencyKey: 'active-link-key', now: 2 },
    );
    expect(r.backlog_count).toBe(0);
  });
  it('defers on storage delete failure without deleting the object', async () => {
    db = createMigratedTestDatabase();
    seed('obj-c');
    const r = await reconcileInstructionAssetOrphans(
      db,
      ...
      { limit: 1 },
      { actor, idempotencyKey: 'failure-key', now: 2 },
    );
    expect(r.deferred).toBe(1);
    expect(
      (await db.prepare("SELECT status FROM file_objects WHERE id='obj-c'").first())?.['status'],
    ).toBe('DELETION_PENDING');
  });
  it('resumes equal-time candidates by item id, then resets the round for earlier work', async () => {
    ...
    const first = await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 1 },
      { actor, idempotencyKey: 'cursor-one', now: 2 },
    );
    expect(first.deleted).toBe(1);
    expect(first.next_cursor?.item_id).toBe('item-obj-d');
    expect(first.backlog_count).toBe(1);
    const second = await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 1, cursor: first.next_cursor },
      { actor, idempotencyKey: 'cursor-two', now: 2 },
    );
    ...
    expect(
      (
        await db
          .prepare(
            "SELECT COUNT(*) AS count FROM file_objects WHERE id IN ('obj-d','obj-e') AND status='DELETED'",
          )
          .first()
      )?.['count'],
    ).toBe(2);
    seed('obj-0');
    const reset = await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 1 },
      { actor, idempotencyKey: 'cursor-reset', now: 2 },
    );
  ...
  it('persists the last attempted cursor when the time budget stops a larger page', async () => {
    ...
    const first = await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 3, deadlineReached: () => checks++ > 0 },
      { actor, idempotencyKey: 'deadline-first', now: 2 },
    );
    ...
    const resumed = await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 3, cursor: first.next_cursor },
      { actor, idempotencyKey: 'deadline-resume', now: 2 },
    );
  ...
  it('does not delete an already-cleaned object again on duplicate reconciliation', async () => {
    db = createMigratedTestDatabase();
    seed('obj-once');
    let deletes = 0;
    const storage = {
    ...
    await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 1 },
      { actor, idempotencyKey: 'once-first', now: 2 },
    );
    await reconcileInstructionAssetOrphans(
      db,
      storage,
      { limit: 1 },
      { actor, idempotencyKey: 'once-second', now: 2 },
    );
