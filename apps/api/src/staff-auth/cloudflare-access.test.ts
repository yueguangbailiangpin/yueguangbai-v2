...
describe('Cloudflare Access Staff identity',()=>{
  ...
  it('bootstraps only a pre-existing active Moonwhite email identity and issues an opaque session',async()=>{
    ...
    database.raw.prepare(`INSERT INTO staff_email_identities(id,staff_id,normalized_email,status,verified_at,last_login_at,created_at,updated_at,revoked_at)
      VALUES('owner-email-identity-0001','zz-phase3h-test-owner','owner@example.test','ACTIVE',NULL,NULL,1,1,NULL)`).run();
    const fixture=await jwtFixture('https://team-bootstrap.cloudflareaccess.com','audience-bootstrap-001','OWNER@EXAMPLE.TEST');
    mockJwks(fixture.jwk);
    const env={DB:database,STAFF_ACCESS_TEAM_DOMAIN:fixture.issuer,STAFF_ACCESS_AUD:fixture.audience,STAFF_AUTH_ALLOWED_ORIGINS:'https://app.example.test'};
    const response=await app.request('https://app.example.test/api/staff-auth/access/bootstrap',{
    ...
    const unknown=await jwtFixture('https://team-unknown.cloudflareaccess.com','audience-unknown-001','unknown@example.test');
    mockJwks(unknown.jwk);
    const denied=await app.request('https://app.example.test/api/staff-auth/access/bootstrap',{method:'POST',headers:{Origin:'https://app.example.test','Cf-Access-Jwt-Assertion':unknown.token}},{...env,STAFF_ACCESS_TEAM_DOMAIN:unknown.issuer,STAFF_ACCESS_AUD:unknown.audience});
    expect(denied.status).toBe(401);expect(await denied.text()).not.toContain('unknown@example.test');
  });
  ...
  it('rejects a foreign Origin before Staff session side effects',async()=>{
    ...
    },{DB:database,STAFF_ACCESS_TEAM_DOMAIN:'https://team.cloudflareaccess.com',STAFF_ACCESS_AUD:'audience-staff-foreign-origin',STAFF_AUTH_ALLOWED_ORIGINS:'https://app.example.test'});
    expect(response.status).toBe(403);
    expect(await database.prepare('SELECT COUNT(*) AS count FROM staff_sessions').first())
      .toEqual({count:0});
...
async function jwtFixture(issuer:string,audience:string,email:string){
  ...
  const header=encode({alg:'RS256',kid,typ:'JWT'});const payload=encode({iss:issuer,aud:[audience],email,sub:'access-subject',iat:nowSeconds-10,nbf:nowSeconds-10,exp:nowSeconds+300});
  const data=new TextEncoder().encode(`${header}.${payload}`);const signature=new Uint8Array(await crypto.subtle.sign('RSASSA-PKCS1-v1_5',pair.privateKey,data));
