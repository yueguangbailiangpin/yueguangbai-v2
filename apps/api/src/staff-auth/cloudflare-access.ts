...
export async function verifyCloudflareAccessIdentity(
  ...
  if (token.length < 16 || token.length > 16 * 1024) throw new CloudflareAccessError('UNAUTHENTICATED');
  const segments = token.split('.');
  if (segments.length !== 3 || segments.some((segment) => segment.length < 1)) throw new CloudflareAccessError('UNAUTHENTICATED');
  const header = jsonSegment<AccessJwtHeader>(segments[0]!);
  const payload = jsonSegment<AccessJwtPayload>(segments[1]!);
  if (header.alg !== 'RS256' || typeof header.kid !== 'string' || header.kid.length < 1 || header.kid.length > 512) throw new CloudflareAccessError('UNAUTHENTICATED');
  ...
  if (!Number.isSafeInteger(payload.exp) || exp <= nowSeconds) throw new CloudflareAccessError('UNAUTHENTICATED');
  if (payload.nbf !== undefined && (!Number.isSafeInteger(payload.nbf) || Number(payload.nbf) > nowSeconds + CLOCK_SKEW_SECONDS)) throw new CloudflareAccessError('UNAUTHENTICATED');
  if (payload.iat !== undefined) {
    if (!Number.isSafeInteger(payload.iat)
      || Number(payload.iat) > nowSeconds + CLOCK_SKEW_SECONDS
      || Number(payload.iat) > exp) throw new CloudflareAccessError('UNAUTHENTICATED');
  }
...
async function signingKeys(teamDomain: string, now: number, force = false): Promise<RemoteJwk[]> {
  ...
  if (Number.isFinite(declared) && declared > JWKS_MAX_BYTES) throw new CloudflareAccessError('UNAUTHENTICATED');
  const text = await response.text().catch(() => '');
  if (new TextEncoder().encode(text).byteLength > JWKS_MAX_BYTES) throw new CloudflareAccessError('UNAUTHENTICATED');
  let value: { keys?: unknown } | null = null;
  try { value = JSON.parse(text) as { keys?: unknown }; } catch { throw new CloudflareAccessError('UNAUTHENTICATED'); }
  if (!value || !Array.isArray(value.keys) || value.keys.length < 1 || value.keys.length > 20) throw new CloudflareAccessError('UNAUTHENTICATED');
...
function jsonSegment<T>(value: string): T {
  if (value.length > 8 * 1024) throw new CloudflareAccessError('UNAUTHENTICATED');
  try {
