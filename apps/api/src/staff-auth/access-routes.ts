...
async function bootstrap(context: Context<any>): Promise<Response> {
    ...
    if (error instanceof CloudflareAccessError) {
      console.warn(
        JSON.stringify({
          ...
          access_email_header_present: Boolean(
            context.req.header('Cf-Access-Authenticated-User-Email'),
          ),
        }),
      );
    }
  ...
  if (existing.value && !existing.malformed) {
    const trusted = await resolveTrustedStaffSession(context.env.DB, existing.value, now).catch(
      () => null,
    );
    if (trusted)
      await revokeStaffSession(context.env.DB, {
        session: trusted.session,
        reason: 'SESSION_REPLACED_BY_ACCESS',
  ...
  `,
    ...
    .bind(now, now, now, identity.identity_id)
    .run();
...
async function logout(context: Context<any>): Promise<Response> {
  ...
  if (cookie.value && !cookie.malformed) {
    const trusted = await resolveTrustedStaffSession(context.env.DB, cookie.value).catch(
      () => null,
    );
    if (trusted)
      await revokeStaffSession(context.env.DB, {
  ...
  context.header('Cache-Control', 'no-store');
  return context.json(
    apiSuccess({ logged_out: true, all_devices_logged_out: false }, requestIdFromContext(context)),
  );
}
...
async function logoutAll(context: Context<any>): Promise<Response> {
  ...
  if (key.length < 8 || key.length > 128 || /[\u0000-\u001f\u007f]/u.test(key))
  ...
  if (replay) {
    clearStaffSessionCookie(context);
    context.header('Cache-Control', 'no-store');
    return context.json(apiSuccess(replay.response, requestIdFromContext(context)));
  }
  const trusted = await resolveTrustedStaffSession(context.env.DB, cookie.value);
  const now = Date.now();
  ...
  const response =
    acquired.kind === 'REPLAY'
      ? acquired.response
      : await logoutAllStaffSessions(context.env.DB, {
...
async function emailIdentity(database: SqlDatabase, rawEmail: string): Promise<EmailIdentityRow> {
  ...
  if (!email) throw new StaffAuthError('UNAUTHENTICATED', 401);
...
function requireAllowedOrigin(context: Context<any>): void {
  ...
  const configured = String(context.env.STAFF_AUTH_ALLOWED_ORIGINS ?? '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean);
  if (configured.length > 0) {
    if (!configured.includes(origin)) throw new StaffAuthError('FORBIDDEN', 403);
    return;
  }
...
function normalizeIdempotencyError(error: unknown): StaffAuthError {
  ...
  if (
    candidate &&
    (candidate.status === 400 || candidate.status === 409 || candidate.status === 503) &&
  ...
  return async (context: Context<any>) => {
    try {
      ...
      return staffAuthFailure(
        context,
        error instanceof StaffAuthError ? error : new StaffAuthError('DEPENDENCY_UNAVAILABLE', 503),
      );
    }
  };
}
