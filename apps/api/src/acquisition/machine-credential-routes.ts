...
export function registerAcquisitionMachineCredentialRoutes(app: Hono<any>): void {
  app.get(
    ...
    wrap(async (context) => {
      owner(context);
      return ok(context, { machines: await listMachineCredentials(context.env.DB) });
    }),
  );
  app.post(
    '/api/staff/acquisition/machines',
    ...
    wrap(async (context) => {
      ...
      if (
        ...
        !Array.isArray(body['marketplace_codes']) ||
        !Array.isArray(body['channel_ids']) ||
        body['marketplace_codes'].some((v) => typeof v !== 'string') ||
        body['channel_ids'].some((v) => typeof v !== 'string') ||
        !Number.isSafeInteger(body['hourly_request_limit'])
      )
        throw new AcquisitionError('VALIDATION_ERROR', 400);
      const result = await createMachineCredential(
        context.env.DB,
        {
          machineName: body['machine_name'],
          marketplaceCodes: body['marketplace_codes'] as string[],
          channelIds: body['channel_ids'] as string[],
          hourlyRequestLimit: Number(body['hourly_request_limit']),
        },
  ...
  app.post(
    '/api/staff/acquisition/machines/:id/revoke',
    customerAuthOriginGuard(),
    wrap(async (context) => {
      const actor = owner(context);
      await exact(context, []);
...
function owner(context: Context<any>) {
  ...
  if (
    ...
    !actor.permissions.has('ACQUISITION_ADMIN')
  )
...
function command(
  ...
  if (!key) throw new AcquisitionError('VALIDATION_ERROR', 400);
  return { actor, idempotencyKey: key, requestId: requestIdFromContext(context) };
}
async function exact(context: Context<any>, keys: string[]) {
  const value = await readBoundedJson(context.req.raw, 32 * 1024);
...
function ok(context: Context<any>, data: unknown, status = 200) {
    ...
    try {
      ...
      return context.json(
        apiFailure(
          e.code,
          e.code === 'FORBIDDEN'
            ? '只有总管理员可以管理 Codex 机器密钥'
            : e.code === 'STATE_CONFLICT'
              ? '该机器已经停用或状态已变化'
