...
export function registerAcquisitionMachineCredentialRoutes(app:Hono<any>):void{
  ...
  app.post('/api/staff/acquisition/machines',customerAuthOriginGuard(),wrap(async(context)=>{
    ...
    const machine=await createMachineCredential(context.env.DB,{machineName:body['machine_name'],marketplaceCodes:body['marketplace_codes'] as string[],channelIds:body['channel_ids'] as string[],hourlyRequestLimit:Number(body['hourly_request_limit']),createdByStaffId:actor.staffId});
    return ok(context,{machine},201);
  }));
  app.post('/api/staff/acquisition/machines/:id/revoke',customerAuthOriginGuard(),wrap(async(context)=>{
    const actor=owner(context);return ok(context,{machine:await revokeMachineCredential(context.env.DB,context.req.param('id')??'',actor.staffId)});
  }));
}
...
function wrap(handler:(context:Context<any>)=>Promise<Response>){return async(context:Context<any>)=>{try{return await handler(context);}catch(error){const e=error instanceof AcquisitionError?error:new AcquisitionError('DEPENDENCY_UNAVAILABLE',503);return context.json(apiFailure(e.code,e.code==='FORBIDDEN'?'只有总管理员可以管理 Codex 机器密钥':e.code==='STATE_CONFLICT'?'该机器已经停用或状态已变化':e.code==='VALIDATION_ERROR'?'机器名称、站点、渠道或限流配置不正确':'机器密钥服务暂时不可用',requestIdFromContext(context)),e.status);}};}
