...
export async function authenticateAcquisitionMachine(database:SqlDatabase,request:Request,now=Date.now()):Promise<AcquisitionMachineIdentity>{
  ...
  const hash=await sha256(secret),row=await database.prepare(`SELECT id,machine_name,hourly_request_limit FROM acquisition_machine_credentials WHERE secret_sha256=? AND status='ACTIVE'`).bind(hash).first<{id:string;machine_name:string;hourly_request_limit:number}>();if(!row)throw new AcquisitionError('UNAUTHENTICATED',401);
  const bucket=Math.floor(now/3_600_000),count=await database.prepare(`INSERT INTO acquisition_machine_rate_buckets(machine_id,bucket_hour,request_count,updated_at) VALUES(?,?,1,?)
  ...
  await database.prepare(`DELETE FROM acquisition_machine_rate_buckets WHERE machine_id=? AND bucket_hour<?`).bind(row.id,bucket-RATE_BUCKET_RETENTION_HOURS).run().catch(()=>undefined);
  if(Number(count?.request_count??0)>Number(row.hourly_request_limit))throw new AcquisitionError('RATE_LIMITED',429);
