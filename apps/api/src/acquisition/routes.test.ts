...
describe('acquisition HTTP authority and privacy boundary', () => {
  it('rejects buyer_refund and Personal DENY before any lead fact', async () => {
    ...
    for (const actor of [auth('buyer_refund', 'staff-refund'), deniedPreSales()]) {
      const response = await request(actor, '/api/staff/acquisition/leads', {
        method: 'POST',
        headers: headers(`forbidden-${actor.staffId}`),
        body: JSON.stringify(leadBody('forbidden-channel', 'forbidden_wx')),
      });
      expect(response.status).toBe(403);
    }
    expect(database.raw.prepare('SELECT COUNT(*) AS count FROM acquisition_leads').get()).toEqual({
      count: 0,
    });
  });
  ...
  it('rejects unknown channel authority and cross-site writes', async () => {
    ...
    const clientAuthority = await request(
      auth('pre_sales', 'staff-pre'),
      '/api/staff/acquisition/leads',
      {
        method: 'POST',
        headers: headers('client-channel-0001'),
  ...
  it('creates a lead with an explicit approved channel and returns the WeChat ID to authorized staff', async () => {
    ...
    const response = await request(auth('pre_sales', 'staff-pre'), '/api/staff/acquisition/leads', {
      method: 'POST',
      headers: headers('route-lead-0001'),
      body: JSON.stringify({ ...leadBody(channel, 'route_secret_wx'), display_name: '路由买家' }),
    });
  ...
  it('enforces consultation origin, exact body, idempotency key and strict owner authority', async () => {
    ...
    const ownerResponse = await request(
      auth('owner', 'staff-owner-route'),
      '/api/staff/acquisition/consultations',
      {
        method: 'POST',
        headers: headers('route-consultation-owner-0001'),
    ...
    expect(await ownerResponse.json()).toMatchObject({
      data: {
        consultation: { channel_id: channel, person_count: 12, version: 1 },
        replayed: false,
      },
    });
    ...
    for (const [name, init] of [
      [
        ...
        {
          headers: {
            ...headers('route-consultation-origin-0001'),
            Origin: 'https://evil.example',
            'Sec-Fetch-Site': 'cross-site',
          },
          body,
      ...
      [
        'extra-body',
        { headers: headers('route-consultation-extra-0001'), body: { ...body, owner: true } },
      ],
      [
        'missing-idempotency',
        {
    ...
    for (const [index, actor] of [
      ...
      auth('buyer_refund', 'staff-refund'),
    ].entries()) {
      const response = await request(actor, '/api/staff/acquisition/consultations', {
        method: 'POST',
        headers: headers(`route-consultation-forbidden-${index}`),
        body: JSON.stringify({
          ...body,
          business_date: `2025-01-${String(index + 3).padStart(2, '0')}`,
        }),
      });
      expect(response.status).toBe(403);
    }
    expect(
      ...
      (SELECT COUNT(*) FROM command_idempotency_records
        WHERE action='RECORD_ACQUISITION_CONSULTATION') AS idempotency`,
        )
        .get(),
  ...
  it('lets acquisition use scoped operator reads but conceals cross-scope history and formal Leads', async () => {
    ...
    const us = await createAcquisitionChannel(
      database,
      {
        code: 'ROUTE_XHS_US',
        platformName: '小红书',
      ...
      {
        actor: owner,
        idempotencyKey: 'route-channel-us-0001',
        requestId: 'route-channel-us-request',
        now: 1000,
      },
    );
    const jp = await recordAcquisitionConsultation(
      database,
      {
        channelId: jpChannel,
      ...
      {
        actor: owner,
        idempotencyKey: 'route-consultation-jp-0001',
        requestId: 'route-consultation-jp-request',
        now: 1001,
      },
    );
    const crossScope = await recordAcquisitionConsultation(
      database,
      {
        channelId: us.channel.channel_id,
        businessDate: '2025-01-01',
        personCount: 7,
        expectedVersion: 0,
        reason: '美国站路由汇总',
      ...
      {
        actor: owner,
        idempotencyKey: 'route-consultation-us-0001',
        requestId: 'route-consultation-us-request',
        now: 1002,
      },
    );
  ...
  it('maps an unknown consultation batch failure to 503 after cleaning the claim', async () => {
    ...
    const response = await request(
      ...
      {
        method: 'POST',
        headers: headers('route-consultation-dependency-0001'),
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM audit_events
        WHERE aggregate_type='ACQUISITION_DAILY_CONSULTATION') AS audits,
      (SELECT COUNT(*) FROM command_idempotency_records
        WHERE action='RECORD_ACQUISITION_CONSULTATION' AND status='COMMITTED') AS committed`,
        )
        .get(),
    ...
    expect(
      database.raw
        .prepare(
          `SELECT status,error_code,response_json
      FROM command_idempotency_records
      WHERE idempotency_key='route-consultation-dependency-0001'`,
        )
        .get(),
  ...
  it('recomputes consultation authority from a real Staff cookie and current D1 facts', async () => {
    ...
    const missing = await trustedRequest('/api/staff/acquisition/consultations', {
      method: 'POST',
      headers: headers('trusted-missing-cookie-0001'),
      body,
    });
    ...
    database.raw
      .prepare(
        `UPDATE staff_sessions SET status='REVOKED',revoked_at=?,
    ...
    const revoked = await trustedRequest('/api/staff/acquisition/consultations', {
      method: 'POST',
      headers: trustedHeaders('trusted-revoked-cookie-0001', revokedToken),
      body,
    });
    ...
    const deniedOwner = await trustedRequest('/api/staff/acquisition/consultations', {
      method: 'POST',
      headers: trustedHeaders('trusted-owner-deny-0001', deniedOwnerToken),
      body,
    });
    ...
    database.raw
      ...
      updated_at=updated_at+1 WHERE id='staff-pre'`,
      )
      .run();
    const drift = await trustedRequest('/api/staff/acquisition/consultations', {
      method: 'POST',
      headers: trustedHeaders('trusted-version-drift-0001', driftToken),
      body,
    });
  ...
  it('creates and revokes a machine credential with non-secret replay and atomic cleanup', async () => {
    ...
    const createBody = {
      machine_name: '日本买家 Agent',
      marketplace_codes: ['AMAZON_JP'],
      channel_ids: [channel],
      hourly_request_limit: 120,
    };
    const first = await trustedRequest('/api/staff/acquisition/machines', {
      method: 'POST',
      headers: trustedHeaders('route-machine-create-0001', token),
    ...
    await disableAcquisitionChannel(
      database,
      {
        channelId: channel,
        expectedVersion: 1,
        reason: '验证 credential replay 不依赖可变渠道状态',
      },
      {
        actor: auth('owner', 'staff-owner-route'),
        idempotencyKey: 'route-channel-disable-replay',
        requestId: 'route-channel-disable-replay',
        now: 2000,
      },
    );
    const replay = await trustedRequest('/api/staff/acquisition/machines', {
      method: 'POST',
      headers: trustedHeaders('route-machine-create-0001', token),
      body: JSON.stringify(createBody),
    });
    ...
    await expect(replay.json()).resolves.toMatchObject({
      data: {
        replayed: true,
        machine: {
          machine_id: firstBody.data.machine.machine_id,
          machine_secret: null,
    ...
    const conflict = await trustedRequest('/api/staff/acquisition/machines', {
      method: 'POST',
      headers: trustedHeaders('route-machine-create-0001', token),
    ...
    database.raw
      .prepare(
        `INSERT INTO acquisition_machine_rate_buckets(
    ...
    const revoke = await trustedRequest(
      ...
      {
        method: 'POST',
        headers: trustedHeaders('route-machine-revoke-0001', token),
        body: '{}',
      },
    );
    ...
    await expect(revokeReplay.json()).resolves.toMatchObject({
      data: { replayed: true, machine: { revoked_at: revoked.data.machine.revoked_at } },
    });
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM command_idempotency_records WHERE action LIKE '%ACQUISITION_MACHINE_CREDENTIAL' AND status='COMMITTED') AS commands`,
        )
        .get(),
    ...
    const persisted = JSON.stringify(
      ...
      (SELECT group_concat(payload_json) FROM integration_outbox WHERE aggregate_type='ACQUISITION_MACHINE_CREDENTIAL') AS outbox`,
        )
        .get(),
    );
  ...
  it('runs machine Prospect commands through Bearer auth with replay, concealment, OCC and atomic failure cleanup', async () => {
    ...
    const channel = await seedChannelAndAssignment(database);
    const token = await seedTrustedSession('staff-owner-route');
    const credential = await trustedRequest('/api/staff/acquisition/machines', {
      method: 'POST',
      headers: trustedHeaders('machine-route-create-0001', token),
      body: JSON.stringify({
        machine_name: '正式 Bearer Agent',
        marketplace_codes: ['AMAZON_JP'],
        channel_ids: [channel],
        hourly_request_limit: 120,
      }),
    ...
    const credentialBody = (await credential.json()) as {
      data: { machine: { machine_secret: string; machine_id: string } };
    };
    const secret = credentialBody.data.machine.machine_secret;

    const prospect = await machineRequest('/api/acquisition-machine/prospects', {
      method: 'POST',
      headers: machineHeaders(secret, 'machine-prospect-create-0001'),
      body: JSON.stringify({
        lead_type: 'BUYER',
        marketplace_code: 'AMAZON_JP',
        channel_id: channel,
        display_name: 'Bearer 并发潜客',
        contact_value: null,
        source_url: null,
        note: null,
        ai_score: null,
    ...
    const prospectId = ((await prospect.json()) as { data: { prospect: { prospect_id: string } } })
      .data.prospect.prospect_id;
    const signalBody = {
      signal_type: 'PUBLIC_POST',
      signal_content: '机器发现的公开事实',
      source_url: 'https://example.test/machine/1',
      confidence: 'HIGH',
    };
    const missingKey = await machineRequest(
      `/api/acquisition-machine/prospects/${prospectId}/signals`,
      { method: 'POST', headers: machineHeaders(secret), body: JSON.stringify(signalBody) },
    );
    expect(missingKey.status).toBe(400);
    const firstSignal = await machineRequest(
      `/api/acquisition-machine/prospects/${prospectId}/signals`,
      {
        method: 'POST',
        headers: machineHeaders(secret, 'machine-signal-0001'),
        body: JSON.stringify(signalBody),
      },
    );
    expect(firstSignal.status).toBe(201);
    const firstSignalBody = (await firstSignal.json()) as {
      data: { signal: { signal_id: string }; replayed: boolean };
    };
    expect(firstSignalBody.data.replayed).toBe(false);
    const signalReplay = await machineRequest(
      `/api/acquisition-machine/prospects/${prospectId}/signals`,
      {
        method: 'POST',
        headers: machineHeaders(secret, 'machine-signal-0001'),
    ...
    await expect(signalReplay.json()).resolves.toMatchObject({
      data: { replayed: true, signal: { signal_id: firstSignalBody.data.signal.signal_id } },
    });
    const signalConflict = await machineRequest(
      `/api/acquisition-machine/prospects/${prospectId}/signals`,
      {
        method: 'POST',
        headers: machineHeaders(secret, 'machine-signal-0001'),
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM command_idempotency_records WHERE idempotency_key='machine-signal-0001' AND status='COMMITTED') AS committed`,
        )
        .get(prospectId, prospectId, prospectId),
    ).toEqual({ signals: 1, audits: 1, outbox: 1, committed: 1 });

    const us = await createAcquisitionChannel(
      database,
      {
        code: 'MACHINE_SCOPE_US',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_US',
        displayName: '越范围渠道',
      ...
      {
        actor: auth('owner', 'staff-owner-route'),
        idempotencyKey: 'machine-scope-us-channel',
        requestId: 'machine-scope-us-channel',
        now: 3000,
      },
    );
    const outside = await request(
      ...
      '/api/staff/acquisition/prospects',
      {
        ...
        body: JSON.stringify({
          ...
          channel_id: us.channel.channel_id,
          display_name: '越范围潜客',
          contact_value: null,
          source_url: null,
          origin_mode: 'HUMAN',
    ...
    const outsideId = ((await outside.json()) as { data: { prospect: { prospect_id: string } } })
      .data.prospect.prospect_id;
    const concealed = [] as Array<{ status: number; code: string; message: string }>;
    for (const [id, key] of [
      [outsideId, 'machine-scope-real-0001'],
      ['random-prospect-id', 'machine-scope-random-0001'],
    ] as const) {
      const response = await machineRequest(`/api/acquisition-machine/prospects/${id}/signals`, {
        method: 'POST',
        headers: machineHeaders(secret, key),
        body: JSON.stringify(signalBody),
      });
    ...
    const analysis = (key: string, status: string) =>
      machineRequest(
        `/api/acquisition-machine/prospects/${prospectId}/analysis`,
        {
          method: 'POST',
          headers: machineHeaders(secret, key),
          body: JSON.stringify({ expected_version: 1, status, ai_score: 70, note: status }),
        },
        barrier,
      );
    const raced = await Promise.all([
      analysis('machine-analysis-race-a', 'RESEARCHING'),
      analysis('machine-analysis-race-b', 'QUALIFIED'),
    ]);
    expect(barrier.arrivals).toBe(2);
    expect(raced.map((value) => value.status).sort()).toEqual([200, 409]);
    const winner = raced.find((value) => value.status === 200)!;
    const loser = raced.find((value) => value.status === 409)!;
    const winnerBody = (await winner.json()) as {
      data: { prospect: { status: string; version: number } };
    };
    expect(winnerBody.data.prospect.version).toBe(2);
    expect(await loser.json()).toMatchObject({ error: { code: 'VERSION_CONFLICT' } });
    expect(
      database.raw
        .prepare('SELECT status,version FROM acquisition_prospects WHERE id=?')
        .get(prospectId),
    ).toEqual({ status: winnerBody.data.prospect.status, version: 2 });
    expect(
      database.raw
        .prepare(
          `SELECT
      ...
      (SELECT COUNT(*) FROM command_idempotency_records WHERE action='CODEX_UPDATE_ACQUISITION_PROSPECT_ANALYSIS' AND status='COMMITTED') AS committed`,
        )
        .get(prospectId, prospectId),
    ).toEqual({ audits: 1, outbox: 1, committed: 1 });

    const failed = await machineRequest(
      `/api/acquisition-machine/prospects/${prospectId}/signals`,
      {
        method: 'POST',
        headers: machineHeaders(secret, 'machine-signal-failure-0001'),
        body: JSON.stringify({ ...signalBody, signal_content: '故障注入事实' }),
      },
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM command_idempotency_records WHERE idempotency_key='machine-signal-failure-0001' AND status='COMMITTED') AS committed`,
        )
        .get(prospectId, prospectId, prospectId),
    ).toEqual({ signals: 0, audits: 0, outbox: 0, committed: 0 });
    expect(
      database.raw
        .prepare(
          `SELECT status,error_code FROM command_idempotency_records WHERE idempotency_key='machine-signal-failure-0001'`,
        )
        .get(),
  ...
  it('guards Prospect update and replays Prospect signals through the real route chain', async () => {
    ...
    const created = await request(ownerActor, '/api/staff/acquisition/prospects', {
      method: 'POST',
      headers: headers('route-prospect-create-0001'),
      body: JSON.stringify({
        lead_type: 'BUYER',
        marketplace_code: 'AMAZON_JP',
        channel_id: channel,
    ...
    const prospectId = (
      (await created.json()) as {
        data: {
          prospect: { prospect_id: string };
        };
      }
    ).data.prospect.prospect_id;

    const update = (key: string, status: 'RESEARCHING' | 'QUALIFIED') =>
      request(ownerActor, `/api/staff/acquisition/prospects/${prospectId}/update`, {
        method: 'POST',
        headers: headers(key),
        body: JSON.stringify({
          expected_version: 1,
    ...
    expect(winnerBody.data).toMatchObject({ replayed: false, prospect: { version: 2 } });
    ...
    const signal = await request(
      ...
      {
        method: 'POST',
        headers: headers('route-prospect-signal-0001'),
        body: JSON.stringify(signalBody),
      },
    );
    expect(signal.status).toBe(201);
    const signalBodyFirst = (await signal.json()) as {
      data: {
        signal: {
          signal_id: string;
        };
    ...
    const signalReplay = await request(
      ownerActor,
      `/api/staff/acquisition/prospects/${prospectId}/signals`,
      {
        method: 'POST',
    ...
    expect(signalReplay.status).toBe(201);
    await expect(signalReplay.json()).resolves.toMatchObject({
      data: { replayed: true, signal: { signal_id: signalBodyFirst.data.signal.signal_id } },
    });
    const signalConflict = await request(
      ownerActor,
      `/api/staff/acquisition/prospects/${prospectId}/signals`,
      {
        method: 'POST',
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM integration_outbox WHERE aggregate_id=? AND event_type='ACQUISITION_PROSPECT_SIGNAL_ADDED') AS outbox`,
        )
        .get(prospectId, prospectId, prospectId),
    ).toEqual({ signals: 1, audits: 1, outbox: 1 });

    const unknownFailure = await request(
      ownerActor,
      `/api/staff/acquisition/prospects/${prospectId}/update`,
      {
        method: 'POST',
        headers: headers('route-prospect-update-dependency-0001'),
        body: JSON.stringify({
          expected_version: 2,
          status: 'CONTACTED',
          ai_score: 70,
    ...
    expect(
      ...
      WHERE idempotency_key='route-prospect-update-dependency-0001'`,
        )
        .get(),
    ).toEqual({ status: 'FAILED', error_code: 'ACQUISITION_COMMAND_FAILED' });
    expect(
      database.raw.prepare(`SELECT version FROM acquisition_prospects WHERE id=?`).get(prospectId),
    ).toEqual({ version: 2 });
  });
  ...
  it('replays assignment revoke and serializes source correction by expected sequence', async () => {
    ...
    const assignment = database.raw
      .prepare(
        `SELECT id FROM acquisition_staff_channel_assignments
      WHERE staff_id='staff-pre' AND channel_id=?`,
      )
      .get(channel) as { id: string };
    ...
    const revoked = await trustedRequest(
      ...
      {
        method: 'POST',
        headers: trustedHeaders('route-assignment-revoke-0001', token),
    ...
    const failureAssignment = await createAcquisitionAssignment(
      ...
      {
        actor: ownerActor,
        idempotencyKey: 'route-assignment-failure-create',
        requestId: 'route-assignment-failure-create',
        now: 1100,
      },
    );
    const failedRevoke = await trustedRequest(
      `/api/staff/acquisition/channel-assignments/${failureAssignment.assignment.assignment_id}/revoke`,
      {
        method: 'POST',
        headers: trustedHeaders('route-assignment-revoke-failure', token),
        body: JSON.stringify(revokeBody),
      },
      new ConsultationBatchFailureDatabase(database),
    );
    expect(failedRevoke.status).toBe(503);
    expect(await failedRevoke.json()).toMatchObject({ error: { code: 'DEPENDENCY_UNAVAILABLE' } });
    expect(
      database.raw
        .prepare(
          `SELECT
      ...
      (SELECT COUNT(*) FROM acquisition_staff_channel_assignments WHERE id=? AND status='ACTIVE' AND version=1) AS active`,
        )
        .get(
          failureAssignment.assignment.assignment_id,
          failureAssignment.assignment.assignment_id,
          failureAssignment.assignment.assignment_id,
          failureAssignment.assignment.assignment_id,
        ),
    ).toEqual({ events: 0, audits: 0, outbox: 0, committed: 0, active: 1 });

    const correctionTarget = await createAcquisitionChannel(
      database,
      {
        code: 'ROUTE_CORRECTION',
        platformName: '小红书',
        leadType: 'BUYER',
      ...
      {
        actor: ownerActor,
        idempotencyKey: 'route-correction-channel-0001',
        requestId: 'route-correction-channel-request',
        now: 2000,
      },
    );
    const lead = await request(ownerActor, '/api/staff/acquisition/leads', {
      method: 'POST',
      headers: headers('route-correction-lead-0001'),
      body: JSON.stringify(leadBody(channel, 'route_correction_wx')),
    });
    expect(lead.status).toBe(201);
    const leadId = ((await lead.json()) as { data: { lead: { lead_id: string } } }).data.lead
      .lead_id;
    const correctionBody = {
      lead_id: leadId,
      new_channel_id: correctionTarget.channel.channel_id,
      expected_correction_sequence: 0,
      reason: '首次登记渠道选择错误',
    };
    const correction = await trustedRequest('/api/staff/acquisition/source-corrections', {
      method: 'POST',
      headers: trustedHeaders('route-source-correction-0001', token),
    ...
    expect(correctionResponse.data).toMatchObject({
      replayed: false,
      correction: { correction_sequence: 1 },
    });
    const correctionReplay = await trustedRequest('/api/staff/acquisition/source-corrections', {
      method: 'POST',
      headers: trustedHeaders('route-source-correction-0001', token),
      body: JSON.stringify(correctionBody),
    });
    ...
    const stale = await trustedRequest('/api/staff/acquisition/source-corrections', {
      method: 'POST',
      headers: trustedHeaders('route-source-correction-stale-0001', token),
      body: JSON.stringify({ ...correctionBody, new_channel_id: channel }),
    });
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM integration_outbox WHERE aggregate_id=? AND event_type='ACQUISITION_SOURCE_CORRECTED') AS outbox`,
        ...
        .get(leadId, leadId, leadId),
    ).toEqual({ corrections: 1, audits: 1, outbox: 1 });

    const raceA = await createAcquisitionChannel(
      database,
      {
        code: 'ROUTE_CORRECTION_RACE_A',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_JP',
        displayName: '来源更正竞态 A',
      },
      {
        actor: ownerActor,
        idempotencyKey: 'route-correction-race-channel-a',
        requestId: 'route-correction-race-channel-a',
        now: 2100,
      },
    );
    const raceB = await createAcquisitionChannel(
      database,
      {
        code: 'ROUTE_CORRECTION_RACE_B',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_JP',
        displayName: '来源更正竞态 B',
      },
      {
        actor: ownerActor,
        idempotencyKey: 'route-correction-race-channel-b',
        requestId: 'route-correction-race-channel-b',
        now: 2101,
      },
    );
    const barrier = new SourceCorrectionReadBarrierDatabase(database);
    const race = (key: string, target: string) =>
      trustedRequest(
        '/api/staff/acquisition/source-corrections',
        {
          method: 'POST',
          headers: trustedHeaders(key, token),
          body: JSON.stringify({
            lead_id: leadId,
            new_channel_id: target,
            expected_correction_sequence: 1,
            reason: '受控并发来源更正',
          }),
        },
        barrier,
    ...
    const raced = await Promise.all([
      race('route-source-correction-race-a', raceA.channel.channel_id),
      race('route-source-correction-race-b', raceB.channel.channel_id),
    ]);
    expect(barrier.arrivals).toBe(2);
    expect(raced.map((value) => value.status).sort()).toEqual([201, 409]);
    expect(
      database.raw
        .prepare(
          `SELECT
      ...
      (SELECT COUNT(*) FROM audit_events WHERE aggregate_id=? AND event_type='ACQUISITION_SOURCE_CORRECTED') AS audits,
      (SELECT COUNT(*) FROM integration_outbox WHERE aggregate_id=? AND event_type='ACQUISITION_SOURCE_CORRECTED') AS outbox,
      (SELECT COUNT(*) FROM command_idempotency_records WHERE action='CORRECT_ACQUISITION_LEAD_SOURCE' AND status='COMMITTED') AS committed`,
        )
        .get(leadId, leadId, leadId),
    ).toEqual({ corrections: 2, audits: 2, outbox: 2, committed: 2 });

    const failureTarget = await createAcquisitionChannel(
      database,
      {
        code: 'ROUTE_CORRECTION_FAILURE',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_JP',
        displayName: '来源更正故障目标',
      },
      {
        actor: ownerActor,
        idempotencyKey: 'route-correction-failure-channel',
        requestId: 'route-correction-failure-channel',
        now: 2200,
      },
    );
    const failed = await trustedRequest(
      '/api/staff/acquisition/source-corrections',
      {
        method: 'POST',
        headers: trustedHeaders('route-source-correction-failure', token),
        body: JSON.stringify({
          lead_id: leadId,
          new_channel_id: failureTarget.channel.channel_id,
          expected_correction_sequence: 2,
          reason: '注入 batch 故障',
        }),
      },
      new ConsultationBatchFailureDatabase(database),
    ...
    expect(
      ...
      (SELECT COUNT(*) FROM command_idempotency_records WHERE idempotency_key='route-source-correction-failure' AND status='COMMITTED') AS committed`,
        )
        .get(leadId, leadId),
    ).toEqual({ corrections: 2, audits: 0, outbox: 0, committed: 0 });
    expect(
      database.raw
        .prepare(
          `SELECT status,error_code FROM command_idempotency_records WHERE idempotency_key='route-source-correction-failure'`,
        )
        .get(),
    ).toEqual({ status: 'FAILED', error_code: 'ACQUISITION_COMMAND_FAILED' });
  });
  ...
  it('uses formal Staff sessions and conceals source-correction resource scope as not found', async () => {
    ...
    const jpChannel = await seedChannelAndAssignment(database);
    const ownerActor = auth('owner', 'staff-owner-route');
    const usChannel = await createAcquisitionChannel(
      database,
      {
        code: 'CORRECTION_SCOPE_US',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_US',
        displayName: '美国更正渠道',
      ...
      {
        actor: ownerActor,
        idempotencyKey: 'correction-scope-us-channel',
        requestId: 'correction-scope-us-channel',
        now: 4000,
      },
    );
    database.raw
      .prepare(
        `UPDATE acquisition_channel_privacy_profiles SET intake_wechat_label='美国范围测试工作微信',version=version+1,updated_at=4000 WHERE channel_id=?`,
      )
      .run(usChannel.channel.channel_id);
    const usTarget = await createAcquisitionChannel(
      database,
      {
        code: 'CORRECTION_SCOPE_US_TARGET',
        platformName: '小红书',
        leadType: 'BUYER',
        marketplaceCode: 'AMAZON_US',
        displayName: '美国更正目标',
      ...
      {
        actor: ownerActor,
        idempotencyKey: 'correction-scope-us-target',
        requestId: 'correction-scope-us-target',
        now: 4001,
      },
    );
    const usLead = await request(ownerActor, '/api/staff/acquisition/leads', {
      method: 'POST',
      headers: headers('correction-scope-us-lead'),
      body: JSON.stringify({
        lead_type: 'BUYER',
        marketplace_code: 'AMAZON_US',
        channel_id: usChannel.channel.channel_id,
        prospect_id: null,
        wechat_id: 'correction_scope_us_wx',
        display_name: null,
        note: null,
      }),
    });
    expect(usLead.status).toBe(201);
    const usLeadId = ((await usLead.json()) as { data: { lead: { lead_id: string } } }).data.lead
      .lead_id;
    const jpLead = await request(ownerActor, '/api/staff/acquisition/leads', {
      method: 'POST',
      headers: headers('correction-scope-jp-lead'),
      body: JSON.stringify(leadBody(jpChannel, 'correction_scope_jp_wx')),
    });
    ...
    const invoke = async (body: Record<string, unknown>, key: string, token = acquisitionToken) => {
      ...
      return { status: response.status, code: value.error.code, message: value.error.message };
    };
    const common = { expected_correction_sequence: 0, reason: '验证越范围资源统一隐藏' };
    const realLead = await invoke(
      { ...common, lead_id: usLeadId, new_channel_id: usTarget.channel.channel_id },
      'correction-scope-real-lead',
    );
    const randomLead = await invoke(
      { ...common, lead_id: 'random-lead-id', new_channel_id: usTarget.channel.channel_id },
    ...
    const randomChannel = await invoke(
      { ...common, lead_id: jpLeadId, new_channel_id: 'random-channel-id' },
    ...
    const denied = await invoke(
      { ...common, lead_id: 'random-lead-id', new_channel_id: 'random-channel-id' },
      'correction-global-capability-denied',
      deniedToken,
    );
    ...
    expect(
      database.raw
        .prepare(
          `SELECT COUNT(*) AS count FROM command_idempotency_records WHERE idempotency_key='correction-global-capability-denied'`,
        )
        .get(),
    ).toEqual({ count: 0 });
  });
...
function machineHeaders(secret: string, key?: string) {
  return {
    'Content-Type': 'application/json',
...
async function seedChannelAndAssignment(db: SqliteDatabase) {
  ...
  const channel = await createAcquisitionChannel(
    ...
    {
      actor: owner,
      idempotencyKey: 'route-channel-0001',
      requestId: 'route-channel-request',
      now: 1000,
    },
  );
  db.raw
    ...
    .run(channel.channel.channel_id);
  await createAcquisitionAssignment(
    ...
    {
      actor: owner,
      idempotencyKey: 'route-assignment-0001',
      requestId: 'route-assignment-request',
      now: 1000,
    },
  );
...
function auth(role: StaffRoleCode, staffId: string): AssignmentStaffAuthorization {
  const effective = calculateEffectiveStaffAuthorization({
    ...
    memberTeamIds: role === 'owner' ? [] : ['phase3h-test-team'],
    leaderTeamIds: [],
  });
  return {
    staffId,
...
async function seedTrustedSession(staffId: string): Promise<string> {
  const staff = database!.raw
    .prepare(
      `SELECT display_name,status,authorization_version,
    session_version FROM staff_users WHERE id=?`,
    )
    .get(staffId) as {
    display_name: string;
    status: 'ACTIVE';
...
class ProspectReadBarrierDatabase implements SqlDatabase {
  ...
  prepare(sql: string): SqlStatement {
    ...
    return sql.includes('FROM acquisition_prospects prospect JOIN acquisition_channels channel')
      ? new BarrierStatement(statement, () => this.arrive())
      : statement;
  }
...
class SourceCorrectionReadBarrierDatabase implements SqlDatabase {
  ...
  prepare(sql: string): SqlStatement {
    const statement = this.target.prepare(sql);
    return sql.includes(
      'SELECT COUNT(*) AS sequence,(SELECT new_channel_id FROM acquisition_lead_source_corrections',
    )
      ? new BarrierStatement(statement, () => this.arrive())
      : statement;
  }
...
class BarrierStatement implements SqlStatement {
  ...
  bind(...values: unknown[]): SqlStatement {
    return new BarrierStatement(this.target.bind(...values), this.wait);
  }
  async first<T = Record<string, unknown>>(): Promise<T | null> {
    const result = await this.target.first<T>();
    await this.wait();
    return result;
  }
