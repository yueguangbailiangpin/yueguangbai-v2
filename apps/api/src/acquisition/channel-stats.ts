...
export interface AcquisitionChannelStatsDto {
  ...
  channel_status:'ACTIVE'|'DISABLED';lead_type:'BUYER'|'SELLER'|'BOTH';marketplace_code:string;
  consultation_count:number|null;consultation_data_complete:boolean;
  consultation_days_recorded:number;consultation_days_expected:number;
  prospect_count:number;codex_prospect_count:number;lead_count:number;registered_count:number;
  reservation_submitted_count:number;cooperation_count:number;formal_order_count:number;
  buyer_formal_order_count:number;seller_formal_order_count:number;
...
export async function readAcquisitionChannelStats(
  ...
  for(const channel of channels){
    const meta=await database.prepare(`SELECT created_at,disabled_at,status FROM acquisition_channels WHERE id=?`)
      .bind(channel.channel_id).first<{created_at:number;disabled_at:number|null;status:'ACTIVE'|'DISABLED'}>();
    if(!meta)continue;
    const expected=expectedConsultationDays(from,to,Number(meta.created_at),meta.disabled_at===null?null:Number(meta.disabled_at));
    const consultation=await database.prepare(`SELECT COALESCE(SUM(person_count),0) AS total,COUNT(*) AS days
      FROM acquisition_daily_consultations WHERE channel_id=? AND business_date BETWEEN ? AND ?`)
      .bind(channel.channel_id,from,to).first<{total:number;days:number}>();
    ...
    result.push({
      ...
      consultation_data_complete:consultationComplete,consultation_days_recorded:recorded,consultation_days_expected:expected,
      prospect_count:Number(prospect?.total??0),codex_prospect_count:Number(prospect?.codex??0),lead_count:Number(lead?.total??0),
      registered_count:Number(lead?.registered??0),reservation_submitted_count:Number(lead?.reserved??0),cooperation_count:Number(lead?.cooperation??0),
      formal_order_count:allOrderIds.size,buyer_formal_order_count:new Set(buyerOrders.map((row)=>row.formal_order_id)).size,
      seller_formal_order_count:new Set(sellerOrders.map((row)=>row.formal_order_id)).size,
      buyer_projected_gross_profit_cny_fen:buyerProfit.projected,buyer_completed_gross_profit_cny_fen:buyerProfit.completed,
      seller_projected_gross_profit_cny_fen:sellerProfit.projected,seller_completed_gross_profit_cny_fen:sellerProfit.completed,
    });
  }
  return Object.freeze(result);
}

async function ordersForChannel(database:SqlDatabase,type:'BUYER'|'SELLER',channelId:string,from:string,to:string):Promise<OrderFinanceRow[]>{
  ...
  const rows=await database.prepare(`SELECT DISTINCT finance.formal_order_id,
