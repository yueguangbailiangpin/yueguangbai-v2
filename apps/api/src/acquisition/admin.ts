...
export async function createAcquisitionChannel(
  ...
  input: {
    code: string;
    platformName: string;
    leadType: AcquisitionChannelAudience;
  ...
  const platformName = text(input.platformName, 100);
  ...
  try {
    await database.batch([
      ...
      database
        .prepare(
          `INSERT INTO acquisition_channel_events(id,channel_id,event_type,previous_version,next_version,actor_staff_id,idempotency_key,request_hash,reason,created_at)
      ...
      audit(
        ...
        'ACQUISITION_CHANNEL',
        id,
        'ACQUISITION_CHANNEL_CREATED',
        null,
        channel,
        null,
        acquired.now,
      ),
      ...finishAcquisitionCommand(database, acquired.acquired.claim, { channel }, acquired.now, {
        channel_id: id,
      }),
      assertion(
        database,
        `SELECT 1 FROM acquisition_channels WHERE id=? AND status='ACTIVE' AND version=1`,
        [id],
      ),
    ]);
...
export async function disableAcquisitionChannel(
  ...
  if (existing.version !== expected) throw new AcquisitionError('VERSION_CONFLICT', 409);
  const acquired = await acquireAcquisitionCommand<{ channel: AcquisitionChannelDto }>(
    database,
    command,
    'DISABLE_ACQUISITION_CHANNEL',
    'ACQUISITION_CHANNEL',
    channelId,
  ...
  try {
    await database.batch([
      database
        .prepare(
          `UPDATE acquisition_channels SET status='DISABLED',version=version+1,updated_at=?,disabled_at=? WHERE id=? AND status='ACTIVE' AND version=?`,
        )
        .bind(acquired.now, acquired.now, channelId, expected),
      database
        .prepare(
          `INSERT INTO acquisition_channel_events(id,channel_id,event_type,previous_version,next_version,actor_staff_id,idempotency_key,request_hash,reason,created_at)
      VALUES(?,?,'DISABLED',?,?,?,?,?,?,?)`,
...
export async function createAcquisitionAssignment(
  ...
  const staffId = identifier(input.staffId),
    ...
    effectiveFrom = epoch(input.effectiveFrom),
    effectiveUntil = input.effectiveUntil === null ? null : epoch(input.effectiveUntil);
  if (effectiveUntil !== null && effectiveUntil <= effectiveFrom) validation();
  const channel = await readChannel(database, channelId);
  if (!channel || channel.status !== 'ACTIVE') throw new AcquisitionError('NOT_FOUND', 404);
  const acquired = await acquireAcquisitionCommand<{ assignment: AcquisitionChannelAssignmentDto }>(
    database,
    command,
    'CREATE_ACQUISITION_CHANNEL_ASSIGNMENT',
    'STAFF_ACQUISITION_ASSIGNMENT',
    `${staffId}:${input.leadType}:${effectiveFrom}`,
    {
      staff_id: staffId,
      lead_type: input.leadType,
      channel_id: channelId,
  ...
  try {
    await database.batch([
      database
        .prepare(
          `INSERT INTO acquisition_staff_channel_assignments(id,staff_id,lead_type,channel_id,effective_from,effective_until,status,version,created_by_staff_id,created_at,updated_at,revoked_at,revoke_reason)
      ...
      ...finishAcquisitionCommand(database, acquired.acquired.claim, { assignment }, acquired.now, {
        assignment_id: id,
      }),
    ]);
  } catch (error) {
...
export async function revokeAcquisitionAssignment(
  ...
  requireAcquisitionAdmin(command.actor);
  const id = identifier(input.assignmentId),
    expected = version(input.expectedVersion),
    reason = text(input.reason, 1000);
  const existing = await readAssignment(database, id);
  if (!existing) throw new AcquisitionError('NOT_FOUND', 404);
  const acquired = await acquireAcquisitionCommand<{ assignment: AcquisitionChannelAssignmentDto }>(
    database,
    command,
    'REVOKE_ACQUISITION_CHANNEL_ASSIGNMENT',
    'STAFF_ACQUISITION_ASSIGNMENT',
    id,
    { expected_version: expected, reason },
  );
  if (acquired.acquired.kind === 'REPLAY') return { ...acquired.acquired.response, replayed: true };
  try {
    if (existing.status !== 'ACTIVE' || existing.version !== expected)
      throw new AcquisitionError('VERSION_CONFLICT', 409);
    const assignment = {
    ...
    const outbox = await prepareOutboxEvent({
      ...
      dedupKey: `acquisition-assignment-revoked:${id}`,
      eventType: 'ACQUISITION_ASSIGNMENT_REVOKED',
      aggregateType: 'STAFF_ACQUISITION_ASSIGNMENT',
      aggregateId: id,
      payload: {
        assignment_id: id,
    ...
    await database.batch([
      database
        .prepare(
          `UPDATE acquisition_staff_channel_assignments SET status='REVOKED',version=version+1,updated_at=?,revoked_at=?,revoke_reason=? WHERE id=? AND status='ACTIVE' AND version=?`,
        )
        .bind(acquired.now, acquired.now, reason, id, expected),
      assertPreviousStatementChangedOnce(database),
      database
        .prepare(
          `INSERT INTO acquisition_assignment_events(id,assignment_id,event_type,actor_staff_id,idempotency_key,request_hash,reason,created_at) VALUES(?,?,'REVOKED',?,?,?,?,?)`,
        )
        .bind(
          crypto.randomUUID(),
          id,
      ...
      audit(
        ...
        'STAFF_ACQUISITION_ASSIGNMENT',
        id,
        'ACQUISITION_ASSIGNMENT_REVOKED',
        toAssignment(existing),
        assignment,
        reason,
        acquired.now,
      ...
      assertion(
        database,
        `SELECT 1 FROM acquisition_staff_channel_assignments WHERE id=? AND status='REVOKED' AND version=? AND revoked_at=? AND revoke_reason=?`,
        [id, expected + 1, acquired.now, reason],
      ),
    ]);
    return { assignment, replayed: false };
  } catch (error) {
    ...
    if (String(error).includes('transaction_assertion_failed')) {
      const latest = await database
        .prepare(`SELECT version FROM acquisition_staff_channel_assignments WHERE id=?`)
        .bind(id)
...
export async function recordAcquisitionConsultation(
  ...
  if (!Number.isSafeInteger(expected) || expected < 0) validation();
  const reason = text(input.reason, 1000);
  const acquired = await acquireAcquisitionCommand<{
    consultation: AcquisitionDailyConsultationDto;
  }>(
    database,
    command,
    'RECORD_ACQUISITION_CONSULTATION',
    'ACQUISITION_DAILY_CONSULTATION',
    `${channelId}:${businessDate}`,
    {
      channel_id: channelId,
  ...
  try {
    const existing = await database
      .prepare(
        `SELECT id,channel_id,lead_type,business_date,person_count,version,updated_by_staff_id,created_at,updated_at FROM acquisition_daily_consultations WHERE channel_id=? AND business_date=?`,
      )
      .bind(channelId, businessDate)
      .first<ConsultationRow>();
    if ((!existing && expected !== 0) || (existing && Number(existing.version) !== expected))
      throw new AcquisitionError('VERSION_CONFLICT', 409);
    ...
    const consultation: AcquisitionDailyConsultationDto = {
      ...
      person_count: personCount,
      version: next,
      updated_by_staff_id: command.actor.staffId,
      updated_at: acquired.now,
    };
    const mutation: SqlStatement = existing
      ? database
          .prepare(
            `UPDATE acquisition_daily_consultations SET person_count=?,version=version+1,updated_by_staff_id=?,updated_at=? WHERE id=? AND version=?`,
          )
          .bind(personCount, command.actor.staffId, acquired.now, id, expected)
      : database
          .prepare(
            `INSERT INTO acquisition_daily_consultations(id,channel_id,lead_type,business_date,person_count,version,updated_by_staff_id,created_at,updated_at) VALUES(?,?,?,?,?,1,?,?,?)`,
          )
          .bind(
            id,
            channelId,
    ...
    await database.batch([
      ...
      database
        .prepare(
          `INSERT INTO acquisition_daily_consultation_events(id,consultation_id,event_type,previous_count,next_count,previous_version,next_version,actor_staff_id,idempotency_key,request_hash,reason,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
        )
        .bind(
          crypto.randomUUID(),
          id,
          existing ? 'CORRECTED' : 'RECORDED',
          existing?.person_count ?? null,
          personCount,
      ...
      audit(
        ...
        existing ? toConsultation(existing) : null,
        consultation,
        reason,
        acquired.now,
      ),
      ...
      assertion(
        database,
        `SELECT 1 FROM acquisition_daily_consultations WHERE id=? AND version=? AND person_count=?`,
        [id, next, personCount],
      ),
    ]);
...
export async function listAcquisitionChannels(
  database: SqlDatabase,
  actor: AssignmentStaffAuthorization,
): Promise<AcquisitionChannelDto[]> {
  ...
  const audience = actor.roles.has('pre_sales')
    ? 'BUYER'
    : actor.roles.has('seller_ops')
      ? 'SELLER'
      : null;
  const clauses = [`1=1`];
  const bindings: unknown[] = [];
  if (markets.length) {
    clauses.push(`marketplace_code IN (${markets.map(() => '?').join(',')})`);
  ...
  const rows = await database
    .prepare(
      `SELECT id,code,channel_type,platform_name,lead_type,marketplace_code,display_name,status,version,created_at,updated_at FROM acquisition_channels WHERE ${clauses.join(' AND ')} ORDER BY status,display_name,id`,
    )
    .bind(...bindings)
    .all<ChannelRow>();
...
export async function listAcquisitionConsultations(
  ...
  try {
    from = parseChinaBusinessDate(fromDate);
    to = parseChinaBusinessDate(toDate);
  } catch {
    validation();
  }
...
export async function listAcquisitionConsultationHistory(
  ...
  const markets = actor.roles.has('owner')
    ? []
    : await resolveStaffMarketplaceCodes(database, actor);
  if (!actor.roles.has('owner') && markets.length === 0)
  ...
  const exists = await database
    .prepare(
      `SELECT 1 AS present FROM acquisition_daily_consultations consultation JOIN acquisition_channels channel ON channel.id=consultation.channel_id WHERE consultation.id=? ${marketClause}`,
    )
    .bind(id, ...markets)
    .first();
  ...
  const rows = await database
    .prepare(
      `SELECT id,event_type,previous_count,next_count,previous_version,next_version,actor_staff_id,reason,created_at FROM acquisition_daily_consultation_events WHERE consultation_id=? ORDER BY created_at,id`,
    )
    .bind(id)
    .all<{
      id: string;
  ...
  return rows.results.map((row) => ({
    ...
    next_version: Number(row.next_version),
...
async function requireMarketplace(database: SqlDatabase, code: string): Promise<void> {
  const row = await database
    .prepare(`SELECT 1 AS present FROM marketplace_registry WHERE code=?`)
    .bind(code)
    .first();
  if (!row) validation();
}
async function readChannel(database: SqlDatabase, id: string) {
  return database
    .prepare(
      `SELECT id,code,channel_type,platform_name,lead_type,marketplace_code,display_name,status,version,created_at,updated_at FROM acquisition_channels WHERE id=?`,
    )
    .bind(id)
    .first<ChannelRow>();
}
async function readAssignment(database: SqlDatabase, id: string) {
  return database
    .prepare(
      `SELECT assignment.id,assignment.staff_id,assignment.lead_type,assignment.channel_id,channel.display_name AS channel_name,assignment.effective_from,assignment.effective_until,assignment.status,assignment.version FROM acquisition_staff_channel_assignments assignment JOIN acquisition_channels channel ON channel.id=assignment.channel_id WHERE assignment.id=?`,
    )
    .bind(id)
    .first<AssignmentRow>();
}
...
function legacyType(platform: string): AcquisitionChannelType {
  const value = platform.toLocaleLowerCase('zh-CN');
  if (value.includes('小红书')) return 'XIAOHONGSHU';
  if (value.includes('微信')) return 'PRIVATE_WECHAT';
  if (value.includes('转介绍')) return 'REFERRAL';
  return 'OTHER';
}
function audit(
  database: SqlDatabase,
...
function assertion(database: SqlDatabase, query: string, bindings: unknown[]) {
  return database
    .prepare(
      `INSERT INTO transaction_assertions(assertion_value) SELECT CASE WHEN EXISTS(${query}) THEN 1 ELSE 0 END`,
    )
    .bind(...bindings);
}
function assertPreviousStatementChangedOnce(database: SqlDatabase) {
...
export function consultationDayStart(value: string): number {
