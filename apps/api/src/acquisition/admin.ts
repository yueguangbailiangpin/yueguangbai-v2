...
export async function recordAcquisitionConsultation(
  ...
  requireAcquisitionAdmin(command.actor);const channelId=identifier(input.channelId);const channel=await readChannel(database,channelId);if(!channel||channel.status!=='ACTIVE')throw new AcquisitionError('NOT_FOUND',404);
  ...
  try {
    ...
    const id=existing?.id??crypto.randomUUID();const next=expected+1;
    ...
    await database.batch([
      mutation,
      assertPreviousStatementChangedOnce(database),
      database.prepare(`INSERT INTO acquisition_daily_consultation_events(id,consultation_id,event_type,previous_count,next_count,previous_version,next_version,actor_staff_id,idempotency_key,request_hash,reason,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),id,existing?'CORRECTED':'RECORDED',existing?.person_count??null,personCount,existing?.version??null,next,command.actor.staffId,command.idempotencyKey,acquired.requestHash,reason,acquired.now),
      audit(database,command,'ACQUISITION_DAILY_CONSULTATION',id,existing?'ACQUISITION_CONSULTATION_CORRECTED':'ACQUISITION_CONSULTATION_RECORDED',existing?toConsultation(existing):null,consultation,reason,acquired.now),
      ...finishAcquisitionCommand(database,acquired.acquired.claim,{consultation},acquired.now,{consultation_id:id}),
      assertion(database,`SELECT 1 FROM acquisition_daily_consultations WHERE id=? AND version=? AND person_count=?`,[id,next,personCount]),
    ]);
...
export async function listAcquisitionConsultationHistory(database:SqlDatabase,actor:AssignmentStaffAuthorization,consultationId:string):Promise<AcquisitionConsultationEventDto[]>{
  requireAcquisitionOperator(actor);const id=identifier(consultationId);const markets=actor.roles.has('owner')?[]:await resolveStaffMarketplaceCodes(database,actor);
  if(!actor.roles.has('owner')&&markets.length===0)throw new AcquisitionError('NOT_FOUND',404);
  const marketClause=markets.length?`AND channel.marketplace_code IN (${markets.map(()=>'?').join(',')})`:'';
  const exists=await database.prepare(`SELECT 1 AS present FROM acquisition_daily_consultations consultation JOIN acquisition_channels channel ON channel.id=consultation.channel_id WHERE consultation.id=? ${marketClause}`).bind(id,...markets).first();
  if(!exists)throw new AcquisitionError('NOT_FOUND',404);
  const rows=await database.prepare(`SELECT id,event_type,previous_count,next_count,previous_version,next_version,actor_staff_id,reason,created_at FROM acquisition_daily_consultation_events WHERE consultation_id=? ORDER BY created_at,id`).bind(id).all<{id:string;event_type:'RECORDED'|'CORRECTED';previous_count:number|null;next_count:number;previous_version:number|null;next_version:number;actor_staff_id:string;reason:string;created_at:number}>();
  return rows.results.map((row)=>({event_id:row.id,event_type:row.event_type,previous_count:row.previous_count===null?null:Number(row.previous_count),next_count:Number(row.next_count),previous_version:row.previous_version===null?null:Number(row.previous_version),next_version:Number(row.next_version),actor_staff_id:row.actor_staff_id,reason:row.reason,created_at:Number(row.created_at)}));
}
...
function assertPreviousStatementChangedOnce(database:SqlDatabase){return database.prepare(`INSERT INTO transaction_assertions(assertion_value) SELECT CASE WHEN changes()=1 THEN 1 ELSE 0 END`);}
