...
export async function readAcquisitionFunnel(
  ...
  if(from>to)validation();const types=visibleTypes(actor);
...
function visibleTypes(actor:AssignmentStaffAuthorization):AcquisitionLeadType[]{
  if(actor.roles.has('owner')||actor.roles.has('acquisition'))return['BUYER','SELLER'];
  if(actor.roles.has('pre_sales'))return['BUYER'];
  if(actor.roles.has('seller_ops'))return['SELLER'];
  validation();
}
...
async function counts(database:SqlDatabase,type:'BUYER'|'SELLER',from:string,to:string,markets:readonly string[]):Promise<CountRow>{
  const marketSql=markets.length?`AND lead.marketplace_code IN (${markets.map(()=>'?').join(',')})`:'';
  const row=await database.prepare(`SELECT
    COUNT(*) AS added,
    SUM(CASE WHEN EXISTS(SELECT 1 FROM acquisition_lead_links link WHERE link.lead_id=lead.id AND link.link_type='BUYER_CUSTOMER') THEN 1 ELSE 0 END) AS registered,
    SUM(CASE WHEN EXISTS(SELECT 1 FROM acquisition_lead_links link WHERE link.lead_id=lead.id AND link.link_type='RESERVATION') THEN 1 ELSE 0 END) AS reserved,
    SUM(CASE WHEN lead.lead_type='BUYER' AND NOT EXISTS(SELECT 1 FROM acquisition_lead_links link WHERE link.lead_id=lead.id AND link.link_type='RESERVATION') THEN 1 ELSE 0 END) AS no_participation,
    SUM((SELECT COUNT(*) FROM acquisition_lead_links link WHERE link.lead_id=lead.id AND link.link_type='FORMAL_ORDER')) AS formal_orders,
    SUM(CASE WHEN EXISTS(SELECT 1 FROM acquisition_lead_links link WHERE link.lead_id=lead.id AND link.link_type='SELLER_ORGANIZATION') THEN 1 ELSE 0 END) AS cooperation
    FROM acquisition_leads lead WHERE lead.status='ACTIVE' AND lead.lead_type=? AND lead.created_business_date BETWEEN ? AND ? ${marketSql}`)
...
async function buyerProfit(database:SqlDatabase,from:string,to:string):Promise<{projected:string;completed:string}>{
  const rows=await database.prepare(`SELECT DISTINCT finance.formal_order_id,finance.projected_gross_profit_cny_fen,finance.completed_gross_profit_cny_fen
    FROM acquisition_leads lead JOIN acquisition_lead_links link ON link.lead_id=lead.id AND link.link_type='FORMAL_ORDER'
    JOIN internal_order_finance_positions finance ON finance.formal_order_id=link.target_id
    WHERE lead.status='ACTIVE' AND lead.lead_type='BUYER' AND lead.created_business_date BETWEEN ? AND ?`).bind(from,to).all<{projected_gross_profit_cny_fen:string|null;completed_gross_profit_cny_fen:string|null}>();
