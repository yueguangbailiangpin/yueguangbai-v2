...
export async function readAcquisitionFunnel(
  ...
  const types = visibleTypes(actor),
    markets = actor.roles.has('owner') ? [] : await resolveStaffMarketplaceCodes(database, actor);
  const buyer = types.includes('BUYER') ? await counts(database, 'BUYER', from, to, markets) : null;
  const seller = types.includes('SELLER')
    ? await counts(database, 'SELLER', from, to, markets)
    : null;
  const buyerConsultationCount = types.includes('BUYER')
    ? await consultations(database, 'BUYER', from, to, markets)
    : 0;
  const sellerConsultationCount = types.includes('SELLER')
    ? await consultations(database, 'SELLER', from, to, markets)
    : 0;
  let projected: string | null = null,
    completed: string | null = null;
  if (buyer && actor.roles.has('owner') && actor.permissions.has('FINANCIAL_VIEW')) {
...
async function counts(
  ...
  const marketSql = markets.length
    ? `AND fact.marketplace_code IN (${markets.map(() => '?').join(',')})`
    : '';
...
async function consultations(
  ...
  const row = await database
    .prepare(
      `SELECT COALESCE(SUM(consultation.person_count),0) AS count
...
async function buyerProfit(
  ...
  const rows = await database
    .prepare(
      `SELECT DISTINCT finance.formal_order_id,finance.projected_gross_profit_cny_fen,finance.completed_gross_profit_cny_fen
    FROM acquisition_customer_intake_facts fact
    ...
    WHERE fact.lead_type='BUYER' AND fact.business_date BETWEEN ? AND ?`,
    )
    .bind(from, to)
    .all<{
