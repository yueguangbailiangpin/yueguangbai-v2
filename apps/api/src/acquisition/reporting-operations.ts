...
export async function correctLeadSource(
  ...
  try{
    ...
    const correctionId=crypto.randomUUID();const correction=Object.freeze({correction_id:correctionId,lead_id:leadId,previous_channel_id:currentState.channelId,new_channel_id:newChannelId,new_channel_name:String(channel.display_name),reason,corrected_at:acquired.now,correction_sequence:expected+1});
    const outbox=await prepareOutboxEvent({id:crypto.randomUUID(),dedupKey:`acquisition-source-corrected:${correctionId}`,eventType:'ACQUISITION_SOURCE_CORRECTED',aggregateType:'ACQUISITION_LEAD',aggregateId:leadId,payload:{...correction},createdAt:acquired.now});
    await database.batch([
      database.prepare(`INSERT INTO acquisition_lead_source_corrections(id,lead_id,previous_channel_id,new_channel_id,reason,corrected_by_staff_id,corrected_at)
        SELECT ?,?,?,?,?,?,? WHERE
          ...
          AND COALESCE((SELECT new_channel_id FROM acquisition_lead_source_corrections WHERE lead_id=? ORDER BY corrected_at DESC,id DESC LIMIT 1),?)=?`).bind(correctionId,leadId,currentState.channelId,newChannelId,reason,command.actor.staffId,acquired.now,leadId,expected,leadId,String(lead.original_channel_id),currentState.channelId),
      database.prepare(`INSERT INTO transaction_assertions(assertion_value) SELECT CASE WHEN changes()=1 THEN 1 ELSE 0 END`),
      createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'ACQUISITION_LEAD',aggregateId:leadId,eventType:'ACQUISITION_SOURCE_CORRECTED',actor:{type:'STAFF',id:command.actor.staffId,roles:[...command.actor.roles]},requestId:command.requestId,idempotencyKey:command.idempotencyKey,previousState:{channel_id:currentState.channelId,correction_sequence:expected},nextState:{channel_id:newChannelId,channel_name:String(channel.display_name),correction_sequence:expected+1},reason,createdAt:acquired.now}),
      ...createOutboxStatements(database,outbox),
      ...finishAcquisitionCommand(database,acquired.acquired.claim,{correction},acquired.now,{lead_id:leadId,correction_id:correctionId}),
      database.prepare(`INSERT INTO transaction_assertions(assertion_value) SELECT CASE WHEN
        ...
        THEN 1 ELSE 0 END`).bind(leadId,expected+1,leadId,correctionId),
    ]);
