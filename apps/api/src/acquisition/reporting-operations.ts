...
export async function readReportingPrecisionConfig(database:SqlDatabase,actor:AssignmentStaffAuthorization){
  ...
  return Object.freeze({precision_started_business_date:row.precision_started_business_date as string|null,
    activated_at:row.activated_at===null?null:Number(row.activated_at),activated_by_staff_id:row.activated_by_staff_id as string|null,
    version:Number(row.version),updated_at:Number(row.updated_at)});
}

export async function activateReportingPrecisionBoundary(
  database:SqlDatabase,actor:AssignmentStaffAuthorization,input:{businessDate:string;expectedVersion:number},
){
  requireOwner(actor);let businessDate:string;
  ...
  if(current.precision_started_business_date!==null){if(current.precision_started_business_date===businessDate)return current;throw new AcquisitionError('STATE_CONFLICT',409);}
  if(current.version!==input.expectedVersion)throw new AcquisitionError('VERSION_CONFLICT',409);
  const now=Date.now();
  await database.batch([
    database.prepare(`UPDATE acquisition_reporting_config SET precision_started_business_date=?,activated_at=?,activated_by_staff_id=?,version=version+1,updated_at=?
      WHERE singleton_id=1 AND version=? AND precision_started_business_date IS NULL`).bind(businessDate,now,actor.staffId,now,input.expectedVersion),
    ...
    createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'ACQUISITION_REPORTING',aggregateId:'precision-boundary',
      eventType:'ACQUISITION_REPORTING_PRECISION_ACTIVATED',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:null,idempotencyKey:null,
      previousState:{precision_started_business_date:null,version:current.version},nextState:{precision_started_business_date:businessDate,version:current.version+1},createdAt:now}),
...
export async function listSourceCorrectionCandidates(database:SqlDatabase,actor:AssignmentStaffAuthorization,limit=100){
  requireAcquisitionOperator(actor);if(!Number.isSafeInteger(limit)||limit<1||limit>200)throw new AcquisitionError('VALIDATION_ERROR',400);
  const markets=actor.roles.has('owner')?[]:await resolveStaffMarketplaceCodes(database,actor);if(!actor.roles.has('owner')&&markets.length===0)return Object.freeze([]);
  ...
  const rows=await database.prepare(`SELECT fact.lead_id,lead.lead_type,fact.marketplace_code,fact.business_date,
    ...
    ORDER BY fact.recorded_at DESC,fact.lead_id DESC LIMIT ?`).bind(...markets,limit).all<any>();
...
export async function correctLeadSource(
  database:SqlDatabase,actor:AssignmentStaffAuthorization,input:{leadId:string;newChannelId:string;reason:string},
){
  requireAcquisitionOperator(actor);const reason=input.reason.normalize('NFKC').trim();if(reason.length<3||reason.length>1000)throw new AcquisitionError('VALIDATION_ERROR',400);
  ...
  if(!lead)throw new AcquisitionError('NOT_FOUND',404);await requireMarket(database,actor,String(lead.marketplace_code));
  ...
  if(!channel||String(channel.marketplace_code)!==String(lead.marketplace_code)||!(channel.lead_type===lead.lead_type||channel.lead_type==='BOTH'))throw new AcquisitionError('VALIDATION_ERROR',400);
  const current=await effectiveChannel(database,String(lead.id),String(lead.original_channel_id));if(current===String(channel.id))throw new AcquisitionError('STATE_CONFLICT',409);
  const now=Date.now(),correctionId=crypto.randomUUID();
  await database.batch([
    database.prepare(`INSERT INTO acquisition_lead_source_corrections(id,lead_id,previous_channel_id,new_channel_id,reason,corrected_by_staff_id,corrected_at)
      VALUES(?,?,?,?,?,?,?)`).bind(correctionId,lead.id,current,channel.id,reason,actor.staffId,now),
    createAuditEventStatement(database,{id:crypto.randomUUID(),aggregateType:'ACQUISITION_LEAD',aggregateId:String(lead.id),
      eventType:'ACQUISITION_SOURCE_CORRECTED',actor:{type:'STAFF',id:actor.staffId,roles:[...actor.roles]},requestId:null,idempotencyKey:null,
      previousState:{channel_id:current},nextState:{channel_id:String(channel.id),channel_name:String(channel.display_name)},reason,createdAt:now}),
  ]);
  return Object.freeze({correction_id:correctionId,lead_id:String(lead.id),previous_channel_id:current,new_channel_id:String(channel.id),new_channel_name:String(channel.display_name),reason,corrected_at:now});
}
async function effectiveChannel(database:SqlDatabase,leadId:string,original:string):Promise<string>{const row=await database.prepare(`SELECT new_channel_id FROM acquisition_lead_source_corrections WHERE lead_id=? ORDER BY corrected_at DESC,id DESC LIMIT 1`).bind(leadId).first<{new_channel_id:string}>();return row?.new_channel_id??original;}
