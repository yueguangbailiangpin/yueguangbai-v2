...
import { addMachineProspectSignal,createMachineProspect,updateMachineProspectAnalysis } from './prospects';
...
export function registerAcquisitionMachineRoutes(app:Hono<any>):void{
  app.post('/api/acquisition-machine/prospects',wrap(async(context)=>{
    ...
    const command=machineCommand(context,machine),body=await bodyObject(context.req.raw);
  ...
  app.post('/api/acquisition-machine/prospects/:id/signals',wrap(async(context)=>{
    ...
    const result=await addMachineProspectSignal(context.env.DB,prospectId,{signalType:body['signal_type'],signalContent:body['signal_content'],sourceUrl:body['source_url'],confidence:body['confidence'] as 'LOW'|'MEDIUM'|'HIGH'|'CONFIRMED'},command);
    return context.json(apiSuccess(result,requestId(context)),201,{'Cache-Control':'no-store'});
  }));
  ...
  app.post('/api/acquisition-machine/prospects/:id/analysis',wrap(async(context)=>{
    ...
    const result=await updateMachineProspectAnalysis(context.env.DB,id,{expectedVersion:Number(body['expected_version']),status:body['status'] as 'NEW'|'RESEARCHING'|'QUALIFIED'|'READY_CONTACT',aiScore:body['ai_score']===null?null:Number(body['ai_score']),note:body['note']},command);
    return context.json(apiSuccess(result,requestId(context)),200,{'Cache-Control':'no-store'});
  }));
}

function machineCommand(context:any,machine:AcquisitionMachineIdentity):AcquisitionMachineCommandContext{let key;try{key=parseIdempotencyKey(context.req.header('Idempotency-Key'));}catch{validation();}if(!key)validation();return{machineId:machine.machineId,marketplaceCodes:machine.marketplaceCodes,channelIds:machine.channelIds,idempotencyKey:key,requestId:requestId(context)};}
async function bodyObject(request:Request){const value=await readBoundedJson(request,BODY_LIMIT);if(!value)validation();return value;}
