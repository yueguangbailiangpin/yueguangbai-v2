...
export function registerAcquisitionHandoffRoutes(app: Hono<AppEnv>): void {
  app.get('/api/staff/acquisition/handoffs', async (context) => {
    ...
    try {
      ...
      if ([...parameters.keys()].some((key) => key !== 'lead_type'))
        throw new AcquisitionError('VALIDATION_ERROR', 400);
      const leadType = context.req.query('lead_type');
      if (!isAcquisitionLeadType(leadType)) throw new AcquisitionError('VALIDATION_ERROR', 400);
      return context.json(
        apiSuccess(
          { items: await listAcquisitionHandoffs(context.env.DB, actor, leadType) },
          requestId,
        ),
      );
    } catch (error) {
