...
interface ServiceFeeFacts{seller_organization_id:string;financial_snapshot_id:string;service_fee_cny_fen:number}
...
export async function prepareBuyerRefundObligationFromReviewApproval(
  ...
  const payable=await prepareSellerPayableCreation(database,{
    sellerOrganizationId:facts.seller_organization_id,formalOrderId:input.formalOrderId,payableType:'SELLER_SERVICE_FEE',amountCnyFen:Number(facts.service_fee_cny_fen),
    financialSnapshotId:facts.financial_snapshot_id,sourceType:'REVIEW_APPROVAL',sourceId:input.reviewCaseId,dueAt:input.now,createdAt:input.now,
    actor:{type:'STAFF',id:input.actorStaffId,roles:input.actorRoles},requestId:input.requestId,idempotencyKey:input.idempotencyKey,
  });
  return Object.freeze({...base,sellerServiceFeePayableId:payable.payableId,statements:Object.freeze([...base.statements,...payable.statements])});
}
...
export async function prepareBuyerRefundObligationStatements(
  ...
  const response:EnsureBuyerRefundObligationResult={
    obligation_id:obligationId,source_review_event_id:input.sourceReviewEventId,review_case_id:input.reviewCaseId,formal_order_id:input.formalOrderId,buyer_customer_id:input.buyerCustomerId,
    due_amount_cny_fen:String(input.dueAmountCnyFen),gross_paid_cny_fen:String(paid),reversed_cny_fen:'0',net_paid_cny_fen:String(paid),status,version:1 as const,replayed:false,
  };
  const outbox=await prepareOutboxEvent({id:crypto.randomUUID(),dedupKey:`buyer-refund-obligation:${input.sourceReviewEventId}`,eventType:'BUYER_REFUND_OBLIGATION_CREATED',aggregateType:'BUYER_REFUND_OBLIGATION',aggregateId:obligationId,payload:response,createdAt:input.now});
  return{obligationId,eventId,auditId,result:response,statements:[
    database.prepare(`INSERT INTO buyer_refund_obligations(id,source_review_event_id,review_case_id,formal_order_id,buyer_customer_id,due_amount_cny_fen,version,created_at,updated_at) VALUES(?,?,?,?,?,?,1,?,?)`).bind(obligationId,input.sourceReviewEventId,input.reviewCaseId,input.formalOrderId,input.buyerCustomerId,input.dueAmountCnyFen,input.now,input.now),
    insertBuyerRefundEventStatement(database,{eventId,obligationId,eventType:'BUYER_REFUND_OBLIGATION_CREATED',actorType:input.actorType,actorId:input.actorId,obligationVersion:1,amountCnyFen:input.dueAmountCnyFen,netPaidAfterCnyFen:0,metadata:{source_review_event_id:input.sourceReviewEventId,source:'BUYER_REFUND_BECAME_DUE.amount_cny_fen',advance_principal_settlement_count:advance.settlementCount,advance_principal_overpayment_count:advance.overpaymentCount,advance_principal_overpayment_cny_fen:advance.overpaymentCnyFen},idempotencyKey:input.idempotencyKey,createdAt:input.now}),
    ...advance.statements,
    createAuditEventStatement(database,{id:auditId,aggregateType:'BUYER_REFUND_OBLIGATION',aggregateId:obligationId,eventType:'BUYER_REFUND_OBLIGATION_CREATED',actor:{type:input.actorType,id:input.actorId,roles:input.actorRoles},requestId:input.requestId,idempotencyKey:input.idempotencyKey,previousState:null,nextState:response,metadata:{source_review_event_id:input.sourceReviewEventId,advance_principal_settled_cny_fen:String(paid),advance_principal_overpayment_cny_fen:String(advance.overpaymentCnyFen)},createdAt:input.now}),
...
async function requireServiceFeeFacts(database:SqlDatabase,reviewCaseId:string,formalOrderId:string):Promise<ServiceFeeFacts>{
  const row=await database.prepare(`SELECT review_case.seller_organization_id,snapshot.id AS financial_snapshot_id,snapshot.service_fee_cny_fen
    ...
    JOIN formal_order_financial_snapshots snapshot ON snapshot.formal_order_id=formal_order.id WHERE review_case.id=? AND formal_order.id=?`).bind(reviewCaseId,formalOrderId).first<ServiceFeeFacts>();
  if(!row||!Number.isSafeInteger(Number(row.service_fee_cny_fen))||Number(row.service_fee_cny_fen)<0)throw new Error('invalid_seller_service_fee_payable_facts');
