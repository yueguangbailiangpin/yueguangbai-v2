...
export async function allocateBuyerCustomerNumber(
  ...
  const acquired =
    await acquireIdempotency<AllocateBuyerNumberResult>(
      database,
      {
        actorType: 'STAFF',
  ...
  try {
    const source = await requireBuyerNumberSource(
      database,
      buyerCustomerId,
    );
    ...
    if (source.access_status !== 'ACTIVE') {
      throw new CustomerMasterDataError(
        'CUSTOMER_NOT_ACTIVE',
        409,
      );
    }
    ...
    const statements: SqlStatement[] = [
      ...
      assertBuyerNumberAllocatedStatement(
        database,
        acquired.claim,
        source,
        buyerNumber,
...
function existingResult(
  ...
  return {
    ...
    first_valid_order_business_date:
      source.first_valid_order_business_date,
    ...
    SELECT CASE WHEN
      ...
      AND EXISTS (
        ...
        WHERE id=?
          ...
          AND buyer_sequence=?
          AND first_valid_order_business_date=?
          AND version=?
      )
      AND EXISTS (
        SELECT 1
        FROM buyer_number_allocation_events
