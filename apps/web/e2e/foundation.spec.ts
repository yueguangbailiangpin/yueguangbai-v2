...
test('public entry and login routes are Chinese identity boundaries', async ({ page }) => { await page.goto('/'); await expect(page.getByRole('heading', { name: '月光白' })).toBeVisible(); await expect(page.getByText('员工入口')).toHaveCount(0); await page.goto('/buyer/login'); await expect(page.getByRole('heading', { name: '买家登录' })).toBeVisible(); await page.goto('/seller/login'); await expect(page.getByRole('heading', { name: '卖家登录' })).toBeVisible(); await page.goto('/staff/login'); await expect(page.getByRole('heading', { name: '员工登录' })).toBeVisible(); });
