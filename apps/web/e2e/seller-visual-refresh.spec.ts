...
const products = [
  {
    ...
    version: 5,
    created_at: fixedNow - 70_000_000,
    updated_at: fixedNow - 2_000_000,
    current_version: {
      id: 'version-rice',
      version_no: 2,
      product_name: '象印 IH 电饭煲 5.5 合',
      search_keywords: ['电饭煲', '象印'],
      ordering_guide_expected_amount_jpy: 22800,
      color_spec_mode: null,
      main_image: null,
      product_url: 'https://example.invalid/rice',
      buyer_visible_notes: '白色款',
      created_at: fixedNow - 60_000_000,
    },
  },
  {
    id: 'product-kettle',
    ...
    version: 2,
    created_at: fixedNow - 50_000_000,
    updated_at: fixedNow - 1_000_000,
    current_version: {
      id: 'version-kettle',
      version_no: 1,
...
const applications = [
  {
    ...
    product_id: null,
    version: 4,
    submitted_at: fixedNow - 7200_000,
    updated_at: fixedNow - 7200_000,
    reviewed_at: null,
    withdrawn_at: null,
  },
  {
    ...
    seller_notes: null,
    status: 'REJECTED',
    review_reason: '请补充清晰的商品主图。',
    product_id: null,
    version: 2,
    submitted_at: fixedNow - 172_800_000,
    updated_at: fixedNow - 86_400_000,
    reviewed_at: fixedNow - 86_400_000,
    withdrawn_at: null,
  },
] as const;
...
const demands = [
  {
    ...
    submitted_at: fixedNow - 172_800_000,
    updated_at: fixedNow - 3_600_000,
    reviewed_at: fixedNow - 90_000_000,
    published_at: fixedNow - 86_400_000,
    withdrawn_at: null,
    closed_at: null,
  },
  {
    ...
    seller_notes: null,
    open_at: fixedNow + 86_400_000,
    reservation_deadline: fixedNow + 4 * 86_400_000,
    order_deadline: fixedNow + 12 * 86_400_000,
    status: 'SUBMITTED',
    review_reason: null,
    close_reason: null,
    version: 1,
    submitted_at: fixedNow - 3_600_000,
    updated_at: fixedNow - 3_600_000,
    reviewed_at: null,
    published_at: null,
    withdrawn_at: null,
...
const orders = [
  ...
  },
  {
    ...
    seller_expected_principal_cny_fen: '33000',
    seller_principal_rate_snapshot: {
      platform_order_date: '2026-08-08',
      payment_amount_minor: '4599',
      ...
      final_rate_scale: '100000000',
      rounding_rule: 'HALF_UP',
      seller_expected_principal_amount_minor: '33000',
    },
    locked_service_fee_snapshot: {
      fee_version_id: 'fee-text',
      version_no: 3,
      review_type: 'TEXT',
    ...
    confirmed_at: fixedNow - 10_800_000,
...
const reviews = [
  {
    ...
    submitted_at: fixedNow - 20_000_000,
    approved_at: fixedNow - 18_000_000,
    evidence: {
      version_id: 'review-evidence-rice',
      version_no: 2,
      submitted_at: fixedNow - 20_000_000,
      files: [
        {
          file_entity_link_id: 'review-file-safe',
          file_version: 2,
          content_type: 'image/png',
          byte_size: 2048,
          created_at: fixedNow - 20_000_000,
        },
      ],
    },
    service_fee_accrued: { amount_cny_fen: '3200', accrued_at: fixedNow - 18_000_000 },
  ...
  },
  {
    ...
    version: 2,
    review_url: null,
    submitted_at: fixedNow - 6_000_000,
    approved_at: null,
    evidence: {
      version_id: 'review-evidence-kettle',
      version_no: 1,
      submitted_at: fixedNow - 6_000_000,
      files: [],
    },
...
function success(data: unknown) {
  return { data, meta: { request_id: 'seller-visual-refresh' } };
}
...
async function installSellerFixture(page: Page, access = me.access): Promise<void> {
  await page.route('**/api/**', async (route) => {
      ...
      await json(
        ...
        success({
          session: {
            ...
            identity_subject_id: 'seller-visual-subject',
    ...
    if (path === '/api/seller-portal/me') {
      await json(route, success({ me: { ...me, access } }));
      return;
    }
    if (path === '/api/seller-portal/stores') {
      await json(route, success({ items: stores, page: pageInfo }));
      return;
    }
    if (path === '/api/seller-portal/products') {
      await json(route, success({ items: products, page: pageInfo }));
      return;
    }
    if (path === '/api/seller-portal/product-applications') {
      await json(route, success({ items: applications, page: pageInfo }));
      return;
    }
    if (path === '/api/seller-portal/product-applications/application-serum') {
      await json(route, success({ application: applications[0] }));
      return;
    }
    if (path === '/api/seller-portal/demand-batches') {
      await json(route, success({ items: demands, page: pageInfo }));
      return;
    }
    if (path === '/api/seller-portal/formal-orders') {
      await json(route, success({ items: orders, page: pageInfo }));
      return;
    }
    if (path === '/api/seller-portal/reviews') {
      await json(route, success({ items: reviews, page: pageInfo }));
      return;
    }
    ...
    if (path === '/api/seller-portal/settlement/payables') {
      await json(route, success({ items: payables, page: pageInfo }));
      return;
    }
    await json(
      route,
      {
        ...
        meta: { request_id: 'seller-visual-not-found' },
      },
      404,
    );
  });
...
test('Seller visual refresh captures deterministic before and after matrix', async ({ page }) => {
  ...
  for (const viewport of extendedViewports) {
    await page.setViewportSize(viewport);
    for (const [name, path, heading] of surfaces.filter(([name]) =>
      ['dashboard', 'products', 'demand-form', 'orders', 'settlements'].includes(name),
    )) {
      await page.goto(path);
      await expect(page.getByRole('heading', { name: heading, exact: true })).toBeVisible();
      await noHorizontalOverflow(page);
...
test('Seller shell keeps context, navigation, entries, and disclosure boundaries', async ({
  page,
}) => {
  await installSellerFixture(page);
...
test('Seller pages preserve keyboard, zoom, reduced motion, targets, and overflow', async ({
  ...
  for (const viewport of [...extendedViewports, ...primaryViewports]) {
    ...
    await page.goto('/seller');
    await noHorizontalOverflow(page);
    const targets = await page.locator('a, button, select, input').evaluateAll((elements) =>
      elements
        .filter((element) => {
...
test('Seller route remains isolated on a cold load', async ({ page }) => {
  ...
  expect(
    [...scripts].some((name) =>
      /BuyerRouteModule|BuyerOrderRouteModule|BuyerAfterSalesRouteModule|StaffRouteModule/u.test(
        name,
      ),
    ),
  ).toBe(false);
