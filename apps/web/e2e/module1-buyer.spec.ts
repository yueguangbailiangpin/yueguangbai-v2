...
function failure(code: string, requestId = 'module1-browser-error') {
  return {
    error: { code, message: 'safe fixture', details: null },
...
const instructionState = (status: MockOptions['instructionStatus'] = 'ACTIVE') => ({
  ...
  can_submit_evidence: status === 'ACTIVE',
  can_read_images: status === 'ACTIVE' || status === 'COMPLETED',
  content_updated: status === 'COMPLETED',
});
...
const instruction = (invalid = false) => ({
  ...
  main_image: {
    ...
    width: 800,
    height: 800,
    read_intent_path: invalid
      ? '/api/staff/files/private/read-intent'
      : '/api/buyer-portal/reservations/reservation-1/order-instruction/images/main/read-intent',
  },
});

const evidenceFile = {
  file_object_id: 'evidence-file',
  ...
  file_entity_link_id: 'evidence-link',
  version: 3,
...
const review = (status: MockOptions['reviewStatus'] = 'CHANGES_REQUESTED') => ({
  ...
  review_approved_at: status === 'APPROVED' ? now - 1_000 : null,
  buyer_refund_due: status === 'APPROVED' ? { amount_cny_fen: '19734' } : null,
  file_count: 1,
  allowed_actions: status === 'CHANGES_REQUESTED' ? ['RESUBMIT', 'WITHDRAW'] : [],
});
const reviewDetail = (status: MockOptions['reviewStatus'] = 'CHANGES_REQUESTED') => ({
  ...review(status),
  files: [
    {
      file_object_id: 'review-file',
      file_entity_link_id: 'review-link',
      client_file_name: '评论截图.png',
      mime: 'image/png',
      byte_size: 3,
      status: 'VERIFIED',
      version: 3,
      verified_at: now - 2_400,
      allowed_actions: ['CREATE_READ_INTENT'],
    },
  ],
});
...
const refund = (status: MockOptions['refundStatus'] = 'PARTIALLY_PAID') => ({
  ...
  net_paid_cny_fen: status === 'OVERPAID' ? '21000' : '10000',
  remaining_amount_cny_fen: status === 'PARTIALLY_PAID' ? '9734' : '0',
  overpaid_amount_cny_fen: status === 'OVERPAID' ? '1266' : '0',
  status,
  order: {
    formal_order_id: 'formal-1',
    marketplace: 'AMAZON_JP',
...
const refundDetail = (status: MockOptions['refundStatus'] = 'PARTIALLY_PAID') => ({
  ...refund(status),
  activities: [
    {
      activity_id: 'pay-1',
...
async function installBuyerApi(page: Page, options: MockOptions = {}): Promise<void> {
  ...
  let activeUpload: {
    purpose: 'ORDER_EVIDENCE' | 'REVIEW_EVIDENCE';
    visibility: 'BUYER_VISIBLE' | 'SELLER_VISIBLE';
    count: number;
  } | null = null;
  await page.route('**/api/**', async (route) => {
    ...
    if (options.networkFailureOnce === path && !failedOnce.has(`network:${path}`)) {
      failedOnce.add(`network:${path}`);
      await route.abort('failed');
      return;
    }
    if (options.failureOnce === path && !failedOnce.has(`http:${path}`)) {
      failedOnce.add(`http:${path}`);
      await json(route, failure('DEPENDENCY_UNAVAILABLE', 'module1-retry-source'), 503);
      return;
    }
    ...
    if (path === '/api/customer-auth/session') {
      ...
      await json(route, success({ session: session(options.sessionType) }));
      return;
    }
    if (path === '/api/customer-auth/logout') {
      await json(route, success({ logged_out: true, all_devices_logged_out: false }));
      return;
    }
    ...
    if (path === '/api/buyer-auth/register') {
      ...
      if (status !== 201) {
        await json(
          route,
          failure(status === 429 ? 'RATE_LIMITED' : 'FEATURE_DISABLED'),
          status,
      ...
      await json(
        route,
        success({
          identity: { buyer_number: 'B-1001', wechat_id: 'buyer_wx' },
    ...
    if (path === '/api/buyer-portal/demands') {
      ...
      if (options.cursorPages && cursor === 'demand-cursor-2') {
        await json(
          route,
          success({
            items: [{ ...demand, demand_id: 'demand-2', product_name: '月白补充装' }],
            next_cursor: 'demand-cursor-3',
          }),
        );
        return;
      }
      if (options.cursorPages && cursor === 'demand-cursor-3') {
        await json(
          route,
          success({
            items: [{ ...demand, demand_id: 'demand-3', product_name: '月白旅行装' }],
            next_cursor: null,
          }),
        );
        return;
      ...
      await json(
        ...
        success({
          items: [demand],
          next_cursor: options.cursorPages ? 'demand-cursor-2' : 'more-demands',
        }),
      );
      return;
    }
    if (path === '/api/buyer-portal/demands/demand-1') {
      await json(route, success({ demand }));
      return;
    }
    if (path === '/api/buyer-portal/demands/demand-1/reservations') {
      if (options.reservationConflict) {
        await json(route, failure('VERSION_CONFLICT'), 409);
        return;
      }
      await json(route, success({ reservation, replayed: false }), 201);
      return;
    }
    ...
    if (path === '/api/buyer-portal/reservations/reservation-1/cancel') {
      await json(
        route,
        success({
          reservation: {
    ...
    if (path.endsWith('/order-instruction/state')) {
      await json(
        route,
        success({ order_instruction: instructionState(options.instructionStatus) }),
      );
      return;
    }
    if (path.endsWith('/order-instruction')) {
      // 真实后端合同：非 ACTIVE 内容读取失败（EXPIRED→410，其余→409），
      // 页面按 409/410 回退到 state 端点。
      if (options.instructionStatus && options.instructionStatus !== 'ACTIVE') {
        await json(
          route,
          failure(options.instructionStatus === 'EXPIRED' ? 'INSTRUCTION_EXPIRED' : 'INSTRUCTION_NOT_PUBLISHED'),
          options.instructionStatus === 'EXPIRED' ? 410 : 409,
        );
        return;
      }
      await json(
        route,
        success({ order_instruction: instruction(options.invalidInstructionPath) }),
      );
      return;
    }
    if (path.includes('/order-instruction/images/') && path.endsWith('/read-intent')) {
      await json(
        route,
        success({
          read_intent: {
            read_intent_id: 'image-intent',
    ...
    if (path === '/api/buyer-portal/order-evidence/eligible-reservations') {
      const cursor = url.searchParams.get('cursor');
      if (options.taskCursorPages && cursor === 'task-eligible-2') {
        await json(
          route,
          success({
            items: [
      ...
      await json(
        ...
        success({
          ...
          next_cursor: options.taskCursorPages ? 'task-eligible-2' : null,
        }),
      );
      return;
    }
    ...
    if (path === '/api/buyer-portal/order-evidence' && request.method() === 'POST') {
      await json(
        route,
        success({
          order_evidence: {
    ...
    if (path === '/api/buyer-portal/order-evidence/evidence-1') {
      await json(route, success({ order_evidence: evidence }));
      return;
    }
    if (path.endsWith('/order-evidence/evidence-1/resubmit')) {
      await json(
        route,
        success({
          order_evidence: {
    ...
    if (path.endsWith('/order-evidence/evidence-1/withdraw')) {
      await json(
        route,
        success({
          order_evidence: { ...evidence, status: 'WITHDRAWN', version: 3, allowed_actions: [] },
          replayed: false,
        }),
      );
      return;
    }
    if (path.endsWith('/order-evidence/evidence-1/files/evidence-link/read-intent')) {
      await json(
        route,
        success({
          read_intent_id: 'evidence-intent',
          file_object_id: 'evidence-file',
          access_token: 'x'.repeat(40),
          access_token_available: true,
          expires_at: now + 1000,
    ...
    if (path === '/api/buyer-portal/reviews/eligible-orders') {
      await json(
        route,
        success({
          items: [{ order: reviewOrder, current_review: null, allowed_actions: ['SUBMIT'] }],
          next_cursor: null,
        }),
      );
      return;
    ...
    if (path === '/api/buyer-portal/reviews' && request.method() === 'GET') {
      await json(route, success({ items: [review(options.reviewStatus)], next_cursor: null }));
      return;
    }
    if (path === '/api/buyer-portal/reviews' && request.method() === 'POST') {
      await json(route, success({ review: reviewDetail('PENDING_REVIEW'), replayed: false }), 201);
      return;
    }
    if (path === '/api/buyer-portal/reviews/review-1') {
      await json(route, success({ review: reviewDetail(options.reviewStatus) }));
      return;
    }
    if (path.endsWith('/reviews/review-1/resubmit')) {
      await json(route, success({ review: reviewDetail('PENDING_REVIEW'), replayed: false }));
      return;
    }
    if (path.endsWith('/reviews/review-1/withdraw')) {
      await json(route, success({ review: reviewDetail('WITHDRAWN'), replayed: false }));
      return;
    }
    if (path.endsWith('/reviews/review-1/files/review-link/read-intent')) {
      await json(
        route,
        success({
          read_intent_id: 'review-intent',
          file_object_id: 'review-file',
          access_token: 'x'.repeat(40),
          access_token_available: true,
          expires_at: now + 1000,
    ...
    if (path === '/api/buyer-portal/refunds') {
      await json(route, success({ items: [refund(options.refundStatus)], next_cursor: null }));
      return;
    }
    if (path === '/api/buyer-portal/refunds/refund-1') {
      await json(route, success({ refund: refundDetail(options.refundStatus) }));
      return;
    }
    if (
      ...
      activeUpload = {
        purpose: reviewUpload ? 'REVIEW_EVIDENCE' : 'ORDER_EVIDENCE',
        visibility: reviewUpload ? 'SELLER_VISIBLE' : 'BUYER_VISIBLE',
        count: body.files.length,
      };
      await json(
        route,
        success({
          upload_intent_id: 'upload-intent',
          purpose: activeUpload.purpose,
          visibility: activeUpload.visibility,
          status: 'ISSUED',
          version: 1,
          expires_at: now + 1000,
          uploads: body.files.map((_, index) => ({
            file_object_id: `upload-file-${index + 1}`,
            slot_no: index + 1,
    ...
    if (path === '/api/buyer-portal/file-upload-intents/upload-intent/complete') {
      ...
      await json(
        ...
        success({
          ...
          status: 'VERIFIED',
          version: 2,
          files: Array.from({ length: activeUpload.count }, (_, index) => ({
            file_object_id: `upload-file-${index + 1}`,
            purpose: activeUpload!.purpose,
            visibility: activeUpload!.visibility,
            detected_mime: 'image/png',
    ...
    if (path.includes('/file-read-intents/') && path.endsWith('/content')) {
      if (options.fileContentFailureOnce && !failedOnce.has('file-content')) {
        ...
        await json(
          route,
          failure(
            options.fileContentFailureOnce === 429 ? 'RATE_LIMITED' : 'DEPENDENCY_UNAVAILABLE',
          ),
          options.fileContentFailureOnce,
          options.fileContentFailureOnce === 429 ? { 'Retry-After': '1' } : {},
        );
        return;
      }
      await route.fulfill({
...
test('Buyer registration succeeds only after the verified session read', async ({ page }) => {
  await installBuyerApi(page);
  await page.goto(`/buyer/register?token=${buyerInvitationToken}`);
  ...
  // 当前批准交互：注册成功后停在成功页，由用户人工点击进入买家中心（不自动跳转）。
...
for (const [name, status, message] of [
  ['disabled', 503, '当前暂未开放注册'],
  ['rate limited', 429, '操作过于频繁'],
] as const) {
  test(`Buyer registration ${name} fails safely`, async ({ page }) => {
    await installBuyerApi(page, { registrationStatus: status });
    await page.goto(`/buyer/register?token=${buyerInvitationToken}`);
    await page.getByLabel('微信号').fill('buyer_wx');
    await page.getByLabel('密码', { exact: true }).fill('safe-password-123');
    await page.getByLabel('确认密码').fill('safe-password-123');
...
test('Buyer registration mismatch logs out and stays fail closed', async ({ page }) => {
  await installBuyerApi(page, { sessionType: 'SELLER_MEMBER' });
  await page.goto(`/buyer/register?token=${buyerInvitationToken}`);
  await page.getByLabel('微信号').fill('buyer_wx');
  await page.getByLabel('密码', { exact: true }).fill('safe-password-123');
  await page.getByLabel('确认密码').fill('safe-password-123');
...
test('Root has no registration link', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('link')).toHaveCount(0);
});
test('Buyer login has no registration link', async ({ page }) => {
...
test('Dashboard reload restores the product list', async ({ page }) => {
  ...
  page.on('request', (request) => {
    if (
      request.method() === 'GET' &&
      new URL(request.url()).pathname === '/api/buyer-portal/demands'
    )
      demandReads += 1;
  });
...
test('Task center follows an empty cursor page to actionable Buyer work', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '您有 4 件待办事项' })).toBeVisible();
...
test('Demand list shows real rules and deadlines', async ({ page }) => {
  await gotoBuyer(page, '/buyer/demands');
  await expect(page.getByText('月白护肤套装')).toBeVisible();
...
test('Demand cursor loads and retains three pages', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '加载更多' }).click();
  await expect(page.getByText('月白护肤套装')).toBeVisible();
  await expect(page.getByText('月白补充装')).toBeVisible();
  await expect(page.getByText('月白旅行装')).toBeVisible();
});
test('Demand confirmation defaults unchecked', async ({ page }) => {
  await gotoBuyer(page, '/buyer/demands/demand-1');
  await expect(
    page.getByText('提交前确认一下当前自费比例哦，如果产品信息有变化需要重新确认。'),
  ).toBeVisible();
  await expect(page.getByRole('checkbox')).not.toBeChecked();
  await expect(page.getByRole('button', { name: '确认并预约' })).toBeDisabled();
});
test('Demand version conflict resets acceptance', async ({ page }) => {
  await gotoBuyer(page, '/buyer/demands/demand-1', { reservationConflict: true });
  await page.getByRole('checkbox').check();
  await page.getByRole('button', { name: '确认并预约' }).click();
  await expect(page.getByRole('checkbox')).not.toBeChecked();
});
test('Ambiguous reservation retry preserves the exact idempotency key and body', async ({
  page,
}) => {
  const operations: { key: string | undefined; body: string | null }[] = [];
  page.on('request', (request) => {
    if (new URL(request.url()).pathname.endsWith('/demands/demand-1/reservations'))
      operations.push({ key: request.headers()['idempotency-key'], body: request.postData() });
  });
  await gotoBuyer(page, '/buyer/demands/demand-1', {
    networkFailureOnce: '/api/buyer-portal/demands/demand-1/reservations',
  });
  await page.getByRole('checkbox').check();
  await page.getByRole('button', { name: '确认并预约' }).click();
  await page.getByRole('button', { name: '重新尝试同一操作' }).click();
  await expect(page).toHaveURL(/\/buyer\/reservations\/reservation-1$/u);
  expect(operations).toHaveLength(2);
  expect(operations[0]).toEqual(operations[1]);
});
test('Reservation creation navigates to authoritative detail', async ({ page }) => {
  await gotoBuyer(page, '/buyer/demands/demand-1');
  await page.getByRole('checkbox').check();
  await page.getByRole('button', { name: '确认并预约' }).click();
  await expect(page).toHaveURL(/\/buyer\/reservations\/reservation-1$/u);
});
test('Reservation list shows status', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reservations');
  await expect(page.getByText('已确认')).toBeVisible();
});
test('Reservation cancellation is offered only by can_cancel', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reservations/reservation-1');
  await expect(page.getByRole('button', { name: '取消预约' })).toBeVisible();
});
test('Reservation cancel uses an explicit confirmation dialog', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reservations/reservation-1');
  await page.getByRole('button', { name: '取消预约' }).click();
  await expect(page.getByRole('dialog', { name: '取消预约' })).toBeVisible();
});
...
for (const status of ['UNPUBLISHED', 'EXPIRED', 'CANCELLED'] as const) {
  test(`Instruction ${status} state does not expose a submit action`, async ({ page }) => {
    await gotoBuyer(page, '/buyer/reservations/reservation-1/instruction', {
      instructionStatus: status,
    });
    await expect(
      page.getByRole('heading', {
        name: status === 'UNPUBLISHED' ? '尚未发布' : status === 'EXPIRED' ? '已到期' : '已取消',
      }),
    ).toBeVisible();
...
test('Instruction ACTIVE reads content after state and shows image controls', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reservations/reservation-1/instruction');
  await expect(page.getByText('商品图片')).toBeVisible();
  // 主图按批准交互内联加载（懒加载需滚入视口），成功后可打开大图。
  await page.locator('.instruction-image-item').scrollIntoViewIfNeeded();
  await expect(page.getByRole('img', { name: `${demand.product_name} 主图` })).toBeVisible();
});
test('Instruction COMPLETED is terminal and makes zero image read requests', async ({ page }) => {
  let imageReadRequests = 0;
  page.on('request', (request) => {
    if (new URL(request.url()).pathname.includes('/order-instruction/images/')) imageReadRequests += 1;
  });
  await gotoBuyer(page, '/buyer/reservations/reservation-1/instruction', {
    instructionStatus: 'COMPLETED',
  });
  // 真实合同：内容端点对非 ACTIVE 返回 409，页面回退到 state 视图（已完成终态）。
  await expect(page.getByRole('heading', { name: '已完成' })).toBeVisible();
  await expect(page.getByText('商品图片')).toHaveCount(0);
  expect(imageReadRequests).toBe(0);
});
test('Instruction arbitrary image path fails closed without pageerror', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  await gotoBuyer(page, '/buyer/reservations/reservation-1/instruction', {
    invalidInstructionPath: true,
  });
  await expect(page.getByRole('heading', { name: '暂时无法读取内容' })).toBeVisible();
  expect(errors).toEqual([]);
});

test('Order materials show actionable and submitted sections', async ({ page }) => {
  await gotoBuyer(page, '/buyer/order-materials');
  await expect(page.getByText('可提交')).toBeVisible();
  await expect(page.getByText('已提交资料')).toBeVisible();
});
test('Evidence detail survives direct deep link refresh', async ({ page }) => {
  await gotoBuyer(page, '/buyer/order-materials/evidence-1');
  await page.reload();
  await expect(page.getByText('123-1234567-1234567')).toBeVisible();
});
test('Evidence stale deep link remains concealed', async ({ page }) => {
  await gotoBuyer(page, '/buyer/order-materials/new?reservation_id=foreign');
  await expect(page.getByRole('heading', { name: '无法打开提交页面' })).toBeVisible();
});
test('Evidence form rejects non-Gregorian browser text input', async ({ page }) => {
  await gotoBuyer(page, '/buyer/order-materials/new?reservation_id=reservation-1');
  await expect(page.getByLabel('Amazon 下单日期')).toHaveAttribute('type', 'date');
});
...
test('Evidence detail shows fixed PRICE_MISMATCH copy and signed direction', async ({ page }) => {
  ...
  await expect(page.getByText('+¥120 JPY（实际支付高于参考金额）')).toBeVisible();
});
test('Evidence historical metadata falls back without a read action', async ({ page }) => {
  const metadata = {
    ...evidenceFile,
  ...
  await installBuyerApi(page);
  await page.route('**/api/buyer-portal/order-evidence/evidence-1', (route) =>
    json(route, success({ order_evidence: { ...evidence, files: [metadata] } })),
  );
  await page.goto('/buyer/order-materials/evidence-1');
  await expect(page.getByText('历史文件已不再提供下载，只能看到文件信息。')).toBeVisible();
});
test('Evidence withdrawal requires confirmation', async ({ page }) => {
  await gotoBuyer(page, '/buyer/order-materials/evidence-1');
  await page.getByRole('button', { name: '撤回资料' }).click();
  await expect(page.getByRole('dialog', { name: '撤回订单资料' })).toBeVisible();
});
test('Evidence changes-requested resubmit requires date and one new screenshot', async ({
  page,
}) => {
  await gotoBuyer(page, '/buyer/order-materials/evidence-1');
  await expect(page.getByRole('heading', { name: '按说明重新提交' })).toBeVisible();
  await expect(page.getByLabel('新的订单截图')).not.toHaveAttribute('multiple');
});
test('Evidence protected file read consumes the shared content endpoint', async ({ page }) => {
  const contentTokens: string[] = [];
  page.on('request', (request) => {
    if (new URL(request.url()).pathname.includes('/file-read-intents/'))
      contentTokens.push(request.headers()['x-file-read-token'] ?? '');
  });
  await gotoBuyer(page, '/buyer/order-materials/evidence-1');
  // 图片类受保护文件按批准交互内联加载（read-intent → 共享 content 端点）；
  // 缩略图按懒加载设计，需要滚动进入视口后才发起读取。
  await page.getByText('订单截图.png', { exact: true }).scrollIntoViewIfNeeded();
  await expect(page.getByRole('img', { name: '订单截图.png' })).toBeVisible();
  expect(contentTokens.length).toBeGreaterThanOrEqual(1);
});
...
for (const status of [429, 503] as const) {
  test(`Provider ${status} recovery retries the same content token`, async ({ page }) => {
    ...
    page.on('request', (request) => {
      if (new URL(request.url()).pathname.includes('/file-read-intents/'))
        tokens.push(request.headers()['x-file-read-token'] ?? '');
    });
    await gotoBuyer(page, '/buyer/order-materials/evidence-1', { fileContentFailureOnce: status });
    // 懒加载：滚入视口后自动读取，失败一次必须出现重试入口；重试复用同一 token。
    await page.getByText('订单截图.png', { exact: true }).scrollIntoViewIfNeeded();
    await expect(page.getByRole('button', { name: '重试' })).toBeVisible({
      timeout: status === 429 ? 2_500 : 1_000,
    });
...
test('Formal order list exposes all six business filters plus paging', async ({ page }) => {
  await gotoBuyer(page, '/buyer/orders');
  await expect(page.getByRole('search', { name: '正式订单筛选' })).toBeVisible();
  await expect(page.getByLabel('Amazon 订单号')).toBeVisible();
});
test('Formal order detail displays historical null date and readable immutable rate', async ({
  page,
}) => {
  await gotoBuyer(page, '/buyer/orders/formal-1');
...
test('Review list shows eligible and submitted cases', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews');
  await expect(page.getByText('可提交评论')).toBeVisible();
  await expect(page.getByText('已提交评论')).toBeVisible();
});
test('Review stale deep link remains concealed', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews/new?formal_order_id=foreign');
  await expect(page.getByRole('heading', { name: '无法打开评论提交页面' })).toBeVisible();
});
test('Review form enforces business 1–3 evidence guidance', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews/new?formal_order_id=formal-1');
  await expect(page.getByText('请选择 1–3 个图片或 PDF 文件')).toBeVisible();
  await expect(page.getByLabel('评论文件')).toHaveAttribute('multiple', '');
});
test('Review upload and submit sends 1–3 verified file versions', async ({ page }) => {
  ...
  await page
    .getByLabel('评论文件')
    .setInputFiles({ name: 'review-a.png', mimeType: 'image/png', buffer: Buffer.from('png') });
  await page.getByRole('button', { name: '提交评论资料' }).click();
  await expect
    .poll(() => ({ requests, pageErrors }))
    .toEqual(
...
test('Review detail shows public change reason and resubmit form', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews/review-1');
  await expect(page.getByText('修改说明：请补充完整评论截图')).toBeVisible();
  await expect(page.getByRole('heading', { name: '按说明重新提交' })).toBeVisible();
});
test('Approved review shows refund amount without inventing payment', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews/review-1', { reviewStatus: 'APPROVED' });
  await expect(page.getByText(/返款金额/u)).toBeVisible();
  await expect(page.getByText(/已支付/u)).toHaveCount(0);
});
test('Review protected file read uses entity-bound provider', async ({ page }) => {
  await gotoBuyer(page, '/buyer/reviews/review-1');
  // 图片类证据按批准交互内联加载（懒加载需滚入视口）；read-intent 走 review 实体绑定 provider。
  await page.getByText('评论截图.png', { exact: true }).scrollIntoViewIfNeeded();
  await expect(page.getByRole('img', { name: '评论截图.png' })).toBeVisible();
});

test('Refund detail keeps payment and reversal activities visible', async ({ page }) => {
  await gotoBuyer(page, '/buyer/refunds/refund-1');
  await expect(page.getByText('记录付款')).toBeVisible();
  await expect(page.getByText('付款撤回')).toBeVisible();
});
test('Refund OVERPAID displays overpaid amount', async ({ page }) => {
  await gotoBuyer(page, '/buyer/refunds/refund-1', { refundStatus: 'OVERPAID' });
  await expect(page.getByText('超额返款')).toBeVisible();
  await expect(page.getByText('¥12.66 CNY')).toBeVisible();
});
...
test('Me REVIEW_REQUIRED shows only safe limitation guidance', async ({ page }) => {
  ...
  await expect(page.getByText(/部分业务操作会受到限制/u)).toBeVisible();
  await expect(page.getByRole('button', { name: /编辑/u })).toHaveCount(0);
});
test('Me logout returns to Buyer login', async ({ page }) => {
  await gotoBuyer(page, '/buyer/me');
  await page.getByRole('button', { name: '退出登录' }).click();
  await expect(page).toHaveURL(/\/buyer\/login$/u);
});

test('Buyer 401 redirects before shell renders', async ({ page }) => {
  await gotoBuyer(page, '/buyer', { failures: { '/api/customer-auth/session': 401 } });
  await expect(page.getByText('月光白')).toBeVisible();
});
for (const [status, path, message] of [
  [403, '/api/buyer-portal/me', '当前账号没有查看'],
  [404, '/api/buyer-portal/order-evidence/evidence-1', '内容不存在'],
  [503, '/api/buyer-portal/formal-orders/formal-1', '服务暂时不可用'],
] as const) {
  test(`Buyer ${status} renders a safe request-bound error`, async ({ page }) => {
    await gotoBuyer(
      page,
      status === 403
        ? '/buyer/me'
    ...
    await expect(page.getByText(new RegExp(message, 'u'))).toBeVisible();
    await expect(page.getByText('请求编号：module1-browser-error')).toBeVisible();
  });
}
...
for (const viewport of [
  ...
  test(`Buyer layout reflows without horizontal overflow at ${viewport.width}px`, async ({
    page,
  }) => {
...
test('Buyer remains readable at 200 percent text zoom', async ({ page }) => {
  ...
  await noOverflow(page);
  await expect(page.getByRole('heading', { name: '月白买家', level: 2 })).toBeVisible();
});
test('Buyer respects reduced motion', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await gotoBuyer(page, '/buyer');
  const duration = await page.evaluate(() => {
    const element = document.createElement('span');
    element.className = 'buyer-loading-mark';
    document.body.append(element);
    return getComputedStyle(element).animationDuration;
  });
  expect(duration).not.toBe('0.9s');
});
test('Buyer keyboard focus remains visible', async ({ page }) => {
  await gotoBuyer(page, '/buyer/me');
  const focused = page.getByRole('link', { name: '正式订单' });
  await focused.focus();
...
test('capture Module1 root desktop acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/');
  await expect(page.getByRole('heading', { name: '月光白' })).toBeVisible();
  await captureAcceptance(page, 'root-desktop-1440x900.png');
});
test('capture Module1 registration mobile acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/buyer/register');
  await expect(page.getByRole('heading', { name: '邀请注册' })).toBeVisible();
  await captureAcceptance(page, 'buyer-register-mobile-390x844.png');
});
test('capture Module1 login mobile acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/buyer/login');
  await expect(page.getByText('月光白')).toBeVisible();
  await captureAcceptance(page, 'buyer-login-mobile-390x844.png');
});
...
for (const [name, path] of [
  ...
  [
    'buyer-order-evidence-form-mobile-390x844.png',
    '/buyer/order-materials/new?reservation_id=reservation-1',
  ],
  ...
  test(`capture Module1 ${name}`, async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await gotoBuyer(page, path);
    await expect(page.locator('.buyer-page')).toBeVisible();
    await captureAcceptance(page, name);
  });
}

test('capture Module1 partial failure mobile acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await gotoBuyer(page, '/buyer', { failures: { '/api/buyer-portal/demands': 503 } });
  await expect(page.getByRole('heading', { name: '暂时无法读取内容' })).toBeVisible();
  await captureAcceptance(page, 'buyer-dashboard-partial-error-mobile-390x844.png');
});
test('capture Module1 320 reflow acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 800 });
  await gotoBuyer(page, '/buyer/order-materials/evidence-1');
  await noOverflow(page);
  await captureAcceptance(page, 'buyer-320-reflow-320x800.png');
});
test('capture Module1 200 percent acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await gotoBuyer(page, '/buyer/me');
  await page.evaluate(() => {
    document.documentElement.style.fontSize = '200%';
  ...
  await expect(page.getByRole('heading', { name: '月白买家', level: 2 })).toBeVisible();
  await noOverflow(page);
  await captureAcceptance(page, 'buyer-200-percent-390x844.png');
});
test('capture Module1 permission error mobile acceptance', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await gotoBuyer(page, '/buyer/me', { failures: { '/api/buyer-portal/me': 403 } });
  await expect(page.getByText(/当前账号没有查看/u)).toBeVisible();
  await captureAcceptance(page, 'buyer-permission-error-mobile-390x844.png');
});
