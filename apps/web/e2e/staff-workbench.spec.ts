...
const success = (data: unknown, requestId = 'm5-browser') => ({
  data,
...
const item = {
  work_item_id: 'work-m5',
  work_type: 'ORDER_EVIDENCE_REVIEW',
  source_entity_type: 'ORDER_EVIDENCE',
  source_entity_id: 'evidence-m5',
  buyer_customer_id: 'buyer-m5',
  seller_organization_id: 'org-m5',
  store_id: 'store-m5',
  duty_code: 'BUYER_PRE_SALES_OWNER',
  fixed_assignment_id: 'assignment-m5',
  assigned_staff_id: 'staff-m5',
  status: 'OPEN',
  version: 2,
...
const evidence = {
  submission_id: 'evidence-m5',
  reservation_id: 'reservation-m5',
  ...
  final_paid_jpy: '12880',
  buyer_note: '订单页面已包含折扣后的实付金额',
  public_change_reason: null,
  submitted_at: 1_786_000_000_000,
  updated_at: 1_786_000_000_000,
  verified_at: null,
  withdrawn_at: null,
  buyer_customer_id: 'buyer-m5',
  internal_review_note: '请核对折扣',
  verified_by_staff_id: null,
  duplicate_signal_count: 0,
  reference_order_amount_jpy: '13000',
  price_difference_jpy: '-120',
  price_mismatch: true,
  screenshot: {
    file_object_id: 'file-m5',
    file_version: 4,
  ...
  instruction: {
    instruction_id: 'instruction-m5',
    instruction_version_id: 'instruction-version-m5',
    buyer_self_pay_bps: 1000,
    buyer_self_pay_jpy: '1300',
    buyer_refundable_principal_jpy: '11700',
  },
  reservation: { reservation_id: 'reservation-m5', status: 'ORDER_EVIDENCE_SUBMITTED', version: 4 },
  version_history: [
    {
      evidence_version_id: 'evidence-version-m5',
      version_no: 1,
      final_paid_jpy: '12880',
      submitted_at: 1_786_000_000_000,
    },
...
async function mockWorkbench(
  page: Page,
  observe?: { approveBody?: unknown; key?: string | null },
): Promise<void> {
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/staff/me/work-items')
      return json(route, success({ work_items: [item], next_cursor: null }));
    if (url.pathname === '/api/staff/me/work-items/work-m5')
      return json(route, success({ work_item: item }));
    if (url.pathname === '/api/staff/order-evidence/evidence-m5')
      return json(route, success({ order_evidence: evidence }));
    ...
    if (url.pathname === '/api/staff/order-evidence/evidence-m5/approve') {
      if (observe) {
        observe.approveBody = route.request().postDataJSON();
        observe.key = route.request().headers()['idempotency-key'] ?? null;
      }
      return json(
        route,
        {
          error: { code: 'VERSION_CONFLICT', message: '已更新', details: null },
          meta: { request_id: 'm5-version-conflict' },
        },
        409,
      );
    }
...
test('Staff completes queue to authoritative order detail and sees explicit conflict recovery', async ({
  page,
}) => {
  ...
  // 顶栏以头像保持轻量视觉，同时保留可访问的员工姓名与角色会话信息。
  await expect(page.getByLabel('当前会话信息：售前员工（售前）')).toBeAttached();
  await expect(page.getByLabel('账户：售前员工，售前')).toBeVisible();
  ...
  expect(observed.approveBody).toEqual({
    expected_version: 2,
    price_mismatch_acknowledged: true,
    price_mismatch_reason: '截图清晰显示平台折扣后的实付金额',
  });
...
test('Staff explicit retry preserves ambiguous request authority and changed body starts a new operation', async ({
  page,
}) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/staff/order-evidence/evidence-m5/approve') {
      ...
      return json(
        ...
        {
          error: { code: 'VERSION_CONFLICT', message: '已更新', details: null },
          meta: { request_id: `retry-deterministic-${calls.length}` },
        },
        409,
      );
    }
    return json(
      ...
      {
        error: { code: 'NOT_FOUND', message: 'not found', details: null },
        meta: { request_id: 'retry-unhandled' },
      },
      404,
    );
  });
...
for (const viewport of [
  ...
  test(`Staff workbench reflows at ${viewport.width}px with keyboard controls`, async ({
    page,
  }) => {
...
test('capture deterministic Staff workbench desktop and narrow views', async ({ page }) => {
  ...
  if (screenshotDirectory) {
    ...
    await page.screenshot({
      path: join(screenshotDirectory, 'staff-workbench-desktop-1600x1000.png'),
      fullPage: true,
    });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.screenshot({
      path: join(screenshotDirectory, 'staff-workbench-narrow-390x844.png'),
      fullPage: true,
    });
  }
});
