...
export function useCursorPages<T>(input: Readonly<{
  ...
  const [chain, setChain] = useState<Readonly<{ resetKey: string; cursors: readonly (string | null)[] }>>({
  ...
  useEffect(() => {
    setChain((current) => current.resetKey === input.resetKey
      ? current : { resetKey: input.resetKey, cursors: [null] });
  }, [input.resetKey]);
  ...
  const items = useMemo(() => queries.flatMap((query) => query.data?.items ?? []), [queries]);
  ...
  return Object.freeze({
    ...
    hasMore: last?.isSuccess === true && nextCursor !== null && !cursors.includes(nextCursor),
