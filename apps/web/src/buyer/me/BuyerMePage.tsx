...
export function BuyerMePage(): React.JSX.Element {
  ...
  return <section className="buyer-page"><PageHeader eyebrow="我的" title={me.buyer.display_name}>
    ...
    <Card><dl className="buyer-facts"><div><dt>客户编号</dt><dd>{me.buyer.customer_number ?? '尚未分配'}</dd></div>
      <div><dt>市场</dt><dd>{me.buyer.marketplace_code}</dd></div><div><dt>会话到期</dt><dd>{formatShanghai(me.session.expires_at)}</dd></div></dl></Card>
