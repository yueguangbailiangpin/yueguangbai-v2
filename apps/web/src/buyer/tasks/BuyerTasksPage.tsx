...
export function BuyerTasksPage(): React.JSX.Element {
  ...
  const results = useQueries({
    queries: [
      {
        queryKey: buyerQueryKeys.reservationsPage({
          limit: BUYER_TASK_CURSOR_PAGE_LIMIT,
          cursor: null,
        }),
        queryFn: ({ signal }) =>
          fetchAllCursorPages({
            ...
            fetchPage: (cursor) =>
              buyerApi
                .reservations(
                  client,
                  cursorQuery({ limit: BUYER_TASK_CURSOR_PAGE_LIMIT, cursor }),
                  signal,
                )
                .then((r) => r.data),
            itemKey: (item) => `reservation:${item.reservation_id}`,
          }),
      },
      {
        queryKey: buyerQueryKeys.evidenceEligiblePage({
          limit: BUYER_TASK_CURSOR_PAGE_LIMIT,
          cursor: null,
        }),
        queryFn: ({ signal }) =>
          fetchAllCursorPages({
            source: 'eligible order evidence',
            signal,
            fetchPage: (cursor) =>
              buyerApi
                .evidenceEligible(
                ...
                .then((r) => r.data),
            itemKey: (item) => `eligible-evidence-reservation:${item.reservation_id}`,
          }),
      },
      {
          ...
          fetchAllCursorPages({
            ...
            fetchPage: (cursor) =>
              buyerApi
                ...
                .then((r) => r.data),
            itemKey: (item) => `order-evidence:${item.submission_id}`,
          }),
      },
      {
        queryKey: buyerQueryKeys.reviewEligiblePage({
          limit: BUYER_TASK_CURSOR_PAGE_LIMIT,
          cursor: null,
        }),
        queryFn: ({ signal }) =>
          fetchAllCursorPages({
            source: 'eligible reviews',
            signal,
            fetchPage: (cursor) =>
              buyerApi
                .reviewEligible(
                ...
                .then((r) => r.data),
            itemKey: (item) => `eligible-review-order:${item.order.formal_order_id}`,
          }),
      },
      {
          ...
          fetchAllCursorPages({
            ...
            fetchPage: (cursor) =>
              buyerApi
                ...
                .then((r) => r.data),
            itemKey: (item) => `review:${item.review_case_id}`,
          }),
      },
      {
        queryKey: buyerQueryKeys.refundsPage({
          limit: BUYER_TASK_CURSOR_PAGE_LIMIT,
          cursor: null,
          ...
          fetchAllCursorPages({
            ...
            fetchPage: (cursor) =>
              buyerApi
                ...
                .then((r) => r.data),
            itemKey: (item) => `refund:${item.refund_obligation_id}`,
          }),
      },
    ],
  });
  ...
  const title = failed
    ? '任务状态暂时无法完整读取'
    : actionableCount > 0
      ? `您有 ${actionableCount} 件待办事项`
      : '暂时没有待办事项，休息一下～';
  return (
    <section className="buyer-page buyer-tasks-page">
      <PageHeader
        eyebrow="买家任务"
        title={title}
      ...
      {failed ? (
        <Alert tone="warning">
          部分任务状态暂时无法加载，请稍后刷新；已成功读取的事项仍可继续处理。
        </Alert>
      ) : null}
      {!pending && !failed && urgent.length === 0 && action.length === 0 && system.length === 0 ? (
        <EmptyState
          title="暂无任务"
          description="新的预约、订单资料、评论或返款状态会出现在这里哦。"
        />
      ) : (
...
function TaskSection({
  ...
  items: readonly BuyerTask[];
  system?: boolean;
}): React.JSX.Element {
  return (
    <section className={`buyer-task-section${system ? ' buyer-task-system' : ''}`}>
      <header>
        <span>{icon}</span>
        <h2>{title}</h2>
        <StatusBadge tone={system ? 'neutral' : title === '紧急' ? 'warning' : 'processing'}>
          {items.length}
        </StatusBadge>
      </header>
      <div className="buyer-task-list">
        {items.map((item) => (
          <Link className={`buyer-task-row buyer-task-${item.kind}`} key={item.id} to={item.href}>
            <div>
