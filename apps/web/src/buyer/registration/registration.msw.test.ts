...
describe('Module 1 registration transport transition', () => {
  it('does not authenticate from 201, clears both customer roots, then strictly reads Buyer session', async () => {
    ...
    let body: unknown;
    server.use(
      http.post(apiUrl('/api/buyer-auth/register'), async ({ request }) => {
        events.push('register');
        body = await request.json();
        return HttpResponse.json(
          { data: registration(), meta: { request_id: 'register-request' } },
          { status: 201 },
        );
      }),
    );
    ...
    vi.spyOn(client, 'cancelQueries').mockImplementation(async (filters) => {
      events.push(`cancel:${String(filters?.queryKey?.[0])}`);
      return cancel(filters);
    });
    const controller = new BuyerRegistrationController(
      client,
      {
    ...
    expect(body).toEqual({
      ...
      password: 'safe-password-123',
      password_confirmation: 'safe-password-123',
      human_verification_token: 'trusted-human-token',
    });
    expect(events.indexOf('session')).toBeGreaterThan(events.indexOf('register'));
    expect(events).toContain('cancel:buyer');
    expect(events).toContain('cancel:seller');
  ...
  it('omits a human token when the production provider is disconnected', async () => {
    ...
    server.use(
      http.post(apiUrl('/api/buyer-auth/register'), async ({ request }) => {
        body = await request.json();
        return HttpResponse.json(
          { data: registration(), meta: { request_id: 'register-no-human' } },
          { status: 201 },
        );
      }),
    );
    await new BuyerRegistrationController(
      queryClient(),
      { token: async () => null },
      authAdapter(buyerSession, []),
    ).register(
      {
        invitation_token: 'a'.repeat(43),
        marketplace_code: 'AMAZON_JP',
  ...
  it('logs out and fails closed when the post-registration session is Seller', async () => {
    server.use(
      http.post(apiUrl('/api/buyer-auth/register'), () =>
        HttpResponse.json(
          { data: registration(), meta: { request_id: 'register-mismatch' } },
          { status: 201 },
        ),
      ),
    );
    ...
    await expect(
      new BuyerRegistrationController(
        queryClient(),
        { token: async () => null },
        authAdapter(seller, events),
      ).register(
        {
          invitation_token: 'a'.repeat(43),
          marketplace_code: 'AMAZON_JP',
...
function authAdapter(session: CustomerSession, events: string[]): CustomerAuthApiAdapter {
  return {
    readSession: async () => {
      events.push('session');
      return { data: { session }, requestId: 'session-request' };
    },
    logout: async () => {
      events.push('logout');
      return {
    ...
    changePassword: async () => {
      throw new Error('unexpected_change_password');
    },
  };
}
...
function registration() {
  return {
    identity: { buyer_number: 'B-1', wechat_id: 'buyer' },
