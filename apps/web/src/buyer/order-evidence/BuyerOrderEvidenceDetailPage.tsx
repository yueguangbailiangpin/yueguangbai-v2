...
export function BuyerOrderEvidenceDetailPage(): React.JSX.Element {
  ...
  const query = useQuery({
    ...
    queryFn: ({ signal }) =>
      buyerApi.evidence(client, submissionId, signal).then((r) => r.data.order_evidence),
    enabled: submissionId.length > 0,
  });
  const withdraw = useBuyerMutation({
    operation: (body: { expected_version: number }, key, signal) =>
      buyerApi.withdrawEvidence(client, submissionId, body.expected_version, key, signal),
  ...
  return (
    <section className="buyer-page buyer-flow-page buyer-detail-page">
      ...
      <PageHeader
        ...
        description={item.reservation.store_display_name}
      >
        ...
        <dl className="buyer-facts">
          <div>
            ...
            <dd className="copyable-fact">
              {item.amazon_order_number_display}
              <Button
                className="secondary compact-button"
                onClick={() => {
                  void navigator.clipboard.writeText(item.amazon_order_number_display);
                }}
              >
                复制
              </Button>
          ...
          <div>
            <dt>价差</dt>
            <dd>
              {formatSignedJpyDifference(item.price_difference_jpy)}（
              {priceDifferenceDirection(item.price_difference_jpy)}）
            </dd>
          </div>
          <div>
            <dt>自费比例</dt>
            <dd>{formatBps(item.buyer_self_pay_bps)}</dd>
          </div>
          <div>
            <dt>自费金额</dt>
            <dd>{formatJpy(item.buyer_self_pay_jpy)}</dd>
          </div>
          <div>
            <dt>可返本金</dt>
          ...
          <div>
            <dt>提交时间</dt>
            <dd>{formatShanghai(item.submitted_at)}</dd>
          </div>
          <div>
            <dt>更新时间</dt>
            <dd>{formatShanghai(item.updated_at)}</dd>
          </div>
          <div>
            <dt>核验时间</dt>
            <dd>{formatShanghai(item.verified_at)}</dd>
          </div>
        </dl>
      </Card>
      <Card className="buyer-support-card">
        <h2>订单截图</h2>
        <div className="buyer-file-list">
          {item.files.map((file) => (
            <EvidenceFile key={file.file_object_id} submissionId={submissionId} file={file} />
          ))}
        </div>
      </Card>
      {item.allowed_actions.includes('RESUBMIT') ? (
        <EvidenceResubmitForm
          evidence={item}
          onRefresh={() => {
            void query.refetch();
          }}
      ...
      {item.allowed_actions.includes('WITHDRAW') ? (
        <Button className="danger" onClick={() => setConfirmWithdraw(true)}>
          撤回资料
        </Button>
      ) : null}
      <Dialog
      ...
      >
        ...
        <div className="entry-actions">
          <Button className="secondary" onClick={() => setConfirmWithdraw(false)}>
            取消
          </Button>
          <Button
            className="danger"
            loading={withdraw.isPending}
            onClick={() => withdraw.mutate({ expected_version: item.version })}
          >
            确认撤回
          </Button>
        </div>
...
function EvidenceFile({
  ...
  const provider = useMemo(
    () =>
      file.file_entity_link_id && file.version
  ...
  return (
    <article className="buyer-file-item">
      <div>
        <strong>{file.client_file_name}</strong>
        <p>
          {file.mime} · {file.byte_size} 字节 · {statusLabel(file.status)}
        </p>
      </div>
      {provider ? (
...
function EvidenceResubmitForm({
  ...
  const mutation = useBuyerMutation({
    operation: (body: unknown, key, signal) =>
      buyerApi.resubmitEvidence(client, evidence.submission_id, body, key, signal),
    onSuccess: async (result) => {
      client.setQueryData(
        buyerQueryKeys.evidence(evidence.submission_id),
        result.data.order_evidence,
      );
      await Promise.all([
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (
      !date.success ||
      !/^\d{3}-\d{7}-\d{7}$/u.test(order) ||
      !Number.isSafeInteger(paid) ||
      paid < 0 ||
      !file.current
    ) {
      setMessage('请完整填写资料并选择一张新的截图。');
      return;
    }
  ...
  return (
    <Card className="buyer-action-panel">
      ...
      >
        <FormField label="Amazon 订单号" htmlFor="resubmit-order" required>
          <TextInput
            name="amazon_order_number"
            defaultValue={evidence.amazon_order_number_display}
            required
          />
        </FormField>
        <FormField label="Amazon 下单日期" htmlFor="resubmit-date" required>
          <TextInput
            name="amazon_order_date"
            type="date"
            lang="zh-CN"
            defaultValue={evidence.amazon_order_date ?? ''}
            required
          />
        </FormField>
        <FormField label="最终支付金额（JPY）" htmlFor="resubmit-paid" required>
          <TextInput
            name="final_paid_jpy"
            type="number"
            min="0"
            step="1"
            defaultValue={evidence.final_paid_jpy}
            required
          />
        </FormField>
        <FormField
        ...
        <FormField label="备注（可选）" htmlFor="resubmit-note">
