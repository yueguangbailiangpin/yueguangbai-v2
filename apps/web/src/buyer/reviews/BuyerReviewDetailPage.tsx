...
export function BuyerReviewDetailPage(): React.JSX.Element {
  ...
  const query = useQuery({
    queryKey: buyerQueryKeys.review(reviewCaseId),
    queryFn: ({ signal }) =>
      buyerApi.review(client, reviewCaseId, signal).then((r) => r.data.review),
    enabled: reviewCaseId.length > 0,
  });
  const withdraw = useBuyerMutation({
    operation: (body: { expected_version: number }, key, signal) =>
      buyerApi.withdrawReview(client, reviewCaseId, body.expected_version, key, signal),
    onSuccess: async (result) => {
      client.setQueryData(buyerQueryKeys.review(reviewCaseId), result.data.review);
  ...
  return (
    <section className="buyer-page buyer-flow-page buyer-detail-page">
      ...
      {item.status === 'APPROVED' && item.buyer_refund_due ? (
        <Alert tone="success">返款金额 {formatCnyFen(item.buyer_refund_due.amount_cny_fen)}</Alert>
      ) : null}
      <Card className="buyer-summary-card">
        ...
        <dl className="buyer-facts">
          <div>
            <dt>评论类型</dt>
            <dd>{reviewTypeLabel(item.review_type)}</dd>
          </div>
          <div>
            <dt>Amazon 订单号</dt>
            <dd>{item.order.amazon_order_number}</dd>
          </div>
          <div>
            <dt>Amazon 下单日期</dt>
...
function ReviewFile({
  ...
  reviewId: string;
  file: ReviewDetail['files'][number];
}): React.JSX.Element {
  const provider = useMemo(
    () =>
      new BuyerReviewFileReadIntentAdapter(
        reviewId,
...
function ReviewResubmitForm({
  ...
  const mutation = useBuyerMutation({
    operation: (body: unknown, key, signal) =>
      buyerApi.resubmitReview(client, review.review_case_id, body, key, signal),
    onSuccess: async (result) => {
      client.setQueryData(buyerQueryKeys.review(review.review_case_id), result.data.review);
      await client.invalidateQueries({ queryKey: buyerQueryKeys.reviewsRoot });
    },
    onError: () => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (files.current.length < 1 || files.current.length > 3) {
  ...
  return (
    <Card className="buyer-action-panel">
      ...
      >
        <FormField label="评论链接（可选）" htmlFor="review-resubmit-url">
        ...
        <FormField label="备注（可选）" htmlFor="review-resubmit-note">
