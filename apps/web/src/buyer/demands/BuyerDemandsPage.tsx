...
export function BuyerDemandsPage(): React.JSX.Element {
  ...
  const pages = useCursorPages({
    ...
    queryFn: (cursor, signal) => buyerApi.demands(client, cursorQuery({ limit: 100, cursor }), signal).then((r) => r.data),
  });
  const filtered = useMemo(() => {
    ...
    return pages.items.filter((item) => {
      if (query && !`${item.product_name}\u0000${item.store_display_name}`.toLocaleLowerCase('zh-CN').includes(query)) return false;
  ...
  function changeSearch(value: string): void { setSearch(value); setPage(1); }
  ...
  return <section className="buyer-page buyer-products-page buyer-products-s3">
    ...
    {!pages.isInitialPending && !pages.initialError && filtered.length > 0 ? <nav className="buyer-local-pagination" aria-label="产品分页">
      <Button className="secondary" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>上一页</Button>
