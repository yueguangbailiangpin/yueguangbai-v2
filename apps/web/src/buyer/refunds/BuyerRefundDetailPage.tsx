...
export function BuyerRefundDetailPage(): React.JSX.Element {
  ...
  const query = useQuery({
    queryKey: buyerQueryKeys.refund(refundId),
    queryFn: ({ signal }) => buyerApi.refund(client, refundId, signal).then((r) => r.data.refund),
    enabled: refundId.length > 0,
  });
  const remind = useMutation({
    mutationFn: () =>
      buyerApi.remindRefund(client, refundId, startOperation({}).key).then((r) => r.data),
    ...
    <section className="buyer-page buyer-flow-page buyer-detail-page buyer-refund-page">
      ...
      <Card className="buyer-summary-card buyer-refund-summary">
        ...
        <dl className="buyer-facts">
          <div>
            <dt>对应订单</dt>
            <dd>{item.order.formal_order_id}</dd>
          </div>
          <div>
            <dt>Amazon 订单号</dt>
          ...
          <div>
            <dt>超额金额</dt>
            <dd>{formatCnyFen(item.overpaid_amount_cny_fen)}</dd>
          </div>
        </dl>
      </Card>
      {item.status === 'DUE' || item.status === 'PARTIALLY_PAID' ? (
        <Card className="buyer-refund-reminder">
          <h2>催返款</h2>
          {item.reminder.last_reminded_at === null ? (
            <p>如尚未收到返款，可提交一次催办。</p>
          ) : (
            <p>
              已于 {formatShanghai(item.reminder.last_reminded_at)} 催办，本单 24
              小时内不能重复催办。
            </p>
          )}
          <button
            ...
            disabled={
              remind.isPending ||
              (item.reminder.next_reminder_at !== null &&
                item.reminder.next_reminder_at > Date.now())
            }
      ...
      <section aria-labelledby="refund-activity-title">
        ...
        {item.activities.length === 0 ? (
          ...
          <ol className="refund-activity-list">
            {item.activities.map((activity) => (
              <li key={activity.activity_id}>
                <Card as="article">
                  <div className="record-card-heading">
                    <strong>{statusLabel(activity.activity_type)}</strong>
                    <span>{formatCnyFen(activity.amount_cny_fen)}</span>
                  </div>
                  <p>
                    {formatShanghai(activity.occurred_at)} ·{' '}
                    {paymentChannelLabel(activity.payment_channel)}
                  </p>
                  <dl className="compact-facts">
                    ...
                    <div>
                      <dt>活动后剩余</dt>
                      <dd>{formatCnyFen(activity.balance_after.remaining_amount_cny_fen)}</dd>
                    </div>
                    <div>
                      <dt>活动后超额</dt>
                      <dd>{formatCnyFen(activity.balance_after.overpaid_amount_cny_fen)}</dd>
                    </div>
                  </dl>
                </Card>
              </li>
