...
export function BuyerFormalOrdersPage(): React.JSX.Element {
  ...
  const emptyFilters = { marketplace: null, productName: null, reviewType: null, confirmedBusinessDate: null, formalOrderId: null, amazonOrderNumber: null } as const;
  const [filters, setFilters] = useState<Omit<FormalOrderPageParameters, 'limit' | 'cursor'>>(emptyFilters);
  const resetKey = JSON.stringify(filters);
  const query = useCursorPages({
    resetKey,
    queryKey: (cursor) => buyerQueryKeys.formalOrdersPage({ ...filters, limit: 20, cursor }),
    queryFn: (cursor, signal) => buyerApi.formalOrders(client, formalOrderQuery({ ...filters, limit: 20, cursor }), signal).then((r) => r.data),
  });
  function submit(event: FormEvent<HTMLFormElement>): void {
    ...
    setFilters({
      marketplace: read('marketplace'), productName: read('product_name'), reviewType: read('review_type'),
      confirmedBusinessDate: read('confirmed_business_date'), formalOrderId: read('formal_order_id'),
