...
export function safeReturnPath(value: string | null, identity: Identity): string {
  ...
  try {
    ...
    if (url.origin !== window.location.origin || !allowed[identity].some((prefix) => url.pathname === prefix || url.pathname.startsWith(`${prefix}/`)) || url.pathname.endsWith('/login') || url.pathname.includes('/auth/callback') || url.pathname.includes('change-password')) return allowed[identity][0] ?? '/';
