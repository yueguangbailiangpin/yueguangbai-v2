...
export function SellerSettlementPanel({ item }: { item: StaffWorkItem }): React.JSX.Element | null {
  ...
  const summary = useQuery({
    ...
    queryFn: ({ signal }) => staffApi.settlementSummary(client, organizationId!, signal).then((response) => response.data.settlement),
    enabled,
    retry: false,
  });
  const payables = useQuery({
    queryKey: staffWorkbenchKeys.payables(organizationId ?? 'unavailable'),
    queryFn: ({ signal }) => staffApi.settlementPayables(client, organizationId!, signal).then((response) => response.data.items),
    enabled,
    retry: false,
  });
  const payments = useQuery({
    queryKey: staffWorkbenchKeys.payments(organizationId ?? 'unavailable'),
    queryFn: ({ signal }) => staffApi.settlementPayments(client, organizationId!, signal).then((response) => response.data.items),
    enabled,
    retry: false,
  });
        ...
        : <Card className="internal-note"><h3>付款、分配与凭证</h3>{payments.data?.length === 0 ? <p>暂无付款事实。</p> : payments.data?.map((payment) => <section key={payment.payment_id}>
          ...
          {capabilities.canRecord && payment.status !== 'REVERSED' && payment.unallocated_amount_cny_fen !== '0' ? <form onSubmit={(event) => {
            ...
            mutation.mutate({
              ...
              body: { payable_id: String(data.get('payable_id')), amount_cny_fen: String(data.get('amount')), expected_payment_version: payment.version },
            });
          }}>
          ...
          {capabilities.canReverse && payment.status !== 'REVERSED' && payment.allocated_amount_cny_fen === '0' ? <form onSubmit={(event) => {
            event.preventDefault();
          ...
          }}><FormField label="整笔冲正原因" htmlFor={`payment-reason-${payment.payment_id}`}><TextInput id={`payment-reason-${payment.payment_id}`} name="reason" required /></FormField><Button className="danger" disabled={mutation.isPending}>整笔冲正</Button></form> : null}
        </section>)}</Card>}
    </section>
    <aside className="staff-actions">
      <Card className="staff-current-customer"><h3>当前客户</h3><Fact label="卖家组织" value={organizationId} /><Fact label="业务范围" value="按当前 Marketplace 权限过滤" /></Card>
      {capabilities.canRecord ? <Card className="sensitive-action"><h3>记录卖家付款</h3><Alert tone="info">付款入账后再明确分配到本金或服务费；两类应结事实不会合并。</Alert><input aria-label="卖家结算付款凭证" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploader.start('staffSellerSettlementProof', [file]); }} /><p role="status">凭证状态：{upload.state}</p>
        <form onChange={() => { if (!mutation.isPending) { authority.release(); mutation.reset(); } }} onSubmit={(event) => {
          event.preventDefault();
          ...
          mutation.mutate({
            ...
            body: { amount_cny_fen: String(data.get('amount')), paid_at: Date.now(), proof_file: { file_object_id: file.file_object_id, expected_file_version: file.file_version } },
          });
        }}><FormField label="付款金额（人民币分）" htmlFor="seller-payment-amount"><TextInput id="seller-payment-amount" name="amount" inputMode="numeric" required /></FormField><Button className="danger" disabled={upload.state !== 'VERIFIED' || mutation.isPending}>确认记录卖家付款</Button></form>
        {mutation.isError ? <StaffPanelError error={mutation.error} retry={authority.canRetry() ? () => mutation.mutate(null) : () => { mutation.reset(); void Promise.all([summary.refetch(), payables.refetch(), payments.refetch()]); }} retryLabel={authority.canRetry() ? '重试原请求' : '刷新服务器事实'} /> : null}
      </Card> : <Alert tone="info">当前权限仅可查看结算事实，不能记录或分配付款。</Alert>}
      <Card className="staff-audit-collapsed"><p>完整付款、分配、冲正、幂等与审计事实由后端保存；页面只显示服务器返回或重新读取的事实。</p></Card>
    </aside>
  </>;
}

function Fact({ label, value }: { label: string; value: string }): React.JSX.Element {
  return <dl className="fact-row"><dt>{label}</dt><dd>{value}</dd></dl>;
}
