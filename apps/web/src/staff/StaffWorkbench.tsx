...
function OrderEvidenceActions({ value, refresh }: { value: StaffOrderEvidence; refresh: () => Promise<unknown> }): React.JSX.Element {
  ...
  const mutation = useMutation({ mutationFn: ({ action, body }: { action: 'approve' | 'request-changes'; body: unknown }) => {
    key.current ??= crypto.randomUUID(); return staffApi.mutateOrderEvidence(client, value.submission_id, action, body, key.current);
  }, onSuccess: () => { key.current = null; void refresh(); } });
  ...
  </form>{mutation.isError ? <PanelError error={mutation.error} retry={() => mutation.reset()} /> : null}</Card>;
}
...
function ReviewPanel({ id }: { id: string }): React.JSX.Element {
  ...
  const mutation = useMutation({ mutationFn: ({ action, body }: { action: 'approve' | 'reject' | 'request-changes'; body: unknown }) => {
    key.current ??= crypto.randomUUID(); return staffApi.mutateReview(client, id, action, body, key.current);
  }, onSuccess: () => { key.current = null; void query.refetch(); } });
  ...
  return <><ReviewFacts value={value} /><Card className="sensitive-action"><h3>评论正式确认</h3><form onSubmit={(event) => {
    ...
    mutation.mutate({ action, body: action === 'approve' ? { expected_version: value.version, ...(internal ? { internal_note: internal } : {}) }
    ...
    {mutation.isError ? <PanelError error={mutation.error} retry={() => mutation.reset()} /> : null}</Card></>;
}
...
function RefundPanel({ id }: { id: string }): React.JSX.Element {
  const client = useQueryClient(); const [uploader, upload] = useFileUpload(); const key = useRef<string | null>(null);
  ...
  const payment = useMutation({ mutationFn: (body: unknown) => { key.current ??= crypto.randomUUID(); return staffApi.recordRefundPayment(client, id, body, key.current); }, onSuccess: () => { key.current = null; void query.refetch(); } });
  const reversal = useMutation({ mutationFn: ({ paymentId, body }: { paymentId: string; body: unknown }) => { key.current ??= crypto.randomUUID(); return staffApi.reverseRefundPayment(client, id, paymentId, body, key.current); }, onSuccess: () => { key.current = null; void query.refetch(); } });
  ...
  return <><Card className="customer-visible"><h3>买家返款事实</h3><Fact label="订单号" value={value.order.amazon_order_number_normalized} /><Fact label="应返" value={formatCny(value.due_amount_cny_fen)} /><Fact label="已返净额" value={formatCny(value.net_paid_cny_fen)} /><Fact label="待返" value={formatCny(value.outstanding_amount_cny_fen)} /></Card>
    <Card className="internal-note"><h3>付款与冲正记录</h3>{value.payments.map((entry) => <section key={entry.payment_entry_id}><Fact label="付款" value={`${formatCny(entry.amount_cny_fen)} · ${formatShanghai(entry.paid_at)}`} />{entry.proofs.map((proof) => <StaffProtectedFileButton key={proof.file_object_id} reference={proof} label="查看返款凭证" />)}
      <form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); reversal.mutate({ paymentId: entry.payment_entry_id, body: { expected_version: value.version, amount_cny_fen: String(data.get('amount')), reversed_at: Date.now(), reason: String(data.get('reason')) } }); }}>
    ...
    <Card className="sensitive-action"><h3>记录买家返款</h3><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploader.start('staffBuyerRefundProof', [file]); }} />
      <p role="status">凭证状态：{upload.state}</p><form onSubmit={(event) => { event.preventDefault(); const file = upload.manifest?.files[0]; if (!file) return; const data = new FormData(event.currentTarget); const paidAt = Date.now(); const date = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(paidAt)); payment.mutate({ expected_version: value.version, amount_cny_fen: String(data.get('amount')), paid_at: paidAt, china_business_date: date, payment_channel: String(data.get('channel')), public_note: String(data.get('public_note') ?? ''), internal_note: String(data.get('internal_note') ?? ''), proof_files: [{ file_object_id: file.file_object_id, expected_file_version: file.file_version }] }); }}>
      ...
      </form>{payment.isError ? <PanelError error={payment.error} retry={() => payment.reset()} /> : null}{reversal.isError ? <PanelError error={reversal.error} retry={() => reversal.reset()} /> : null}</Card></>;
}
...
function SellerSettlementPanel({ organizationId, item }: { organizationId: string; item: StaffWorkItem }): React.JSX.Element {
  ...
  const mutation = useMutation({ mutationFn: ({ kind, paymentId, body }: { kind: 'record' | 'allocate' | 'reverse'; paymentId?: string; body: unknown }) => {
    key.current ??= crypto.randomUUID();
    if (kind === 'record') return staffApi.recordSellerPayment(client, organizationId, body, key.current);
    if (kind === 'allocate' && paymentId) return staffApi.allocateSellerPayment(client, paymentId, body, key.current);
    if (paymentId) return staffApi.reverseSellerPayment(client, paymentId, body, key.current);
    throw new Error('INVALID_SETTLEMENT_ACTION');
  }, onSuccess: () => { key.current = null; void Promise.all([summary.refetch(), payables.refetch(), payments.refetch()]); } });
  return <><Card className="customer-visible"><h3>卖家组织上下文</h3><Fact label="组织" value={organizationId} /><Fact label="店铺" value={item.store_id ?? '当前工作项未绑定店铺'} /><Fact label="Marketplace" value="以业务详情返回事实为准；韩国站不可用" /></Card>
    ...
    {payments.isError ? <PanelError error={payments.error} retry={() => { void payments.refetch(); }} /> : <Card className="internal-note"><h3>付款、分配与凭证</h3>{payments.data?.length === 0 ? <p>暂无付款事实。</p> : payments.data?.map((payment) => <section key={payment.payment_id}>
      ...
      {payment.status !== 'REVERSED' && payment.unallocated_amount_cny_fen !== '0' ? <form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ kind: 'allocate', paymentId: payment.payment_id, body: { payable_id: String(data.get('payable_id')), amount_cny_fen: String(data.get('amount')), expected_payment_version: payment.version } }); }}>
    ...
    <Card className="sensitive-action"><h3>记录卖家付款</h3><Alert tone="info">付款入账后再明确分配到本金或服务费；两类应结事实不会合并。</Alert><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploader.start('staffSellerSettlementProof', [file]); }} /><p role="status">凭证状态：{upload.state}</p>
      <form onSubmit={(event) => { event.preventDefault(); const file = upload.manifest?.files[0]; if (!file) return; const data = new FormData(event.currentTarget); mutation.mutate({ kind: 'record', body: { amount_cny_fen: String(data.get('amount')), paid_at: Date.now(), proof_file: { file_object_id: file.file_object_id, expected_file_version: file.file_version } } }); }}><FormField label="付款金额（人民币分）" htmlFor="seller-payment-amount"><TextInput id="seller-payment-amount" name="amount" inputMode="numeric" required /></FormField><Button className="danger" disabled={upload.state !== 'VERIFIED' || mutation.isPending}>确认记录卖家付款</Button></form>
      {mutation.isError ? <PanelError error={mutation.error} retry={() => mutation.reset()} /> : null}</Card></>;
}
...
function PanelError({ error, retry }: { error: unknown; retry: () => void }): React.JSX.Element {
  const requestId = isFrontendApiError(error) ? error.requestId : null;
  const hidden = isFrontendApiError(error) && error.httpStatus === 404;
  return <div role="alert" className="state"><h3>{hidden ? '资源不存在或当前无权访问' : '当前面板加载失败'}</h3><p>{hidden ? '为保护客户与组织信息，系统不会透露范围外资源。' : '其他面板仍可继续使用，请按需重试。'}</p><RequestIdDisplay requestId={requestId} /><Button className="secondary" onClick={retry}>重试</Button></div>;
}
