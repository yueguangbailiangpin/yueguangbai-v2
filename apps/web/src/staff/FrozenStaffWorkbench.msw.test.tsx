...
describe('canonical Frozen Staff workbench', () => {
  it('keeps the scoped queue usable when the selected detail is concealed', async () => {
    server.use(
      http.get(apiUrl('/api/staff/me/work-items'), () =>
        HttpResponse.json({
          data: { work_items: [staffTestWorkItem], next_cursor: null },
          meta: { request_id: 'queue-request' },
        }),
      ),
  ...
  it('does not infer a total when an opaque next cursor exists', async () => {
    server.use(
      http.get(apiUrl('/api/staff/me/work-items'), () =>
        HttpResponse.json({
          data: { work_items: [staffTestWorkItem], next_cursor: 'opaque-next' },
          meta: { request_id: 'queue-request' },
        }),
      ),
    );
  ...
  it.each([
        ...
        HttpResponse.json(
          {
            ...
            meta: { request_id: 'queue-forbidden' },
          },
          { status: 403 },
        ),
    ],
    [
      ...
      () =>
        HttpResponse.json({
          data: { work_items: 'not-an-array', next_cursor: null },
          meta: { request_id: 'queue-invalid-envelope' },
        }),
    ],
        ...
        data: {
          demand_review: {
            ...
            schedule: {
              ...
              theoretical_last_order_date: '2026-08-17',
              order_interval_days: 2,
              orders_per_run: 5,
              affected_reservation_count: 0,
              preview_hash: 'a'.repeat(64),
              change_reason: '首次发布需求',
              changed_by_staff_id: 'staff-1',
              created_at: 1_787_000_000_000,
            },
          },
        },
  ...
  it('clears retained selection and refetches the same queue when the trusted Staff authorization changes', async () => {
    ...
    const adapter = {
      ...staffTestAdapter(initialSession),
      readSession: async () => ({
        data: { session: currentSession },
        requestId: `session-v${currentSession.authorization_version}`,
      }),
    };
    server.use(
      http.get(apiUrl('/api/staff/me/work-items'), () => {
