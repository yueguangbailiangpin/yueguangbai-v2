...
import {
  BriefcaseBusiness, ChartNoAxesCombined, PackageSearch, Settings,
  Sparkles, UserCog, UserRound, UsersRound,
} from 'lucide-react';
...
export function StaffShell({children}:{children?:ReactNode}={}):React.JSX.Element{
  const session=useCurrentStaffSession();const location=useLocation();const role=session.role.code;
  ...
  return <IdentityShell identity="staff" className="staff-business-shell">
    <aside className="staff-sidebar">
      ...
      <nav className="staff-primary-nav" aria-label="员工工作台导航">
        {role!=='acquisition'?<NavLink to="/staff" end className={workQueue?'active':''} {...(workQueue?{'aria-current':'page' as const}:{})}><BriefcaseBusiness aria-hidden="true"/><span>工作队列</span></NavLink>:null}
        {(owner||role==='acquisition')?<NavLink to="/staff/acquisition"><Sparkles aria-hidden="true"/><span>客户开发</span></NavLink>:null}
        {(owner||role==='pre_sales')?<NavLink to="/staff/buyer-customers"><UsersRound aria-hidden="true"/><span>买家客户</span></NavLink>:null}
        {(owner||role==='seller_ops')?<NavLink to="/staff/seller-customers"><UsersRound aria-hidden="true"/><span>卖家客户</span></NavLink>:null}
        {mayProducts?<NavLink to="/staff/products"><PackageSearch aria-hidden="true"/><span>产品库</span></NavLink>:null}
        {owner?<NavLink to="/staff/admin-business-dashboard"><ChartNoAxesCombined aria-hidden="true"/><span>经营看板</span></NavLink>:null}
        {(owner||role==='seller_ops')&&session.permissions.includes('SELLER_MANAGE')?<NavLink to="/staff/seller-principal-rate-policies"><Settings aria-hidden="true"/><span>本金汇率策略</span></NavLink>:null}
        {owner&&session.permissions.includes('STAFF_MANAGE')?<NavLink to="/staff/access-management"><UserCog aria-hidden="true"/><span>员工管理</span></NavLink>:null}
      </nav>
      <div className="staff-sidebar-person"><UserRound aria-hidden="true"/><span><strong>{session.display_name}</strong><small>{session.role.display_name}</small></span></div>
    </aside>
