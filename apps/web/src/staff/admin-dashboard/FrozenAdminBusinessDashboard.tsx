...
export function FrozenAdminBusinessDashboard():React.JSX.Element{
  ...
  const business=summary.data;const value=acquisition.data;const hasUnattributed=value.totals.buyer_unattributed_orders>0||value.totals.seller_unattributed_orders>0;
  return <main className="admin-dashboard frozen-admin-dashboard">
    ...
    <section><h2>客户与订单概览</h2><div className="dashboard-metric-grid">
      ...
      <Metric label="买家网站注册" value={value.totals.buyer_portal_registrations} detail="新买家或历史买家实际开通网站账号"/>
      <Metric label="卖家网站开通" value={value.totals.seller_portal_registrations} detail="新卖家或历史卖家主账号实际开通"/>
    ...
    </div>{hasUnattributed?<Alert tone="warning">所选范围存在历史客户 / 来源未知订单：买家来源缺失 {value.totals.buyer_unattributed_orders} 单，卖家来源缺失 {value.totals.seller_unattributed_orders} 单。系统不会猜测到任何渠道。</Alert>:null}</section>
    <section><div className="dashboard-section-heading"><div><h2>每日新增</h2><p>新增客户与网站账号开通是两类独立事实。</p></div></div>
      {value.daily.length===0?<EmptyState title="暂无每日数据" description="所选时间范围没有新增客户或订单。"/>:<DataTable caption="每日新增客户、账号开通与订单"><thead><tr><th>日期</th><th>新增买家客户</th><th>新增卖家客户</th><th>买家网站注册</th><th>卖家网站开通</th><th>新增正式订单</th><th>买家来源未知</th><th>卖家来源未知</th></tr></thead><tbody>{[...value.daily].reverse().map((row)=><tr key={row.business_date}><td>{row.business_date}</td><td>{row.new_buyer_customers}</td><td>{row.new_seller_customers}</td><td>{row.buyer_portal_registrations}</td><td>{row.seller_portal_registrations}</td><td><strong>{row.formal_orders}</strong></td><td>{row.buyer_unattributed_orders}</td><td>{row.seller_unattributed_orders}</td></tr>)}</tbody></DataTable>}
    </section>
    <section><div className="dashboard-section-heading"><div><h2>每天各渠道新增</h2><p>同一张正式订单分别归到买家来源渠道和卖家来源渠道；全站订单总数仍只算一单。</p></div></div>
      {value.channel_daily.length===0?<EmptyState title="暂无渠道归因数据" description="新系统客户形成订单后，这里会按首次来源自动汇总；历史来源未知不会被猜测。"/>:<DataTable caption="每日渠道新增客户与新增订单"><thead><tr><th>日期</th><th>类型</th><th>员工渠道</th><th>真实渠道</th><th>平台</th><th>站点</th><th>新增客户</th><th>新增订单</th></tr></thead><tbody>{[...value.channel_daily].reverse().map((row)=><tr key={`${row.business_date}:${row.channel_id}:${row.lead_type}`}><td>{row.business_date}</td><td><StatusBadge tone={row.lead_type==='BUYER'?'processing':'success'}>{row.lead_type==='BUYER'?'买家':'卖家'}</StatusBadge></td><td>{row.channel_label}</td><td><strong>{row.channel_name}</strong></td><td>{row.platform_name}</td><td>{MARKETS[row.marketplace_code]??row.marketplace_code}</td><td>{row.new_customer_count}</td><td><strong>{row.formal_order_count}</strong></td></tr>)}</tbody></DataTable>}
    </section>
    <section className="dashboard-two-column" aria-label="转化漏斗"><Funnel title="买家转化" stages={business.buyer_funnel.stages}/><Funnel title="卖家转化" stages={business.seller_funnel.stages}/></section>
  </main>;
}
