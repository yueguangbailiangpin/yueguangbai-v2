...
import { Alert, Button, DataTable, Dialog, EmptyState, FormField, Select, StatusBadge, TextInput } from '../../ui/primitives';

const roleSchema=z.discriminatedUnion('code',[
  z.object({code:z.literal('owner'),display_name:z.literal('总管理员')}).strict(),z.object({code:z.literal('acquisition'),display_name:z.literal('获客')}).strict(),
  z.object({code:z.literal('pre_sales'),display_name:z.literal('售前')}).strict(),z.object({code:z.literal('seller_ops'),display_name:z.literal('卖家对接')}).strict(),
  z.object({code:z.literal('buyer_refund'),display_name:z.literal('买家返款')}).strict(),
]);
const employeeSchema=z.object({
  staff_id:z.string(),display_name:z.string(),email:z.string().nullable(),status:z.enum(['ACTIVE','DISABLED']),version:z.number().int().positive(),role:roleSchema,
  marketplace_codes:z.array(z.string()),marketplace_scopes:z.array(z.object({code:z.string(),scope_kind:z.enum(['PRIMARY','SUPPORT'])}).strict()).optional(),
  last_login_at:z.number().int().nonnegative().nullable(),updated_at:z.number().int().nonnegative(),
}).strict();
...
type Employee=z.output<typeof employeeSchema>;type Role=Employee['role']['code'];
...
export function StaffAccountsWorkspace():React.JSX.Element{
  const session=useCurrentStaffSession(),client=useQueryClient(),authorized=session.role.code==='owner'&&session.permissions.includes('STAFF_MANAGE');
  const [creating,setCreating]=useState(false),[editing,setEditing]=useState<Employee|null>(null),[disabling,setDisabling]=useState<Employee|null>(null);
...
function AccountDialog({title,open,employee,marketplaces,busy,onClose,onSubmit}:{title:string;open:boolean;employee:Employee|undefined;marketplaces:readonly {code:string;display_name:string;status:'ACTIVE'|'DISABLED'}[];busy:boolean;onClose:()=>void;onSubmit:(body:{display_name:string;email:string;role_code:Role;marketplace_codes:string[]})=>void}){
  ...
  return <Dialog open={open} title={title} description="一个员工只选一个岗位；站点可与同岗位其他员工重复，系统自动决定主负责人/协助。" busy={busy} onClose={onClose}><form onSubmit={submit} className="staff-account-form"><FormField label="员工姓名" htmlFor={`${title}-name`}><TextInput id={`${title}-name`} name="display_name" defaultValue={employee?.display_name??''} required/></FormField><FormField label="登录邮箱" htmlFor={`${title}-email`}><TextInput id={`${title}-email`} name="email" type="email" defaultValue={employee?.email??''} required/></FormField><FormField label="岗位" htmlFor={`${title}-role`}><Select id={`${title}-role`} value={role} onChange={(event)=>setRole(event.target.value as Role)}>{ROLES.map(([code,label])=><option key={code} value={code}>{label}</option>)}</Select></FormField>{role!=='owner'?<fieldset className="staff-marketplace-options"><legend>负责站点</legend>{marketplaces.filter((market)=>market.status==='ACTIVE').map((market)=><label key={market.code}><input type="checkbox" name="marketplace_codes" value={market.code} defaultChecked={employee?.marketplace_codes.includes(market.code)??market.code==='AMAZON_JP'}/><span>{market.display_name}</span></label>)}</fieldset>:<Alert tone="info">总管理员自动拥有全部站点。</Alert>}<div className="entry-actions"><Button type="button" className="secondary" onClick={onClose}>取消</Button><Button type="submit" loading={busy}>保存</Button></div></form></Dialog>;
}
...
function scopeLabels(employee:Employee,markets:readonly {code:string;display_name:string}[]){const scopes=employee.marketplace_scopes??employee.marketplace_codes.map((code)=>({code,scope_kind:'PRIMARY' as const}));return scopes.map((scope)=>`${marketLabel(markets,scope.code)} · ${scope.scope_kind==='PRIMARY'?'主负责人':'协助'}`).join(' / ')||'未配置';}
