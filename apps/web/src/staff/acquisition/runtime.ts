...
export const acquisitionInternalChannelViewSchema=acquisitionChannelSchema.extend({
  visibility:z.literal('INTERNAL'),staff_label:z.string(),intake_wechat_label:z.string().nullable(),profile_version:z.number().int().positive(),
}).strict();
export const acquisitionStaffChannelViewSchema=z.object({
  visibility:z.literal('STAFF'),channel_id:z.string(),staff_label:z.string(),
  lead_type:z.enum(['BUYER','SELLER','BOTH']),marketplace_code:z.string(),
  status:z.enum(['ACTIVE','DISABLED']),version:z.number().int().positive(),
}).strict();
export const acquisitionVisibleChannelSchema=z.discriminatedUnion('visibility',[
  acquisitionInternalChannelViewSchema,acquisitionStaffChannelViewSchema,
]);
export const acquisitionChannelsResponseSchema=z.object({channels:z.array(acquisitionVisibleChannelSchema)}).strict();
...
export const acquisitionHandoffSchema=z.object({
  prospect_id:z.string(),lead_type:acquisitionLeadTypeSchema,marketplace_code:z.string(),
  origin_channel_id:z.string(),channel_label:z.string(),display_name:z.string(),
  contact_value:z.string().nullable(),status:z.literal('HUMAN_HANDOFF'),
...
export const acquisitionLeadSchema=z.object({
  ...
  origin_channel_id:z.string(),channel_label:z.string(),current_owner_staff_id:z.string(),
  ...
  formal_order_count:z.number().int().nonnegative(),seller_cooperation:z.boolean(),created_at:epoch,updated_at:epoch,
}).strict();
