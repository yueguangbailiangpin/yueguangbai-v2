...
import {
  acquisitionChannelSchema,acquisitionChannelsResponseSchema,acquisitionConsultationsResponseSchema,acquisitionFunnelResponseSchema,
  acquisitionHandoffSchema,acquisitionInternalChannelViewSchema,acquisitionLeadSchema,acquisitionLeadsPageSchema,
  acquisitionProspectDetailSchema,acquisitionProspectSchema,acquisitionProspectsPageSchema,acquisitionProspectSignalSchema,
} from './runtime';
...
const channelStat=z.object({
  channel_id:z.string(),channel_name:z.string(),platform_name:z.string(),channel_status:z.enum(['ACTIVE','DISABLED']),lead_type:z.enum(['BUYER','SELLER','BOTH']),marketplace_code:z.string(),consultation_count:z.number().int().nonnegative().nullable(),consultation_data_complete:z.boolean(),consultation_days_recorded:z.number().int().nonnegative(),consultation_days_expected:z.number().int().nonnegative(),prospect_count:z.number().int().nonnegative(),codex_prospect_count:z.number().int().nonnegative(),lead_count:z.number().int().nonnegative(),registered_count:z.number().int().nonnegative(),reservation_submitted_count:z.number().int().nonnegative(),cooperation_count:z.number().int().nonnegative(),formal_order_count:z.number().int().nonnegative(),buyer_formal_order_count:z.number().int().nonnegative(),seller_formal_order_count:z.number().int().nonnegative(),buyer_projected_gross_profit_cny_fen:z.string().nullable(),buyer_completed_gross_profit_cny_fen:z.string().nullable(),seller_projected_gross_profit_cny_fen:z.string().nullable(),seller_completed_gross_profit_cny_fen:z.string().nullable(),
}).strict();
const channelStatsSchema=z.object({channels:z.array(channelStat)}).strict();
const consultationMutation=z.object({consultation:z.object({consultation_id:z.string(),channel_id:z.string(),lead_type:z.enum(['BUYER','SELLER']),business_date:z.string(),person_count:z.number().int().nonnegative(),version:z.number().int().positive(),updated_by_staff_id:z.string(),updated_at:z.number().int().nonnegative()}).strict(),replayed:z.boolean()}).strict();
const reportingConfig=z.object({precision_started_business_date:z.string().nullable(),activated_at:z.number().int().nonnegative().nullable(),activated_by_staff_id:z.string().nullable(),version:z.number().int().positive(),updated_at:z.number().int().nonnegative()}).strict();
...
export const acquisitionApi=Object.freeze({
  ...
  prospects:(client:QueryClient,input:{leadType:string|null;status:string|null;cursor:string|null},signal?:AbortSignal)=>{const query=new URLSearchParams({limit:'50'});if(input.leadType)query.set('lead_type',input.leadType);if(input.status)query.set('status',input.status);if(input.cursor)query.set('cursor',input.cursor);return read(client,`/api/staff/acquisition/prospects?${query}`,acquisitionProspectsPageSchema,signal);},
  ...
  reportingConfig:(client:QueryClient,signal?:AbortSignal)=>read(client,'/api/staff/acquisition/reporting-config',reportingConfigEnvelope,signal),
  activateReportingConfig:(client:QueryClient,body:unknown,key:string)=>write(client,'/api/staff/acquisition/reporting-config/activate',body,reportingConfigEnvelope,key),
  sourceCorrectionCandidates:(client:QueryClient,signal?:AbortSignal)=>read(client,'/api/staff/acquisition/source-corrections/candidates?limit=100',correctionCandidatesEnvelope,signal),
  correctSource:(client:QueryClient,body:unknown,key:string)=>write(client,'/api/staff/acquisition/source-corrections',body,sourceCorrectionEnvelope,key),
  machines:(client:QueryClient,signal?:AbortSignal)=>read(client,'/api/staff/acquisition/machines',machinesEnvelope,signal),
  createMachine:(client:QueryClient,body:unknown,key:string)=>write(client,'/api/staff/acquisition/machines',body,machineCreatedEnvelope,key),
  revokeMachine:(client:QueryClient,id:string,key:string)=>write(client,`/api/staff/acquisition/machines/${encodeURIComponent(id)}/revoke`,{},machineRevokedEnvelope,key),
});
