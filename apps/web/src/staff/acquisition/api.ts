...
const channelStat=z.object({
  channel_id:z.string(),channel_name:z.string(),platform_name:z.string(),channel_status:z.enum(['ACTIVE','DISABLED']),
  lead_type:z.enum(['BUYER','SELLER','BOTH']),marketplace_code:z.string(),consultation_count:z.number().int().nonnegative().nullable(),
  consultation_data_complete:z.boolean(),consultation_days_recorded:z.number().int().nonnegative(),consultation_days_expected:z.number().int().nonnegative(),
  prospect_count:z.number().int().nonnegative(),codex_prospect_count:z.number().int().nonnegative(),lead_count:z.number().int().nonnegative(),
  registered_count:z.number().int().nonnegative(),reservation_submitted_count:z.number().int().nonnegative(),cooperation_count:z.number().int().nonnegative(),
  formal_order_count:z.number().int().nonnegative(),buyer_formal_order_count:z.number().int().nonnegative(),seller_formal_order_count:z.number().int().nonnegative(),
  buyer_projected_gross_profit_cny_fen:z.string().nullable(),buyer_completed_gross_profit_cny_fen:z.string().nullable(),
  seller_projected_gross_profit_cny_fen:z.string().nullable(),seller_completed_gross_profit_cny_fen:z.string().nullable(),
}).strict();
...
const reportingConfig=z.object({
  precision_started_business_date:z.string().nullable(),activated_at:z.number().int().nonnegative().nullable(),
  activated_by_staff_id:z.string().nullable(),version:z.number().int().positive(),updated_at:z.number().int().nonnegative(),
}).strict();
const reportingConfigEnvelope=z.object({config:reportingConfig}).strict();
const sourceCorrectionCandidate=z.object({
  lead_id:z.string(),lead_type:z.enum(['BUYER','SELLER']),marketplace_code:z.string(),business_date:z.string(),display_name:z.string().nullable(),
  wechat_masked:z.string(),original_channel_id:z.string(),original_channel_name:z.string(),effective_channel_id:z.string(),effective_channel_name:z.string(),
...
const sourceCorrectionEnvelope=z.object({correction:z.object({
  correction_id:z.string(),lead_id:z.string(),previous_channel_id:z.string(),new_channel_id:z.string(),new_channel_name:z.string(),
  reason:z.string(),corrected_at:z.number().int().nonnegative(),
}).strict()}).strict();
