...
function CustomerIntakeWorkspace({leadType}:{leadType:'BUYER'|'SELLER'}):React.JSX.Element{
  ...
  return <main className="customer-intake-workspace">
    <header className="staff-customer-heading"><div><p className="eyebrow">{buyer?'买家客户接入':'卖家客户接入'}</p><h2>{buyer?'买家客户':'卖家客户'}</h2>
      <p>先查询微信是否属于历史客户；新客户成功保存后立即计入当日新增，并可在同一位置生成注册链接。</p></div></header>
    ...
    <div className="customer-intake-grid">
      ...
      <Card className="customer-intake-list"><h3>正式{buyer?'买家':'卖家'}线索</h3>
        {leads.isPending?<p role="status">正在加载</p>:leads.isError?<Alert tone="danger">客户线索暂时无法加载。</Alert>:leads.data.items.length===0?<EmptyState title={`暂无${buyer?'买家':'卖家'}线索`} description="加微信后在左侧建立正式线索。"/>:<DataTable caption={`${buyer?'买家':'卖家'}线索与业务进度`}><thead><tr><th>客户</th><th>站点</th><th>渠道</th><th>{buyer?'业务进度':'合作状态'}</th></tr></thead><tbody>
          {leads.data.items.map((lead)=><tr key={lead.lead_id}><td><strong>{lead.display_name??lead.wechat_masked}</strong><small>{lead.wechat_masked}</small></td><td>{marketLabel(lead.marketplace_code)}</td><td><StatusBadge tone="neutral">{lead.channel_label}</StatusBadge></td><td>{buyer?`${lead.registered?'网站已开通':'网站未开通'} · ${lead.formal_order_count} 单`:lead.seller_cooperation?'已建立卖家组织':'待开通卖家账号'}</td></tr>)}
        </tbody></DataTable>}
      </Card>
    </div>
    {buyer?<Card className="buyer-security-tools"><h3>已有账号恢复</h3><p>这里只处理已经开通过网站账号的买家忘记密码；新客户和历史客户首次开通都在上方完成。</p><StaffCustomerSecurityPanel showInvitationIssuer={false}/></Card>:null}
  </main>;
}
...
function HistoricalCustomerOnboarding({leadType}:{leadType:'BUYER'|'SELLER'}){
  ...
  const invite=useMutation({mutationFn:(match:HistoricalMatch)=>buyer
    ?issueBuyerInvite(client,wechat,match.marketplace_code)
  ...
  return <Card className="historical-customer-onboarding"><div className="staff-section-toolbar"><div><h3>历史客户 / 已有客户查询</h3><p>先查微信，避免把历史客户重复新增。</p></div><StatusBadge tone="neutral">不计新增客户</StatusBadge></div>
    ...
    {matches.map((match)=><div className="historical-customer-result" key={`${match.customer_type}:${match.subject_id}`}><div><strong>{match.display_name}</strong><p>{marketLabel(match.marketplace_code)} · 历史订单 {match.historical_order_count} 单 · 历史客户 / 来源未知</p></div>
      {match.has_portal_account?<StatusBadge tone="success">账号已开通</StatusBadge>:<Button loading={invite.isPending} onClick={()=>invite.mutate(match)}>{buyer?'开通网站账号':'开通卖家账号'}</Button>}</div>)}
...
function LeadCreateCard({leadType,channels,handoffs}:{leadType:'BUYER'|'SELLER';channels:readonly AcquisitionChannel[];handoffs:readonly AcquisitionHandoff[]}){
  ...
  const [saved,setSaved]=useState<SavedLead|null>(null);const [link,setLink]=useState<string|null>(null);
  ...
  const mutation=useMutation({mutationFn:(input:{body:unknown;draft:Omit<SavedLead,'leadId'>})=>acquisitionApi.createLead(client,input.body,crypto.randomUUID()).then((response)=>({response,draft:input.draft})),
    onSuccess:({response,draft})=>{setSaved({leadId:response.data.lead.lead_id,...draft});setLink(null);void client.invalidateQueries({queryKey:['staff','customer-intake']});}});
  const invite=useMutation({mutationFn:(value:SavedLead)=>buyer?issueBuyerInvite(client,value.wechatId,value.marketplaceCode):issueSellerInvite(client,{leadId:value.leadId,sellerOrganizationId:null,wechatId:value.wechatId,marketplaceCode:value.marketplaceCode}),
    onSuccess:(response)=>setLink(`${window.location.origin}${response.data.invitation.registration_path}`)});
  function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setSaved(null);setLink(null);const form=event.currentTarget,data=new FormData(form);const selected=handoffs.find((item)=>item.prospect_id===String(data.get('handoff_id')))??null;
    const marketplaceCode=selected?.marketplace_code??String(data.get('marketplace_code'));const wechatId=String(data.get('wechat_id'));const displayName=String(data.get('display_name')??'').trim()||wechatId;
    mutation.mutate({body:{lead_type:leadType,marketplace_code:marketplaceCode,channel_id:selected?.origin_channel_id??String(data.get('channel_id')),prospect_id:selected?.prospect_id??null,wechat_id:wechatId,display_name:nullable(data.get('display_name')),note:nullable(data.get('note'))},draft:{wechatId,marketplaceCode,displayName}},{onSuccess:()=>{form.reset();setProspectId('');}});}
  return <Card className="customer-intake-create"><h3>新{buyer?'买家':'卖家'}客户</h3><p>保存成功即计入当天新增客户；账号开通是后续独立步骤。</p>
    <form onSubmit={submit}>
      {handoffs.length>0?<FormField label="待接入客户（可选）" htmlFor={`${leadType}-handoff`} description="获客岗位交接的客户会自动继承渠道编号"><Select id={`${leadType}-handoff`} name="handoff_id" value={prospectId} onChange={(event)=>setProspectId(event.target.value)}><option value="">不是交接客户</option>{handoffs.map((item)=><option key={item.prospect_id} value={item.prospect_id}>{item.display_name} · {item.channel_label}</option>)}</Select></FormField>:<input type="hidden" name="handoff_id" value=""/>}
      ...
      <Button loading={mutation.isPending} loadingLabel="正在保存">保存新{buyer?'买家':'卖家'}客户</Button>
      {mutation.isError?<Alert tone="danger">保存未完成。如果这个微信属于历史客户，请使用上方“历史客户 / 已有客户查询”。</Alert>:null}
    </form>
...
async function issueBuyerInvite(client:ReturnType<typeof useQueryClient>,wechatId:string,marketplaceCode:string){
  const body={wechat_id:wechatId,marketplace_code:marketplaceCode};return identityApiRequest('staff',client,{path:'/api/staff/customer-security/buyer-invitations',method:'POST',schema:buyerInvitationSchema,body,headers:operationHeaders({key:crypto.randomUUID(),body})});
}
