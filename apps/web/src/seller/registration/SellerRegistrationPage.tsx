...
const invitationEnvelope=z.object({data:z.object({invitation:z.object({
  ...
  onboarding_kind:z.enum(['NEW_CUSTOMER','HISTORICAL_ACCOUNT_ONLY']),expires_at:z.number().int(),
...
export function SellerRegistrationPage():React.JSX.Element{
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const data=new FormData(event.currentTarget);const parsed=formSchema.safeParse({wechat_id:data.get('wechat_id'),password:data.get('password'),password_confirmation:data.get('password_confirmation')});
    if(!parsed.success){setMessage('请检查微信号和密码。密码至少为 12 位，两次输入必须一致。');return;}
    ...
    }catch{setMessage('账号开通未完成。请确认微信号与工作人员登记的一致，或联系工作人员重新生成链接。');}finally{setBusy(false);}}
  return <main className="login-page identity-seller seller-registration-page"><Card className="login-card seller-login-card seller-registration-card">
    ...
    {invitation?<Alert tone="info"><strong>{invitation.seller_name}</strong><br/>邀请微信：{invitation.wechat_hint}<br/>{invitation.onboarding_kind==='HISTORICAL_ACCOUNT_ONLY'?'历史卖家：只开通登录账号，原店铺、产品和订单保持不变。':'新卖家：完成后进入卖家专业控制台。'}</Alert>:null}
    <form onSubmit={(event)=>{void submit(event);}}><FormField label="微信号" htmlFor="seller-register-wechat" required><TextInput id="seller-register-wechat" name="wechat_id" autoComplete="username" required/></FormField>
      <FormField label="密码" htmlFor="seller-register-password" description="至少 12 位" required><TextInput id="seller-register-password" name="password" type="password" autoComplete="new-password" minLength={12} required/></FormField>
      <FormField label="确认密码" htmlFor="seller-register-confirm" required><TextInput id="seller-register-confirm" name="password_confirmation" type="password" autoComplete="new-password" minLength={12} required/></FormField>
      {message?<Alert tone="danger">{message}</Alert>:null}<Button type="submit" disabled={!invitation} loading={busy} loadingLabel="正在开通">完成开通</Button></form>
