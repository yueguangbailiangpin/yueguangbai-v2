...
export const sellerStoresSchema = z.object({ items: z.array(z.object({
  ...
  canonical_marketplace_code: z.enum(['AMAZON_JP', 'AMAZON_US', 'COUPANG_KR', 'RAKUTEN_JP', 'TIKTOK_JP']),
...
const canonicalMarketplace = z.enum(['AMAZON_JP', 'AMAZON_US', 'COUPANG_KR', 'RAKUTEN_JP', 'TIKTOK_JP']);
const sellerFormalOrderCommon = {
  formal_order_id: z.string(), status: z.literal('CONFIRMED'),
  platform_order_identifier: z.string(),
  store: z.object({ id: z.string(), display_name: z.string() }).strict(),
  platform_product_identifier: z.string(), product_name: z.string(),
  chat_screenshot: z.object({ status: z.enum(['AVAILABLE', 'NONE']), file_version: z.number().int().positive().nullable() }).strict(),
  confirmed_at: epoch,
} as const;
const sellerAmazonFormalOrderSchema = z.object({
  ...sellerFormalOrderCommon,
  legacy_projection: z.literal('AMAZON'), marketplace_code: z.literal('JP'),
  canonical_marketplace_code: z.enum(['AMAZON_JP', 'AMAZON_US']),
  amazon_order_number: z.string(), asin: z.string(),
...
const sellerPlatformFormalOrderSchema = z.object({
  ...sellerFormalOrderCommon,
  legacy_projection: z.literal('NONE'), marketplace_code: z.null(),
  canonical_marketplace_code: z.enum(['RAKUTEN_JP', 'TIKTOK_JP']),
  amazon_order_number: z.null(), asin: z.null(), product_version: z.null(),
...
export const sellerFormalOrdersSchema = z.object({
  items: z.array(z.discriminatedUnion('legacy_projection', [
