...
export function SellerProductApplicationFormPage(): React.JSX.Element {
  ...
  return <section className="seller-page"><PageHeader title="提交产品申请" description="提交后由工作人员审核。图片仅用于本次申请。" />
    <Card>{me.isPending || stores.isPending ? <p role="status">正在核验可提交范围</p> : me.isError || stores.isError ? <><Alert tone="danger">暂时无法读取店铺和权限，请刷新后重试。</Alert><Button className="secondary" onClick={() => { void Promise.all([me.refetch(), stores.refetch()]); }}>重新读取</Button></> : !me.data?.access.can_submit_product_applications ? <Alert tone="warning">当前账号无权提交产品申请。</Alert> : <form className="buyer-form" onSubmit={(event) => { void submit(event); }}>
      ...
      <Button type="submit" loading={mutation.isPending || uploadBusy} loadingLabel={mutation.isPending ? '正在提交' : '正在上传图片'}>提交申请</Button>
    </form>}</Card>
  </section>;
}
...
export function SellerDemandFormPage(): React.JSX.Element {
  ...
  return <section className="seller-page"><PageHeader title="提交需求" description="每次追加数量都会新建一个需求批次，不会修改历史记录。" /><Card>{me.isPending || products.isPending ? <p role="status">正在核验可提交产品</p> : me.isError || products.isError ? <><Alert tone="danger">暂时无法读取产品和权限，请刷新后重试。</Alert><Button className="secondary" onClick={() => { void Promise.all([me.refetch(), products.refetch()]); }}>重新读取</Button></> : !me.data?.access.can_submit_demand_batches ? <Alert tone="warning">当前账号无权提交需求。</Alert> : <form className="buyer-form" onSubmit={submit}>
    ...
    {message ? <Alert tone="danger">{message}</Alert> : null}<BuyerMutationRecovery mutation={mutation} onRefresh={() => { void Promise.all([me.refetch(), products.refetch()]); }} /><Button type="submit" loading={mutation.isPending}>提交需求</Button>
  </form>}</Card></section>;
}
