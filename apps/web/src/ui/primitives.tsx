...
export function Dialog({ open, title, description, busy = false, onClose, children }: PropsWithChildren<{ open: boolean; title: string; description: string; busy?: boolean; onClose: () => void }>) {
  const dialog = useRef<HTMLDivElement>(null); const opener = useRef<HTMLElement | null>(null); const closeRef = useRef(onClose); const busyRef = useRef(busy); const titleId = useId(); const descriptionId = useId(); closeRef.current = onClose; busyRef.current = busy;
  useEffect(() => { if (!open) return; opener.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; const focus = (): void => { const first = dialog.current?.querySelector<HTMLElement>('button:not([disabled]),input:not([disabled])'); (first ?? dialog.current)?.focus(); }; focus(); const key = (event: KeyboardEvent) => { if (event.key === 'Escape' && !busyRef.current) closeRef.current(); if (event.key === 'Tab' && dialog.current) { const nodes = Array.from(dialog.current.querySelectorAll<HTMLElement>('button:not([disabled]),input:not([disabled])')); const firstNode = nodes[0]; const lastNode = nodes[nodes.length - 1]; if (!firstNode || !lastNode) { event.preventDefault(); dialog.current.focus(); } else if ((event.shiftKey && document.activeElement === firstNode) || (!event.shiftKey && document.activeElement === lastNode)) { event.preventDefault(); (event.shiftKey ? lastNode : firstNode).focus(); } } }; window.addEventListener('keydown', key); return () => { window.removeEventListener('keydown', key); opener.current?.focus(); }; }, [open]);
  if (!open) return null;
  return <div className="overlay" role="presentation"><div className="drawer" ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId}><h2 id={titleId}>{title}</h2><p id={descriptionId}>{description}</p>{children}</div></div>;
}
