...
describe('Wave 14A foundation policy', () => {
  it('accepts only origin-relative formal API paths and bounded query values', () => {
    expect(approvedApiPath('/api/customer-auth/session')).toBe(true);
    expect(approvedApiPath('/api/buyer-portal/demands?limit=20&cursor=safe')).toBe(true);
    expect(approvedApiPath('/api/v2/session')).toBe(false);
    expect(approvedApiPath('https://example.test/api/x')).toBe(false);
