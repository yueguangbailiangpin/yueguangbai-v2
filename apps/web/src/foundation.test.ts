...
describe('Wave 14A foundation policy', () => {
  ...
  it('bounds Retry-After and semantic retry policy', () => { expect(retryAfterMilliseconds('3')).toBe(3000); expect(retryAfterMilliseconds('61')).toBeNull(); expect(shouldRetryQuery(0, new FrontendApiError('FORBIDDEN', 403, 'r', 'PERMISSION'))).toBe(false); });
