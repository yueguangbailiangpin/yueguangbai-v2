...
import { Button, Card, DependencyUnavailable, Dialog, Drawer, ErrorState, LoadingState, NotFound, PageHeader, PermissionDenied, RequestIdDisplay, TextInput } from './ui/primitives';
...
function CustomerLogin({ target }: { target: CustomerTarget }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.session.password_change_required) { navigate(`/${target}/change-password`, { replace: true }); return; }
...
function ChangePassword({ target }: { target: CustomerTarget }) {
  const navigate = useNavigate(); const client = useQueryClient(); const [message, setMessage] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const current = String(form.get('current_password') ?? ''); const next = String(form.get('new_password') ?? ''); const confirm = String(form.get('confirm_password') ?? ''); if (!current || !next || next !== confirm) { setMessage('请完整填写并确认新密码。'); return; } setBusy(true); try { const response = await apiRequest({ path: '/api/customer-auth/change-password', method: 'POST', schema: z.object({ session: customerSessionSchema }).strict(), body: { current_password: current, new_password: next }, headers: { 'Idempotency-Key': crypto.randomUUID() } }); await CUSTOMER_TRANSPORT_INVALIDATION_GROUP.clear(client); if (response.data.session.account_type === (target === 'buyer' ? 'BUYER' : 'SELLER_MEMBER')) { client.setQueryData(queryKeys[target].session, response.data.session); navigate(`/${target}`, { replace: true }); } } catch { setMessage('修改密码未完成，请稍后重试。'); } finally { setBusy(false); } }
  return <main className="login-page"><Card><h1>{target === 'buyer' ? '买家修改密码' : '卖家修改密码'}</h1><form onSubmit={submit}><label>当前密码<TextInput name="current_password" type="password" autoComplete="current-password" required /></label><label>新密码<TextInput name="new_password" type="password" autoComplete="new-password" required /></label><label>确认新密码<TextInput name="confirm_password" type="password" autoComplete="new-password" required /></label>{message && <p className="inline-error" role="alert">{message}</p>}<Button type="submit" disabled={busy}>{busy ? '正在提交' : '修改密码'}</Button></form></Card></main>;
}
