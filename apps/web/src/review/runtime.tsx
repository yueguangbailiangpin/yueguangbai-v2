...
export function currentSellerReviewRole(): SellerReviewRole {
  return sellerRole;
}
...
function customerSession(target: CustomerTarget): CustomerSession {
  return {
    account_id: `review-${target}-account`,
    identity_subject_id: 'review-customer-multi-persona',
    account_type: target === 'buyer' ? 'BUYER' : 'SELLER_MEMBER',
    available_personas: ['BUYER', 'SELLER_MEMBER'],
    session_version: 1,
    password_change_required: false,
    issued_at: 1_786_368_000_000,
...
export function reviewCustomerAuthApi(target: CustomerTarget): CustomerAuthApiAdapter {
  const result = () =>
    Promise.resolve({
      data: { session: customerSession(target) },
      requestId: `review-${target}-session`,
    });
  return Object.freeze({
    ...
    logout: () =>
      Promise.resolve({
        data: { logged_out: true as const, all_devices_logged_out: false as const },
        requestId: 'review-customer-logout',
      }),
  });
}
...
export function reviewStaffSession(): StaffSession {
  ...
  return {
    ...
    authorization_version: STAFF_REVIEW_ROLES.indexOf(staffRole) + 1,
    session_version: 1,
    expires_at: 4_102_444_800_000,
  };
}
...
export const reviewStaffAuthApi: StaffAuthApiAdapter = Object.freeze({
  readSession: () =>
    Promise.resolve({ data: { session: reviewStaffSession() }, requestId: 'review-staff-session' }),
  bootstrap: () =>
    Promise.resolve({
      data: { session: reviewStaffSession(), access_email: 'review@example.invalid' },
      requestId: 'review-staff-bootstrap',
    }),
  logout: () =>
    Promise.resolve({
      data: { logged_out: true as const, all_devices_logged_out: false as const },
      requestId: 'review-staff-logout',
    }),
  logoutAll: () =>
    Promise.resolve({
      data: {
        logged_out: true as const,
        all_devices_logged_out: true as const,
        session_version: 2,
      },
      requestId: 'review-staff-logout-all',
    }),
});
...
export function ReviewRuntimeProvider({ children }: { children: ReactNode }): React.JSX.Element {
  ...
  const value = useMemo<ReviewRuntimeValue>(
    () => ({
      revision,
      sellerRole: selectedSellerRole,
      staffRole: selectedStaffRole,
      chooseSellerRole: (role) => {
        sellerRole = role;
        setSelectedSellerRole(role);
        client.clear();
  ...
  return (
    <div className="review-runtime">
      <header className="review-mode-bar">
        <Link to="/" className="review-mode-label">
          前端评审 · Demo 数据
        </Link>
        {seller ? (
          <label>
            ...
            <Select
              aria-label="卖家评审角色"
              value={runtime.sellerRole}
              onChange={(event) => runtime.chooseSellerRole(event.target.value as SellerReviewRole)}
            >
              {SELLER_REVIEW_ROLES.map((role) => (
        ...
        {staff ? (
          <label>
            ...
            <Select
              aria-label="员工评审角色"
              value={runtime.staffRole}
              onChange={(event) => {
                runtime.chooseStaffRole(event.target.value as StaffReviewRole);
                void navigate('/staff');
              }}
            >
              {STAFF_REVIEW_ROLES.map((role) => (
                <option key={role}>{role}</option>
              ))}
            </Select>
          </label>
      ...
      <div key={`${runtime.revision}:${seller ? 'seller' : staff ? 'staff' : 'buyer'}`}>
        {children}
      </div>
    </div>
  );
