...
-- Wave 12 / Phase 3M: immutable audit metadata for synchronous CSV exports.
INSERT INTO transaction_assertions (assertion_value)
SELECT CASE WHEN EXISTS (
  SELECT 1 FROM app_schema_state
  WHERE singleton_id=1 AND schema_version=25
) THEN 1 ELSE 0 END;
...
SELECT CASE WHEN
  EXISTS (SELECT 1 FROM sqlite_master
    WHERE type='table' AND name='financial_export_events')
  AND EXISTS (SELECT 1 FROM sqlite_master
    WHERE type='trigger' AND name='trg_financial_export_events_no_update')
  AND EXISTS (SELECT 1 FROM sqlite_master
    WHERE type='trigger' AND name='trg_financial_export_events_no_delete')
THEN 1 ELSE 0 END;
