...
-- Wave 11 / Phase 3K: seller payments, allocations and append-only reversals.
INSERT INTO transaction_assertions (assertion_value)
SELECT CASE WHEN EXISTS (
  SELECT 1 FROM app_schema_state
  WHERE singleton_id=1 AND schema_version=23
) THEN 1 ELSE 0 END;
...
DROP VIEW seller_payable_balances;
...
INSERT INTO transaction_assertions (assertion_value)
SELECT CASE WHEN
  EXISTS (SELECT 1 FROM sqlite_master WHERE type='table' AND name='seller_payments')
  AND EXISTS (SELECT 1 FROM sqlite_master WHERE type='table' AND name='seller_payment_allocations')
  AND EXISTS (SELECT 1 FROM sqlite_master WHERE type='table' AND name='seller_payment_allocation_reversals')
  AND EXISTS (SELECT 1 FROM sqlite_master WHERE type='table' AND name='seller_payment_reversals')
  AND EXISTS (SELECT 1 FROM sqlite_master WHERE type='view' AND name='seller_payment_balances')
  AND EXISTS (SELECT 1 FROM sqlite_master WHERE type='view' AND name='seller_payable_balances')
THEN 1 ELSE 0 END;
