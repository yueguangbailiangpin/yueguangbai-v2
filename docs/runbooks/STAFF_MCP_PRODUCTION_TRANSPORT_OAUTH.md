...
当前只允许本地匿名检查。禁止登录或调用真实 OpenAI/ChatGPT/OAuth/Cloudflare，禁止部署、域名/DNS、Secret、远程 Migration、生产数据和任何真实资源写入。checked-in 模板不可部署。
...
production activation 要求 `STAFF_MCP_CLEANUP_ENABLED=true`，但 checked-in staging/production/local 模板均为 false。启用后每个 `/mcp` 请求在认证前执行一次有界清理：
