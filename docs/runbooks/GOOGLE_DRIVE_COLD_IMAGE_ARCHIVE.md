...
发布前导出完整 D1，并分别保存 R2 清单与 Drive Manifest。当前候选的隔离恢复演练必须验证连续 `0001`–`0037`、schema 37、0032 引入的归档表与约束、Manifest 数量/哈希、已归档文件 Drive 可读、已回灌对象 R2 HEAD 一致、所有原 Audience 仍有效。未来 Migration 推进后必须使用当时重新核验的连续末号，不得继续硬编码 37。生产 D1、R2、Drive 的真实写入不属于本地验收。
