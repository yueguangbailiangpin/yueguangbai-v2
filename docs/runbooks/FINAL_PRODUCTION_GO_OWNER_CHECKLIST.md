...
- [ ] 确认工作树干净、无未审查变更、无 open PR、依赖 audit 0、`0001`–`0037` 连续且尾部为 `0037_stage75_multimarket_staff_order_list_index.sql`、全量本地/Chromium/OpenSpec 门禁通过。
...
3. [ ] 只读比较线上 ledger 与 release SHA 的完整 `0001`–`0037` 链（尾部为 `0037_stage75_multimarket_staff_order_list_index.sql`）；线上可以是该链的连续前缀，但发现未知、跳号、重复、并行或部分 Migration 立即停止。
