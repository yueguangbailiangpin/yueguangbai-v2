...
| G3 | Production database migration + backup | STAGING / LOCAL | T10 备份/恢复证据（隔离 staging：bundle + attestation + restored.sqlite，2026-08-16，Production accessed: false）；本地 `db:verify` 40 migrations / Schema 40 | NOT VERIFIED | 无生产 D1 ledger 只读核验、无迁移窗口、无 release-bound 生产备份与隔离恢复证据 |
...
- [ ] 确认工作树干净、无未审查变更、无 open PR、依赖 audit 0、`0001`–`0040` 连续且尾部为 `0040_owner_seed_yueguangbai_channel.sql`、全量本地/Chromium/OpenSpec 门禁通过。
...
3. [ ] 只读比较线上 ledger 与 release SHA 的完整 `0001`–`0040` 链（尾部为 `0040_owner_seed_yueguangbai_channel.sql`）；线上可以是该链的连续前缀，但发现未知、跳号、重复、并行或部分 Migration 立即停止。
