...
本任务不执行部署。经最终批准后的顺序必须是：确认 release SHA/配置快照/备份可恢复 → 保持外部开关关闭 → 只读核验线上 Migration ledger → 按 `0001`–`0038` 连续顺序应用获批且尚未应用的 Migration → 部署 schema-compatible Worker → 匿名 smoke → 分阶段启用 Provider/Job。Migration、部署、Scheduler、Drive delete、Provider 和 Production GO 必须分别批准；不得因为仓库当前末号为 `0038` 就推断线上已应用到 `0038`。
