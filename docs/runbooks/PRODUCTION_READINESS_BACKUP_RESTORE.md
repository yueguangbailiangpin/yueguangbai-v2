...
M10 当时不增加业务事实、权限、状态机或审计表，因此没有为发布证据创建 Migration；本最终本地准备 Change 同样不创建新 Migration。备份 Manifest 继续保存在加密外部包中，Git 只保存匿名发布证据。后续业务 Change 已把当前仓库 Migration 链推进为连续 `0001`–`0042`，当前候选的 `app_schema_state.schema_version=42`。乐天与 TikTok 日本站基础模块由 Migration 0042 追加平台注册和平台中性身份承载，不写入真实历史数据。任何生产操作都必须在窗口开始时重新核验候选 SHA、连续 Migration 末号和线上 ledger，并把核验出的当前 schema 作为 `--expected-schema` 显式传入。备份与恢复 CLI 已删除 schema 34 默认值；省略、非整数或非正数均在读写数据库前失败关闭。不得因为仓库当前末号为 `0042` 就推断线上已应用到 `0042`。如果未来必须在 D1 持久化生产发布事实，必须另建 OpenSpec Change，并使用届时下一连续 Migration。
...
本任务不执行部署。经最终批准后的顺序必须是：确认 release SHA/配置快照/备份可恢复 → 保持外部开关关闭 → 只读核验线上 Migration ledger → 按 `0001`–`0042` 连续顺序应用获批且尚未应用的 Migration → 部署 schema-compatible Worker → 匿名 smoke → 分阶段启用 Provider/Job。Migration、部署、Scheduler、Drive delete、Provider 和 Production GO 必须分别批准；不得因为仓库当前末号为 `0042` 就推断线上已应用到 `0042`。
