...
- 当前 checkout（2026-09-01）的连续尾部是 `0041_owner_alias_yueguangbai_ygbceping.sql`（Schema 41）：0038/0039 为 Owner 授权零消费者对象清理（A+C 与 B+D 收窄版），0040 补种 yueguangbai 通道后 0041 按"月光白=ygbceping"同日终裁撤销该种子（净效果六通道）；下列 `0037`–`0043` 是旧历史链的边界说明，不是当前 checkout 的 schema tail，也不是当前可执行的并行 Migration。
