...
当前发布候选要求 Migration 连续为 `0001`–`0065`，尾部为 `0065_retire_feishu_artifacts.sql`；仓库连续性只证明本地候选结构，不能据此推断生产 ledger。
