...
`audit:dependencies` 明确审计 production 和 dev dependencies。`verify:dependency-lifecycle` 离线读取 committed `package-lock.json` 的 `packages[*].hasInstallScript`，并要求每个实际 lifecycle package 与 [`scripts/dependency-lifecycle-allowlist.mjs`](../../scripts/dependency-lifecycle-allowlist.mjs) 中的包名和精确版本一一对应；新包、版本漂移、失效批准都会失败。lockfile 不记录精确脚本文本，所以 CI 不伪造这种信息；本地审计时才可读取已安装 package metadata 作为补充证据。

当前批准的最小集合为：`@fission-ai/openspec@1.8.0`（OpenSpec CLI）、`esbuild@0.28.1`（Vite/Vitest binary）、`fsevents@2.3.3` 与 `fsevents@2.3.2`（可选 macOS watcher）、`msw@2.15.0`（测试 mock runtime）、`workerd@1.20260730.1`（Wrangler local dry-run runtime）。这不是“允许所有 install scripts”；CI 的 `npm ci` 没有设置 `--ignore-scripts`，而 custom verifier 只负责对 committed lockfile 做批准和漂移检测，不假装接管 npm 的安装策略。
