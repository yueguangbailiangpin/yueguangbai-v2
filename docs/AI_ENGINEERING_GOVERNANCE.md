...
当前 Staff 固定使用 Cloudflare Access 身份证明、Moonwhite D1 Staff 权限和 Worker 内部会话，不使用旧共享角色链接。现行架构不接入飞书登录、绑定、同步、回调或告警；重新引入任何外部员工入口必须另建 OpenSpec Change，不得形成第二套身份或业务事实权威。
...
| 6：内部运营 | 强制 | 选择性只读 Review | 仅普通页面、普通 API glue、DTO/内部通知格式化；不得简化 Staff 权限、Assignment、Personal DENY、Audit 或客户隔离。 |
