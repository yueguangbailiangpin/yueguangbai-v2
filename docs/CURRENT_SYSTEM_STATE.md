...
- 当前目标 Schema：36（阶段 7.5 追加 0031 员工订单列表索引、0032 公司公开客服渠道、0033 卖家结算批次；阶段 7.5R 追加 0034 `SERVICE_CHANNEL_QR` 受控文件链、0035 结算批次取消修复、0036 `BATCH_CANCELLED` 保留标记）
- **当前 API inventory（2026-08-30，本地运行时核验）：240 个唯一端点 = 238 个 `/api/*` + `/health` + `/ready`。** 历史数字保留其原有阶段语义：阶段 6.6 为 219 个端点，阶段 6.6E 为 224 个端点；阶段 7.5 在 224 的基础上新增 14 个（第一批员工工作台摘要 1 个、第二批公司公开客服渠道 3 个、第三批卖家结算批次 10 个），形成 238 个；阶段 7.5R 再新增客服二维码受控文件链的上传意图与挂载/清除 2 个端点，形成当前 240 个。`npm run verify:api-contract` 由现有 verifier 直接统计并由运行时 `app.routes` 双向断言，当前退出码为 0。
