...
- 当前 Schema：36（阶段 7.5 追加 0031 员工订单列表索引、0032 公司公开客服渠道、0033 卖家结算批次；阶段 7.5R 追加 0034 `SERVICE_CHANNEL_QR` 受控文件链、0035 结算批次取消修复、0036 `BATCH_CANCELLED` 保留标记）
...
- 当前实现与未完成边界：后端干净基线重建已完成；当前 Schema、API inventory 和 Seller 结算读取边界以上方条目为准。真实历史订单导入与真实图片盘点仍未运行（`REAL_HISTORICAL_IMPORT=NOT_RUN`、`REAL_IMAGE_INVENTORY=NOT_RUN`）；Stage 8 部署准备仍待总控授权。
- 历史前端 7A-1R-B → 7A-2 路线已走完；当前剩余前端工作以 active OpenSpec Change `stage7f-frontend-complete-rebuild` 为准，Stage 7F-4 legacy CSS 收尾仍未完成。
