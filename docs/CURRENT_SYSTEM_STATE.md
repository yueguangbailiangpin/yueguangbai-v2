...
- 当前目标 Schema：26（阶段 3 建立 0001–0019 干净 baseline；阶段 4 追加 0020–0023 marketplace canonical 统一与机器获客字段删除；阶段 5 追加 `0024_cold_archive_bundle_model.sql`（ZIP Bundle 冷归档 7 张表 + 队列/恢复/开关，schema 24）；阶段 6 追加 `0025_historical_order_import.sql`（历史导入 5 张事实表，schema 25）；阶段 6.5 追加 `0027_stage66_single_source_convergence.sql`（schema 26：quarantine CHECK 扩展 `MULTI_LINE_ORDER_REQUIRES_MAPPING`、identity override 增 `import_batch_id` 审计列、图片盘点三表 `historical_image_inventory_batches`/`_files`/`_findings`——源目录只读、字节事实不可变、仅 business_* 列可由 reconciliation 更新）），`app_schema_state.schema_version=27`）。当前 inventory：190 表 / 566 索引 / 345 触发器 / 12 视图（SHA-256 由 `db:verify` 锚定）
