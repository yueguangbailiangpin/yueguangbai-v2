...
- M10 历史提交 `8c4fdaa382fd1e2c56d76aa23bb6b960c4f6f72c` 只作历史谱系证据。当前仓库 Migration 为连续 `0001`–`0072`，尾部为 `0072_unified_order_day_rate_center.sql`，不能据此推断线上 ledger 已到 0072。
- 备份与恢复 CLI 已删除 schema 34 默认回退，遗漏、无效或非正整数 `--expected-schema` 会在读取数据库或备份文件前失败关闭。当前操作目标是 Schema 72 / `0001`–`0072`，仍要求上线前授权只读核对真实 ledger。
...
- Migration verifier 只证明仓库 `0001`–`0072` 连续（尾部为 `0072_unified_order_day_rate_center.sql`）、fresh/顺序/错序/重复/部分失败关闭；真实 D1 ledger、备份和恢复必须另获授权。
