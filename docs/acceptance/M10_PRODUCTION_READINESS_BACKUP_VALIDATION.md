...
- P0-01：独立外部健康告警接收人与升级通道未配置/未测试；现行系统不使用飞书告警。
...
- P0-04：Cloudflare 账号、域名/DNS、production secrets 未配置；无线上 Migration、部署、Scheduler/Queue、R2/Drive/MCP 启用。
...
- P1-01（既有治理项）：`pre-wave13-baseline-conformance-audit` 仍为 `28/40`。未完成项包含真实 R2、真实浏览器/网络矩阵、Integration/main/PR/部署等；本任务不越权勾选或虚假归档。总控需决定继续完成、拆分或正式关闭该历史 change。它不否定 M10 本地代码证据，但禁止声称“全部 change 已归档”。
...
2. 完成 Google Drive、OpenAI/ChatGPT、Cloudflare 三组独立外部激活清单与凭证托管。
3. 批准备份保留、异地副本、密钥轮换、恢复频率和销毁流程。
4. 完成三大运营商、微信内置浏览器和主要移动端的根页/登录/上传/受控图片/长列表/中文/北京时间/错误恢复实测。
5. 批准隐私、AI 处理、跨境、保留/删除/注销和平台政策风险处置。
6. 若导入历史数据，先批准不可变 AUDIT/PREVIEW，再分别批准分批导入；不得覆盖 V2 已完成事实。
7. 分别批准线上 Migration、部署、Scheduler、Drive proxy、首次 R2 delete、MCP；最后单独签发 Production GO。
